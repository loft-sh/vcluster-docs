...
function validate(data) {
  ...
  if (!Array.isArray(data.matrix)) {
    ...
    for (const row of data.matrix) {
      ...
      for (const v of data.kubernetesVersions) {
        ...
        } else if (!['tested', 'compatible'].includes(row.vcluster[v])) {
          errors.push(`invalid status "${row.vcluster[v]}" for host ${row.host} / vcluster ${v}`);
        }
      }
    }
  }
  ...
  if (!data.statuses || !data.statuses.tested || !data.statuses.compatible) {
    errors.push('statuses must define "tested" and "compatible"');
  }
