...
In order to sync KubeVirt [DataVolumes](https://kubevirt.io/user-guide/storage/disks_and_volumes/),  make sure to install the [KubeVirt Containerized Data Importer](https://kubevirt.io/user-guide/storage/containerized_data_importer/). Then enable the following setting in the `vcluster.yaml`:
```yaml
integrations:
  kubeVirt:
    enabled: true
