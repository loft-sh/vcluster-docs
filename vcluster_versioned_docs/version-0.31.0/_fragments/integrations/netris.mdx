...
- BCM Node Provider with Netris (requires vCluster Platform 4.4+)
- KubeVirt Node Provider with Netris (requires vCluster Platform 4.4+)
...
The bridge must exist on the nodes where the vCluster pods are running and be connected to the Netris fabric (e.g. through VLAN or an extension of the L2VPN).

If not set, `nbr-<vxlan id>` will be used with the VXLAN ID of the first Vnet that is managed by the server cluster with a valid gateway IP.
