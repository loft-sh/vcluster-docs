...
This applies to Docker Desktop and Rancher Desktop on macOS in particular. Run the `vcluster create` command with `sudo` to enable load balancer services:
...
If you don't need load balancer services, you can [disable them](#disable-load-balancer) and run without sudo.
:::
