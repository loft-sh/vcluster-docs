...
To connect a virtual cluster to the vCluster Platform, you can use an API key—referred to as an [access key](/docs/platform/next/administer/authentication/access-keys) in the platform's console. There are two authentication methods:

- **Shared access key**: Create a single Kubernetes Secret containing the access key, and reference it in multiple virtual clusters, even if they are deployed in different namespaces. This approach simplifies management, reduces overhead, and eases credential rotation.
  ...
  ### Configure shared access for a virtual cluster
    ...
    <Flow>
      <Step title="Create the access key">
        ...
        Log in to vCluster Platform and create an [access key](/docs/platform/next/administer/authentication/access-keys).
...
### Automatic vCluster registration and API key management

When you are authenticated to the vCluster Platform in your terminal and create a vCluster using the vCluster CLI, the vCluster Platform automatically registers the new vCluster as an externally deployed cluster.
You do not need to manually add it through the platform UI. 

During this CLI-driven deployment, a Kubernetes Secret, default as `vcluster-platform-api-key`, is created in the vCluster’s host namespace (the namespace where the vCluster is deployed).
This Secret contains the API key that the vCluster Platform uses to communicate with the vCluster. In your vCluster or vCluster Platform configuration, the `apiKey` setting defaults to reading the key from this Secret. 
As a result, you typically don’t need to set `apiKey` manually.

You should configure `apiKey` explicitly if:

- You did not use the vCluster CLI to authenticate to the vCluster Platform and deploy the vCluster, or
- You prefer to use a single, centrally managed API key (one Secret shared across vClusters) instead of per-namespace Secrets.
