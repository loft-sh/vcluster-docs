...
Distro holds virtual cluster related distro options. A distro cannot be changed after vCluster is deployed, with one exception: starting with vCluster 0.25.0, migration from K3s to K8s is supported. For more details, see the [K3s to K8s migration guide](/docs/vcluster/0.31.0/deploy/upgrade/distro-migration).
