...
Need automated, scheduled snapshots? [vCluster Platform AutoSnapshots](/docs/platform/4.6.0/use-platform/virtual-clusters/key-features/snapshots) lets you configure automatic snapshot creation on a cron schedule with retention policies—no need to write custom Kubernetes jobs.
:::
