...
An administrator must first set up a [database connector secret](/docs/platform/administer/connector/database) in the platform. Your virtual cluster must have access to the platform.

Once the connector secret has been configured, assign the name of the secret to the `connector` field.

:::note
Any virtual cluster can connect to the platform. This is usually done by configuring a platform [API key](../../../../../../configure/vcluster-yaml/external/platform/api-key). Connecting to the platform allows for centralized management and monitoring of virtual clusters, enhancing overall operational efficiency.
:::
