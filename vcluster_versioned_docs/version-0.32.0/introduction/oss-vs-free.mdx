...
The Platform requires outbound HTTPS access to `admin.loft.sh` for license retrieval. Virtual clusters only need connectivity to the Platform, not to the external license API. For air-gapped environments, see the [air-gapped installation guide](/platform/install/advanced/air-gapped).
:::
