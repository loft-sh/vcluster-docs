...
After deploying the platform, [create a new access key](/platform/administer/users-permissions/access-keys) to authenticate with the vCluster Platform API. This key is required to retrieve the synced Kubernetes Service Account (KSA) name and complete the Workload Identity setup.
