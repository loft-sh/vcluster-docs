...
#### `tag` <span className="config-field-required" data-required="false">required</span> <span className="config-field-type">string</span> <span className="config-field-default">v1.34.0</span> <span className="config-field-enum"></span> {#k8s-image-tag}
