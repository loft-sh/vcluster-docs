...
* **Pod to another virtual cluster service** - Services can also be mapped between [different virtual clusters](../configure/vcluster-yaml/networking/resolve-dns. This is achieved through DNS configurations that direct service requests from one virtual cluster to another.
