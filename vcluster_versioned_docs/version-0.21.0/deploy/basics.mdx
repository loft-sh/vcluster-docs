...
In order to use an emptyDir to store the data instead of a persistent volume,  create a `values.yaml` with the following contents:
...
More advanced configuration parameters are available. For more information on these,  see https://godoc.org/github.com/lib/pq.
