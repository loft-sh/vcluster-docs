...
When running vCluster with high availability, the default backing store option (i.e. embedded database) **must be changed** to any of the other [backing store options](../../../configure/vcluster-yaml/control-plane/components/backing-store/overview.mdx). 
