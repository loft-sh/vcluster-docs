...
For an example guide demonstrating volume snapshots in action, see this [walkthrough using AWS EBS volumes](./aws-ebs-guide.mdx).
...
If the vCluster is deployed externally, ensure the vCluster ClusterRole (`vc-<vClusterName>-v-<vClusterNamespace>`) has permission to read the secret.
:::
...
- Volume snapshots are currently not supported for [vCluster standalone](../../../deploy/control-plane/binary/README.mdx).
