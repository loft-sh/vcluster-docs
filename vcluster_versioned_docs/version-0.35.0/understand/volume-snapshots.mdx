...
:::warning
The Volume snapshot beta feature is now deprecated and slated for removal in an upcoming release.
See vCluster [Lifecycle Policy](../manage/upgrade/supported_versions.mdx#feature-versioning) for more information.
:::
...
**Control plane cluster user**:
- Runs the vCluster CLI commands;
- Installs snapshot-controller and CSI driver in the control plane cluster or in the tenant cluster, depending on the tenancy model.

**vCluster CLI**:
- Creates snapshot requests in the control plane cluster.

**vCluster**:
- Processes and updates snapshot request that are created in the control plane cluster;
- Creates dynamic VolumeSnapshots in the control plane cluster or in the tenant cluster, depending on the tenancy model.

**Kubernetes snapshot-controller**:
- Runs in the control plane cluster or in the tenant cluster, depending on the tenancy model;
...
- Runs as a sidecar container in the CSI driver pod in the control plane cluster or in the tenant cluster, depending on the tenancy model;
...
vCluster with shared nodes creates volume snapshots in the control plane cluster.

Control plane cluster admin needs to complete the following setup before being able to create volume snapshots with vCluster:
- Install common Kubernetes snapshot-controller and volume snapshot CRDs in the control plane cluster.
- Installs CSI driver in the control plane cluster.
...
sequenceDiagram
    ...
    participant ETCD as vCluster etcd
    participant VSCTRL as vCluster
    participant KSCTRL as snapshot-controller
    participant ESS as external-snapshotter sidecar
    participant CSI as CSI Driver
    end
    box S3/OCI/container
    participant SNAPSTORE as Snapshot location
    end
    ...
    VSCTRL->>ETCD: Create etcd backup
    VSCTRL->>SNAPSTORE: Save etcd backup
```

### Create volume snapshots with private nodes

vCluster with private nodes creates volume snapshots in the tenant cluster.

The tenant cluster has to be created with `deploy.volumeSnapshotController.enabled` config set to `true`. With this config, vCluster installs the common snapshot-controller, and VolumeSnapshot, VolumeSnapshotContent and VolumeSnapshotClass CRDs in your tenant cluster.

Then, the following setup needs to be completed to be able to create volume snapshots with vCluster:
- Installs CSI driver in the tenant cluster.
- Install VolumeSnapshotClass and annotate it as default for the CSI driver.
...
sequenceDiagram
    ...
    User->>CLI: run `vcluster snapshot create`
    CLI->>HAPI: Create snapshot request Secret
    CLI->>HAPI: Create snapshot request ConfigMap

    HAPI-->>VSCTRL: Watch event: snapshot request ConfigMap
    VSCTRL->>API: Create VolumeSnapshot for PVC
    ...
    ESS->>API: Update VolumeSnapshotContent.status (ready, handle)
    KSCTRL->>API: Update VolumeSnapshot.status (readyToUse=true, bound)
    VSCTRL->>API: Get: VolumeSnapshot and VolumeSnapshotContent

    VSCTRL->>HAPI: Update snapshot request (snapshot handle, phase=Completed)
    VSCTRL->>SNAPSTORE: Save snapshot request details
    VSCTRL->>ETCD: Create etcd backup
    VSCTRL->>SNAPSTORE: Save etcd backup
```
...
- Creates volume restore requests in the control plane cluster.

**vCluster**:
- Processes and updates restore request that are created in the control plane cluster;
- Creates pre-provisioned VolumeSnapshotContent and VolumeSnapshot in the control plane cluster or in the tenant cluster, depending on the tenancy model. The pre-provisioned VolumeSnapshotContent references a volume snapshot that has been already created when vCluster snapshot was taken.
...
- Syncs virtual PersistentVolumeClaims with restored PersistentVolumeClaims in the control plane cluster, if using shared nodes.
...
vCluster with shared nodes restores volumes in the control plane cluster.

Control plane cluster admin needs to complete the following setup before being able to restore volumes with vCluster:
- Install common Kubernetes snapshot-controller and volume snapshot CRDs in the control plane cluster.
- Installs CSI driver in the control plane cluster.
- Install VolumeSnapshotClass and annotate it as default for the CSI driver.
...
sequenceDiagram
    ...
    %% 5) Sync back PVC
    VCL->>API: Get: Restored PVC
    VCL->>VAPI: Sync back: Recreate virtual PVC (new volumeName)
```

### Restore volumes with private nodes

vCluster with private nodes restores volumes in the tenant cluster.
