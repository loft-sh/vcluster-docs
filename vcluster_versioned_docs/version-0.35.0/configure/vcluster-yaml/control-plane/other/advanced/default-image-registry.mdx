---
title: Default image registry
sidebar_label: defaultImageRegistry
sidebar_position: 1
description: Configure the default image registry air-gapped installation.
sidebar_class_name: host-nodes private-nodes
---

import DefaultImageRegistry from '../../../../../_partials/config/controlPlane/advanced/defaultImageRegistry.mdx'
import TenancySupport from '../../../../../_fragments/tenancy-support.mdx';

<TenancySupport hostNodes="true" privateNodes="true" />

Use `defaultImageRegistry` to configure a private image registry.

For example:

```yaml
controlPlane:
  advanced:
    defaultImageRegistry: my-private-registry:5000/vcluster/
```

vCluster prepends the image registry to all images used by vCluster, such as syncer, K8s, and CoreDNS. For example, `coredns/coredns:v1.11` becomes `my-private-registry:5000/vcluster/coredns/coredns:v1.11`.

You can find a list of all needed images by vCluster in the file `images.txt` at the releases page, as well as two scripts (`download-images.sh` and `push-images.sh`) to pull and push those to your private registry.

To use your private registry, you need to [configure `imagePullSecrets`](./service-account.mdx).

{/*
`advanced.defaultImageRegistry`:
- existing docs covers this well, however it may be needed to configure the vCluster service account with a pullSecret to use the private registry. See [this issue](https://github.com/loft-sh/vcluster/issues/333)
- multiple default image registries are not supported, and instead users need to override each image, see [this issue](https://github.com/loft-sh/vcluster/issues/1200)
*/}

## Config reference

<DefaultImageRegistry/>
