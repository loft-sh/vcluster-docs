...
Not all workloads need to run continuously. Scaling them down reduces costs. With auto sleep, you can scale workloads based on a set schedule, user activity, ingress traffic, or Gateway API HTTP traffic when the selected Gateway controller supports request mirroring.
...
If a controller supports request mirroring but does not advertise it in `GatewayClass` status, set the `sleepmode.loft.sh/request-mirror-controller-allowlist` annotation on the Platform `Cluster` resource with the controller names that should be treated as request-mirror-capable.
...
For the broader adoption path, see [Gateway API](../../key-features/gateway-api.mdx) and [Migrate from ingress-nginx to Gateway API](../../manage/ingress-nginx-to-gateway-api.mdx).
