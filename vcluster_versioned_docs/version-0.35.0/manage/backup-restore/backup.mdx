...
:::warning
Volume snapshot for CSI persistent volumes (the `--include-volumes` flag) is now deprecated and slated for removal in an upcoming release. See vCluster [Lifecycle Policy](../upgrade/supported_versions.mdx#feature-versioning) for more information.
