...
- An existing Amazon EKS cluster with the EBS CSI Driver installed. Follow the [EKS deployment guide](../../../deploy/control-plane/kubernetes-pod/environment/eks.mdx) to set up your cluster
- The vCluster CLI installed
- Completed the [volume snapshots setup](./setup.mdx#setup) based on your chosen tenancy model
...
Choose the deployment option based on your worker node type. See [worker nodes](../../../introduction/architecture.mdx#worker-nodes) for an overview of shared and private node options.
<Tabs
