...
To map a synced object on the control plane cluster back to its original name in the tenant cluster, read its `vcluster.loft.sh/object-name` annotation. vCluster sets it on every synced object, and it always carries the untruncated name.

For namespaced objects, `vcluster.loft.sh/object-namespace` carries the original namespace in the tenant cluster. Cluster-scoped objects have no namespace, so vCluster removes that annotation from them. Reverse-mapping automation must tolerate its absence rather than assume both annotations are present.
