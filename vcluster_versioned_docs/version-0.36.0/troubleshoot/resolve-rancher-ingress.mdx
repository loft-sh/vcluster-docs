...
description: How to resolve ingress conflicts when deploying Rancher in both vCluster and the control plane cluster.
...
When deploying the Rancher agent in both vCluster and the control plane cluster with a single shared ingress, both agents repeatedly update the ingress object. This causes excessive load on the database and API server.

You might observe the following signs of ingress update conflicts:

- Endless updates to ingress resources in the control plane cluster
...
This issue occurs due to a conflict between Rancher agents deployed in both vCluster and the control plane clusters. Both agents attempt to update annotations on the ingress using different naming conventions, creating a continuous update loop.
...
:::note
You must [compact the etcd](./etcd-compaction.mdx) database before seeing performance improvements.
:::
