...
These policies apply through your own policy engine, not vCluster, so the YAML itself doesn't depend on a vCluster edition. The first one's documented delivery path does. It needs [`policies.centralAdmission`](./shared-nodes-hardening.mdx#prerequisites), an Enterprise feature, for the tamper-resistant delivery this guide assumes. [Manually mapping a host policy service into the tenant cluster](../configure/vcluster-yaml/policies/admission-control.mdx), or installing a webhook directly inside it, reaches the tenant cluster too, without that dependency. You then have to protect that setup from tenant modification yourself.
