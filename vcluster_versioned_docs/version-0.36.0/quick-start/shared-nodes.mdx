...
This guide shows you how to deploy a tenant cluster onto your existing cluster in under two minutes. What you deploy is a nested cluster, the tenancy model vCluster OSS provides. See [What is vCluster?](../introduction/what-is-vcluster.mdx) for how it compares to private nodes. You'll also see the isolation boundary in action. From inside the tenant cluster, everything looks like a normal Kubernetes cluster. From the outside, each tenant fits in a single namespace with no visibility into anyone else's workloads.
