...
This guide shows you how to deploy a tenant cluster onto your existing cluster in under two minutes. What you deploy is a nested cluster, the tenancy model vCluster OSS provides. See [What is vCluster?](../introduction/what-are-virtual-clusters.mdx) for how it compares to private nodes. You'll also see the isolation boundary in action. From inside the tenant cluster, everything looks like a normal Kubernetes cluster. From the outside, each tenant fits in a single namespace with no visibility into anyone else's workloads.
...
On shared nodes, tenant workloads run on the control plane cluster, so the control plane cluster's CNI does the enforcement. A NetworkPolicy created inside the tenant cluster only takes effect once vCluster syncs it to the control plane cluster. That sync is off by default, so enable it first.

<Flow>
  <Step>
    Enable NetworkPolicy sync on the tenant cluster.
