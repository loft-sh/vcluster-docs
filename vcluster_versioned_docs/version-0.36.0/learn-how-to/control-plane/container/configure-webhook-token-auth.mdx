...
sidebar_label: Configure webhook token auth
sidebar_position: 10
...
:::note
Webhook token authentication uses kube-apiserver, which supports the `--authentication-token-webhook-config-file` flag.
:::
...
- [Hardening guide - authentication](../../../deploy/control-plane/kubernetes-pod/security/hardening-guide/host-nodes/3-control-plane.mdx)
