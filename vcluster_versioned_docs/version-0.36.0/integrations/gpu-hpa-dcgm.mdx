...
The GPU Operator deploys DCGM Exporter by default. Installing the standalone chart below alongside it creates two competing DaemonSets on the same GPU nodes. If the Operator manages your GPU stack, skip this step. To add Kubernetes pod labels such as endpoint, customer, or tier to the exported metrics, enable the optional label enrichment shown in [GPU and accelerator support](../introduction/gpu-and-accelerator-support.mdx#install-the-nvidia-gpu-operator-in-a-tenant-cluster). Then continue with Step 2, and follow the GPU Operator path in Step 3 for the ServiceMonitor.
...
:::info What enablePodLabels adds
DCGM Exporter's Kubernetes mapping adds the core `pod`, `namespace`, and `container` attribution through the kubelet pod-resources API. The HPA example below only requires this core attribution.

To copy labels from the Kubernetes Pod object onto GPU metrics, add `--set kubernetes.enablePodLabels=true` to the Helm command. These extra dimensions let you query by endpoint, customer, model, or tier.
:::
...
Choose one owner for the workload's autoscaler. The following HPA and KEDA examples target a direct Deployment. If KServe manages the workload, configure its autoscaler through the `InferenceService` instead of creating a competing HPA or `ScaledObject`.
...
### Scale a direct Deployment with HPA
...
### Scale a direct Deployment with KEDA
...
Use KEDA for query-driven scaling or scale-to-zero patterns. The example keeps one warm replica because `minReplicaCount` is `1`. Set it to `0` only after you design and test the endpoint's cold-start path.

Use HPA with Prometheus Adapter when you want Kubernetes-native custom metrics and HPA behavior. In both cases, set conservative scale-down windows. Loading large models is slow, and repeated cold starts degrade response latency.
...
2. Check that DCGM Exporter metrics include `pod` and `namespace` labels. If these labels are missing, verify that Kubernetes mapping is enabled and that the exporter can access the kubelet pod-resources socket. If labels from the Kubernetes Pod object are missing, confirm label enrichment is enabled with `kubernetes.enablePodLabels=true` for the standalone chart or `dcgmExporter.enablePodLabels=true` for the GPU Operator.
