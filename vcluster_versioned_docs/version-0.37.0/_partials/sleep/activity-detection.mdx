...
All tenant cluster Kubernetes API requests that are made through the platform count as activity in the {props.name}.
...
For [ingress-nginx](https://github.com/kubernetes/ingress-nginx/tree/main) (deprecated) based Ingresses, [Istio](https://istio.io/) Gateways, and Gateway API `HTTPRoute` resources with request mirroring support, activity detection works automatically.
...
For Gateway API, the selected Gateway controller must support `HTTPRoute` request mirroring. Gateway endpoints can still work with a controller that does not support request mirroring.

Gateway-backed auto wakeup is not available through that controller.

:::note Gateway API CRD discovery
If Gateway API CRDs are installed after vCluster Platform is already running, restart Platform so the Gateway API sleep-mode controllers are discovered and started:
