...
* `vcluster platform share` : Shares a tenant cluster/namespace with another vCluster platform user or team
...
### New flags within each command
 ...
 `vcluster connect`
    ...
    * `--driver` string : Specifies the driver to use for managing the tenant cluster (helm or platform).

 `vcluster create`

    * `--add` : Automatically adds the tenant cluster to the current vCluster platform when using the helm driver (default true).
    ...
    * `--description` string : Description to show in the platform UI for this tenant cluster. 

    * `--display-name` string : Display name to show in the platform UI for this tenant cluster.
    ...
    * `--recreate` : Deletes an existing tenant cluster with the same name before creating a new one.

    * `--set-parameter` stringArray : Sets specific parameters if a template is used.

    * `--skip-wait` : Does not wait until the tenant cluster is running if true. 
    ...
    * `--use` : Uses the tenant cluster if it's already there. 
...
`vcluster pause`
    ...
    * `--driver` string : Specifies the driver for the tenant cluster (helm or platform).
...
`vcluster connect`
    ...
    * `--kube-config-context-name` string: This flag was used to override the context name of the generated tenant cluster kube config with a specified name. 
...
`vcluster create`
    ...
    * `--distro` string: This flag was used to specify the Kubernetes distro to use for the tenant cluster. Allowed distros were k3s, k0s, k8s, eks, with the default being "k3s". In v0.20.0, configuring the distro is now in the vcluster.yaml with the default being "k8s".
