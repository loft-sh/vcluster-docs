...
sidebar_position: 6
description: How to deploy and use custom schedulers in both control plane and tenant cluster
---
...
- Run a custom scheduler in the control plane cluster.
- Run a custom scheduler in the tenant cluster.
- Use vCluster Hybrid scheduling, so Pods from the tenant cluster can use schedulers from both control plane and tenant cluster.

How pods are scheduled with Hybrid scheduling enabled:
- Pods without `spec.schedulerName` use the default scheduler inside the tenant cluster.
...
- Pods whose `spec.schedulerName` matches a scheduler running inside the tenant cluster are scheduled by that virtual
  scheduler.

See [Hybrid scheduling](../../../configure/vcluster-yaml/sync/to-host/core/pods/hybrid-scheduling.mdx) docs for more details.
...
newer. It requires shared host nodes and host nodes have to be synced into the tenant cluster. This guide enables node
...
## Create control plane and tenant cluster
...
Create a vCluster config file with Hybrid scheduling enabled and node syncing from the control plane cluster:
    ...
    spec:
      ...
      containers:
        - command:
          ...
          image: registry.k8s.io/kube-scheduler:${DEFAULT_K8S_VERSION}
...
:::tip
Use a kube-scheduler image tag compatible with your cluster's Kubernetes version. The minor versions should match.
:::
...
Apply manifests in the control plane cluster:
...
Apply manifests in the tenant cluster:
...
- one pod is using the custom tenant cluster scheduler.
...
Apply the manifest in the tenant cluster:
...
- No nodes in the tenant cluster:
  - Ensure `sync.fromHost.nodes.enabled` is `true` and that your control plane cluster has Ready nodes.
