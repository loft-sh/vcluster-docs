...
import EnableSyncing from './class-syncing-enable.mdx';
import HowSyncingWorks from './class-syncing-how-it-works.mdx';
import UseSelectors from './class-syncing-selectors.mdx';
...
Synced {props.resourceClass} resources function like any other Kubernetes resource in the tenant cluster. You
...
When you modify the {props.resourceClass} resource in the control plane cluster, vCluster re-evaluates whether it still matches the selector criteria. If the {props.resourceClass} continues to match, vCluster updates the corresponding resource in the tenant cluster to reflect the changes. If the {props.resourceClass} no longer matches the selector criteria, vCluster removes it from the tenant cluster.

The selector system acts as both a resource filter and a validation mechanism. As a resource filter, it
ensures that vCluster creates only {props.resourceClass} resources matching the defined selector criteria. The selector also functions as a creation validation mechanism - when you create {props.resource} {props.showExtraResource && (<> and {props.extraResource} </>)}resources in the tenant cluster, the resources can only reference {props.resourceClass} resources that exist in the tenant cluster.

### Orphaned resources

When vCluster removes a synced {props.resourceClass} from the tenant cluster due to selector changes or deletion from the control plane cluster,
any {props.resource} {props.showExtraResource && (<> and {props.extraResource} </>)}resources that reference it remain in the tenant and control plane clusters. These orphaned {props.resource} {props.showExtraResource && (<> and {props.extraResource} </>)}resources stop
...
- The {props.resource} {props.showExtraResource && (<> or {props.extraResource} </>)}fails to sync to the control plane cluster
- vCluster records an event on the {props.resource} {props.showExtraResource && (<> or {props.extraResource} </>)}resource in the tenant cluster
