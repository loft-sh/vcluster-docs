...
vCluster certificates will change when you create the virtual cluster with a new name or in a new namespace, which is expected as virtual clusters shouldn't share the same certificates.
:::
...
- Persistent volumes (PVs) or persistent volume claims (PVCs) will not be in the snapshot. 

**Sleeping Virtual Clusters**
- Snapshots require a running vCluster control plane, which means snapshots will not work with sleeping virtual clusters. 

**Virtual clusters using the k0s distro** 
- When taking a snapshot of a k0s virtual cluster, the `--pod-exec` flag is required. 
- k0s virtual clusters cannot be restored or cloned to a new virtual cluster, but can only be migrated to use k8s as the distro. 

**Virtual clusters using an external database** 
- Virtual clusters using an external database, backup and restore are handled outside of vCluster. The database administrator needs to back up the external database, or restore it from a snapshot or dump using your database's documentation. We do not recommend using vCluster CLI backup and restore functionality for virtual clusters using an external database.
