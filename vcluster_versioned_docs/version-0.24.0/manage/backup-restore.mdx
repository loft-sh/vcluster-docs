...
## Restore a snapshot to a new virtual cluster (Clone)

You can also use snapshots to clone an existing virtual cluster and create a new virtual cluster.

:::warning Restore Failures
If the restore failes while using `vcluster create`, the virtual cluster is automatically deleted.
:::
...
## Override vcluster options from a snapshot

When upgrading virtual clusters, there are a couple of configuration options that are not supported to change without a snapshot, e.g. changing backing store, changing distro, etc. 

You can use a snapshot to upgrade an existing virtual cluster and apply new options to migrate to these new confifguration options. 
