...
<TabItem value="ingress">
  ...
  If you are using the ingress nginx controller,  make sure you have [enabled the SSL passthrough feature](https://kubernetes.github.io/ingress-nginx/user-guide/tls/#ssl-passthrough) as it is disabled by default.
  :::
  ...
  In order for this ingress to work correctly, you will need to enable SSL passthrough as TLS termination has to happen at the vCluster level and not ingress controller level. If you cannot do that,  take a look below for using an ingress without ssl passthrough.
  :::
...
<TabItem value="ingress-wo-ssl">
  ...
  With this configuration you will need to use service account authentication to connect as the ingress controller won't be able to resolve the client-cert and client-key which is used by default as authentication method. To create a kube config that uses a service account,  run the following command:
