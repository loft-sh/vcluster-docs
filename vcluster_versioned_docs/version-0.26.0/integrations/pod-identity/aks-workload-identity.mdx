...
If you have not set one up yet, follow the [vCluster Platform installation guide](/docs/platform/install/quick-start-guide).

After deploying the platform, [create a new access key](/docs/platform/administer/users-permissions/access-keys) to authenticate with the vCluster Platform API. This key is required to retrieve the synced Kubernetes Service Account (KSA) name and complete the Workload Identity setup.
