...
You can also specify a label selector to determine which host nodes the virtual cluster can be scheduled onto. Set labels on the worker nodes on the shared host cluster before using label selectors to select those nodes.
