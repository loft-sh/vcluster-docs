...
- Can be disabled in the control plane vCluster when using [isolated control planes](../../experimental/isolated-control-plane.mdx), where all workloads are instead scheduled in the workload vCluster (which should have node syncing enabled).
