...
A good example would be nodes, which are nice to view inside the virtual cluster and can be also used to enabled certain features such as [scheduling inside the vCluster](/docs/vcluster/configure/vcluster-yaml/control-plane/other/advanced/virtual-scheduler),
