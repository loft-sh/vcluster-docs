...
##### `image` <span className="config-field-required" data-required="false">required</span> <span className="config-field-type">string</span> <span className="config-field-default">map&#91;registry:mirror.gcr.io repository:library/alpine tag:3.20&#93;</span> <span className="config-field-enum"></span> {#pods-rewriteHosts-initContainer-image}
