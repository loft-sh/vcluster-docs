...
```yaml title="vcluster.yaml" {5,7,10}
controlPlane:
  statefulSet:
    env:
      - name: HTTP_PROXY
        value: http://corp-proxy.example.com:3128  # Replace with your proxy
      - name: HTTPS_PROXY  
        value: http://corp-proxy.example.com:3128  # Replace with your proxy
      - name: NO_PROXY
        # IMPORTANT: Include <vcluster-name>-etcd explicitly for external etcd
