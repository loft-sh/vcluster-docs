...
However, when using features such as [syncing custom resources](/vcluster/configure/vcluster-yaml/sync/to-host/advanced/custom-resources), it can be tedious to add all used references, or it can become very difficult or even impossible to rewrite certain references.
