...
Most of the settings are for the init container from which vCluster copies the plugin. Adjust `controlPlane.statefulSet.resources` [configuration](../vcluster-yaml/control-plane/deployment/statefulset) to ensure the vCluster deployment has enough CPU  and memory to also run the plugin.
