...
<!-- vale off -->
## Shared host nodes vs. private nodes vs. standalone vs. Docker
<!-- vale on -->
...
- **Docker (vind)**: vCluster runs entirely in Docker containers, providing a lightweight standalone environment without requiring a host Kubernetes cluster.
...
* **[Binary](./vcluster-yaml/control-plane/components/standalone.mdx)** - The control plane is deployed as a binary on individual nodes.
* **[Docker Container](./vcluster-yaml/experimental/docker.mdx)** - The control plane runs in Docker containers without requiring a host Kubernetes cluster.
...
* **[Dedicated Nodes](./vcluster-yaml/sync/from-host/nodes.mdx#selecting-a-set-of-nodes)** - A set of worker nodes of the host cluster are selected based on node labels. That set of worker nodes can be used by the virtual cluster. By default, all resources are synced to the single namespace that the vCluster is deployed on. 
...
* **[Namespace Syncing](./vcluster-yaml/sync/to-host/advanced/namespaces.mdx)** - Advanced - Enable syncing namespaces between the virtual cluster and host cluster allows the specified namespaces and its resources to be synced from the virtual cluster onto the host cluster. There is no 
...
- **[Private Nodes](./vcluster-yaml/private-nodes/README.mdx)** 
...
### Docker containers (vind)

vCluster in Docker (vind) provides a standalone deployment model that runs entirely in Docker containers without requiring a host Kubernetes cluster:

- **[Docker Deployment](./vcluster-yaml/experimental/docker.mdx)** - Both control plane and worker nodes run as Docker containers on the local machine or any Docker-enabled environment. This provides complete isolation and is ideal for development, testing, and CI/CD scenarios.

## Deployment model comparison

The following table outlines the differences between the various deployment models:

| Feature                                    |  Host Nodes           |  Private Nodes        | Docker (vind)        |
|--------------------------------------------|------------------------------|------------------------------|---------------------|
| Isolated control plane                     | Yes                          | Yes                          | Yes                 |
| Ability to install custom CNI              | No                          | Yes                           | Yes                 |
| Ability to install custom CSI drivers      | No                          | Yes                           | Limited             |
| Isolated network                           | No                          | Yes                           | Yes                 |
| Ability to re-use host controllers         | Yes                           | No                          | No                  |
| Ability to view vCluster resources in the host cluster | Yes              | No                          | No                 |
| Ability to automatically provision and add nodes based on resource requirements | No | Yes | Limited |
| Requires host Kubernetes cluster          | Yes                          | Yes                          | No                  |
| Suitable for local development            | Limited                      | No                           | Yes                 |
| Suitable for CI/CD                        | Yes                          | Limited                      | Yes                 |
