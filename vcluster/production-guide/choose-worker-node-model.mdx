...
Shared nodes may be suitable only when all three answers are no. You own the images, workload definitions, Kubernetes configuration, and operations, and the customer only consumes your application or API. Treat "may be suitable" as provisional, not final, until you apply the [shared node hardening guide](../security/shared-nodes-hardening.mdx) and explicitly accept the shared-kernel risk.
...
Any "yes" routes to private nodes regardless of the other answers. All "no" routes to shared nodes, once you've applied the hardening guide and accepted the shared-kernel risk. Hardening reduces risk on an accepted shared-node deployment. It never turns a "private nodes" result from this decision into "shared nodes."
...
| Suitable when the tenant controls Kubernetes access or workload execution | Yes | No |
