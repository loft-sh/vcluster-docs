...
**Private nodes** dedicate worker nodes to a single <GlossaryTerm term="tenant-cluster">tenant cluster</GlossaryTerm>, so no other tenant's workloads share that node or its kernel. **Shared nodes** run multiple tenant clusters' workloads on the same worker nodes, isolated from each other at the control plane, API, and namespace level, but not at the kernel. See [Architecture](../introduction/architecture.mdx#worker-nodes) for the full mechanics.

:::info Provisioning through a vCluster Platform template?
This decision may already be made for you. Templates fix `privateNodes.enabled` in the underlying `vcluster.yaml`. You'll see the resolved config before confirming creation, but you can only change it if the template allows customization. Check with whoever owns the template, or see [Create templates](/docs/platform/administer/templates/create-templates) if you administer them yourself.
:::

This page covers customer-facing and other production offerings where tenants aren't your own trusted staff. "Trusted" here means an explicit, enforced trust domain, not just familiarity. If your tenants are internal and meet that bar, skip to [trusted internal tenants](#trusted-internal-tenants-are-a-separate-model) for the full criteria.
...
Shared nodes may be suitable only when all three answers are no. You own the workload definitions and the cluster or scheduler's configuration and operations, and the customer only consumes your application or API. Treat "may be suitable" as provisional, not final, until you apply the [shared node hardening guide](../design/security/shared-nodes-hardening.mdx) and explicitly accept the shared-kernel exposure.

<figure>
  <ChooseWorkerNodeModelDecision style={{width: '80%', height: 'auto', display: 'block', margin: '0 auto'}} role="img" aria-label="Decision tree: any yes answer to the three customer-access questions routes to private nodes. A no to all three leads to a hardening and risk-acceptance check, which routes to shared nodes if passed or private nodes if not." />
...
| --- | --- | --- |
| Shared nodes | Existing Kubernetes worker capacity on the control plane cluster. Requires a CNI that enforces NetworkPolicy. | [Shared Nodes quick start](../quick-start/shared-nodes.mdx) |
| Private nodes | vCluster Platform (free mode supported), plus dedicated Linux worker capacity per tenant cluster, joined manually or provisioned automatically through a node provider. | [Private Nodes quick start](../quick-start/private-nodes.mdx) |
...
- **Private nodes** are the security boundary for tenants with cluster, scheduler, or workload execution access. See [Private Nodes](../deploy/worker-nodes/private-nodes/overview.mdx) for the full architecture.
- **Shared nodes** are a supported model for trusted tenants, not a security boundary for untrusted tenants with cluster or scheduler access or arbitrary workload execution. See [Architecture](../introduction/architecture.mdx#shared-nodes) for the full architecture.

A dedicated, labeled node pool selected from the <GlossaryTerm term="control-plane-cluster">control plane cluster</GlossaryTerm> is still the shared-node architecture. It scopes placement, not isolation, and it doesn't become private nodes.
...
| Suitable for trusted internal tenants | Yes | Yes, with [hardening](../design/security/shared-nodes-hardening.mdx) |
...
- **An internal CI/CD platform that creates ephemeral clusters for your own engineering teams** is a trusted internal tenant. Shared nodes are appropriate, hardened according to the [shared node hardening guide](../design/security/shared-nodes-hardening.mdx).
...
- Answered "shared nodes may be suitable"? Continue to the [Shared Nodes quick start](../quick-start/shared-nodes.mdx) or [Nodes from the Control Plane Cluster](../deploy/worker-nodes/host-nodes/overview.mdx) to deploy, then apply the [shared node hardening guide](../design/security/shared-nodes-hardening.mdx) and document your acceptance of the shared-kernel exposure, per the guide's own checklist, before treating the decision as final.
