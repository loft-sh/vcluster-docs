...
**Private nodes** dedicate physical nodes to a single tenant cluster, so no other tenant's workloads ever land on them. **Shared nodes** run multiple tenant clusters' workloads on the same physical nodes, isolated from each other at the control plane, API, and namespace level, but not at the kernel. See [Architecture](../introduction/architecture.mdx#worker-nodes) for the full mechanics.
...
- You accept that tenant workloads share a kernel and physical nodes with each other, and that shared-kernel exposure is documented.
