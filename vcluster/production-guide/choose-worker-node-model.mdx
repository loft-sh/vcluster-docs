...
**Private nodes** dedicate worker nodes to a single cluster, so no other tenant's workloads share that node or its kernel. **Shared nodes** run multiple clusters' workloads on the same worker nodes, isolated from each other at the control plane, API, and namespace level, but not at the kernel. See [Architecture](../introduction/architecture.mdx#worker-nodes) for the full mechanics.
...
This page covers customer-facing and other production offerings where tenants aren't your own trusted staff. "Trusted" here means an explicit, enforced trust domain, not just familiarity.

:::note "Tenant" on this page
Here a tenant is whoever consumes a cluster, which is the usual security sense of the word. It isn't the Platform [Tenant](/docs/platform/next/understand/what-are-tenants) resource. A Platform Tenant is one way to draw this boundary, and the decision below applies whether or not you use one.
::: If your tenants are internal and meet that bar, skip to [trusted internal tenants](#trusted-internal-tenants-are-a-separate-model) for the full criteria.
...
| Private nodes | vCluster Platform (free mode supported), plus dedicated Linux worker capacity per cluster, joined manually or provisioned automatically through a node provider. | [Private Nodes quick start](../quick-start/private-nodes.mdx) |
...
- Answered "shared nodes may be suitable"? Continue to the [Shared Nodes quick start](../quick-start/shared-nodes.mdx) or [Nodes from the Control Plane Cluster](../deploy/worker-nodes/host-nodes/overview.mdx) to deploy, then apply the [shared node hardening guide](../security/shared-nodes-hardening.mdx) and document your acceptance of the shared-kernel exposure, per the guide's own checklist, before treating the decision as final.
