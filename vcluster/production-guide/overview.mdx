...
- **A worker node model matched to your isolation needs:** private nodes for customer-facing or untrusted tenants, or shared nodes for trusted internal tenants. A dedicated node pool scopes placement within shared nodes. It doesn't provide private-node isolation. See [Choose a worker node model](./choose-worker-node-model.mdx).
