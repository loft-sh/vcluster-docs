...
Please make sure to [install the vCluster CLI](../quick-start-guide.mdx). Connect to the vCluster via:
...
If you have manually [exposed the vCluster](./exposing-vcluster.mdx), you can specify the domain where the vCluster is reachable via the `--server` flag:
...
If you have [exposed the vCluster](./exposing-vcluster.mdx), you can also tell the vCluster to create the kube config secret with another server endpoint through the `--out-kube-config-server` flag.
