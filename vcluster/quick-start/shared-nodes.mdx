...
<Flow>
  ...
  <Step>
    ...
    This is the isolation boundary. The tenant gets a clean Kubernetes API with no visibility into the Control Plane Cluster's structure. Other tenant clusters have their own dedicated namespaces, so neither tenant can see the other's workloads. This isolation is at the API and namespace level. Tenants still share the same kernel and physical nodes, which is why shared nodes are for internal, trusted tenants rather than externally facing or resold offerings.
  </Step>
</Flow>

## Verify network policy enforcement

Shared-node isolation relies on your cluster's CNI actually enforcing NetworkPolicy. Some CNIs accept policy resources without enforcing them, which silently leaves tenant traffic open. Confirm enforcement before you rely on it.

<Flow>
  <Step>
    Reconnect to the tenant cluster, deploy a target pod with a service, and a client pod.
  ...
  <Step>
    ...
    The request should now time out. If it still succeeds, your CNI isn't enforcing NetworkPolicy, so shared-node network isolation isn't in effect. Review the [security baseline](/docs/vcluster/security) before you run tenant workloads that depend on it.
  </Step>
</Flow>
