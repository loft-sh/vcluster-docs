...
sidebar_position: 1
slug: /
description: Choose the right deployment path and get a cluster running in minutes.
...
vCluster provisions isolated clusters, delivering managed Kubernetes, Slurm, inference, Ray, and other cluster types. The clusters can go on your existing infrastructure, or they can be built from scratch on bare metal and VMs. Each tenant gets a full Kubernetes API experience while the virtualized control plane stays completely invisible to them.

:::tip Planning a customer-facing or production platform?
These quick starts prove the deployment mechanics. They don't select a production security architecture. [Choose a worker node model](../production-guide/choose-worker-node-model.mdx) first.
:::
...
<div style={{display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '1rem', marginBottom: '2rem', marginTop: '1rem'}}>
  <QuickStartCard path="quick-start/shared-nodes" icon="group" title="Shared Nodes" description="You have a Kubernetes cluster and want isolated environments for internal, trusted teams. Tenant workloads run on the existing node pool. No extra infrastructure needed. Not for external or resold tenants. Works with OSS CLI or Platform." />
  <QuickStartCard path="quick-start/private-nodes" icon="lock" title="Private Nodes" description="You need dedicated infrastructure per tenant. Each cluster runs on its own nodes. Best for hard tenancy: GPU clouds, regulated platforms, and sovereign deployments. Requires vCluster Platform (free mode OK)." />
...
- Use an existing Kubernetes cluster for internal platforms, CI, testing, or development with trusted teams → [Shared Nodes](./shared-nodes.mdx)
- Create a Kubernetes cluster on Docker for local development or CI → [Docker (vind)](./docker.mdx)

Still unsure? Read [What is vCluster?](../introduction/what-is-vcluster.mdx) to understand the deployment models and tradeoffs.
