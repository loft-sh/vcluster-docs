...
All of the deployment options deploy a vCluster called `my-vcluster` into a namespace `team-x`. Each example expects a `vcluster.yaml`, which is an optional configuration file. Refer to the `vcluster.yaml` [reference docs](../configure/vcluster-yaml/README.mdx) to explore all configuration options.
