...
It allows for stronger isolation between the clusters, since the Pods in a cluster don't share the same Nodes.
...
It enables you to isolate clusters spanning mixed bare metal and VM private nodes that share the same underlying hardware with an exclusive network.
...
The goal of this guide is to have a cluster on an isolated network:

- Cluster managed by vCluster platform
...
This is transparent to the cluster; however, the control plane cluster remains inaccessible.

This can be repeated for any number of clusters.
The VPCs of each cluster may also be peered to access shared services across clusters.
...
1. Create ServerClusterTemplates
    ...
    Create another one for the vCluster tenant network with a non-overlapping CIDR, we choose `192.168.67.254/21`.
...
The following tools are required in the control plane cluster to manage interfaces that allow adding Pods and VMs to the cluster network.

#### Multus

See install instructions in [Multus CNI](https://github.com/k8snetworkplumbingwg/multus-cni).

Multus adds additional interfaces to the vCluster control plane Pods and KubeVirt VMs to attach them to the cluster VLAN.
...
2. Create vCluster with Netris integration
    ...
    Connect to the control plane cluster and run:
...
4. Verify node provisioning
    ...
    The BCM node provider will automatically provision the bare metal server as a node in the cluster.
    ...
    Once the node joins the cluster, the claim transitions from `Pending` to `Available` and the node will appear in the cluster:
      ...
      autoNodes:
        ...
        - provider: kubevirt-netris
          ...
          properties:
            ...
            kubevirt.vcluster.com/network-attachment-definition: vcluster-platform/vcluster-netris-vms
    ```

3. Verify VM provisioning

    The KubeVirt node provider will automatically create VMs that join the cluster.
    ...
    kubectl get nodeclaims -A
    NAMESPACE   NAME           STATUS      VCLUSTER   TYPE                        CREATED AT
    p-default   netris-swwnh   Available   netris     bcm-netris.dgx-node         2025-10-27T19:48:06Z
    p-default   netris-8gnqh   Available   netris     kubevirt-netris.netris-vm   2025-10-27T19:39:45Z
    ```

    Once the VM joins the cluster, the node claim transitions from `Pending` to `Available` and the VM will appear in the cluster:

    ```bash
    kubectl get nodes
    NAME           STATUS   ROLES    AGE     VERSION
    dgx-01         Ready    <none>   5m11s   v1.34.1
    netris-8gnqh   Ready    <none>   20s     v1.34.1
    ```
