...
By default, vCluster runs one instance of each of its components. This deployment method is recommended for any use cases that are very ephemeral, such as dev environments and CI/CD, but for production
use cases, it's recommended to run vCluster with more redundancy. We recommend deploying vCluster in high availability (HA) to run multiple copies of the vCluster components so that the cluster is more resistant to failures.

HA protects a cluster from failures such as a single vCluster control plane pod or eligible node becoming unavailable. It does not protect against a full outage of the Kubernetes cluster that hosts the vCluster control plane. For more information, see [What happens during control plane outages?](../../../understand/control-plane-outages.mdx).

## Enable high availability 

When running vCluster with high availability, the default backing store option (that is, embedded database) **must be changed** to any of the other [backing store options](../../../configure/vcluster-yaml/control-plane/components/backing-store/README.mdx). 
...
## Deploy vCluster with high availability
...
:::warning 
If you've used the vCluster CLI, your kubecontext automatically switches and connects to the cluster.
:::

You can get the pods in the namespace of the cluster. The namespace is
