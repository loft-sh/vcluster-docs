...
By default, vCluster runs one instance of each of its components. This deployment method is recommended for any uses cases that are very ephemeral, such as dev environments and CI/CD, but for production
...
HA protects a tenant cluster from failures such as a single vCluster control plane pod or eligible node becoming unavailable. It does not protect against a full outage of the Kubernetes cluster that hosts the vCluster control plane. For more information, see [What happens during control plane outages?](../../../understand/control-plane-outages.mdx).
