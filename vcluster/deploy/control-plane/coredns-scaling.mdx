...
sidebar_position: 3
description: How to scale CoreDNS in a cluster when a single replica can't keep up with DNS query volume.
---

CoreDNS handles DNS resolution for every pod in the cluster. By default,
it runs a single replica (`controlPlane.coredns.deployment.replicas: 1`) as a
separate Deployment, sized for small and medium clusters. High DNS
...
For clusters where node count changes over time, such as
...
inside the cluster, not a synced mirror of something on the control
plane cluster. Run the autoscaler directly against the cluster's own
API server. It watches the cluster's node count and scales `coredns`
directly. The cluster's own controller manager reconciles the change,
and the resulting pods sync to the control plane cluster automatically.

Apply the following to the cluster, not the control plane cluster:

```yaml title="coredns-autoscaler.yaml — apply inside the cluster"
apiVersion: v1
kind: ServiceAccount
metadata:
...
is `true`. With the default `sync.fromHost.nodes.enabled: false`, the
...
size. A node the physical cluster just added doesn't show up in the
...
node-derived replica count to 2 once the cluster has more than one
