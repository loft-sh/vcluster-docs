...
vind requires outbound HTTPS access during cluster creation. Use a forward proxy and an internal registry to run vind in a restricted network.

:::warning Fully disconnected environments aren't supported
vind can't create a cluster from an empty local cache without outbound access. The CLI downloads artifacts from GitHub and `ghcr.io` through hardcoded URLs.
...
| Cluster startup | Internal vCluster images | Set `controlPlane.advanced.defaultImageRegistry` |
...
vind's registry proxy is enabled by default. It serves this image from the host Docker cache for subsequent pulls inside the cluster.
...
:::note
Steps 5 and 6 aren't required for a basic cluster. Complete them when your workloads need the described images.
:::
...
vind enables a registry proxy by default. The cluster pulls workload images from the host Docker daemon's image store through this proxy.
...
The registry proxy serves the images from the host store for pulls inside the cluster.

## Step 6: Override the rewriteHosts init container image

Complete this step if you run StatefulSets inside the cluster. vCluster pulls `mirror.gcr.io/library/alpine:3.20` to rewrite fully qualified domain names for StatefulSet pods.
...
After configuring network access and images, switch to the Docker driver and create the cluster:
...
### Enterprise features error when not logged into Platform
...
- vind doesn't support cluster creation from an empty cache without outbound access.
