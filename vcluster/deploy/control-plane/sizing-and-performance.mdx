...
sidebar_position: 1
description: How to evaluate, size, and tune a Cluster's Virtual Control Plane for your environment.
---

This page explains how to evaluate and tune a Cluster's Virtual Control
...
- **P3 (4 CPU / 8 GiB) is the right default** for most production Clusters.
...
operator-rich Clusters with many persistent controllers, double them to
...
The vCluster syncer reflects Cluster objects to the Control Plane
...
For high-density Clusters, raise all three rate-limit stacks together.
...
When provisioning a new HA Cluster with embedded etcd, bring up replica
...
**Tenant Isolation is per-Tenant-Cluster.** Each Cluster is fully
isolated. Control-plane sizing is per-tenant, not aggregate. A single Control
Plane Cluster can host many Clusters concurrently, within its own
