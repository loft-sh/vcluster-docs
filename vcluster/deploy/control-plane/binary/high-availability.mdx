...
# Create a token
vcluster token create --control-plane --expires=1h
```
...
If you want to reuse a node that was already running vCluster standalone, you can do so by reseting the node using `--reset-only` flag and running install script again.
