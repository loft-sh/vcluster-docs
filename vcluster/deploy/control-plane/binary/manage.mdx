...
import { DEFAULT_K8S_VERSION } from '../../../_fragments/default-k8s-version.mdx';
...
When a standalone cluster is connected to <Link to="/docs/platform">vCluster Platform</Link>, configuration updates and version upgrades can be coordinated from the Platform UI with rolling upgrades:

<Flow>
  <Step>
    Go to `Clusters` and select the vCluster.
  </Step>
  <Step>
    Click `Edit`.
  </Step>
...
For the full configuration reference, see `config.yaml` in the [standalone configuration reference](../../../configure/vcluster-yaml/control-plane/components/standalone.mdx). Review the [standalone limitations](./troubleshooting.mdx#unsupported-configuration-options) before changing settings.

### Add and remove nodes

When a standalone cluster is connected to <Link to="/docs/platform">vCluster Platform</Link>, add or remove nodes by updating the `quantity` in your configuration and saving the changes in the Platform UI. Worker node lifecycle is managed through the Platform UI when <Link to="/docs/vcluster/deploy/worker-nodes/private-nodes/auto-nodes/">Auto Nodes</Link> are used, and provisioning is backed by [Node Providers](/docs/platform/administer/node-providers/overview).

```yaml title="Example: scale control plane nodes"
controlPlane:
  standalone:
    autoNodes:
      quantity: 3
...
Self-managed standalone vCluster can be connected to <Link to="/docs/platform">vCluster Platform</Link>. Once connected, configuration updates and version upgrades can be coordinated from the platform, but this does not make the nodes platform-managed.

Use the following command to connect a standalone cluster:

```bash title="Connect a vCluster Standalone to vCluster Platform"
...
<Flow>
  <Step>
    Create a control plane join token on an existing control plane node.
  ...
  <Step>
    Run the returned join command on each new control plane node.

    ```bash title="Join a control plane node"
...
For control plane nodes that are joined in the cluster (`controlPlane.standalone.joinNode.enabled: true`), first follow the [worker node removal steps](../../worker-nodes/private-nodes/manage.mdx#remove-worker-node) and [node cleanup](../../worker-nodes/private-nodes/manage.mdx#clean-up-orphaned-node).
  ...
  <TabItem value="cp2cp">
    <Flow>
      <Step>
        ...
        ```bash
🧹 Resetting vCluster installation...
✅ Reset complete.
        ```
      </Step>
      <Step>
...
✅ Reset complete.
      ...
      <Step>
        From the primary node for the cluster, create a node join token.
...
curl -fsSLk "https://10.244.0.16:8443/node/join?token=hzndlx.vhf08ob7pg97r2hb" | sh -
      ...
      <Step>
        Back on your node, in order to join you'll need to add `--force-join` flag to the command.
        ...
        <InterpolatedCodeBlock
          code={`Detected OS: ubuntu
Preparing node for Kubernetes installation...
Kubernetes version: ${DEFAULT_K8S_VERSION}
