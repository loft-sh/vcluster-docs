...
When a standalone cluster is connected to <Link to="/docs/platform">vCluster Platform</Link>, control plane nodes can be removed from the Platform UI. Worker nodes can also be removed from the Platform UI when <Link to="/docs/vcluster/deploy/worker-nodes/private-nodes/auto-nodes/">Auto Nodes</Link> are used.
