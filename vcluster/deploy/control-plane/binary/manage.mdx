...
✅ Reset complete.
      ...
      <Step>
        Create node join token in vCluster you want to join this node into.
...
curl -fsSLk "https://10.244.0.16:8443/node/join?token=hzndlx.vhf08ob7pg97r2hb" | sh -
      ...
      <Step>
        In order to join the node, you'll need to add `--force-join` flag to the join command.
...
Run 'kubectl get nodes' on the control-plane to see this node join the cluster.
  ...
  <TabItem value="wn2cp">
    <Flow>
      <Step>
        Follow steps in [Worker Nodes Documentation](../../worker-nodes/private-nodes/manage.mdx) to reset the node.
      </Step>
      <Step>
