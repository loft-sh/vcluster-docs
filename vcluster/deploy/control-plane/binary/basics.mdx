...
To access the cluster from other machines, copy the kubeconfig from `/var/lib/vcluster/kubeconfig.yaml` on the control plane node, or use the vCluster CLI to [generate access credentials](../../../manage/accessing-vcluster), then replace the server field with relevant IP or DNS in order to access the cluster.
