...
### Worker nodes
 ...
 With vCluster Standalone, worker node pools can only be [private nodes](../../worker-nodes/private-nodes/overview.mdx). Since there is no Control Plane Cluster,
...
ip-192-168-3-131   Ready    control-plane,master   11m   ${DEFAULT_K8S_VERSION}`}
          language="bash"
        />
...
1. Add a [Node Provider](/docs/platform/next/vmetal/concepts/machine-types-and-nodetypes).
2. Add the vCluster configuration (example below).
3. Provision the cluster from the platform UI.

For the infrastructure side of this, including how to size and select a control plane node type, what the OS image has to provide, and how to diagnose an install that fails on the server, see [Control plane machines](/docs/platform/next/vmetal/deploy/control-plane-machines).

<PageVariables K8S_VERSION={DEFAULT_K8S_VERSION} NODE_PROVIDER="aws" />
