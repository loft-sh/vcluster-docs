---
title: vCluster Standalone
sidebar_label: As a binary - Standalone
sidebar_position: 2
sidebar_class_name: standalone private-nodes
---

import Standalone from '../../../_fragments/standalone.mdx'
import PrivateNodeLimitations from '../../../_fragments/private-nodes-limitations.mdx'

import TenancySupport from '../../../_fragments/tenancy-support.mdx';

<TenancySupport standalone="true" />

<Standalone />

## Compute resource requirements
vCluster standalone can be deployed on bare metal, virtual machines of containers. The minimal recommended resource requirements for vCluster Standalone are as follows:
- 0.2 vCPU
- 512 MiB of RAM
- 1 GiB of ephemeral storage

## Installation overview

vCluster Standalone needs its own set of nodes for the control plane, and you can add extra nodes later if you want them to act as workers. 
The steps are to bring up a vCluster Standalone cluster are:

1. Install an initial control plane node.
2. (Optional) Join additional control plane nodes.
3. (Optional) Join worker nodes.

## vcluster.yaml configuration

To create a vCluster Standalone with one node that is the control plane and worker node. Read more about [high availability](./high-availability).

```yaml title="Enable standalone"
controlPlane:
  # Enable standalone
  standalone:
    enabled: true
    joinNode:
      enabled: true  # Optional: Control Plane node will also be considered a worker node 
  distro:
    k8s:
      image:
        tag: v1.31.2 # Kubernetes version you want to use
  service:
    spec:
      type: LoadBalancer # If you want to expose vCluster using LoadBalancer (Recommended option)

# Required for adding additional worker nodes
privateNodes: 
  enabled: true
  
# Networking configuration
networking:
  # Specify the pod CIDR
  podCIDR: 10.64.0.0/16
  # Specify the service CIDR
  serviceCIDR: 10.128.0.0/16
```



<PrivateNodeLimitations />