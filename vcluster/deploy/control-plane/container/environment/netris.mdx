---
title: Deploy on NVIDIA DGX SuperPod with Netris
sidebar_label: NVIDIA DGX
sidebar_position: 5
id: netris
...
We deploy vCluster on a SuperPod on bare metal while being isolated from any other clusters or infrastructure through an exclusive L2 domain using vlan.

We add bare metal DGX servers & VMs on management nodes as private nodes.
...
This is transparent to the vCluster, and the host cluster remains inaccessible to the vCluster.

This can be repeated for any number of clusters, the limitations being the exhaustion of compute resources and vlan ids.
...
- Netris:
    - Software vendor
    - configures switch fabric for isolated virtual networks

## Prerequisites

We assume the following:

- Nvidia SuperPod deployed
- Netris controller & agent installed & configured
- Bare metal Kubernetes "host cluster" (See [below](#prepare-host-cluster))
...
The host cluster gets its own vlan, which spans all management servers.
...
2. Create ServerCluster in netris
    ...
    Create a ServerCluster in netris using the template and add the management servers to it.

    This configures the ports on the management servers as access ports with the same vlan.

    ![ServerCluster](/img/netris_create_host_cluster_servercluster.png)

### Install Kubernetes

Most Kubernetes distributions should work.
...
The _Whereabouts_ IPAM plugin assigns well-known IPs to the vCluster control plane Pods and load balancer.
It tracks the allocations of the pool cluster wide, to avoid IP conflicts between different nodes.

<!-- vale off --> // vale doesn't like the capitalization
#### KubeVirt
<!-- vale on -->
...
To use private nodes, which is an enterprise features, we need to add the vCluster to the vCluster platform.

To start it in the host cluster, run:
...
2. Create ServerCluster
    ...
    Create a ServerCluster in netris using the template and add the management servers to it as shared endpoints.

    This configures the ports on the management servers as trunk ports (additionally) carrying tagged vlan traffic.

    ![ServerCluster](/img/netris_create_vcluster_servercluster.png)

3. Look up the Vnet's vlan id

    Netris creates a vnet and assign it a vlan id.
...
2. Create vlan interface and bridge
    ...
    The brigde operator creates a bridge with the name `br-vlan-3` on every Node and enslave the vlan interface connected to the default interface.
    Egress from any (vlan unaware) device connected to the bridge is tagged and thus be part of the L2 domain of the vCluster.
...
2. Deploy load balancer in front of vCluster Pods
    ...
    A kube-vip sidecar assigns the IP only one of the load balancer Pods at a time and provides active failover.
    ...
    The Pods may reside on the same host as the load balancer, then the traffic would remain on the bridge and not be tagged.
    In this case, pick a different Pod to run the command.

## Join bare metal DGX server

1. Add DGX server to ServerCluster

    ![Add DGX server](/img/netris_add_vcluster_dgx_server.png)

2. Provision DGX server with OS and configure the default interface (bond)
...
4. Run join command on DGX server

    This downloads the necessary Kubernetes binaries and container runtime, configure Kubelet and join the vCluster as a new node.
...
5. Verify that the DGX server joined the vCluster
