...
We deploy vCluster on bare metal while being isolated from any other clusters or infrastructure through an exclusive L2 domain using vlan.
To configure the switch fabric, we use [Netris](https://netris.ai/).

We add bare metal servers and run VMs on the host cluster nodes as vCluster private nodes.
...
Then, the switches must be configured manually.
...
To use private nodes, which is an enterprise feature, we need to add the vCluster to the vCluster platform.
...
2. Create vlan interface and bridge
    ...
    Egress from any (vlan unaware) device connected to the bridge is tagged and thus part of the L2 domain of the vCluster.
