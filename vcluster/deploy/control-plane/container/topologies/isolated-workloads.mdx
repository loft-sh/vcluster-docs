...
for the syncer component. If you enable either `baseline` or `restricted` policy, it follows the standards outlined in Kubernetes. For exampke, with `baseline` as a Pod Security Standard,
...
pods stay pending in the vCluster and in newer Kubernetes version they will be denied by the admission controller
as well.
