...
1. **Recommended approach (Flux v0.40.0+)**: Use `skipTLSVerify: true` in the `kubeConfig` section as shown above, which tells Flux to skip certificate verification when connecting to the virtual cluster.

2. **Alternative approach**: Configure both the TLS SAN and `insecure: true` in your vCluster configuration as we did in the example.

3. **If you still encounter certificate errors**: Use a modified Secret created with the solution in the [troubleshooting section](#tls-certificate-verification-errors):
...
1. Detect the new vCluster configuration
2. Create the required namespace
3. Deploy the vCluster using the Helm chart
4. Create the KubeConfig Secret
5. Use the exported KubeConfig to deploy apps to the vCluster
