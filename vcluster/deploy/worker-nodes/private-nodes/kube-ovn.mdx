...
description: Route directly to tenant pod IPs and expose LoadBalancer services on kube-ovn VIPs for private-nodes clusters, without an external load balancer or MetalLB.
...
integration lets a private-nodes cluster route traffic through the
...
The same fields are available in vCluster Platform. On a cluster in
...
form appears only for clusters running vCluster 0.37 or later.
:::
...
3. **The VPC and its subnets exist.** The `Vpc` named in `kubeOvn.vpc` and the
   ...
   creates these through network environment properties. See [kube-ovn
   networking](/docs/platform/administer/node-providers/kubevirt#integrate-with-kube-ovn).
...
5. **Egress out of the custom VPC configured.** Tenant nodes need egress from the
   ...
   the network environment's `kube-ovn.vcluster.com/egress-enabled` property. See
   [kube-ovn
   networking](/docs/platform/administer/node-providers/kubevirt#integrate-with-kube-ovn).
