...
A node can be re-used for another cluster by using the `--force-join` flag. Before joining the node to a new vCluster, it
...
Before loading any images to the node, the node has to be in a `Ready` state and part of a cluster. The command pulls the image locally
