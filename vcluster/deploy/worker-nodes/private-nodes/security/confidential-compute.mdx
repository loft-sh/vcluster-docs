...
- **Pod-level runtime** (Kata Containers, Confidential Containers): each pod
  ...
  node image, a RuntimeClass object available to the cluster, and a
...
strongest isolation guarantee, assign private nodes to a single cluster
...
node type when a cluster requests TDX-capable capacity.
...
image, and join each server to the cluster as a private node. Any node
...
A platform admin configures the cluster to use confidential nodes by
requiring the confidential-capable node type property. vCluster Platform
provisions matching nodes when the cluster needs capacity.

```yaml title="vcluster.yaml — platform admin"
privateNodes:
  # Enable dedicated worker nodes for this cluster.
  enabled: true
  autoNodes:
    - provider: bare-metal
      dynamic:
...
the cluster needs confidential-capable worker capacity.

Private nodes join the cluster as real nodes, so their OS-level labels
...
RuntimeClass installed in the control plane cluster and exposed to the
cluster. RuntimeClass syncing is disabled by default, so the platform admin must
sync the allowed RuntimeClass objects into the cluster before tenants can
reference them.
