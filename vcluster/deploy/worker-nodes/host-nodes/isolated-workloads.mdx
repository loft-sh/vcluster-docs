...
vCluster offers several different policies to automatically isolate workloads in a tenant
cluster. For all options within each policy, review the [policies documentation](../../../configure/vcluster-yaml/policies/overview.mdx).
...
container or mount a host path are not synced to the control plane cluster. Though Pod Security Standard is a Kubernetes concept and only applicable
...
which affect scheduling decisions on the shared control plane cluster. To prevent the pods from being scheduled
to the undesirable nodes, you can use the `sync.fromHost.nodes.selector.labels` to select specific nodes from the control plane cluster.
