...
:::important
Enterprise features require the vCluster Platform, even in air-gapped environments. 
...
:::note
You must have the vCluster Platform deployed in your air-gapped environment. For more information, see the [platform air-gapped installation guide](/platform/install/advanced/air-gapped) for instructions.
:::
...
Before deploying vCluster, you should [create an access key](/vcluster/configure/vcluster-yaml/external/platform/api-key) to authenticate your virtual cluster:

1. Generate an access key in the vCluster Platform.
2. Assign the `vcluster` role to the access key.
3. Create a Kubernetes Secret that contains the access key.
