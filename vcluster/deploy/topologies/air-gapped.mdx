...
- vCluster Platform deployed in your air-gapped environment. See the [platform air-gapped installation guide](/platform/next/install/advanced/air-gapped) for instructions.
...
## Create platform access key {#create-platform-access-key}

Before deploying vCluster, you need to create an access key in the platform to authenticate your virtual cluster. Follow the instructions in the [API key configuration guide](/vcluster/next/configure/vcluster-yaml/external/platform/api-key) to:

1. Create an access key in the vCluster Platform
2. Configure the access key with the required `vcluster` role
3. Create a Kubernetes Secret containing the access key

This step is essential for connecting your air-gapped vCluster to the platform.

## Deploy vCluster {#deploy-vcluster}

Now that you have the platform deployed, created an access key, and populated your private registry with all necessary images and charts, you can deploy vCluster.
...
For more details on FIPS, refer to the [FIPS configuration guide](/vcluster/next/deploy/security/fips).
