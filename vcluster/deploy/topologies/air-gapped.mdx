...
If you use the `sync.toHost.pods.rewriteHosts` feature, manually replace the full path of the Alpine image with your private registry path.  
...
<Flow>
  ...
  **Optional**: Create a secret to authenticate to your private registry.
  ...
  Use the `vcluster.yaml` file with the necessary configuration for your air-gapped vCluster installation: 
