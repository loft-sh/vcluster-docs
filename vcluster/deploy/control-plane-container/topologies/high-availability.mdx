...
use cases, you want to run vCluster with more redundancy. We recommend deploying vCluster in High Availability (HA) mode 
...
In order to enable the HA mode, you'll need to change the backing store from the embedded database (SQLite) to any of the other [backing store options](../../../configure/vcluster-yaml/control-plane/components/backing-store). 
