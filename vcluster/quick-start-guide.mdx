...
Deploy a tenant cluster called `my-vcluster` to namespace `team-x` on a Control Plane Cluster. This guide uses the vCluster CLI, but there are multiple ways to deploy vCluster using other tools like Helm, Terraform, ArgoCD, ClusterAPI, etc. Read about the additional options in the [deployment section of the docs](./deploy/control-plane/kubernetes-pod/basics.mdx).
...
<Flow id="deploy-vcluster-docker">
  ...
  <Step>
    Check out the namespaces in your tenant cluster. When the tenant cluster deployment finishes, your kube context is automatically updated to the tenant cluster.
...
After creating your tenant cluster, your kube context is updated to point to the tenant cluster and all these commands are directed at it.

### Deploy resources inside the tenant cluster
...
Unlike Kubernetes deployments, Docker-based tenant clusters are fully self-contained. All resources exist within the Docker containers that make up your tenant cluster—no Control Plane Cluster required. The containers simulate a complete Kubernetes environment without any underlying cluster.
:::
...
Deleting a vCluster also deletes all resources within the tenant cluster and all state related to it.
