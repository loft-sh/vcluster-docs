...
An ingress controller manages external HTTP/S traffic to services in a Kubernetes cluster. To configure sleep mode for the ingress controller, make sure it stays responsive to incoming traffic and can wake up the cluster if needed. The objective is to keep the ingress controller active, even when the vCluster is in sleep mode. This enables the ingress controller to handle requests that trigger the vCluster to wake up.
