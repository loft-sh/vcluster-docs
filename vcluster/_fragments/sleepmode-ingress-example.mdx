...
#### In this example we will
 ...
 - Test the ingress after waking to show it woke successfully
...
### 3. Install the NGINX `IngressController`
...
Keep trying the `Ingress` endpoint within the `30 second` activity window with

`curl --silent backend.local/bar`

You should see the name of whichever pod in the `Deployment` responds.
