...
:::info
This example continues from the [Istio integration example](../configure/vcluster-yaml/integrations/istio.mdx).
:::

<Flow id="sleepmode-ingress-example">
  <Step>
    Complete the [Istio integration example](../configure/vcluster-yaml/integrations/istio.mdx).
  </Step>
  ...
  <Step>
    ...
    Use the following `vcluster.yaml` to update your tenant cluster on your control plane cluster with `--upgrade`.
  ...
  <Step>
    Enable local DNS resolution for the tenant cluster.
  ...
  <Step>
    Allow the tenant cluster to go to sleep.
