...
<Flow id="sleepmode-ingress-example">
  ...
  <Step>
    ...
    Use the following `vcluster.yaml` to update your cluster on your control plane cluster with `--upgrade`.
  ...
  <Step>
    Enable local DNS resolution for the cluster.
  ...
  <Step>
    Allow the cluster to go to sleep.
