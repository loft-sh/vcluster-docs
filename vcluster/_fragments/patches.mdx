...
If the `path` in a patch references a field where the parent object is `null` or doesn't exist, vCluster will fail to create the object in the host cluster and log an error in the vCluster container.
For example, if you specify `path: spec.template.metadata.labels["my-label"]` but `spec.template` is `null`, the patch will fail and you'll see an error in the vCluster logs.
:::
