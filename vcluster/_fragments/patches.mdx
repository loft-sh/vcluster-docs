...
If the `path` in a patch references a field where the parent object is `null` or doesn't exist, vCluster logs an error in the vCluster container but it will not block the synchronization to the host.
