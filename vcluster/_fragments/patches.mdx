...
These functions are useful for encoding/decoding values or translating services when syncing between clusters:
<CodeBlock language="yaml">
{`sync:
    toHost:
      ${props.resource}:
        ...
        - path: metadata.annotations["encoded-value"]
          expression: 'valueExists ? btoa(value) : null'
        - path: metadata.annotations["decoded-value"]
          expression: 'valueExists ? atob(value) : null'
        - path: metadata.annotations["referenced-service"]
          expression: 'valueExists ? virtualToHostDNS(value) : null'`}
</CodeBlock>
:::
