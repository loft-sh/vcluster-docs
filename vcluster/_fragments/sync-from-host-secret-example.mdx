...
This guide shows how to sync Kubernetes Secret from control plane clusters and how you can use them in your workload running inside clusters.
...
Enable the from host syncing for secrets in your cluster configuration:
...
### Sync Secret to cluster and use it in pod
...
<Flow id="secrets-from-host-example">
  ...
  #### Ensure that Secret got synced to the cluster

  Your Secret should be now accessible in the cluster.
  Keep in mind, that any edit made in the virtual object is overwritten by the host object data.

  <Step>
    Check if the Secret is accessible in the cluster:
  ...
  <Step>
    ...
    From host Secret syncing allow you to make specific Secret(s) from control plane clusters accessible inside your clusters. You can make them accessible from different namespaces and/or with different names in the cluster.
    They are always synced from the control plane to the cluster, so it is also possible to sync one control plane cluster Secret to multiple cluster ones.
