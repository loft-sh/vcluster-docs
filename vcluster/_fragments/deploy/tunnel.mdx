import Tunnel from '../../_partials/config/privateNodes/tunnel.mdx'

Tunnel allows you to connect vCluster worker nodes to the vCluster control plane through the platform. 
...
Tunnel builds a vpn across the control plane and the worker nodes via an in-built [tailscale VPN](https://tailscale.com/). 
You don't need a tailscale account for tunnel to work as vCluster only reuses tailscale as a library under the hood and will never reach out to any tailscale services directly.
The only requirement for tunnel to work is that both the vCluster control plane as well as the worker nodes can reach the platform url (e.g. `https://platform.my-domain.com`).

In most cases tunnel will be able to figure out a direct connection between worker nodes and the control plane even if they are not exposed via a public ip or can reach each other directly. 
...
privateNodes:
  enabled: true
  tunnel:
    enabled: true   # This enables node to control plane tunnel
    nodeToNode:
      enabled: true # This enables node to node tunnel
```

## When to use tunnel

### Tunnel is useful in the following scenarios:
...
### When to *not* use tunnel:
* Only the control plane cannot reach the worker nodes directly. This is already solved by using [Konnectivity](https://kubernetes.io/docs/tasks/extend-kubernetes/setup-konnectivity/) and does not require vCluster tunnel or any extra configuration
...
* You cannot change the networking setup on the node. Tunnel will create a new network interface (tailscale0) and needs permissions to create it
* When the goal is to isolate node networking. While tunnel will encrypt all traffic, it cannot prevent nodes reaching each other or block traffic between them. If that is the goal, use a proper VLAN or underlay setup for this. 

:::tip Only use tunnel if you have to
If there is a direct way for node-to-node or node-to-control-plane communication it's usually preferred over using tunnel.
:::

## How does it work?

When enabled, the join script will setup a systemd service called `vcluster-tunnel` that connects the node to the platform. The service runs a modified version of [tailscaled](https://tailscale.com/kb/1278/tailscaled).
...
## Node to control plane tunnel

Node to control plane tunnel allows your nodes to communicate to the control plane over the tailscale vpn.
...
Node to node tunnel allows your nodes to communicate with each other over the tailscale vpn. 
...
To configure node to node tunnel, you just need to set the following in your `vcluster.yaml`:
```yaml
privateNodes:
  enabled: true
  tunnel:
...
Using node to node tunnel requires node to control plane tunnel, so you cannot opt-out of that.
...
If you experience any problems with the connection, you can check the vCluster tunnel logs directly or via the [tailscale CLI](https://tailscale.com/kb/1080/cli)
...
### Check vCluster tunnel logs

vCluster tunnel is installed as a systemd service on the node that should join, you can check the logs via the following command on the node:
```
journalctl -u vcluster-tunnel --no-pager
```

### Check with tailscale CLI

:::warning Do not install tailscaled daemon
Make sure to only install the CLI and NOT tailscaled as vCluster tunnel is replacing that and if you install tailscale the regular way it will override and conflict with vCluster tunnel.
:::
