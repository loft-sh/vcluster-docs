import { DEFAULT_K8S_VERSION } from '../default-k8s-version.mdx';
...
<InterpolatedCodeBlock
  code={`$ kubectl get node -o wide
NAME       STATUS   ROLES    AGE     VERSION   INTERNAL-IP    EXTERNAL-IP   OS-IMAGE   KERNEL-VERSION  CONTAINER-RUNTIME
my-node    Ready    <none>   5d19h   ${DEFAULT_K8S_VERSION}   100.64.0.5     <none>        Ubuntu     6.14.10         containerd://2.0.5`}
  language="bash"
/>
...
When changing the way the cluster control plane is exposed, for example switching from LoadBalancer to VPN, you will need to re-join the nodes to pick up the new settings.

### Configure firewall to enable Tailscale to establish peer-to-peer connections

While it is possible to utilize the vCluster Platform as a DERP relay server, there are cases where the additional latency is not acceptable. You may check whether your nodes are using a relay using the `tailscale status` command:
```
1           2         3          4         5
...
If you notice that two devices are connected over a relay server, you can try opening firewall ports to enable them to connect peer-to-peer (direct):
- UDP connections from `:41641` to `*:*`.
- UDP connections to `*:3478`
- TCP connections to `*:80`
- TCP connections to `*:443`

In some cases it may be enough to enable connections to UDP port 41641.
