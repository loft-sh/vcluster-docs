...
This guide shows how to sync Kubernetes config maps from control plane clusters and how you can use them in your workload running inside clusters.
...
Enable the from host syncing for ConfigMap in your cluster configuration:
...
### Sync ConfigMap to cluster and use it in pod
...
<Flow id="config-maps-from-host-example">
  ...
  #### Ensure that ConfigMap got synced to the cluster

  Your ConfigMap should be now accessible in the cluster.
  Keep in mind, that any edit made in the virtual object is overwritten by the host object data.

  <Step>
    Check ConfigMap in the cluster:
  ...
  <Step>
    <Highlight color="secondary">Cluster</Highlight> Wait for pod running
  ...
  <Step>
    ...
    From host ConfigMap syncing allow you to make specific ConfigMap(s) from control plane clusters accessible inside your clusters. You can make them accessible from different namespaces and/or with different names in the cluster.
    They are always synced from the control plane to the cluster, so it is also possible to sync one control plane cluster ConfigMap to multiple cluster ones.
