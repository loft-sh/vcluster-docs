...
This guide shows how to sync Kubernetes config maps from control plane clusters and how you can use them in your workload running inside tenant clusters.
...
then, create a namespace in your control plane cluster, use `foobar2` as an example:
...
Enable the from host syncing for ConfigMap in your tenant cluster configuration:
...
### Sync ConfigMap to tenant cluster and use it in pod

<Flow id="config-maps-from-host-example">
  <Step>
    First, you create a ConfigMap that you want to sync in the control plane cluster:
    ...
    then, create a ConfigMap containing this file in the control plane cluster:
  ...
  #### Ensure that ConfigMap got synced to the tenant cluster

  Your ConfigMap should be now accessible in the tenant cluster.
  Keep in mind, that any edit made in the virtual object is overwritten by the host object data.

  <Step>
    Check ConfigMap in the tenant cluster:
  ...
  <Step>
    ...
    From host ConfigMap syncing allow you to make specific ConfigMap(s) from control plane clusters accessible inside your tenant clusters. You can make them accessible from different namespaces and/or with different names in the tenant cluster.
    They are always synced from the control plane to the tenant cluster, so it is also possible to sync one control plane cluster ConfigMap to multiple tenant cluster ones.
