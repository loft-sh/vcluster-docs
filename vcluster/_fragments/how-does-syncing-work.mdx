...
To map a host object back to its tenant cluster name, read the `vcluster.loft.sh/object-name` and `vcluster.loft.sh/object-namespace` annotations on the host object. vCluster sets these on every synced object, and they always carry the untruncated names.
