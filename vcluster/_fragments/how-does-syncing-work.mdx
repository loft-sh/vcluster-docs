...
All fields of a resources are synced from either the tenant cluster to the control plane cluster or the inverse (that is, from the control plane cluster to the tenant cluster). The direction on how
...
For example, a ServiceAccount named `payments-service-account-with-a-very-long-name` in the `production-workloads` namespace of a tenant cluster named `team-alpha-vcluster` has this full rewritten name:
...
To map a host object back to its tenant cluster name, read the `vcluster.loft.sh/object-name` annotation on the host object. vCluster sets it on every synced object, and it always carries the untruncated name.

For namespaced objects, `vcluster.loft.sh/object-namespace` carries the tenant cluster namespace. Cluster-scoped objects have no namespace, so vCluster removes that annotation from them. Reverse-mapping automation must tolerate its absence rather than assume both annotations are present.

:::info Deriving synced names in automation
External systems that key off synced object names see the truncated name, not the name inside the tenant cluster. ServiceAccount-dependent integrations are the most common place this surfaces, including EKS Pod Identity, IRSA, GKE Workload Identity, and AKS Workload Identity. An IAM trust policy that references the untruncated name silently fails to match.
