vCluster syncs specific Kubernetes resources from the cluster to the control plane cluster. These resources are expected to be created and updated within the cluster.
...
<!-- vale off -->
### Added by the cluster
<!-- vale on -->
When syncing objects from the cluster to the control plane cluster, the cluster adds management labels to the control plane cluster-side objects.
...
:::note
If `sync.toHost.namespaces.enabled` is `true` (multi-namespace sync), the cluster does not add the release label. Only the `vcluster.loft.sh/*` labels are applied.
:::

### Bidirectional syncing

For all resources synced from the cluster to the control plane cluster, `metadata.labels` and `metadata.annotations` are synchronized in both directions.
This means changes made in the cluster are reflected in the control plane cluster, and changes made in the control plane cluster are also reflected in the cluster.
...
All `metadata.labels` and `metadata.annotations` are bidirectionally synced for resources synced from the cluster to the control plane cluster.  
