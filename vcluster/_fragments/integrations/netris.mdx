...
- [BCM Node Provider with Netris](/platform/next/administer/node-providers/bcm)
- [KubeVirt Node Provider with Netris](/platform/next/administer/node-providers/kubevirt)
...
When the Netris integration is enabled, you can configure it to automatically set up kube-vip for your vCluster control plane.
This uses Netris to allocate a VIP from a specified server cluster and creates the necessary NetworkAttachmentDefinition for Multus.
...
integrations:
  netris:
    ...
    kubeVip:
      ...
      ipRange: "10.100.0.10-10.100.0.20,10.100.0.30-10.100.0.40"
```
...
The bridge name to use in the Multus NetworkAttachmentDefinition for connecting the control plane to the Netris network.
...
For more information on manual kube-vip configuration, see [Kube-vip](/vcluster/next/configure/vcluster-yaml/control-plane/other/advanced/kube-vip).
