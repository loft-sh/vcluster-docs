...
Following fields of `Gateway` are modified by vCluster during the sync to the control plane cluster:
 ...
 - additional labels `vcluster.loft.sh/managed-by: [YOUR CLUSTER NAME]` and `vcluster.loft.sh/namespace: [VIRTUAL NAMESPACE]` are automatically added to the `spec.subsets[*].labels`
