...
This guide shows how to set up Istio integration with your tenant cluster.
This enables you to use one Istio installation from the control plane cluster instead of installing Istio in each tenant cluster.

You can include your tenant workloads in the mesh by setting `istio.io/dataplane-mode=ambient` label on the tenant Namespaces or Pods.
You can exclude your tenant workloads from the mesh by setting `istio.io/dataplane-mode=none` label either on the Namespace or on the Pod.
...
- `istio` operator installed on your control plane cluster in ambient mode with DNS Capture disabled
...
Enable the Istio integration in your tenant cluster configuration:
...
- Installs Resource Definitions for `DestinationRules`, `Gateways` and `VirtualServices` into the tenant cluster.
- Exports `DestinationRules`, `Gateways` and `VirtualServices` from the tenant cluster to the control plane cluster (and re-writes) service references to the services translated names in the control plane cluster.
...
<Flow id="istio-example">
  ...
  between them. You can adjust the values below according to your setup. Here,
  the `HOST_CONTEXT` is the context of the control plane cluster, while the `VCLUSTER_CONTEXT` is
  the context to access the created vCluster. The `VCLUSTER_HOST_NAMESPACE` is the namespace
  where the vCluster is created on the control plane cluster. The `ISTIO_NAMESPACE` is the namespace to deploy the Istio integration.
  ...
  <Step>
    ...
    To make it work, you need a Waypoint proxy in the tenant cluster's control plane namespace.
    ...
    create it in the control plane cluster:
  ...
  <Step>
    ...
    <InterpolatedCodeBlock
      code={`kubectl --context="[[GLOBAL:VCLUSTER_CONTEXT]]" create namespace [[GLOBAL:ISTIO_NAMESPACE]]`}
      language="bash"
...
code={`apiVersion: v1
  ...
  metadata:
    ...
    namespace: [[GLOBAL:ISTIO_NAMESPACE]]
  data:
    index.html: |
      <!DOCTYPE html>
      <html>
...
/>
    ...
    <InterpolatedCodeBlock
      code={`kubectl --context="[[GLOBAL:VCLUSTER_CONTEXT]]" create -f configmap1.yaml --namespace [[GLOBAL:ISTIO_NAMESPACE]]`}
      language="bash"
      title="Create v1 config map"
    />

    <InterpolatedCodeBlock
      code={`kubectl --context="[[GLOBAL:VCLUSTER_CONTEXT]]" create -f deployment1.yaml --namespace [[GLOBAL:ISTIO_NAMESPACE]]`}
      language="bash"
    ...
    <InterpolatedCodeBlock
      code={`kubectl --context="[[GLOBAL:VCLUSTER_CONTEXT]]" wait --for=condition=ready pod -l app=nginx --namespace [[GLOBAL:ISTIO_NAMESPACE]] --timeout=300s`}
      language="bash"
...
/>
    ...
    <InterpolatedCodeBlock
      code={`kubectl --context="[[GLOBAL:VCLUSTER_CONTEXT]]" create -f configmap2.yaml --namespace [[GLOBAL:ISTIO_NAMESPACE]]`}
      language="bash"
      title="Create v2 config map"
    />

    <InterpolatedCodeBlock
      code={`kubectl --context="[[GLOBAL:VCLUSTER_CONTEXT]]" create -f deployment2.yaml --namespace [[GLOBAL:ISTIO_NAMESPACE]]`}
      language="bash"
      title="Create v1 deployment"
    />
...
/>
    ...
    <InterpolatedCodeBlock
      code={`kubectl --context="[[GLOBAL:VCLUSTER_CONTEXT]]" create -f service.yaml --namespace [[GLOBAL:ISTIO_NAMESPACE]]`}
      language="bash"
...
/>
    ...
    <InterpolatedCodeBlock
      code={` kubectl --context="[[GLOBAL:VCLUSTER_CONTEXT]]" create -f client_deployment.yaml --namespace [[GLOBAL:ISTIO_NAMESPACE]]`}
      language="bash"
  ...
  <Step>
    ...
    You can create `DestinationRules` and `VirtualService` in the tenant cluster.
    ...
    Save this `DestinationRule` and `VirtualService` definition, and apply it in the tenant cluster:
...
<InterpolatedCodeBlock
  ...
  metadata:
    ...
    namespace: [[GLOBAL:ISTIO_NAMESPACE]]
  spec:
    host: nginx-service.[[GLOBAL:ISTIO_NAMESPACE]].svc.cluster.local # vCluster translates it to the host service automatically
    subsets:
      - name: v1
        labels:
          version: v1
...
<InterpolatedCodeBlock
  ...
  spec:
    hosts:
      - nginx-service.[[GLOBAL:ISTIO_NAMESPACE]].svc.cluster.local # vCluster translates it to the host service automatically
    http:
      - name: "nginx-v2"
        match:
          - uri:
...
/>
    ...
    To apply a `DestinationRule` configuration to the tenant cluster specified by the `VCLUSTER_CONTEXT` context, use the following command:
  ...
  ### Verify that DestinationRule and VirtualService is synced to the control plane cluster
  ...
  <Step>
    ...
    <InterpolatedCodeBlock
      ...
      language="bash"
      title="Check destination rule in the control plane cluster"
    />
    ...
    />
    You should see an output mapping from a hash value prefixed with `v` to `HOST`:
    <InterpolatedCodeBlock
      code={`
    $ kubectl --context="[[GLOBAL:HOST_CONTEXT]]" get destinationrules --namespace "[[GLOBAL:VCLUSTER_HOST_NAMESPACE]]"
    NAME            HOST                                                   AGE
    v7f8a9b2cafds   nginx-destination-x-[[GLOBAL:ISTIO_NAMESPACE]]-x-vcluster       1m

    $ kubectl --context="[[GLOBAL:HOST_CONTEXT]]" get virtualservices --namespace "[[GLOBAL:VCLUSTER_HOST_NAMESPACE]]"
    NAME        GATEWAYS   HOSTS                                                                 AGE
    v3d4e5f6g   ...        ["nginx-service-x-[[GLOBAL:ISTIO_NAMESPACE]]-x-vcluster.[[GLOBAL:VCLUSTER_HOST_NAMESPACE]].svc.cluster.local"]       2m
      `}
    />
  ...
  <Step>
    ...
    <InterpolatedCodeBlock
      code={`kubectl --context="[[GLOBAL:VCLUSTER_CONTEXT]]" exec -it -n [[GLOBAL:ISTIO_NAMESPACE]] deploy/client -- curl nginx-service/v2
      <!DOCTYPE html>
      <html>
      <head>
        <title>Welcome to nginx v2!</title>
    ...
    <InterpolatedCodeBlock
      code={`kubectl --context="[[GLOBAL:VCLUSTER_CONTEXT]]" exec -it -n [[GLOBAL:ISTIO_NAMESPACE]] deploy/client -- curl nginx-service
      <!DOCTYPE html>
      <html>
      <head>
        <title>Welcome to nginx v1!</title>
  ...
  <Step>
    ...
    Istio integration enables you to re-use one Istio instance from the control plane cluster for multiple tenant clusters.
    Tenant cluster users can define their own `Gateway`, `DestinationRule` and `VirtualService` without interfering with each other.

  </Step>
</Flow>

## Fields translated during the sync to control plane cluster

Following fields of `Gateway` are modified by vCluster during the sync to the control plane cluster:
 - reference to the TLS Secret is re-written `spec.servers[*].tls.credentialName`. Secret is automatically synced to the control plane cluster.
 - namespace, `.` and `*` prefix, followed by `/` is stripped from `spec.servers[*].hosts[*]`, so e.g. `foo-namespace/loft.sh` becomes `loft.sh` in the control plane cluster object.
...
For additional information how Secret and Service references are translated, read [How does syncing work?](../../configure/vcluster-yaml/sync/overview.mdx#how-does-syncing-work)

Following fields of `DestinationRule` are modified by vCluster during the sync to the control plane cluster:
  - reference to the virtual Kubernetes Service is re-written for `spec.host`
  - reference to the TLS Secret in `spec.trafficPolicy.portLevelSettings[*].tls.credentialName` & `spec.trafficPolicy.tls.credentialName` is re-written. Secrets are automatically synced to the control plane cluster.
  - additional labels

Following fields of `VirtualService` are modified by vCluster during the sync to the control plane cluster:
