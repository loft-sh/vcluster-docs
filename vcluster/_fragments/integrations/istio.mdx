...
<Flow id="istio-example">
  ...
  <Step>
    Next, you create 3 deployments: two of them are nginx server and you will use third one to curl other two.
  ...
  <Step>
    ...
    kubectl --context="${HOST_CTX}" get destinationrules --namespace vcluster
    ```

    ```bash title="Check virtual service in the host cluster"
    kubectl --context="${HOST_CTX}" get virtualservices --namespace vcluster
    ```
  ...
  <Step>
    Now, you can exec into the client pod, and check if we will get a response containing "Hello from Nginx Version 2!" or "Hello from Nginx Version 1!" depending on request path:
