...
vCluster supports syncing only one version of a custom resource. If you specify multiple versions, the cluster fails to start.
:::

### Upgrade custom resources with explicit CRD API versions

vCluster supports syncing only one version of a CRD. If multiple versions are specified in the sync configuration, the cluster fails to start.

vCluster modifies the CRD inside the cluster during upgrades if it detects that a required version is missing. The following scenarios describe how vCluster handles CRD versions when you upgrade the cluster or change the sync configuration.
...
- If the CRD's storage version in the control plane cluster is `v1`, nothing happens and the CRD in the cluster remains as `certificaterequests.cert-manager.io/v1`.
