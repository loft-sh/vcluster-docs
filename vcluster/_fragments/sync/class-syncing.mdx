...
Synced {props.resourceClass} resources function like any other Kubernetes resource in the cluster. You
...
When you modify the {props.resourceClass} resource in the control plane cluster, vCluster re-evaluates whether it still matches the selector criteria. If the {props.resourceClass} continues to match, vCluster updates the corresponding resource in the cluster to reflect the changes. If the {props.resourceClass} no longer matches the selector criteria, vCluster removes it from the cluster.

The selector system acts as both a resource filter and a validation mechanism. As a resource filter, it
ensures that vCluster creates only {props.resourceClass} resources matching the defined selector criteria. The selector also functions as a creation validation mechanism - when you create {props.resource} {props.showExtraResource && (<> and {props.extraResource} </>)}resources in the cluster, the resources can only reference {props.resourceClass} resources that exist in the cluster.

### Orphaned resources

When vCluster removes a synced {props.resourceClass} from the cluster due to selector changes or deletion from the control plane cluster,
...
- vCluster records an event on the {props.resource} {props.showExtraResource && (<> or {props.extraResource} </>)}resource in the cluster
