...
- If the control plane cluster has a CRD for the resource, vCluster copies that CRD into the cluster.
- If it doesn't, vCluster looks the resource up in the control plane cluster's discovery data. When an extension API server serves the resource, vCluster generates a schemaless CRD for it in the cluster.
- If neither a CRD nor an extension API server serves the resource, the cluster fails to start.
...
- `kubectl explain` returns no field documentation for the resource inside the cluster.
- The cluster doesn't prune unknown fields.
