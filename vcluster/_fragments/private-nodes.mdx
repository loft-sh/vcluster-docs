...
To remove a node from the cluster, follow standard procedures as with any other Kubernetes distro:
1. Cordon the node (this prevents new workloads being scheduled on the node):

```shell title="Node cordoning"
$ kubectl cordon <node-name>
```

2. Drain the node (this evicts all existing workloads from the pod):

```shell title="Node draining"
$ kubectl drain <node-name> --ignore-daemonsets
```

3. Once no more workloads are running on the node, log into the node terminal and run:
```shell title="reset node"
$ /var/lib/vcluster/node/join.sh --reset-only
```
