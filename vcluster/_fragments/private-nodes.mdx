...
:::tip
Private node setup installs `conntrack` if not present.  For air gapped environments, you should already have `conntrack`
installed, or the relevant package e.g. the conntrack ".deb" for Ubuntu, present locally to mitigate the need for network
access
:::
...
Supported OS:
- Ubuntu 24.04 LTS
- RHEL (RedHat Enterprise Linux 9)
- CentOS Stream 9

For CentOS, iptables have to be manually installed:
...
:::tip
Using `vcluster token create` relies on curl to fetch the installation script, which in turn installs `conntrack` if its not already present.
It is recommended you pre-install `conntrack` and using the `--bundle-path` argument. 
:::
...
In case when virtual cluster was deleted first, there is a way to clean up a node.
When virtual cluster is gone, kubelet, kube-proxy and other components running on the node are still trying to connect to control plane.
You can clean them up using a join script with `--reset-only` flag, as described in [Remove worker node section](#remove-worker-node).
