...
*Pre-requisites on worker nodes*
...
The Node has to be in the `Ready` state and able to run a pod.
...
There is a `vcluster` CLI helper command that does it:
...
The `Node` can be reset and reused in another virtual cluster.

To do this, follow steps from the [Remove section](#remove-worker-node) first, and then [Join worker nodes to the virtual cluster](#join-worker-nodes-to-the-virtual-cluster). **Make sure to pass `--force-join` flag to the installation script**.

## Clean up an orphaned node
In cases where the virtual cluster is deleted first the kubelet, kube-proxy and other components running on the node are still trying to connect to control plane.
