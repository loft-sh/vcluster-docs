...
This enables the integration, imports cluster stores (`ClusterSecretStore`) from the host cluster into the virtual cluster, and exports namespaced stores (`SecretStore`) from the virtual cluster into the host cluster.
**NOTE**: After the namespaced stores have been exported any changes to them in the host cluster are also reflected into the virtual cluster and vice versa.
