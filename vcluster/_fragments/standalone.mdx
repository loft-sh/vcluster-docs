...
Optional flags:
- `--expires=24h` - Set token expiration time
...
To add dedicated worker nodes to your vCluster Standalone cluster, follow the [Private Nodes documentation](../configure/vcluster-yaml/private-nodes/README.mdx#join-worker-nodes) for detailed instructions on joining worker nodes.
...
If your control plane node is also acting as a worker node (`controlPlane.standalone.joinNode.enabled: true`), follow the [worker node removal steps](../configure/vcluster-yaml/private-nodes/README.mdx#remove-worker-node).
...
For a complete list of feature limitations, refer to the [Private Nodes limitations](../configure/vcluster-yaml/private-nodes/README.mdx#other-vcluster-feature-limitations) as they share similar constraints.
