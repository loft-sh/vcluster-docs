...
To add dedicated worker nodes to your vCluster Standalone cluster, follow the [Private Nodes documentation](../vcluster-yaml/private-nodes#join-worker-nodes-to-the-virtual-cluster) for detailed instructions on joining worker nodes.
...
If your control plane node is also acting as a worker node (`controlPlane.standalone.joinNode.enabled: true`), follow the [worker node removal steps](../vcluster-yaml/private-nodes#remove-worker-node).
...
For a complete list of feature limitations, refer to the [Private Nodes limitations](../vcluster-yaml/private-nodes#other-vcluster-feature-limitations) as they share similar constraints.
