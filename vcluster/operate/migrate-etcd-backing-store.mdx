...
- **Back up your etcd data** - `vcluster snapshot create` isn't supported for deployed etcd, so back it up with its own tooling, such as `etcdctl snapshot save`, before migrating (see [Back up and restore](../configure/vcluster-yaml/control-plane/components/backing-store/etcd/deploy.mdx#back-up-and-restore))
...
Create a new values file that enables embedded etcd with the migration flag. Since embedded etcd runs inside the vCluster StatefulSet pod, storage is configured under `controlPlane.statefulSet.persistence` (not under the `backingStore` section like deployed etcd).

The `migrateFromDeployedEtcd: true` flag tells vCluster to copy data from the deployed etcd before switching to embedded mode. During migration, both etcd instances temporarily coexist, with the deployed etcd remaining available as a fallback.
...
Upgrade your vCluster with the migration configuration. Note that you should not use `--reuse-values` for the migration, as it will cause a conflict between the deployed and embedded etcd configurations. Instead, include all necessary values in your migration configuration file.
...
### StatefulSet immutability errors

During migration, you might encounter errors about forbidden StatefulSet spec updates:

```
error when patching: StatefulSet.apps "vcluster-name" is invalid:
...
- The migration converts a Deployment to a StatefulSet (for embedded etcd)
- Kubernetes StatefulSets have strict immutability rules for certain fields
- Some fields like `volumeClaimTemplates` cannot be modified after creation
...
2. Then retry the Helm upgrade - it will recreate the StatefulSet with correct configuration

:::info Why cascade=orphan?
Using `--cascade=orphan` removes the StatefulSet object but leaves pods running, minimizing downtime during the migration process.
:::
...
- [Backup and restore](/docs/vcluster/next/operate/backup-restore/backup) - Create snapshots before and after migration
- [High availability setup](../deploy/control-plane/kubernetes-pod/high-availability.mdx) - Scale your embedded etcd for HA
