...
vCluster pauses the tenant cluster and scales workload pods to 0 before the restore pod attempts to download the snapshot. An invalid snapshot URL or expired credential is therefore discovered after downtime begins. Before restoring, run `vcluster snapshot get` with the same URL and credential options. Continue only if it reports the snapshot as saved. This check doesn't pause the tenant cluster.

If the restore fails for this or any other reason, the process stops. You must retry the restore to avoid leaving the <GlossaryTerm term="tenant-cluster">tenant cluster</GlossaryTerm> in an inconsistent or broken state.
:::

Restore a tenant cluster using the following commands. You can use any snapshot option and set the snapshot URL with different credentials. Whichever credentials you provide, vCluster never writes them into the restore pod's spec. It mounts them from a Kubernetes Secret through `secretKeyRef`, and deletes that Secret along with the pod once the restore finishes.
...
When you run `vcluster create --restore` without `-f`, `--values`, or `--set`, vCluster reads the Helm configuration from the snapshot before installing the tenant cluster. If it can't access the snapshot, the command exits before the new tenant cluster starts. Providing configuration through one of those flags skips this check because vCluster uses the supplied configuration instead.
