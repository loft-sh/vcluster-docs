---
title: Snapshot and restore vCluster
sidebar_label: Snapshot and Restore
sidebar_position: 3
---

There are multiple ways to backup and restore a virtual cluster. vCluster provides a built-in method to create and restore snapshots using its CLI.
...
The vCluster CLI provides the easiest way to back up the etcd datastore. When you run a backup, vCluster creates a temporary pod to save the snapshot at the specified location and automatically determines the configured backing store. The snapshot includes:
...
:::info
The vCluster CLI backup method currently does not support backing up persistent volumes. To back up persistent volumes, use the [Velero backup method](https://velero.io/docs/main/file-system-backup/).
:::
...
Use the following flags with the `oci` protocol when saving snapshots to an OCI image registry:

| Flag                     | Description |
|--------------------------|-------------|
| `username`               | Specifies the OCI registry username |
| `password`               | Specifies the OCI registry password |
| `skip-client-credentials` | Disables the use of local credentials for authentication |
...
This command saves the snapshot as an image tagged `my-tag` in the specified repository.

#### s3

Use the `s3` protocol to store snapshots in an S3-compatible bucket, such as AWS S3 or MinIO.

| Flag                    | Description |
|-------------------------|-------------|
| `access-key-id`         | S3 access key ID for authentication |
| `secret-access-key`     | S3 secret access key for authentication |
| `session-token`         | Temporary session token for S3 authentication |
| `region`                | Region of the S3-compatible bucket |
| `profile`               | AWS profile to use for authentication |
| `skip-client-credentials` | Prevents the use of local credentials for authentication |
...
You can use [Velero](https://velero.io/) to backup virtual clusters.
