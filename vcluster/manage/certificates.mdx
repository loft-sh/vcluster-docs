...
state otherwise the cert commands below stop the operation. In case of
...
<!-- vale off --> 
### Rotate the CA certificate {#rotate-the-ca-certificate}
<!-- vale on --> 
...
[Rotate the CA Certificate](#rotate-the-ca-certificate) still apply.
...
stateful application running that requires graceful termination, ensure
