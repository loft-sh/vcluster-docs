...
### Rotating the CA when running vCluster Private Nodes

After rotating the CA you have to rejoin any previously joined private nodes.

#### Steps

1. **Obtain a new join token by connecting to the virtual cluster**
...
3. **Copy the command and adjust it to include the `--force-join` flag like in the example below**

```bash
curl -fsSLk "https://example.com:443/node/join?token=d34db33f" | sh -s -- --force-join
```

4. **Login into your nodes and execute the above command**
