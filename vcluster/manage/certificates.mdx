...
vCluster client and server certificates (also called leaf certificates) remain valid for 1 year from the initial cluster creation.
...
:::info
Checking and rotating certificates is only supported for clusters using the k8s distribution.
:::

:::important
The cluster Pod must be in a
...
certificate expiration during runtime of a cluster it will still be in
...
The `vcluster certs check` command provides the following information for each certificate of the given cluster:
...
- **Restarts cluster**: Restarts the cluster to load the new certificates.
...
The restart does not affect running application workloads because the underlying cluster nodes and container runtime remain operational during the cluster restart process.
...
This means that connecting to the cluster with previously issued kubeconfigs continues to work.
...
When working with clusters with private nodes, there are [extra steps](#additional-steps-required-for-private-nodes) that need to happen before doing any changes to the CA certificate and
after making the changes to the CA certificate.


### Rotate the CA Certificate {#rotate-ca-certificate}
...
The rotation creates a backup in the cluster's data directory using the following pattern:
...
<Flow>
<Step>
To restore from the backup, access the cluster syncer container:
...
you must re-join the nodes again to the cluster. Each step must be performed on each manually provisioned private node.
...
node from the cluster. If there's no other node for the workload to be re-scheduled to, the
...
After a CA rotation or restoration, you need to re-create a token to join the cluster and join each manually provisioned private node back to the cluster.
...
you need to delete all `NodeClaims` related to the cluster, but those `NodeClaims` are in the Kubernetes cluster that
...
<Flow>
  ...
  <Step>
    Find the relevant `NodeClaims` for the cluster.

    From the list of `NodeClaims`, identify the ones associated to your cluster that had the CA certificate change.

    In this example, if you have rotated the CA certificate of the cluster named `vcluster-2`, then
    the associated `NodeClaim` is `vcluster-5xtx3`.
  </Step>
  <Step>
    Delete the `NodeClaim` of the cluster that had a CA change.

    For the cluster that you had changed the CA certificate, you'll need to delete each `NodeClaim`, which
    removes the `NodeClaim` and deletes the underlying `Node`. vCluster Platform recreates the `NodeClaim`
    and the new nodes re-join the cluster using the new CA certificate.

    ```bash title="Example of deleting the NodeClaim for a single cluster"
