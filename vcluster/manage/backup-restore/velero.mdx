...
You can use [Velero](https://velero.io/) to back up clusters.

Ensure your cluster supports [volume snapshots](https://kubernetes.io/docs/concepts/storage/volume-snapshots/) to allow Velero to backup persistent volumes and persistent volume claims that save the cluster state. Alternatively, you can use [Velero's file system backup](https://velero.io/docs/main/file-system-backup/) with the Node Agent to back up the cluster state.
...
This restores the vCluster workloads, configuration, and state in the cluster namespace.
...
2. After enabling `hostpath-mapper`, install the Velero CLI (as described above), connect to your vCluster, and install Velero inside the cluster:

      ```bash title="Velero inside the cluster"
