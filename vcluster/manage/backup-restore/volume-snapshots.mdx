...
vCluster snapshots enable you to create volume snapshots for PersistentVolumeClaims that are provisioned by CSI drivers. This way you can protect not only your virtual cluster resources and config, but also your application data.

See [How are volume snapshots created](../../understand/volume-snapshots-creation) if you would like to learn more about volume snapshots and how vCluster creates them.
...
1. Install common snapshot-controller, and VolumeSnapshot, VolumeSnapshotContent and VolumeSnapshotClass CRDs. You can find the instructions in the [Kubernetes CSI snapshotter repo](https://github.com/kubernetes-csi/external-snapshotter).
2. Install CSI driver with volume snapshots support, and storage classes for that CSI driver.
...
You can create a vCluster snapshot, with volume snapshots included, by running `vcluster snapshot create <MY VCLUSTER> <SNAPSHOT URL> --include-volumes`. vCluster CLI creates a snapshot request in the host cluster, which is then processed in the background by the vCluster snapshot controller, which creates volume snapshots.
...
You can check the status of all volume snapshots with the following command:

```bash
vcluster snapshot get myvcluster "container:///data/my-snapshot.tar.gz" --show-volumes
```
