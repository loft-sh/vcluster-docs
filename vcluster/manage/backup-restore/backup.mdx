...
:::info
The vCluster snapshot feature currently only supports creating snapshots for CSI persistent volumes. See [Volume snapshots](./volume-snapshots) for more details. To back up non-CSI persistent volumes (e.g. local volumes), use the [Velero backup method](./velero).
:::
...
13:52:09 info Beginning snapshot creation... Currently, in the snapshots beta release, you can check the snapshot creation progress by inspecting the following ConfigMap data with the following command: kubectl get configmap -n myvcluster myvcluster-snapshot-request-z82gj -o json | jq '.data["snapshotRequest"] | fromjson'
```

As written in the `vcluster snapshot create` output, during the snapshots beta release, you can check the snapshot creation progress by inspecting the snapshot request ConfigMap data with the following command:

```bash
kubectl get configmap -n myvcluster myvcluster-snapshot-request-z82gj -o json | jq '.data["snapshotRequest"] | fromjson'
```
