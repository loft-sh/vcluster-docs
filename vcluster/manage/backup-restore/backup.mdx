...
:::info
The vCluster snapshot functionality currently only supports creating snapshots for CSI persistent volumes. To back up non-CSI persistent volumes (e.g. local volumes), use the [Velero backup method](./velero).
:::

When you run the command to create a snapshot, vCluster CLI creates a snapshot request, which is then handled in the background by a controller. vCluster snapshot controller automatically determines the configured backing store and saves the snapshot at the specified location. The snapshot includes:
...
- Volume snapshot identifiers (if the snapshot was created with `--include-volumes` flag).
