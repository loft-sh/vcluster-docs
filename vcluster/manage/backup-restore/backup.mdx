...
**Azure CLI credentials**: Log in with `az login` before running the snapshot command. You must also provide the subscription ID and resource group where the storage account is located, either as CLI flags or as environment variables (`AZURE_SUBSCRIPTION_ID` and `AZURE_RESOURCE_GROUP`):

```bash title="Create and store a snapshot in Azure Blob Storage using Azure CLI credentials"
# Log in to Azure.
az login

# Create a snapshot and store it in Azure Blob Storage.
vcluster snapshot create my-vcluster "https://myaccount.blob.core.windows.net/my-container/my-snapshot.tar.gz" \
  --azure-subscription-id my-subscription-id \
  --azure-resource-group my-resource-group
```

**SAS token in the URL**: If the blob URL contains a SAS token, no Azure CLI login, subscription ID, or resource group is needed:

```bash title="Create and store a snapshot in Azure Blob Storage using a SAS token"
vcluster snapshot create my-vcluster "https://myaccount.blob.core.windows.net/my-container/my-snapshot.tar.gz?sv=2022-11-02&ss=b&srt=co&sp=rwlacuptfx&se=2026-01-01T00:00:00Z&sig=YOUR_SAS_SIGNATURE"
```

**Storage account key via `AZURE_STORAGE_KEY`**: Set the `AZURE_STORAGE_KEY` environment variable to the storage account access key. No subscription ID or resource group is needed:

```bash title="Create and store a snapshot in Azure Blob Storage using a storage account key"
