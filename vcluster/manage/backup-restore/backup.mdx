...
Need automated, scheduled snapshots? [vCluster Platform AutoSnapshots](/docs/platform/use-platform/virtual-clusters/key-features/snapshots) lets you configure automatic snapshot creation on a cron schedule with retention policies—no need to write custom Kubernetes jobs.
:::
...
For MinIO, you should follow:
```bash title="Create snapshot in MinIO"
# Set your values and MinIO endpoint
export BUCKET="your-bucket-name"
export KEY="path/to/snapshot.tar.gz"
export ENDPOINT="https://your-minio-endpoint:9000"
...
# Create snapshot through CLI
vcluster snapshot my-vcluster \
"s3://${BUCKET}/${KEY}\
?url=$(printf %s "$ENDPOINT" | base64)\
&access-key-id=$(printf '%s' "$ACCESS_KEY_ID" | base64)\
&secret-access-key=$(printf '%s' "$SECRET_ACCESS_KEY" | base64)
&region="$REGION"\
...
vcluster snapshot my-vcluster \
"s3://${BUCKET}/${KEY}\
?url=$(printf %s "$ENDPOINT" | base64 -w 0)\
&access-key-id=$(printf '%s' "$ACCESS_KEY_ID" | base64 -w 0)\
&secret-access-key=$(printf '%s' "$SECRET_ACCESS_KEY" | base64 -w 0)
&region="$REGION"\
&force-path-style=true"
```
</TabItem>
