...
vCluster pauses the tenant cluster and scales workload pods to 0 before the restore pod attempts to reach or download the snapshot, so an invalid snapshot URL or expired credential is only discovered once the restore pod starts, after downtime has begun. Verify the snapshot URL and credentials first with `vcluster snapshot get`, which reads the same URL and credentials without pausing the tenant cluster.
