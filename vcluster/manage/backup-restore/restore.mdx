...
For virtual clusters with private nodes, [additional steps](#using-snapshots-with-private-nodes) may be required. 
:::
...
## Using snapshots with private nodes

Node resources are also included in the etcd snapshot. If you are restoring to a virtual cluster with 
a different set of nodes, manual intervention will be required for virtual clusters using [private nodes](../../deploy/worker-nodes/private-nodes). When using snapshots with [host nodes](../../deploy/worker-nodes/host-nodes),
...
When nodes exist in the snapshot, but don't exist in the current virtual cluster, you will need to manually delete the node(s) from the restored virtual cluster. 

The node will show up in `kubectl get nodes`, but the node does not exist in the virtual cluster.

For each node that no longer exists, you will need to run `kubectl delete node [name]`. 
...
When nodes were added after the snapshot was taken, the node will not exist in the restored virtual cluster. If you run
`kubectl get nodes`, then those nodes will be missing. 

You will need to re-join each private node with the `--force-join` flag. 
...
The output provides a command to run on your worker node, and you will need to append the `--force-join` flag.
