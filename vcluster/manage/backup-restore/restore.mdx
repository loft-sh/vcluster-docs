...
```bash title="Restore virtual clusters from OCI, S3, Azure, or container snapshots"
...
# Restore from an Azure Blob Storage snapshot using Azure CLI credentials.
...
# Restore from an Azure Blob Storage snapshot using a SAS token in the URL.
# Restore from an Azure Blob Storage snapshot using a SAS token in the URL.
vcluster restore my-vcluster "https://myaccount.blob.core.windows.net/my-container/my-snapshot.tar.gz?sv=2022-11-02&sig=YOUR_SAS_SIGNATURE"

# Restore from an Azure Blob Storage snapshot using a storage account key.
...
See [Volume snapshots](./volume-snapshots/setup) for more details.
