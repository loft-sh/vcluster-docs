...
This page describes how to **manually** put a virtual cluster to sleep and wake it up using CLI commands. This is different from automatic sleep, which automatically manages sleep and wake operations based on activity detection or schedules.

## Manual vs automatic sleep
...
- **Automatic sleep**: Configure the vCluster to automatically sleep and wake based on inactivity detection, schedules, or ingress activity. This is an enterprise feature. For details, see the [Sleep configuration documentation](/vcluster/configure/vcluster-yaml/sleep).
...
### Differences from automatic sleep

Manual sleep differs from automatic sleep in several ways:

- **Control**: Manual sleep requires explicit CLI commands; automatic sleep manages sleep and wake operations automatically based on configuration.
- **Activity detection**: Manual sleep doesn't detect activity or wake automatically; automatic sleep can detect API calls, ingress traffic, and other activity to wake the cluster.
- **Scheduling**: Manual sleep doesn't support schedules; automatic sleep can sleep and wake on specific schedules (for example, weekends or off-hours).
- **Resource management**: Manual sleep scales down workloads but keeps the control plane active; automatic sleep (when connected to the platform via agent) can shut down the control plane completely for greater resource savings.

For automatic sleep with activity detection, scheduling, and other advanced features, see the [Sleep configuration documentation](/vcluster/configure/vcluster-yaml/sleep).
...
## When to use manual vs automatic sleep
...
For automatic sleep configuration, see the [Sleep configuration documentation](/vcluster/configure/vcluster-yaml/sleep).
