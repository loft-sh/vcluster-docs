...
## Access vCluster for control plane running as a container
...
If you have manually [exposed the vCluster](#expose-vcluster), you can specify the domain where the vCluster is reachable with the `--server` flag:
...
Often this might not be desired. Instead of giving a user admin access to the cluster, you can also use [service account authentication](https://kubernetes.io/docs/reference/access-authn-authz/authentication/#service-account-tokens) to the cluster. Say we want to create a kubeconfig file that only has view access in the cluster. Then you would create a new service account inside the vCluster and assign it the cluster role `view` through a cluster role binding. Then we would generate a service account token and use that instead of the client-cert and client-key inside the kubeconfig.
...
- `serviceAccount.name`, `serviceAccount.namespace` (defaults to `kube-system`), and `serviceAccount.clusterRole` generate a service account token instead of the default admin client certificate. vCluster creates the service account and a cluster role binding to the given `clusterRole` inside the cluster.
...
Exposing vCluster is different, depending on whether the control-plane is deployed [as container](../deploy/control-plane/kubernetes-pod/basics.mdx) or [as a binary](../deploy/control-plane/binary/basics.mdx).

<Tabs
  defaultValue="as-container"
  values={[
    { label: 'As Container', value: 'as-container',},
    { label: 'As Binary', value: 'as-binary',}
  ]}>
...
For vCluster Standalone, you can use the method of your choice for exposing API Server endpoint, as described in the [Kubernetes documentation](https://kubernetes.io/docs/tasks/administer-cluster/access-cluster-api/).
  If you decide to use vCluster Standalone as a control plane cluster for clusters, then you can set your current kube context to the vCluster Standalone and create and interact with clusters using vCluster CLI.
</TabItem>
...
The Gateway API [TLSRoute](https://gateway-api.sigs.k8s.io/reference/spec/#gateway.networking.k8s.io/v1alpha2.TLSRoute) is the recommended way to expose the cluster's API server with TLS passthrough. It requires a Gateway API-compatible controller, such as Envoy Gateway, Istio, or Cilium, installed on the control plane cluster with a `Gateway` resource configured for TLS passthrough.

For adoption planning, migration caveats, and Gateway API troubleshooting checks, see [Gateway API](../key-features/gateway-api.mdx) and [Migrate from ingress-nginx to Gateway API](./ingress-nginx-to-gateway-api.mdx).
...
The [ingress-nginx project](https://kubernetes.github.io/ingress-nginx/) is deprecated upstream. Existing ingress-based configurations continue to work in vCluster v0.35 and vCluster Platform v4.10, but new deployments should use the **Gateway API (TLSRoute)** tab. Migration is manual; see [Migrate from ingress-nginx to Gateway API](./ingress-nginx-to-gateway-api.mdx).
:::
  ...
  Make sure your ingress controller is installed and healthy on the cluster that hosts your clusters. Create the following `ingress.yaml` for a vCluster called `my-vcluster` in the namespace `my-vcluster`:
...
<TabItem value="from-host-cluster">
  ...
  To access the cluster from within the control plane cluster, you can directly connect to the vCluster service. Make sure you can access that service and then create a kube config in the following form:
  ```
