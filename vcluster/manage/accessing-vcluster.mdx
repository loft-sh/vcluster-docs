...
Exposing vCluster is different, depending on whether the control-plane is deployed [as container](../deploy/control-plane/container/basics.mdx) or [as a binary](../deploy/control-plane/binary/basics.mdx).
...
For a Standalone cluster, you can use the method of your choice for exposing API Server endpoint, as described in the [Kubernetes documentation](https://kubernetes.io/docs/tasks/administer-cluster/access-cluster-api/).
  If you decide to use a Standalone cluster as a host cluster for virtual clusters, then you can set your current kube context to the Standalone cluster and create and interact with virtual clusters using vCluster CLI.
</TabItem>
