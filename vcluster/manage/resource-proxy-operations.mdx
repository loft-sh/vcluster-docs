...
This feature requires [vCluster Platform](/docs/platform). Both the client and target clusters must be managed as [VirtualClusterInstance](/docs/platform/api/resources/virtualclusterinstance) within the platform.
:::

The resource proxy lets a client <GlossaryTerm term="tenant-cluster">cluster</GlossaryTerm> transparently forward custom resource requests to a target cluster, which stores the objects and can run controllers against them. For how it works and how to set it up, see [How the resource proxy works](../understand/resource-proxy.mdx) and [Set up the resource proxy](../learn-how-to/resource-proxy-setup.mdx).
...
```yaml title="vcluster.yaml - client cluster"
policies:
  networkPolicy:
    enabled: true
    controlPlane:
...
```yaml title="vcluster.yaml - target cluster"
policies:
  networkPolicy:
    enabled: true
    controlPlane:
...
- Enable `LOFT_LOG_TSNET_CONTROL_PLANE` on the platform for verbose TSNet logging, which covers DERP relay and mesh traffic on the same toggle. See [Enable verbose agent and platform logs](/docs/platform/troubleshoot/connected-cluster-connectivity#enable-verbose-agent-and-platform-logs).

## Monitor the proxy {#monitoring}

The proxy exposes Prometheus metrics on the client cluster. Scrape them, alert on the starting points below, pair them with a [synthetic probe](#synthetic-probe), and [verify that all three work](#verify-monitoring) before you depend on them during an incident.

### Scrape proxy metrics {#scrape-metrics}

The client cluster exposes proxy metrics at the `/metrics/syncer` path of its <GlossaryTerm term="api-server">API server</GlossaryTerm>, using the same delegated authentication as the other metrics paths. The caller needs `get` on the non-resource URL `/metrics/syncer` inside the cluster.

```bash title="Check proxy metrics manually"
kubectl get --raw /metrics/syncer | grep vcluster_proxy
```

Run this against the client cluster API.

```yaml title="RBAC for a metrics scraper - apply inside the client cluster"
...
| `namespace_missing` | A namespaced POST, PUT, or PATCH targeted a namespace that doesn't exist in the client cluster. | |
...
- Caller-observed errors from the client cluster's own API server metrics: `apiserver_request_total{group="<your-group>",code=~"5.."}`.
- APIService availability inside the client cluster: `kube_apiservice_status_condition` if you run kube-state-metrics there, or a scheduled `kubectl get apiservice`.
- A [synthetic probe](#synthetic-probe), because counters only move when traffic flows.

### Synthetic probe {#synthetic-probe}

Metrics report the proxy's own view. A synthetic request reports what callers experience, including the parts before the proxy, such as the APIService wiring and the aggregation layer. Run a probe like this on a schedule against the client cluster:
...
Run the CronJob on the control plane cluster, in the client cluster's namespace, so that the control plane cluster's kube-state-metrics observes it. The default `vc-<release>` kubeconfig points to `https://localhost:8443`, which is only reachable from the vCluster control plane pod. Export an [additional kubeconfig secret](../configure/vcluster-yaml/export-kube-config.mdx#export-additional-secrets) that uses the cluster Service instead:
...
The generated secret holds the cluster kubeconfig under the key `config`. If the release name or namespace differs from the example, update both the Service address and the CronJob placeholders.
...
To deploy the ConfigMap and CronJob without applying them by hand, add both manifests to [`experimental.deploy.host.manifests`](../configure/vcluster-yaml/experimental/deploy.mdx#apply-manifests-on-initialization-in-host) in the client's `vcluster.yaml`. vCluster then applies them into the cluster's namespace on the control plane cluster whenever it initializes.
...
- **Scraping works.** Query `up{job=~".*vcluster.*"}` in Prometheus and confirm the client cluster's control plane target is present and `1`. An absent target is indistinguishable from a healthy proxy in every alert above.
