...
The resource proxy lets a client tenant cluster transparently forward custom resource requests to a target tenant cluster, which stores the objects and can run controllers against them. For how it works and how to set it up, see [How the resource proxy works](../understand/resource-proxy.mdx) and [Set up the resource proxy](../learn-how-to/resource-proxy-setup.mdx).
...
Keep the client, the target, and the platform on current versions. Peer staleness fixes span both vCluster and the platform's stale peer cleanup, and running a new client against an old platform keeps the degradation risk alive.
...
Latency distribution for non-watch requests, with buckets up to the 60-second request deadline. Watches are excluded, because their lifetime would swamp the distribution. Fast 503s point at proxy-side degradation. Creeping p99 points at a slow target or a relayed network path.
...
Discovery requests answered from the [local fallback document](../understand/resource-proxy.mdx#degraded-behavior) instead of the target. Callers see a `200`, so this counter is the only signal that discovery is degraded. See [Discovery lists resources with only a get verb, or an empty list](../troubleshoot/resource-proxy.mdx#discovery-fallback).
...
| `ownership_masked` | `accessResources: owned` hid an object the client doesn't own. Tenant isolation working as intended. See [404 on resources that exist in the target](../troubleshoot/resource-proxy.mdx#resources-not-appearing). | |
...
| `upgrade_rejected` | A connection upgrade verb was rejected. See [501: connection upgrades aren't supported](../troubleshoot/resource-proxy.mdx#connection-upgrades). | `request_terminations_total` |
...
Whether the proxy holds a usable peer snapshot (`1`). At `0`, resource requests to that target fail fast with 503, while discovery uses the local fallback. This covers both a peer list that has never arrived and one that aged out after the subscription dropped.
...
Events flowing per target. A connected but silent watch, such as a half-dead connection, shows up only here.
