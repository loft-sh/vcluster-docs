...
For cluster API server exposure, verify that your controller supports `TLSRoute` and TLS passthrough. For workload HTTP routing and Gateway-backed auto sleep, verify `HTTPRoute` support.
...
4. Create a test cluster with a Gateway-backed `TLSRoute` endpoint.
...
- Cluster API access works through the Gateway-backed endpoint.
