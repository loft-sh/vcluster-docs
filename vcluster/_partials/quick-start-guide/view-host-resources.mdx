...
This step shows the core isolation guarantee: most resources in your cluster remain invisible on the control plane cluster. Only the pods required for scheduling are synced across.

To verify this, perform these steps:

<Flow>
   <Step>
      Switch your kube context from the cluster back to the control plane cluster by disconnecting.
   ...
   <Step>
      ...
      This `deployment` resource does **not** exist on the control plane cluster. By default, Deployments are not synced from the cluster to the control plane cluster. Only the pods that need to be scheduled sync.
...
By default, vCluster does not persist data outside of its pods. Deleting a vCluster pod without persistent storage causes data loss. Configure an external [backing store](../../configure/vcluster-yaml/control-plane/components/backing-store/README.mdx) to persist data outside the pod.
