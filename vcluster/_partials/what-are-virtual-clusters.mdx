...
Virtual clusters run inside namespaces on the control plane cluster but operate as independent Kubernetes environments, each with its own API server, control plane, <GlossaryTerm term="syncer">syncer</GlossaryTerm>, and set of resources.

Although they rely on the control plane cluster for resource scheduling and networking, virtual clusters maintain a level of abstraction that isolates their operations from its global state.
...
### Robust security and isolation
  ...
  - **Customizable security policies:** Tenants can implement additional virtual cluster-specific governance, including OPA policies, network policies, resource quotas, limit ranges, and admission control, in addition to the existing policies and security measures in the underlying physical control plane cluster.
...
### Cost-effectiveness and reduced overhead
  ...
  - **Resource efficiency:** By sharing the underlying control plane cluster's resources, virtual clusters minimize the need for additional physical infrastructure, reducing costs and environmental impact.
...
### Improved scalability
  ...
  - **Reduced API server load:** Virtual clusters, each with their own dedicated API server, significantly reduce the operational load on the control plane cluster's Kubernetes API server by isolating and handling requests internally.
