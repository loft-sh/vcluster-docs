...
<Step>
Create a Helm repository source in the Git repository that Flux monitors, so Flux can fetch the vCluster Helm charts automatically. In this example, related files are stored under the `clusters` folder:
...
Create a vCluster `HelmRelease` file in your Git repository. This `HelmRelease` tells Flux how to deploy a vCluster in your Kubernetes cluster using the configured Helm charts in Step 2:
