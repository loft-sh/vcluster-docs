...
- `<CLUSTER_NAME>-app.yaml` for your ArgoCD `Application` definition
...
<InterpolatedCodeBlock 
  code={`export CLUSTER NAME=[[VAR:CLUSTER NAME:my-vcluster]]
export CLUSTER NAMESPACE=[[VAR:NAMESPACE:team-x]]
export VCLUSTER YAML=$(awk '{printf "%s\\n", $0}' [[VAR:CONFIG FILE:vcluster.yaml]])`} 
  language="bash" 
/>
...
<InterpolatedCodeBlock
code={`clusterctl generate cluster \${CLUSTER NAME} \\   --infrastructure vcluster \\   --target-namespace \${CLUSTER NAMESPACE} \\    | kubectl apply -f -`}
language="bash"
/>
...
<InterpolatedCodeBlock
code={`kubectl wait --for=condition=ready vcluster -n \$CLUSTER NAMESPACE \$CLUSTER NAME --timeout=300s`}
language="bash"
/>
