...
description: Diagnose and resolve DNS resolution timeouts and SERVFAIL errors caused by CoreDNS running out of capacity in a cluster.
---
...
Test DNS resolution directly from a pod in the cluster:

```bash title="Test DNS resolution from inside the cluster"
...
through a normal upgrade on your live cluster, no redeploy needed:
...
live cluster without redeploying it. You may see a brief DNS gap
during the switch.
