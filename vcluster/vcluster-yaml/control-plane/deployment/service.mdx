...
:::note
- `.experimental.isolatedControlPlane.headless` [configuration](/vcluster/vcluster-yaml/experimental/isolated-control-plane.mdx) changes the default Service spec to expose the required 8443 `targetPort` for both `https` and `kubelet`.
- `networking.advanced.proxyKubelets.byHostname` or `networking.advanced.proxyKubelets.byIP` [configuration](/vcluster/vcluster-yaml/networking/advanced.mdx) changes the default Service spec to expose the `kubelet` port at 10250. 
:::
