...
This guide shows how to manage clusters using [Flux](https://fluxcd.io/), a GitOps tool for Kubernetes. Flux uses GitOps to keep your Kubernetes clusters in sync with the desired state defined in your Git repository.
...
When using Flux with clusters, choose an architecture that aligns well with your GitOps workflow. You can configure Flux to work with clusters in several ways, each offering different benefits and limitations.

Unlike ArgoCD, which treats other clusters as first-class objects, Flux manages workloads on other clusters through kubeconfig references in `HelmRelease` and `Kustomization` resources. This difference in design influences how you'll structure your GitOps workflows when working with clusters.

The following are common approaches for integrating Flux with clusters, each suited to different use cases:

### 1. Flux instance per control plane cluster {#flux-per-host}

When running Flux on each control plane cluster, it can manage the clusters within that environment. This approach is recommended if you already use Flux for each traditional cluster and want to maintain a similar management pattern.
...
- Each Flux instance manages multiple clusters on that host
- Cluster kubeconfig Secret management is simplified since Secrets are local to the cluster
...
With this approach, a central Flux instance manages multiple clusters across different control plane clusters. This is a good option if you already use a single Flux instance with multiple Kubernetes clusters or if you want centralized control of all virtual environments.
...
Architecture characteristics:
- One central Flux instance manages multiple clusters across different hosts
...
- Provides a single control point for all cluster management
- Can simplify GitOps workflows by having a single source of truth

### 3. Flux inside clusters {#flux-inside-vclusters}

While possible, running Flux inside every cluster adds resource overhead and management complexity. This approach might be suitable when clusters need complete isolation and independent GitOps workflows.
...
The standalone approach of deploying Flux in each cluster might not scale effectively when managing large numbers of clusters, particularly in ephemeral environments like pull request previews. In these cases, a hub-and-spoke model or a Flux instance per control plane cluster is typically more resource-efficient and can reduce the overhead of running multiple Flux controllers.
<!-- vale off -->
## Enable kubeconfig export {#enable-kubeconfig-export}
<!-- vale on -->

To enable Flux to deploy to clusters, you must create a kubeconfig Secret that Flux can reference.
...
This configuration:
- Exports the cluster kubeconfig as a Secret in the host namespace
...
After the vCluster is up and running, use Flux to deploy applications directly into the cluster.
...
spec:
  ...
  values:
    ui:
      message: "Deployed by Flux to cluster"
    ingress:
      enabled: true
      hosts:
...
The `kubeConfig` section references the Secret created by the vCluster using the `exportKubeConfig` setting. To allow Flux to connect to the cluster with a self-signed certificate, you must configure both Flux and vCluster to permit insecure TLS connections:

- **Flux Helm Controller configuration**:  
  Add the `--insecure-kubeconfig-tls` flag to the [Helm Controller configuration](https://fluxcd.io/flux/components/helm/options/). This allows Flux to honor `insecure-skip-tls-verify: true` in the referenced kubeconfig and skip certificate verification when connecting to the cluster.  
...
<Highlight color="secondary">Cluster</Highlight> Verify deployment
...
## Manage multiple clusters {#manage-multiple-vclusters}

When managing multiple clusters with Flux, you can use Kustomize to organize your configurations.
...
A common scenario is having Flux already installed on the control plane cluster and wanting to use it for ephemeral review environments with clusters. This approach follows GitOps principles while avoiding the need to install Flux separately for each environment.

### Use ResourceSet feature for review environments {#flux-resource-sets}

For later versions of Flux, you can use the native Flux Operator `ResourceSet` capability to manage ephemeral environments efficiently. This is a feature for creating review environments that include clusters.
...
2. Deploys clusters for each review environment
3. Use Flux to deploy applications to these clusters
...
For organizations managing many clusters, especially in dynamic, ephemeral environments, vCluster Platform offers advanced lifecycle management capabilities that integrate smoothly with GitOps workflows. It supports automatic creation of kubeconfig `Secrets`, access control management, and simplified bootstrapping of clusters with Flux.
:::

## Troubleshoot

<Highlight>control plane cluster</Highlight>
- Verify the cluster kubeconfig Secret exists with the correct format
...
<Highlight color="secondary">Cluster</Highlight>
- Verify that resources are being created in the cluster
...
```bash title="Check cluster configuration"
...
To allow Flux (v2+) to connect to a cluster with a self-signed certificate, you must configure the Flux Helm Controller to permit insecure TLS connections:
...
1. The cluster's API server is not accessible from Flux
2. Network policies are blocking the communication
3. The cluster is not running or healthy
...
1. Check if the cluster is running and ready:
...
When you deploy a cluster through a `HelmRelease` resource that lives in a different namespace than the cluster itself (for example, `HelmRelease` in `flux-system` and the cluster in `mytestcluster`), Flux stores the Helm release secret in the `HelmRelease` namespace. The snapshot controller searches for the Helm release secret in the cluster's namespace and fails to find it, which causes snapshots to fail.

To fix this, set `spec.storageNamespace` in the `HelmRelease` to the cluster's namespace:

```yaml title="Set storageNamespace to match the cluster namespace" {7}
apiVersion: helm.toolkit.fluxcd.io/v2
kind: HelmRelease
metadata:
  name: my-vcluster
  namespace: flux-system
spec:
  storageNamespace: mytestcluster  # Must match the cluster namespace
