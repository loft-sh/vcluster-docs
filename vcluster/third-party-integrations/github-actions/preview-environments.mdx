...
This guide shows how to use vCluster and GitHub Actions to deploy preview environments on every pull request. The example creates a new cluster for each pull request and installs Argo CD into it as a stand-in workload. It uses the sleep mode feature of vCluster Platform so that idle environments consume no resources.

## Prepare the platform

### Install an ingress controller

Install an ingress controller into the control plane cluster where the preview environments run. The cluster template later reuses this controller to expose each preview environment through `sync.toHost.ingresses`.
...
Create a cluster template for the preview environments and name it `preview-template`. In vCluster Platform, select **Templates**, then **Clusters**, and click **Add Cluster Template**. For the full flow, see [Create a template](/docs/platform/administer/templates/create-templates).
...
```yaml title="Cluster template values"
# Reuse the control plane cluster ingress controller for cluster ingresses
sync:
  toHost:
    ingresses:
      enabled: true
...
With the project configured, write the GitHub workflow that creates the preview environments. On every pull request, this workflow creates a cluster, installs Argo CD, and exposes it through an ingress.
    ...
    steps:
      ...
      - name: Deploy preview environment
        run: |
          ...
          # Create the cluster, or update it on later pushes to the same pull request
...
Environments must be cleaned up when they are no longer needed. Create a separate workflow that deletes the cluster as soon as the pull request is closed or merged:
