...
## Deploy clusters on pull requests

Clusters provide isolated Kubernetes environments for each pull request, enabling:
...
When using [host nodes](../../deploy/worker-nodes/host-nodes/overview.mdx), cluster workloads run on the existing control plane cluster nodes. This is the simplest setup for CI/CD pipelines.
