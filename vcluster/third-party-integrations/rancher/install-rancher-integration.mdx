...
The vCluster Rancher integration uses an operator to connect tenant clusters to Rancher by creating a corresponding Rancher cluster resource for each tenant cluster. The operator runs on the control plane cluster and handles registration automatically. Rancher users don't need cluster management permissions to use this integration. The operator assigns Rancher cluster owner access to the tenant cluster creator, owners of the tenant cluster's project, and owners of the control plane cluster. This setup allows tenant clusters to appear and behave like managed clusters in Rancher.
...
- Admin permissions for the control plane cluster. You must be able to add Helm repositories and install charts on the control plane cluster.
- A Rancher instance running on the control plane cluster. The Docker installation method is not supported for production use.
...
<Step title="Select a control plane cluster">
Use the dropdown list to select the control plane cluster where the tenant cluster is deployed.
...
Since tenant clusters share physical nodes with the control plane cluster, Rancher may count tenant cluster nodes toward your resource usage. To prevent this, use sync patches to zero out the CPU capacity reported by the tenant cluster's nodes.
...
To remove the Rancher integration, uninstall the vCluster Rancher Operator from the control plane cluster. Deleting the operator stops new tenant clusters from appearing in Rancher, but does not remove any existing tenant clusters.
