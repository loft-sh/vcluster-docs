...
To achieve this setup, we'll need to use the [sync.toHost feature][sync-toHost-docs] in order to expose the KSA in the control plane cluster together with the vCluster Platform API to retrieve the updated name of the KSA in the control plane cluster.
...
gcloud iam service-accounts add-iam-policy-binding ${GSA_NAME}@${GCP_PROJECT_ID}.iam.gserviceaccount.com \
  --member "serviceAccount:${GCP_PROJECT_ID}.svc.id.goog[${VCLUSTER_NAMESPACE}/${KSA_NAME}]" \
...
# Bind IAM policy for Workload Identity User
gcloud iam service-accounts add-iam-policy-binding ${GSA_NAME}@${GCP_PROJECT_ID}.iam.gserviceaccount.com \
  --member "serviceAccount:${GCP_PROJECT_ID}.svc.id.goog[${PLATFORM_NAMESPACE}/${KSA_NAME}]" \
  --role "roles/iam.workloadIdentityUser"
...
[access-key-link]: /platform/administer/authentication/access-keys
...
[sync-toHost-docs]: ../../configure/vcluster-yaml/sync/to-host/overview.mdx
