...
To configure Workload Identity in a cluster, you must link the Kubernetes Service Account (KSA) used by your workload with a Microsoft Entra Workload ID. This KSA must exist in the control plane cluster where the cluster runs. 
...
If you have not set one up yet, follow the [vCluster Platform installation guide](/platform/install/quick-start-guide).

After deploying the platform, [create a new access key](/platform/administer/authentication/access-keys) to authenticate with the vCluster Platform API. This key is required to retrieve the synced Kubernetes Service Account (KSA) name and complete the Workload Identity setup.
...
Create a Kubernetes `ServiceAccount` that can be used to interact with the cluster:
...
Then create the federated identity credential. The namespace parameter depends on your vCluster deployment type:
...
For vCluster deployments managed by the platform with a project:
...
az identity federated-credential create \
    ...
    --resource-group "${RESOURCE_GROUP_NAME}" \
    --issuer "${AKS_OIDC_ISSUER}" \
    --subject system:serviceaccount:"${PLATFORM_NAMESPACE}":"${KSA_NAME}" \
    --audience api://AzureADTokenExchange
```
