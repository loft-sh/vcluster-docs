...
- _`KUBERNETES_DISTRO`_ with your virtual cluster's existing Kubernetes distribution. If you aren't sure what distro you were using, the previous default distro was `k3s`.  **You can't change the Kubernetes distribution or backing store during an upgrade**.
