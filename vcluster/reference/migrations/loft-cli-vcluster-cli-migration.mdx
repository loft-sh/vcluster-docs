...
This section provides an overview of `loft` CLI commands that have moved to the `vCluster` CLI.
Most commands should work exactly as before. Any commands that have been changed are documented in the "Comments" column.

## Global --config flag

Previously, the `lof` CLI read its config file from `~/.loft/config.json` if nothing was specified under `--config`. The `vCluster` CLI now assumes the config to be in `~/.vcluster/config.json`, by default.
The config format in the `vCluster` CLI changed in a way that the Loft Platform (now vCluster Platform) related configuration goes into a special section under the key `platform`.
You can convert your already existing Loft configuration using the following (prerequisite: jq >= 1.7 installed):
...
You can pass this path to `--config $CONFIG_PATH` or rely on the `vCluster` CLI to use the default path `~/.vcluster/config.json`.

## Mapp old `loft` CLI commands to `vcluster` CLI commands
...
| `loft` CLI             | `vCluster` CLI                                  | Comments                                                                                                                         |
|------------------------|-------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------|
| `loft connect cluster` | `vcluster platform add cluster`                 | The default agent namespace is now `vcluster-platform`. To restore the previous behavior, use the `--namespace=loft` flag.       |
