...
| `--out-kube-config-server`           | `server`           | If you have exposed the cluster, you can instruct vCluster to create the kubeconfig secret with another server endpoint.   |
...
The following example configures a cluster to use a specific kubeconfig context and server endpoint for the default
kubeconfig secret `vc-NAME`:

- Set the kubeconfig context name to `my-domain-context`.
- Configure the kubeconfig to use `https://my-domain.org:443` as the endpoint for the exposed cluster.
...
The following example configures a cluster to export an additional secret `vc-my-domain`, which is created in
...
- Set the kubeconfig context name to `my-domain-context`.
- Configure the kubeconfig to use `https://my-domain.org:443` as the server endpoint for the exposed cluster.
...
The following example configures a cluster to store the kubeconfig secret in a separate namespace while maintaining
proper access control:

- Set the kubeconfig context name to `my-domain-context`.
- Configure the kubeconfig to use `https://my-domain.org:443` as the cluster endpoint.
