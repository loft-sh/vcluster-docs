...
vCluster can deploy a [KubeVirt](https://kubevirt.io/) CSI driver into the tenant cluster. It provisions PersistentVolumes as CDI DataVolumes in the vCluster's namespace on the control plane cluster. It then hot-plugs them into the KubeVirt virtual machines backing the tenant cluster's private nodes. This is a storage integration, unrelated to the [KubeVirt integration](../../integrations/kubevirt.mdx), which syncs KubeVirt VM resources themselves into the tenant cluster. See [How this differs from KubeVirt CSI](../../integrations/kubevirt.mdx#how-this-differs-from-kubevirt-csi) for the distinction.
...
- The private nodes backing this tenant cluster must themselves be KubeVirt virtual machines, provisioned through a [KubeVirt node provider](/docs/platform/administer/node-providers/kubevirt) and [auto nodes](private-nodes/auto-nodes.mdx). Those virtual machines must already be running in the same namespace as the tenant cluster before you enable KubeVirt CSI. The driver discovers each node's virtual machine by name. It doesn't create virtual machines itself.
