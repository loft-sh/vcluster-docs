...
- The private nodes backing this tenant cluster must be KubeVirt virtual machines in the same namespace as the tenant cluster. You can provision them through a [KubeVirt node provider](/docs/platform/vmetal/drivers/virtual-machines/kubevirt) and [auto nodes](private-nodes/auto-nodes.mdx). The controller first matches a virtual machine to a node by name, then falls back to matching internal IP addresses. It retries until a matching virtual machine is available. The driver doesn't create virtual machines itself.
