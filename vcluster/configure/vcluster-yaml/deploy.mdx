...
vCluster can install [metrics server](https://github.com/kubernetes-sigs/metrics-server) into the tenant cluster. Enable it when you need `kubectl top` output or want the Horizontal Pod Autoscaler to use CPU and memory metrics.
...
vCluster installs [Flannel](https://github.com/flannel-io/flannel) by default. You can disable it and install your own CNI.
...
:::warning User-managed component
...
To customize kube-proxy behavior without disabling it, use the `config` field. Values are merged into the default KubeProxyConfiguration. See the [Kubernetes kube-proxy config API reference](https://kubernetes.io/docs/reference/config-api/kube-proxy-config.v1alpha1/) for available fields.
...
vCluster can install [MetalLB](https://metallb.io/) into the tenant cluster. This can be enabled via:
...
:::warning User-managed component
When you disable the Local Path Provisioner and replace it with your own StorageClass, vCluster support does not cover your storage configuration. Existing PersistentVolumes are not affected, but new PersistentVolumeClaims will not bind until a replacement provisioner is available.
:::
