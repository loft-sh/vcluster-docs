...
namespace, add them to the Namespace. For a tenant cluster, add the supported
annotations under `spec.template.metadata.annotations` in its
VirtualClusterTemplate. `ignore-groups`, `ignore-resources`, and `ignore-verbs`
apply to namespace sleep mode only and have no effect on a tenant cluster.
