...
description: Configure inactivity-based and scheduled auto sleep for clusters, with or without vCluster Platform.
---
...
Auto sleep reduces the compute resources that idle clusters consume. It
...
| --- | --- |
| Cluster without a platform agent | The cluster scales down its workloads. Its control plane stays running to detect activity and restore workloads. |
| Cluster connected to vCluster Platform | The platform agent takes over sleep management and can scale down both workloads and the cluster control plane. |

When an existing cluster connects to a compatible platform agent, the
...
VirtualClusterInstance. Use it when the cluster should keep managing its
...
cluster is already connected, set the scope and then remove the
...
cluster control plane runs on the control plane cluster. The platform
agent leaves the annotation in place when the scope changes, and native auto
sleep remains disabled until you remove it.

See [Sleep clusters](../../key-features/sleep.mdx) for the lifecycle and
[Pause and resume a cluster](../../manage/sleep-wakeup.mdx) for manual
operations.
...
1. Select the project that contains the cluster.
2. Click **Clusters** in the sidebar.
3. Click **Edit** for the cluster.
4. Expand **Auto-Sleep and Auto-Delete**.
5. Set **Sleep after inactivity**.
6. Click **Save**.

For a cluster created from a template, edit the template instead of the
instance. See [Create a cluster template](/docs/platform/administer/templates/create-templates).
...
In vCluster Platform, select the project and click **Clusters**. Click
**Edit** for the cluster, expand **Auto-Sleep and Auto-Delete**, then set
...
This section applies to platform-managed namespaces and clusters alike,
...
Standalone auto sleep, which only applies to a cluster without a
platform agent, can wake from activity after scheduled sleep.

### Kubernetes API activity

For a platform-managed namespace or cluster, Kubernetes API requests
...
Direct requests to a cluster API endpoint that bypasses the platform
...
For both namespaces and clusters, vCluster Platform can detect traffic
...
namespace or cluster can therefore enter sleep while the route is
...
displays a wakeup page while the namespace or cluster wakes. The page
...
shouldn't keep a platform-managed namespace or cluster awake. For a
namespace, add them to the Namespace. For a template-backed cluster, add
...
mode only. They have no effect on a cluster.
...
namespace or cluster active. Choose one of these controls when idle
...
2. Add `sleepmode.loft.sh/ignore-active-connections: "true"` to the Namespace or
   cluster template metadata. Open streams then don't count as activity
...
Auto-delete permanently deletes a platform-managed cluster after an
