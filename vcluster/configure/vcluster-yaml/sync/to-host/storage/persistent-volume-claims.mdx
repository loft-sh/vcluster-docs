---
title: Persistent Volume Claims
sidebar_label: persistentVolumeClaims
sidebar_position: 1
description: Configuration for ...
---


import EnableSwitch from '@site/vcluster/_partials/config/sync/toHost/persistentVolumeClaims.mdx'
import TranslatePatches from '@site/vcluster/_fragments/translate-patches.mdx'

{/*
`persistentVolumeClaims.enabled`:
- covered by the generic section here: https://www.vcluster.com/docs/syncer/config#enable-or-disable-synced-resources
*/}

By default, this is enabled.

Sync [PersistentVolumeClaim](https://kubernetes.io/docs/concepts/storage/persistent-volumes/) resources from the virtual cluster to the host cluster.

### Disable syncing PersistentVolumeClaims from the virtual to host cluster

```YAML
sync:
  toHost:
    persistentVolumeClaims:
      enabled: false
```

## Translate Patches
<TranslatePatches resource="persistentVolumeClaims" path="spec.storageClassName" />

## Config reference

<EnableSwitch/>
