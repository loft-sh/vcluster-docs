...
On shared nodes, enabling this lets a tenant create a PersistentVolume with `spec.hostPath` set to any path on the node, then mount it through an ordinary PersistentVolumeClaim. Pod Security Standards don't catch this, since the Pod's volume is a PVC, not a `hostPath` volume. If you enable PersistentVolume sync on shared nodes, add control plane cluster admission that denies synced PersistentVolumes with `spec.hostPath`, or restricts them to an explicit safe-path allowlist. See [storage breakout prevention](../../../../../design/security/shared-nodes-hardening.mdx#storage-breakout-prevention).
:::
