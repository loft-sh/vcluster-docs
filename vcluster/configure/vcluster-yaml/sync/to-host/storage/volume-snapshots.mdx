---
title: Volume snapshots
sidebar_label: volumeSnapshots
sidebar_position: 3
sidebar_class_name: host-nodes
description: Configuration for syncing volume snapshots to host
---

import Prereqs from '../../../../../_fragments/volume-snapshots-host-prereqs.mdx'
import EnableSwitch from '../../../../../_partials/config/sync/toHost/volumeSnapshots.mdx'
import TenancySupport from '../../../../../_fragments/tenancy-support.mdx';

<TenancySupport hostNodes="true" />

{/*
`volumeSnapshots.enabled`:
- covered by the generic section here: https://www.vcluster.com/docs/syncer/config#enable-or-disable-synced-resources
- Added to close issue: https://github.com/loft-sh/vcluster/issues/258
*/}

By default, this is disabled.

Sync [VolumeSnapshot](https://kubernetes.io/docs/concepts/storage/volume-snapshots/), VolumeSnapshotContent and VolumeSnapshotClass resources from the tenant cluster to the control plane cluster.

Until enabled, creating a VolumeSnapshot custom resource in the tenant cluster has no effect on the control plane cluster.

### Host prerequisites

<Prereqs/>

### Sync VolumeSnapshots from the tenant to control plane cluster

```YAML
sync:
  toHost:
    volumeSnapshots:
      enabled: true
```

## Patches

Use `sync.toHost.volumeSnapshots.patches` to transform VolumeSnapshot fields while syncing to the control plane cluster. See [Patching synced resources](../../patching/README.mdx) for syntax, directionality, and examples.

## Config reference

<EnableSwitch/>
