---
title: Secrets
sidebar_label: secrets
sidebar_position: 2
sidebar_class_name: host-nodes
description: Configuration for syncing secrets to host
---

import SyncAllResource from '../../../../../_partials/config/sync/toHost/secrets.mdx'
import DisableResourceWarning from  '../../../../../_fragments/hints/warning-do-not-disable-resource.mdx'
import TenancySupport from '../../../../../_fragments/tenancy-support.mdx';

<TenancySupport hostNodes="true" />

{/*
`secrets.enabled`:
 - covered by the generic section here: https://www.vcluster.com/docs/syncer/config#enable-or-disable-synced-resources
`secrets.all`:
 - https://www.vcluster.com/docs/syncer/config#sync-all-secrets-and-configmaps
*/}
By default, this is enabled.

Sync [Secret](https://kubernetes.io/docs/concepts/configuration/secret/) resources used by pods from the cluster to the control plane cluster. Apps frequently need secret data to function.

### Sync only utilized secrets from tenant to control plane cluster (Default)

```YAML
sync:
  toHost:
    secrets:
      enabled: true
```

### Sync all secrets from tenant to control plane cluster

vCluster only knows about a couple of cluster resources that actually use secrets and tries to sync only those into the control plane cluster. You can enable syncing all cluster secrets to avoid the problem that needed secrets are not synced to the control plane cluster.

```YAML
sync:
  toHost:
    secrets:
      enabled: true
      all: true
```

## Patches

Use `sync.toHost.secrets.patches` to transform Secret fields while syncing to the control plane cluster. See [Patching synced resources](../../patching/README.mdx) for syntax, directionality, and examples.

## Config reference
<DisableResourceWarning/>

<SyncAllResource/>
