...
slug: /configure/vcluster-yaml/sync/to-host/core/pods
---
...
Sync all [Pod](https://kubernetes.io/docs/concepts/workloads/pods/) resources, including ephemeral containers, from the cluster to the control plane cluster.
...
- This can be used to schedule vCluster pods to dedicated host nodes. Basically you can configure nodes with a "taint" and then make the vCluster pods "tolerate" the taint. Adds the taint to all Pods created from the cluster.
...
In order for the Pod to be able to communicate with the cluster's API, it requires a Service Account Token.
If you don't want to create these secrets in the control plane cluster, you can disable it and then add annotations to the pods which then get mounted as a file.
...
If certain images used within the cluster are not accessible in the control plane cluster due to registry restrictions or security policies, `translateImage` can map these to equivalent, permitted images in the control plane cluster's registry.
...
A host Pod requires a ServiceAccount token to communicate with the cluster's API. If you don't want to create these secrets in the control plane cluster, disable this option. vCluster then adds annotations to the pods.
