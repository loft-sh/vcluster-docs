...
When enabled, sync all [Ingress](https://kubernetes.io/docs/concepts/services-networking/ingress/) resources from the cluster to the control plane cluster. Use this option to make a cluster service available using a hostname/domain without having to configure DNS for each cluster. This requires a compatible Ingress controller running on the control plane cluster, and a single shared controller can reconcile Ingress resources synced from multiple clusters.
