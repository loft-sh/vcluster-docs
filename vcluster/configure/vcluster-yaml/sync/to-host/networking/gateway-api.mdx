...
description: Configuration for syncing Gateway API resources from the cluster to the control plane cluster.
---
...
vCluster can sync the following [Kubernetes Gateway API](https://gateway-api.sigs.k8s.io/) resources from the cluster to the control plane cluster:
...
vCluster can also import selected control plane `Gateway` resources into the cluster with `sync.fromHost.gateways`, so tenants can attach routes to platform-managed infrastructure without creating Gateways directly. `sync.toHost.gatewayApi.gateways` exists for tenant-created Gateway sync, but the examples on this page use imported, control plane-owned Gateways. `GRPCRoute`, `TCPRoute`, and `UDPRoute` are not synced.

vCluster also enforces Gateway API reference authorization before sync. Cross-namespace `allowedRoutes`, `ReferenceGrant`, and unsupported `parentRef`/`backendRef` group+kind combinations are validated in the cluster, and rejected resources surface a `RefNotPermitted` or `SyncError` event on the tenant object.

:::info Prerequisites
Before enabling Gateway API sync, review the [Gateway API prerequisites](../../../../../key-features/gateway-api.mdx#prerequisites). To import a control plane `GatewayClass` into the cluster, also enable [`sync.fromHost.gatewayClasses`](../../from-host/gateway-classes.mdx).
:::
...
Enable sync for tenant-created route and policy resources. Do not enable `sync.toHost.gatewayApi.gateways` for these examples: the platform-owned `Gateway` resources are created in the control plane cluster and imported into the cluster with `sync.fromHost.gateways`.
...
The platform team creates the `Gateway` with an HTTP listener in the control plane cluster. vCluster imports that `Gateway` into the cluster, and the tenant creates an `HTTPRoute` that attaches to the imported Gateway and points at a same-namespace Service backend.

Apply the namespace and backend resources first:

```yaml title="HTTP backend in the cluster"
apiVersion: v1
kind: Namespace
metadata:
  name: gw-demo-01
...
Create the platform-managed Gateway in the control plane cluster. vCluster imports this Gateway into the cluster as `gw-demo-01/edge`; tenants attach routes to the imported copy but do not create the Gateway directly.
...
Then create only the route in the cluster:

```yaml title="HTTPRoute in the cluster"
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
  name: basic
...
```yaml title="TLS backend in the cluster"
apiVersion: v1
kind: ConfigMap
metadata:
...
Create the platform-managed TLS Gateway in the control plane cluster. vCluster imports it into the cluster as `gw-demo-02/edge`.
...
```yaml title="TLSRoute in the cluster"
apiVersion: gateway.networking.k8s.io/v1
kind: TLSRoute
metadata:
  name: passthrough
...
The platform-managed Gateway lives in one namespace in the control plane cluster and is imported into the cluster. Its tenant-facing route policy accepts routes from tenant namespaces whose label matches the selector.
...
Create the platform-managed Gateway in the control plane cluster. vCluster imports it into the cluster as `gw-demo-03-edge/shared`; the `sync.fromHost.gateways.allowedRoutes` override exposes the tenant-facing namespace selector.
...
Create the platform-managed Gateway in the control plane cluster. vCluster imports it into the cluster as `gw-demo-05-routes/edge`.
...
```yaml title="HTTPS route backend resources in the cluster"
apiVersion: v1
kind: Namespace
metadata:
  name: gw-demo-07-edge
...
Create the certificate `ReferenceGrant` and platform-managed Gateway in the control plane cluster. vCluster imports the Gateway into the cluster as `gw-demo-07-edge/edge`.
...
```yaml title="HTTPRoute through the HTTPS listener in the cluster"
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
  name: https-app
...
Create the platform-managed Gateway in the control plane cluster. vCluster imports it into the cluster as `gw-demo-09/edge`.
...
Then create the route and `BackendTLSPolicy` in the cluster:

```yaml title="HTTPRoute with BackendTLSPolicy in the cluster"
apiVersion: gateway.networking.k8s.io/v1
kind: HTTPRoute
metadata:
  name: backend-tls
...
Tenant-owned Gateway API resources such as routes and policies live in the cluster. Imported `Gateway` resources also appear in the cluster, but their source of truth is the control plane cluster. Connect to the tenant with the vCluster CLI before issuing tenant-side `kubectl` commands:
