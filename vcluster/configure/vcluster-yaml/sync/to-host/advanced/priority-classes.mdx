...
On shared nodes, a synced PriorityClass can preempt or starve co-tenant workloads on the same nodes. PriorityClass affects scheduling across the whole control plane cluster, not just the tenant cluster it came from. Leave this disabled unless required. If you enable it, constrain synced values through control plane cluster admission rather than trusting tenant-supplied values. See [operational hardening and lifecycle](../../../../../design/security/shared-nodes-hardening.mdx#operational-hardening-and-lifecycle).
:::
