...
On shared nodes, a synced PriorityClass can preempt or starve co-tenant workloads on the same nodes, since PriorityClass affects scheduling across the whole control plane cluster, not just the tenant cluster it came from. Leave this disabled unless required, and if you enable it, constrain synced values through control plane cluster admission rather than trusting tenant-supplied values. See [availability, disclosure, and lifecycle](../../../../../security/shared-nodes-hardening.mdx#availability-disclosure-and-lifecycle).
:::
