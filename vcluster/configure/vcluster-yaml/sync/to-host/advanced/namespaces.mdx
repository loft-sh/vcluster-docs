...
If namespace is imported through mappings, any workload started in this namespace on host will get automatically imported into vCluster
<Flow>
<Step>
Create namespace matching mappings on host

```bash
kubectl --context kind-vcluster-pro create namespace  h-team-a-namespace-synced-from-host
```
</Step>
<Step>
Run a pod inside this namespace

```bash
kubectl --context kind-vcluster-pro -n h-team-a-namespace-synced-from-host run nginx-from-host --image nginx
```
</Step>
<Step>
...
<Step>
Check pod running in namespace
...
You can set a cleanup policy to control what happens to host cluster namespaces when an associated vCluster is deleted.
