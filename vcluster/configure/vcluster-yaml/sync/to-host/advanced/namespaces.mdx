...
However, tasks like setting up IAM integrations with cloud providers or using custom schedulers can be more complex. For instance, when configuring Workload Identity in Google Cloud Platform (GCP), you need to link a GCP Service Account to a Kubernetes Service Account. This is essential for securely authenticating workloads, but it requires predictable resource names in the host cluster to establish the necessary IAM policy bindings. Similarly, custom schedulers like Run:ai, which manage and allocate resources for AI/ML workloads, rely on a well-defined hierarchy of projects, departments, and node pools with specific quotas and priorities. Without a systematic approach to naming and configuration of resources synced to host in both of these scenarios, managing these integrations and resource allocations can quickly become tedious and error-prone, especially at scale, or just be impossible.
...
Change this behavior to strictly enforce that operations within the vCluster only occur for namespaces defined by your mappings. This means that if you create a namespace in the vCluster which doesn't match any of the configured mappings in vCluster.yaml, the creation of that namespace will be blocked.
...
will get removed from host. All resources that were created on the host and imported into vCluster as result of namespace syncing will remain on the host untouched including the namespace itself.
