...
To use a specific version of a CRD, specify the version before the resource type in the `apiVersion` field, separated by a `/`.

The specified version must exist in the host cluster when vCluster starts. This version becomes the storage version in the vCluster. Custom resources are converted between versions when the storage version differs between the host cluster and vCluster.
