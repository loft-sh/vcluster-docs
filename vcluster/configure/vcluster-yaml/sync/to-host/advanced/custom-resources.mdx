...
If an upgrade introduces a version that is not yet synced, vCluster modifies the CRD inside the virtual cluster to include the new version.
:::
...
This is useful when working with resources like `Certificate`, which reference a `Secret` created by cert-manager. For example:
