...
This feature works for any resource that the control plane cluster serves, either through a CustomResourceDefinition (CRD) or through an [extension API server](#aggregated-api-resources).
...
When an aggregated API resource has a `status` subresource, the status syncs from the control plane cluster to the tenant cluster only. vCluster doesn't write tenant status updates through, and the syncer doesn't push them either. A status written inside the tenant cluster stays there until the extension API server's next status update replaces it.

### Limitations

An aggregated API resource behaves like a CRD-backed one in the tenant cluster. vCluster supports create, read, update, and delete operations only.
