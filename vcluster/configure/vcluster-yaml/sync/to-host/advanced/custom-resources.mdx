...
To use a specific version of a CustomResourceDefinition (CRD), specify the version before the resource type in the `apiVersion` field, using the `/` separator. For example, to use version `v1` of the `certificates.cert-manager.io` CRD:
