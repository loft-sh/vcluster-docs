...
## Resources available to sync from cluster to control plane cluster
<SyncToHostResources/>

## Resources available to sync from control plane cluster to cluster
<SyncFromHostResources/>

## Patch synced resources

Use [patching](./patching/README.mdx) to transform fields, references, and custom resource labels while resources sync between the cluster and the control plane cluster.
