...
With DeviceClass syncing enabled, virtual clusters can reference DeviceClass resources from the host cluster in `resourceclaims` and `resourceclaimtemplates`.
...
When you create a ResourceClaims or ResourceClaimsTemplates resource in the virtual cluster that references a synced DeviceClass, the host cluster's resourceclaim controller handles the resource claims allocation.
...
When DeviceClass syncing is enabled, vCluster can use a label selector to control which DeviceClass, ResourceClaims and ResourceClaimsTemplates resources are synchronized between the host and virtual clusters. 
...
The same selector controls both sync flows. The selector determines which DeviceClass resources are imported from the host cluster and which ResourceClaims and ResourceClaimsTemplates resources can sync to the host cluster based on the referenced DeviceClass resources.
...
and ResourceClaimsTemplates resources specifications with the <code>DeviceClassName</code> field.
...
The selector also functions as a creation validation mechanism - when you create ResourceClaims and ResourceClaimsTemplates
...
any ResourceClaims and ResourceClaimsTemplates resources that reference it remain in the virtual and host clusters. 
These orphaned ResourceClaims and ResourceClaimsTemplates resources stop receiving updates but vCluster does not
...
When you create ResourceClaims and ResourceClaimsTemplates resources that reference a DeviceClass not matching
the selector criteria, several things occur to provide clear feedback:

- The ResourceClaims or ResourceClaimsTemplates fails to sync to the host cluster
- vCluster records an event on the ResourceClaims or ResourceClaimsTemplates resource in the virtual cluster
- The event indicates that the specified DeviceClass is not available according to the current selector configuration

The error output appears as a Kubernetes event that you can view using `kubectl describe`. The event message clearly states that the ResourceClaims or ResourceClaimsTemplates resource was not synced because the referenced DeviceClass does not match the defined selector criteria.
