...
DeviceClass syncing allows clusters to use device classes from the control plane cluster.
With DeviceClass syncing enabled, clusters can reference DeviceClass resources from the
...
You can enable this feature to sync DeviceClass resources from the control plane cluster to the cluster.
...
When you create a ResourceClaims or ResourceClaimTemplates resource in the cluster that references a synced DeviceClass, the control plane cluster's resourceclaim controller handles the resource claims allocation.

This saves resources since you don't need resourceclaim controllers in every cluster.
It also gives you centralized control over which DeviceClass teams can access. 

:::warning Selector scope on shared nodes
The selector controls which `DeviceClass` pool a tenant can draw from. It doesn't control which specific device within an allowed class a tenant receives. It also doesn't stop a synced node from disclosing its GPU capacity and allocatable totals to other tenants on that node. Those are schedulable inventory counts, not another tenant's real-time usage. See [GPU and device isolation](../../../../security/shared-nodes-hardening.mdx#gpu-and-device-isolation) in the shared-node hardening guide.
:::
...
- **Security**: Restrict the DeviceClass resources available in the cluster to enforce access control and prevent unintended configurations.

<EnableSyncing resourceClass="DeviceClass" pluralResourceClass="deviceClasses" />

## How syncing works

When DeviceClass syncing is enabled, vCluster can use a label selector to control which DeviceClass, ResourceClaims and ResourceClaimTemplates resources are synchronized between the control plane cluster and clusters.
This affects the resource types with separate unidirectional sync flows:

- **DeviceClass resources (control plane → tenant)**: vCluster syncs DeviceClass resources from the control plane cluster to the cluster. You cannot create DeviceClass resources directly in the cluster. If a selector is defined, only DeviceClass resources matching the selector will sync. If no selector is defined, **all** DeviceClass resources are synced.

- **ResourceClaims resources (tenant → control plane)**: vCluster syncs ResourceClaims resources from the cluster to the control plane cluster only if all the referenced `deviceClassName` exist in the control plane cluster and any of the following is true:
...
- **ResourceClaimTemplates resources (tenant → control plane)**: vCluster syncs ResourceClaimTemplates resources from the cluster to the control plane cluster only if all the referenced `deviceClassName` exist in the control plane cluster and any of the following is true:
...
Synced DeviceClass resources function like any other Kubernetes resource in the cluster. You
...
When you modify the DeviceClass resource in the control plane cluster, vCluster re-evaluates whether it still matches the selector criteria. If the DeviceClass continues to match, vCluster updates the corresponding resource in the cluster to reflect the changes. If the DeviceClass no longer matches the selector criteria, vCluster removes it from the cluster.
...
resources in the cluster, the resources can only reference DeviceClass resources that exist in the cluster.

### Orphaned resources

When vCluster removes a synced DeviceClass from the cluster due to selector changes or deletion from the control plane cluster,
...
- vCluster records an event on the ResourceClaims or ResourceClaimTemplates resource in the cluster
