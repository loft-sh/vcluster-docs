...
By default, this is disabled by default. vCluster only displays nodes in the cluster if the cluster
...
from the node to be available from the cluster context. In order to access node information, a cluster role is deployed
onto the control plane cluster.

:::warning Node and kubelet disclosure on shared nodes
Syncing real nodes gives every tenant on a shared node visibility into that node's identity. `networking.advanced.proxyKubelets` enables a vCluster-mediated kubelet proxy that backs kubelet metrics scraping and `kubectl top node` against synced nodes. On vCluster v0.34.0 and later, its pod-listing, container-log, and per-pod stats responses are scoped to the tenant's own pods. Node-level metric samples aren't scoped on any version. Disable it unless you need that workflow, both to close that remaining gap and as a safeguard on any version older than v0.34.0. Always set `clearImageStatus: true` alongside this setting on shared nodes, and review [node and kubelet disclosure](../../../../security/shared-nodes-hardening.mdx#operational-hardening-and-lifecycle) before enabling it for trusted tenants.
:::
...
By default, vCluster is able to schedule pods to *any* of the worker nodes in the control plane cluster, but you can isolate a cluster to only use
a set of specific nodes from the control plane cluster. There are multiple reasons why you may want to dedicate nodes to a cluster including:
...
Sync nodes to the cluster to view real node information. The cluster will only display
...
When syncing nodes, only nodes that have pods deployed on them will show up in the cluster, but you can
...
```yaml title="Make all nodes visible to the cluster"
sync:
  fromHost:
    nodes:
      enabled: true
...
By default, node syncing is one directional. Labels and taints travel from the
control plane cluster to the cluster, and changes made in the cluster are not
reflected on the host nodes. 

Enabling `syncBackChanges` allows labels and taints set from the cluster context to be synced to the host nodes. Additional
 permissions are added to the cluster role to edit the host nodes.

```yaml title="Sync labels and taints from the cluster context to the host node"
sync:
  fromHost:
    nodes:
      enabled: true
...
Use `sync.fromHost.nodes.patches` to transform node fields while syncing nodes from the control plane cluster to the cluster. See [Patching synced resources](../patching/README.mdx) for syntax, directionality, and examples.
