...
By default, no API version is required for syncing CRDs. If there are multiple API versions and no version defined, then vCluster uses the storage version. If you have multiple versions of CRD and want to specify a specific API version, you can include that in your `vcluster.yaml`.
...
If you want to use a specific version, you can specify it after the "/" separator, e.g. for version `v1` of the `clusterissuers.cert-manager.io` CRD.
This is quite useful if you want to use a specific version of a CRD that is not the storage version or if you are not sure on what is the storage version. You can specify it like this:
...
Note that vCluster only supports one version of a custom resource to be synced. If you specify multiple versions, vCluster fails starting up.
If an upgrade involves a version not yet synced, the CRD is modified to incorporate the new version in the vCluster."
:::
