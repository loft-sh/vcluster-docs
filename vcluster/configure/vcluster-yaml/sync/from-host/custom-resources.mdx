...
To specify a particular version of a CustomResourceDefinition (CRD), prepend the version to the resource type, separated by a `/`, using the format `<resource>/<version>` in your configuration.
