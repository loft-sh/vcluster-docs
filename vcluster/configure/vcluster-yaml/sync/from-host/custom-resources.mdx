...
When you enable custom resource syncing, vCluster first makes the resource type available in the tenant cluster. It copies the control plane cluster's CustomResourceDefinition (CRD) for a CRD-backed resource. For a resource served by an extension API server, vCluster generates a schemaless CRD instead. It then begins syncing the resource instances. This is particularly useful for sharing cluster-wide resources or `ClusterStores` from [external-secrets](https://external-secrets.io/latest/) with tenant workloads. For details, see [Aggregated API resources](#aggregated-api-resources).
