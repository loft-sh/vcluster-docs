...
By default, vCluster does not require you to specify an API version when syncing custom resources. If a CRD has multiple versions and no version is defined, vCluster will use the storage version.
...
To use a specific version of a CustomResourceDefinition (CRD), include the version before the resource type in the `apiVersion` field, separated by a `/`.
...
During an upgrade, if a new version of the CRD is required and not yet synced, vCluster automatically updates the CRD to include the new version.
:::
...
Enabling this allows you to sync namespaced custom resources from specific namespaces in the host cluster to corresponding namespaces in the virtual cluster.
