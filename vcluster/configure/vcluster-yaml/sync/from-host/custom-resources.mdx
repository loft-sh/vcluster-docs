...
When you enable custom resource syncing, vCluster first copies the CustomResourceDefinition (CRD) from the control plane cluster to the tenant cluster, then begins syncing the actual custom resource instances. This is particularly useful for sharing cluster-wide resources like cluster-scoped custom resources or `ClusterStores` from [external-secrets](https://external-secrets.io/latest/) with your tenant cluster workloads. vCluster can also sync resources that an extension API server serves instead of a CRD. For details, see [Aggregated API resources](#aggregated-api-resources).
...
vCluster automatically adds the required cluster RBAC permissions for retrieving the CustomResourceDefinition and syncing the resources from the control plane cluster to the tenant cluster. Resources served by an extension API server can need extra rules. See [Authorization on the control plane cluster](#authorization-on-the-control-plane-cluster).
:::
...
An aggregated API resource behaves like a CRD-backed one in the tenant cluster. vCluster supports create, read, update, and delete operations only, and from-host sync narrows that to reads because the synced copies are read-only.
