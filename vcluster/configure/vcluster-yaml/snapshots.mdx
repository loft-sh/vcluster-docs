...
For on-demand snapshot creation and restore operations, see the [vCluster Snapshot and Restore](/docs/vcluster/next/operate/backup-restore/backup) documentation.
:::
...
If you use private nodes, see [Use snapshots with private nodes](../../operate/backup-restore/restore.mdx#use-snapshots-with-private-nodes) for additional restore steps.

:::warning Deployed etcd isn't supported
Snapshots aren't supported with [deployed etcd](./control-plane/components/backing-store/etcd/deploy.mdx) (`controlPlane.backingStore.etcd.deploy`), so don't rely on scheduled snapshots to protect those tenant clusters.
:::
