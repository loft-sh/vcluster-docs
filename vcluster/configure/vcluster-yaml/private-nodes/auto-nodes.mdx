...
### Node claim properties

A node provider can set a `properties` map at the provider level. Use it for details the provider needs to create nodes, such as region or subscription. Independently, each static or dynamic node pool can set its own `properties` map, passed through to the node claim the pool creates.

`properties` doesn't select or filter anything, unlike `nodeTypeSelector`. It supplies extra operational details the node provider needs when creating a node from this pool, such as which region or subscription to use.

If a provider and a node pool both set the same property key, the node pool's value takes precedence.

```yaml title="Example of static and dynamic node pools setting properties, with provider-level defaults"
privateNodes:
  enabled: true
  autoNodes:
    - provider: my-provider
      # Provider-level properties apply to every pool using this provider.
      properties:
        region: us-east-1
      static:
        - name: my-static-pool
          quantity: 1
          # Pool-level properties are set on this pool's node claims.
          # A key set here overrides the same key set at the provider level,
          # so this pool's nodes get region "us-west-2", not "us-east-1".
          properties:
            region: us-west-2
      dynamic:
...
If a pool doesn't set `profile`, nodes it provisions fall back to the <GlossaryTerm term="tenant-cluster">cluster</GlossaryTerm>'s `defaultProfile` or the project's default node profile, if either is configured. A node resolves to at most one profile.
