...
If you do not deploy vCluster through the platform (either via CLI, GUI or CRD), create an access key with the role `vcluster` in the platform:
```yaml
...
And then create a secret in the host cluster with the following fields:
```yaml
apiVersion: v1
kind: Secret
metadata:
...
stringData:
  ...
  project: default # the projects the vCluster should show up in the platform
  insecure: "true" # if platform ca cert is untrusted
```

Add the following configuration to the `vcluster.yaml` and vCluster will read the access key from that secret (you can reuse the same secret for all created vClusters in that host cluster):
```yaml
external:
  platform:
    apiKey:
...
```bash
VCLUSTER_VERSION=v0.26.0-alpha.7
