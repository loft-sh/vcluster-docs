...
The configuration file controls resource synchronization between clusters, network access methods, storage persistence, authentication settings, and external service integrations. You can apply most configurations during deployment or upgrades, though some settings like the data store must be configured during initial deployment.
...
:::note
The `vcluster.yaml` file is optional. You can deploy vCluster with default settings or use command-line flags like `--expose` and `--isolate` instead.
:::
