  ...
  <TabItem value="shared" label="Shared access key" default>
    <Flow>
      <Step title="Create the API Key">
        ...
        In the platform console, create an [access key](/platform/next/api/authentication). Assign the `vcluster` role when creating the key.
...
<InterpolatedCodeBlock
  ...
  platform:
    apiKey:
      secretName: [[VAR:SECRET_NAME:vcluster-platform-api-key]]  # Defaults to vcluster-platform-api-key if undefined
      namespace: [[VAR:NAMESPACE:<vcluster-namespace>]]  # Namespace to search for the secret. Defaults to the vCluster namespace if undefined. If different, vCluster needs access to this namespace.
      createRBAC: [[VAR:CREATE_RBAC:true]]  # Defaults to true. Automatically creates the necessary RBAC roles and role bindings.
`}
  language="yaml"
/>
