...
<Tabs>
  ...
  ### Connect a virtual cluster to the platform using a shared access key
    ...
    <Flow>
      ...
      <Step title="Assign the vCluster Role">
        ...
        When creating the access key, you must define the `vcluster` role inside the API key YAML specification in the platform console:
  ...
  <Flow>
    ...
    :::note
    This configuration requires each user to authenticate with their personal access key and eliminates the need to create a shared Kubernetes Secret.
    :::
    </Step>
    ...
    Each user can connect to the virtual cluster using their personal access key via the CLI.
