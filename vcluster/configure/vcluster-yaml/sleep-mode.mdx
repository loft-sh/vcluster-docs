...
:::warning
Sleep mode is intended for pre-production use cases only, and has limitations when used on a vCluster not connected to the platform.
:::
...
:::tip Without an agent 🚫🕵🏻‍♂️
**How it works**: The vCluster manages sleep mode itself by scaling down workloads (pods, Deployments, StatefulSets, etc.) while keeping the control plane active.
...
**Use when**: You want automatic sleep/wake feature but don't have the platform or prefer vCluster-managed sleep mode.
:::

:::tip With a platform agent ✅🕵🏻‍♂️
...
**Use when**: You want maximum resource savings and advanced features like auto-deletion, or you're already using the platform for vCluster management.
:::

### Key differences

| Feature | Without agent 🚫🕵🏻‍♂️ | With agent ✅🕵🏻‍♂️ |
|---------|---------------|------------|
...
| Management | vCluster manages itself | Platform agent manages |
...
When you connect a vCluster that has sleep mode configured to the platform (via an agent), the platform agent automatically takes over sleep mode management. The vCluster stops managing its own sleep mode and the agent handles all sleep/wake operations.

:::tip Transitioning from vCluster-managed to agent-managed sleep mode
When a vCluster with sleep mode configured connects to the platform:
...
- The vCluster's sleep mode configuration continues to work, but is now managed by the platform agent
:::
...
A deployment resource in Kubernetes manages a set of identical pods. Configuring sleep mode for a deployment scales down the pods when the cluster sleeps.

:::tip Control plane behavior
- **Without agent**: The control plane remains active, allowing the vCluster to monitor activity and wake up automatically.
