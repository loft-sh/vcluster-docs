...
:::warning
Sleep mode is intended for pre-production use cases only, and has limitations when used on a standalone vCluster not connected to [the platform](../../../../platform/use-platform/virtual-clusters/key-features/sleep-mode).
:::
...
- Delete bare pods: Bare pods (those not managed by controllers) are removed permanently.
...
You can prevent resources from entering sleep mode by:
...
- Adding the appropriate labels to workloads that should remain active.

## Detect activity in sleep mode

To wake a sleeping cluster or update the last active time, sleep mode captures the following actions:
...
The [resource exemption](#resource-exemption) feature keeps specifically configured resources active when the vCluster enters sleep mode. The table in the following dropdown describes how to ignore requests that keep the cluster awake or wake it up if it is asleep.
...
This requires compatible versions of vCluster and the platform. The table in the following dropdown provides the steps needed for installing or uninstalling an agent.
...
For ingress activity detection:

- Ensure the vCluster pod is discoverable by the ingress controller.
- Verify DNS lookup for `<vcluster-namespace>.<vcluster-svc-name>.svc.cluster.local` resolves correctly.
- Configure the correct `dnsPolicy` in the ingress controller Helm installation.

## Configure sleep mode in a virtual cluster

The following example demonstrates how to configure sleep mode in a virtual cluster.
...
Configure sleep mode with a deployment resource using the following example:
...
Configure sleep mode with an ingress resource using the following example:
