...
an agent on the host cluster can shut down the vCluster workload, i.e. the control plane, entirely. A virtual cluster on its own must remain active
...
This requires compatible versions between vCluster and platform. Below is a table of compatibility and migration.


| vCluster Version | Platform Version | Compatibility |  Installing an agent | Uninstalling an agent |
| -- | -- | -- | -- | -- |
| 0.22.x | 4.2.x | ✅ | Manually update the configuration, moving it from `experimental` to `external.platform`. Translate any durations like `90m` into equivalent seconds e.g. `5400`.  Schema validation prevents configuring both at the same time. | Configuration needs to be manual reverted. |
| 0.23.x | 4.2.x | ✅ | Manually update the configuration, moving it from `experimental` to `external.platform`. Translate any durations like `90m` into equivalent seconds e.g. `5400`.  Schema validation prevents configuring both at the same time. | Configuration needs to be manual reverted. |
