...
The [resource exemption](#resource-exemption) feature keeps specifically configured resources active when the vCluster enters sleep mode. The following describes how to ignore requests that would normally keep the cluster awake or wake it up if it is asleep.
