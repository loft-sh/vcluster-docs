...
The Resource Proxy feature enables <GlossaryTerm term="vcluster">vCluster</GlossaryTerm> to proxy requests for custom resources defined by CustomResourceDefinitions (CRDs) to other tenant clusters. When enabled, the client <GlossaryTerm term="tenant-cluster">tenant cluster</GlossaryTerm> transparently forwards resource operations to a target tenant cluster, where the objects are stored and can be managed by controllers running there.

This page covers the `vcluster.yaml` fields. For the mechanism, see [How the resource proxy works](../../../understand/resource-proxy.mdx). To set one up, see [Set up the resource proxy](../../../learn-how-to/resource-proxy-setup.mdx). For ongoing operation and monitoring, see [Operate the resource proxy](../../../operate/resource-proxy-operations.mdx). If something's broken, see [Troubleshoot the resource proxy](../../../operate/troubleshoot/resource-proxy.mdx).
...
- Connection upgrade verbs such as `exec`, `attach`, and `port-forward` aren't supported through the proxy. See [Connection upgrades return 501](../../../operate/troubleshoot/resource-proxy.mdx#connection-upgrades).
...
- **DERP relay tuning and the monitoring guide** → [Operate the resource proxy](../../../operate/resource-proxy-operations.mdx)
...
- **Troubleshooting steps and the log reference** → [Troubleshoot the resource proxy](../../../operate/troubleshoot/resource-proxy.mdx)
...
- **Best practices** → distributed into the sections above: explicit resource keys under [Wildcard group and version entry](#wildcard-kind-matching), staying on `owned` mode under [Access modes](#access-modes), and the rest into [Operate the resource proxy](../../../operate/resource-proxy-operations.mdx) and [Troubleshoot the resource proxy](../../../operate/troubleshoot/resource-proxy.mdx)
