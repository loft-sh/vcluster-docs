...
# Peer list going stale. This is the one leading indicator: 503s start once the
# age crosses the 60-second staleness tolerance.
- alert: ResourceProxyPeerListStale
  expr: vcluster_proxy_peer_list_age_seconds > 45
  for: 2m
  labels: {severity: warning}
  annotations:
    summary: 'Resource proxy target {{ $labels.target }}: peer list age {{ $value }}s, 503s start at 60s'
...
| `Ready` | `peer cache did not sync in time for the startup probe` or `peer watch subscription is down` in logs | The client cannot reach the platform because of a license, network, or platform issue. The target is fine. The path to it is not. |
