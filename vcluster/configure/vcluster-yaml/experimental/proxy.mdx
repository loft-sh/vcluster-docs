...
The Resource Proxy feature enables vCluster to proxy custom resource requests to other virtual clusters. When enabled, the source virtual cluster transparently stores resources in and delegates management to a target virtual cluster. This enables cross-cluster communication patterns, centralized resource management, and multi-tenant architectures.
...
- **Multi-tenant isolation**: Each source virtual cluster only sees its own resources by default.
...
| `targetVirtualCluster` | string | Name of the target virtual cluster to proxy requests to. Must be in the same project. |
...
Create RBAC rules to allow the client virtual cluster to access resources. The client virtual cluster authenticates using its platform identity in the format `loft:vcluster:<project>:<name>`:
...
The `accessResources` field controls which resources a source virtual cluster can see in the target:

<CodeBlock language="yaml" title="Access modes configuration">{ProxyAccessModesConfig}</CodeBlock>

### `owned` mode - default {#owned-mode}

Each source virtual cluster only sees resources it created. This enables multi-tenant isolation where multiple source virtual clusters can share a target without seeing each other's resources.

### `all` mode {#all-mode}

The source virtual cluster can see all resources in the target, regardless of who created them. This is useful for read-only observers, centralized dashboards, or admin access.
...
- Proxy all resources within the same API group and version to the same target virtual cluster.
- The target virtual cluster must be in the same project as the source virtual cluster.
...
1. **Check RBAC configuration**: Ensure the source virtual cluster's identity has correct permissions in the target.
