...
### Pods add `imagePullSecrets` hooks (Pro)

<ProAdmonition/>

Configure `hooks` to add `imagePullSecrets` to all `pods` in all `namespaces` created in the virtual cluster. The `secrets` specified must be available in the host cluster `namespace` that the `pod` is synced.
