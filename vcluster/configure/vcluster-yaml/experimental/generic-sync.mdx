...
:::warning Deprecated
This experimental feature is now deprecated. There are currently no plans to remove it, but further functionality is not expected to be added. For more advanced capabilities, please consider using the custom resource sync ([fromHost](../sync/from-host/custom-resources), [toHost](../sync/to-host/advanced/custom-resources)) 
