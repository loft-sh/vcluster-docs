...
## Schedule cluster pods

You can configure how and where cluster pods are scheduled within the Kubernetes cluster. Configure the scheduling field with the following options to control pod placement in the cluster:
...
- **`affinity`** 
  ...
  - Use anti-affinity (repel the pod) to keep cluster pods on separate nodes. A common technique is to make cluster pods repel each other so that they are not scheduled on the same nodes. This increases resiliency if a node is scaled down or replaced.  
  - Use affinity (attract the pod) to group related pods together and reduce network latency for critical services.

- **`tolerations`**  
  Allow pods to run on nodes with specific taints. Use this to schedule cluster pods on nodes that would otherwise reject them. A common pattern is to taint nodes for non-cluster workloads and configure cluster pods to tolerate the taint. This keeps clusters separate from critical workloads. See [Taints and Tolerations](https://kubernetes.io/docs/concepts/scheduling-eviction/taint-and-toleration/) for more information.
