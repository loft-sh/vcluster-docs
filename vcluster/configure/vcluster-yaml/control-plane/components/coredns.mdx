...
# CoreDNS in virtual clusters: Complete guide

## Understand CoreDNS in virtual clusters

Each virtual cluster instance operates with its own DNS service, CoreDNS, deployed in two configurations: as a separate pod (default) or integrated within the syncer (vCluster Pro feature). Both configurations ensure DNS functions according to standard Kubernetes expectations. Pods and services within each virtual cluster discover each other using standard Kubernetes DNS names such as `my-service.default.svc.cluster.local`. Each virtual cluster maintains its own DNS view, preventing applications in different virtual clusters from discovering or accessing services outside their designated boundaries, despite sharing the same host cluster infrastructure.

CoreDNS in virtual clusters performs name resolution and resource mapping functions. When a pod queries a service such as `my-app.default.svc.cluster.local`, CoreDNS returns a service IP address that appears as a standard cluster service. The virtual cluster maps this request to a corresponding service in the host cluster with a different name.

The syncer component performs this mapping function regardless of CoreDNS deployment configuration. It translates standard Kubernetes DNS names to actual services running in the host cluster. Applications receive normal Kubernetes DNS responses without awareness of the underlying translation process.

CoreDNS behavior knowledge is required for troubleshooting connectivity issues, implementing custom networking policies, and optimizing performance for specific workloads. The DNS system maintains security boundaries between virtual clusters by providing each instance with an isolated view of available services and resources.

## Learn how DNS resolution works in virtual clusters

DNS resolution in virtual clusters involves a multi-stage process that maintains Kubernetes compatibility while managing the virtual-to-physical resource mapping. This ensures apps receive expected DNS responses while the infrastructure handles routing to actual host cluster resources.

When an application pod initiates a DNS query, the request travels to the virtual cluster's CoreDNS instance on port 1053. This CoreDNS maintains a complete view of all services that exist within the virtual cluster's namespace scope and resolves queries to virtual IP addresses that exist only within the context of that specific virtual cluster.

The application then attempts to establish a connection to the resolved virtual IP address. At this point, the virtual cluster syncer component intercepts the network traffic and performs the translation between virtual cluster resources and their corresponding sync resources in the host cluster. The syncer understands the mapping between virtual services and host services, which typically have names that include prefixes and suffixes to ensure uniqueness across multiple virtual clusters on the same host infrastructure.
...
## Choose deployment architectures

Virtual clusters support two distinct CoreDNS deployment models.

### Deploy separate CoreDNS

A normal virtual cluster deployment consists of two pods per virtual cluster instance: the virtual cluster pod containing the API server and syncer components, and a separate CoreDNS pod.

```mermaid
graph LR
    subgraph "Virtual Cluster Deployment"
...
This isolation provides operational benefits for production environments. DNS issues can be debugged, updated, or restarted independently without affecting the virtual cluster's control plane availability. Resource allocation can be tuned specifically for DNS workloads, which may require different CPU and memory profiles than the API server and syncer components.

The separate model also simplifies monitoring and observability since DNS metrics, logs, and performance characteristics can be tracked independently. This separation becomes valuable when scaling operations, as DNS and control plane components may need different horizontal scaling strategies based on their respective workload patterns.

### Deploy integrated CoreDNS with vCluster Pro

The integrated CoreDNS feature allows you to run CoreDNS as part of the syncer, which saves the overhead of an external CoreDNS pod. This Pro feature consolidates all three virtual cluster components into a single pod, creating a more resource-efficient deployment model.

```mermaid
graph LR
    subgraph "Virtual Cluster Pro Deployment"
...
## Understand why CoreDNS uses port 1053

CoreDNS in virtual clusters listens on port 1053 instead of the default DNS port 53 to avoid conflicts with the host cluster's DNS. This port selection addresses several technical and operational challenges inherent in running DNS services within virtual cluster environments.

```mermaid
graph TD
    subgraph "Host Cluster Network"
        A[Host CoreDNS/kube-dns<br/>Port 53] 
        B[Virtual Cluster A CoreDNS<br/>Port 1053]
        C[Virtual Cluster B CoreDNS<br/>Port 1053]
        D[Virtual Cluster C CoreDNS<br/>Port 1053]
    end
    
    E[Host Cluster Pods] -->|DNS Queries| A
    F[Virtual Cluster A Pods] -->|DNS Queries| B
    G[Virtual Cluster B Pods] -->|DNS Queries| C
    H[Virtual Cluster C Pods] -->|DNS Queries| D
...
The non-standard port also simplifies network policy management and firewall configuration. Network administrators can distinguish between host cluster DNS traffic on port 53 and virtual cluster DNS traffic on port 1053, enabling more granular security policies and clearer traffic analysis for troubleshooting purposes.

### Consider cloud provider networking

Different cloud providers have varying network policies that can affect DNS traffic on port 1053, including EKS clusters where DNS queries might fail when pods are scheduled on different nodes. Understanding these provider-specific requirements is essential for ensuring reliable DNS operation.
...
graph TD
    subgraph "Virtual Cluster DNS Scope"
        A[App Pod] -->|Query: my-service.default.svc.cluster.local| B[Virtual Cluster CoreDNS]
...
When a pod inside a virtual cluster attempts to resolve a DNS name for a service that exists only in the host cluster, the virtual cluster CoreDNS instance returns an NXDOMAIN response because it maintains no knowledge of host cluster resources. This behavior ensures that virtual clusters cannot accidentally discover or access resources they should not have permission to use.
...
Resource synchronization offers a more sophisticated approach by importing actual host cluster services into the virtual cluster namespace. This pattern uses virtual cluster's synchronization capabilities to create mirror services that represent host cluster resources with full Kubernetes service features.
...
When configured with this synchronization pattern, virtual cluster imports services from the host cluster that match the specified label selector. The imported services appear as native virtual cluster services with complete endpoints and all standard Kubernetes service capabilities, enabling full integration with service mesh technologies, network policies, and other Kubernetes-native tools.

## Configure CoreDNS patterns

CoreDNS configuration in virtual clusters supports extensive customization that enables operators to adapt DNS behavior for specific environments, integrate with existing infrastructure, and optimize performance for particular workloads. Understanding these configuration patterns helps in implementing sophisticated DNS policies and troubleshooting complex networking scenarios.
...
### Configure integrated CoreDNS

vCluster Pro's integrated CoreDNS feature combines CoreDNS capability within the syncer container. This configuration requires vCluster Pro and provides optimization for resource-constrained environments.
...
The integrated configuration reduces total resource allocation since DNS and syncer functions share the same container runtime environment. This approach is beneficial in environments where pod limits constrain the number of virtual clusters that can be deployed, or where resource optimization is a primary concern.
...
DNS problems in virtual cluster environments can manifest in various ways, from complete service discovery failures to intermittent connectivity issues. Systematic troubleshooting approaches help identify root causes quickly and implement appropriate solutions that restore normal operation.
...
Testing network connectivity to the CoreDNS service helps identify whether DNS queries can reach the DNS service on port 1053. Network policy restrictions, firewall rules, or security group configurations may block this traffic, especially in cloud environments with restrictive default settings.
...
CoreDNS performance optimization in virtual cluster environments requires understanding DNS query patterns, resource allocation strategies, and configuration tuning approaches that balance performance with resource consumption and operational complexity.
...
Production CoreDNS deployments in virtual cluster environments require careful attention to reliability, security, monitoring, and operational procedures. These practices help ensure stable DNS resolution while minimizing operational complexity and security risks.
