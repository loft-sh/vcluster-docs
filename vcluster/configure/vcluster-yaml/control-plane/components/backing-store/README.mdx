---
title: Backing Store
sidebar_label: Overview
sidebar_position: 0
description: Configuration for backing store
sidebar_class_name: host-nodes private-nodes standalone
slug: /configure/vcluster-yaml/control-plane/components/backing-store
---


import BackingStore from '../../../../../_partials/config/controlPlane/backingStore.mdx'

Each cluster requires a backing store and <GlossaryTerm term="vcluster">vCluster</GlossaryTerm> provides two different types of backing store options:

* [etcd](./etcd/README.mdx)
* [database](./database/README.mdx)

There are different ways to deploy the type of backing store.

By default, vCluster uses an embedded SQLite database as the backing store. This option is great for smaller sandbox clusters, but for production, it's recommended to use a different backing store option.

:::warning Backing store migration has limited support
vCluster doesn't support changing the backing store type after initial deployment by editing configuration in place. A cluster that already runs on deployed etcd also has no [snapshot or restore support](./etcd/deploy.mdx#back-up-and-restore) for backing itself up or recovering in place, so choose a backing store type before you deploy to production.

Two exceptions exist:
- Migration from [deployed etcd to embedded etcd](../../../../../manage/migrate-etcd-backing-store.mdx). Both modes use etcd as the backing store technology.
- Restoring an embedded etcd snapshot onto a <GlossaryTerm term="tenant-cluster">cluster</GlossaryTerm> configured with an embedded database, external database, or deployed etcd backing store. vCluster converts the snapshot automatically before restoring it. See [Migrate from embedded etcd to a different backing store](../../../../../manage/backup-restore/restore.mdx#migrate-from-embedded-etcd-to-a-different-backing-store).

Migrating in other directions, such as from a database backing store to <GlossaryTerm term="etcd">etcd</GlossaryTerm>, isn't documented or tested.
:::

:::note vCluster Standalone
vCluster Standalone has limited backing store options. For single <GlossaryTerm term="control-plane">control plane</GlossaryTerm> nodes setup,
* Embedded database (sqlite)
* Embedded etcd

are supported.

For multiple control plane nodes (HA) setup,
* Embedded etcd

is supported.
:::
## Config reference

<BackingStore/>
