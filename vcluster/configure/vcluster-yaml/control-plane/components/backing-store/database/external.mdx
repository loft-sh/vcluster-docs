...
description: Configure an external database as the cluster's backing store for enhanced performance and scalability.
---
...
This guide explains how to configure an external database as the backing store for a <GlossaryTerm term="tenant-cluster">cluster</GlossaryTerm>. A backing store is a persistent storage solution that maintains the state and data of the cluster. Using an external database can provide better performance, scalability, and data persistence compared to the default embedded storage.

Configure this feature to use an external database such as MySQL or PostgreSQL for your cluster's backing store.
...
Before configuring an external database for your cluster, ensure you have the following prerequisites based on your chosen method:
...
- Network connectivity from the cluster to the database server
...
3. [**Platform API key**](/docs/vcluster/configure/vcluster-yaml/platform): Your cluster must be connected to the platform

:::info
The **connector** option provides automatic database provisioning, credential management, and cleanup when the cluster is deleted. The **dataSource** option gives you direct control but requires manual database and user management.
:::
...
`dataSource`: assign a connection string to `dataSource` that the cluster uses for its <GlossaryTerm term="control-plane">control plane</GlossaryTerm> backing store.

`connector`: assign a name of a [connector secret](/platform/administer/connector/database) that exists in an instance of the platform in the installed namespace. The platform uses the secret to automatically provision a separate database within the database server for the cluster. It also creates a non-privileged user that can only interact with the cluster's database. The cluster receives a connection string built from the user and database.

|                               | Connector | DataSource |
| ----------------------------- | --------- | ---------- |
| MySQL Support                 | Yes       | Yes        |
| PostgreSQL Support            | Yes       | Yes        |
| Share Across clusters  | Yes       | No         |
| Automatic DB Cleanup          | Yes       | No         |
| Credential stored in secret   | Yes       | No         |

:::warning Snapshot and restore
`vcluster snapshot create` and `vcluster restore` work with an external database, whether configured with `dataSource` or `connector`, but they operate on the cluster's Kubernetes key/value data through kine, not as a native database backup. vCluster doesn't back up or roll back the external database itself. See [Restore limitations](../../../../../../manage/backup-restore/restore.mdx#limitations).
:::
...
After completing the [prerequisites](#connector-platform-managed), reference the connector secret in your cluster configuration:
...
:::note
The cluster must be [connected to the platform](/docs/vcluster/configure/vcluster-yaml/platform) to use the connector. This enables centralized management and monitoring of clusters.
:::
...
When you use an external PostgreSQL `dataSource`, the cluster's `kine` connects to your PostgreSQL backend. Instead of a static password in the connection string, that connection can authenticate with **Kerberos/GSSAPI** using a mounted keytab. This suits enterprises that already operate a Kerberos realm (an MIT KDC or Active Directory) and require Kerberos for all database access.
...
- A cluster running the Kubernetes distro image at **v1.36 or later** (see the admonition above).
- An existing **Kerberos environment**:
  - A KDC and realm (for example `EXAMPLE.COM`).
  - A **client principal** for the cluster, for example `krb5-db-user@EXAMPLE.COM`, that you can export to a keytab.
...
Add a `gss` rule to `pg_hba.conf` so remote TCP connections from the cluster authenticate over Kerberos. PostgreSQL uses the first matching `pg_hba.conf` rule, so place this rule before broader `host` rules that use another authentication method. Scope the address range to the cluster network where possible:
...
Create a Kubernetes Secret holding the client keytab and a ConfigMap holding the client `krb5.conf`, both in the cluster's host namespace (for example `vcluster-my-vcluster`). The control plane pod mounts them in [Step 4](#step-4---configure-the-vclusteryaml-file).
...
Write a local `krb5.conf` file that points at your KDC. When your service DNS names lack matching PTR records, disable DNS canonicalization and reverse lookups so the service principal name matches the host the cluster connects to:
...
### Step 5 - Create the cluster and confirm authentication

Create the cluster with your configuration:

```bash
vcluster create my-vcluster --namespace vcluster-my-vcluster -f vcluster.yaml
```

The cluster becomes ready once `kine` completes the GSSAPI handshake and connects to PostgreSQL. The data source carries no password, so Kerberos is the only authentication path available.

If the cluster stays unready, inspect the control plane pod logs:
...
## Network policies
If you are using [network policies](../../../../policies/network-policy.mdx), the cluster control plane traffic to the external database must be allowed.
