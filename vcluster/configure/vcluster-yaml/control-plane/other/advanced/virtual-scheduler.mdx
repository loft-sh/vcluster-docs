...
- Labeling nodes inside the virtual cluster has **no effect** on scheduling unless [node syncing from the host](../../../sync/from-host/nodes) is enabled using `sync.fromHost.nodes.enabled: true`.
- Draining nodes inside the virtual cluster does **not** affect scheduling.
