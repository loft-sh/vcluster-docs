...
sidebar_position: 4
description: Configure embedded kube-vip for cluster control plane.
...
Kube-vip announces the cluster's control plane endpoint IP address on a specified network interface using ARP (layer 2).
...
2. Kube-vip uses leader election within the cluster to determine which control plane replica manages the VIP
...
```yaml
# kube-vip is only compatible with private node clusters
privateNodes:
  enabled: true
