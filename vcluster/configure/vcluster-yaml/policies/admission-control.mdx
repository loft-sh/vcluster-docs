...
This setting wires up the webhook plumbing only. It provides no protection until a reachable policy engine on the other end has active policies whose rules actually cover the resource kinds and operations you care about. Verify the engine has rules loaded, not just that the webhook configuration exists. Tenant-facing admission also never sees translated, synced objects, or workloads a host-side controller generates from a tenant custom resource, such as a KubeVirt `virt-launcher` pod. Those need control plane cluster-side admission instead. See [admission and policy coverage](../../../security/shared-nodes-hardening.mdx#admission-and-policy-coverage).
:::
