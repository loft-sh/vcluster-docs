...
sidebar_position: 1
description: Configure network policies to isolate cluster workloads and implement project-scoped network boundaries.
...
By default, workloads created by vCluster are able to communicate with other workloads in the control plane cluster through their cluster IPs. Configure network policies when you want to isolate namespaces and do not want the pods running inside the cluster to have access to other workloads in the control plane cluster.
...
- Allow traffic between pods within the cluster
...
metadata:
  ...
  labels:
    ...
    release: {name}
spec:
  # Pod selector matching cluster workloads pods.
  podSelector:
    matchLabels:
...
metadata:
  ...
  labels:
    ...
    release: {name}
spec:
  # Pod selector matching cluster control plane pods.
  podSelector:
    matchLabels:
      release: {name}
  policyTypes:
...
:::warning DNS port in clusters
Cluster DNS runs on port 1053, not the standard port 53. Custom egress rules that allow DNS traffic must target port 1053 or DNS resolution silently fails. UDP packets are dropped with no error in pod logs.
...
policies:
  networkPolicy:
    ...
    controlPlane:
      ingress:
        # Allow ingress traffic from anywhere to the cluster control plane api
        - ports:
            - port: 8443
