...
You can alternatively configure the control plane to output logs in JSON format, which provides structured data compatible with external log aggregation tools (such as Elasticsearch). JSON logs simplify log collection, parsing, and analysis across multiple clusters.
...
The console format provides human-readable logs with timestamps, log levels, source locations, and contextual information. This format works well for use cases where users are troubleshooting a single cluster. Choose this format in environments where human readability is more important than structured log parsing. Example use cases include:

- Local development and testing, where developers benefit from quick, readable output.
- Single cluster analysis, where users manually review logs and debug in a terminal.
