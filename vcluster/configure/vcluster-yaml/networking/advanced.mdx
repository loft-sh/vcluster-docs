---
title: Advanced networking
sidebar_label: advanced
sidebar_position: 3
description: Configuration for advanced networking like cluster domain, fallback cluster, and Kubernetes proxy.
sidebar_class_name: host-nodes
---

import NetworkingAdvanced from '../../../_partials/config/networking/advanced.mdx'
import TenancySupport from '../../../_fragments/tenancy-support.mdx';

<TenancySupport hostNodes="true" />

Each vCluster has its own DNS service, which is CoreDNS by default. DNS allows pods in the tenant cluster to get the IP addresses of services that are also running in the tenant cluster. The syncer ensures that the intuitive naming logic of Kubernetes DNS names for services applies, and that users can connect to these DNS names, which map to the IP address of the synchronized services that are present in the control plane cluster.

However, this also means that you cannot directly access host services inside the tenant cluster using DNS. Host pods can only access tenant cluster services by their synced name. vCluster offers a feature to map services from the tenant cluster to the control plane cluster and vice versa.

**Fallback to Host DNS**

When you enable `fallbackHostCluster`, vCluster falls back to the control plane cluster's DNS for resolving domains. This is useful if the control plane cluster is using Istio or Dapr and the sidecar containers cannot connect to the central instance. It is also useful if you want to access the control plane cluster services from within the tenant cluster.

**`proxyKubelets`**

The `proxyKubelets` section replaces `--fake-kubelet-ips` and `--disable-fake-kubelets`. 

You may need to configure `proxyKubelets` for proper resolution of Kubelet metrics when using Prometheus.

:::warning Kubelet exposure on shared nodes
`proxyKubelets.byHostname` and `proxyKubelets.byIP` enable a vCluster-mediated kubelet proxy, which backs kubelet metrics scraping and `kubectl top node` against synced nodes. `kubectl logs` and `exec` don't depend on this setting. They use an independent Pod subresource redirect, validated against pod and container ownership regardless of `proxyKubelets`. The proxy itself scopes per-pod stats to the tenant's own pods. Pod listing and container log access reached through the proxy aren't scoped to tenant ownership, and neither are node-level metric samples without a pod or PVC label. Disable `proxyKubelets` unless you need the metrics workflow. It's the primary mitigation for this disclosure on shared nodes, not defense in depth. See [node and kubelet disclosure](../../../security/shared-nodes-hardening.mdx#availability-disclosure-and-lifecycle).
:::

{/*
`advanced.clusterDomain`:
- used in certificate generation
- looks like it also updates coredns resolution, according to this PR: https://github.com/loft-sh/vcluster/pull/870

`advanced.fallbackHostCluster`:
- seems pretty well covered by the above

`advanced.proxyKubelets`:
- appears to set `--fake-kubelet-ips` and `--disable-fake-kubelets`
- `advanced.proxyKubelets.ByIP` sets `--fake-kubelet-ips`
- `--disable-fake-kubelets` is set to `true` if both `proxyKubelets.ByIP` and `proxyKubelets.ByHostname` are `false`
- The only reason I've seen to use `--disable-fake-kubelets` is for Kubernetes conformance testing.
- `--fake-kubelet-ips` appears to be needed for proper resolution of kubelet metrics when using prometheus
- More information here: https://linear.app/loft/issue/ENG-1197/prometheus-not-discovering-vcluster-kubelets

28 March: (aimee) Russ' comments about this feature should be incorporated in the Go file
*/}

## Config reference

<NetworkingAdvanced/>