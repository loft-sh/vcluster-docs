...
`proxyKubelets.byHostname` and `proxyKubelets.byIP` make node kubelets reachable from inside the tenant cluster, which backs in-tenant-cluster `kubectl logs` / `exec` / `top node` against synced nodes. On shared nodes, this also exposes kubelet read endpoints for every pod on that node, not only the tenant's own. Disable both unless that workflow is required, and validate the tradeoff before disabling in an environment that depends on it. See [node and kubelet disclosure](../../../security/shared-nodes-hardening.mdx#availability-disclosure-and-lifecycle).
:::
