...
sidebar_position: 12
description: Configuration for enabling or disabling metrics that vCluster Labs collects.
...
### What vCluster Labs collects

vCluster Labs is not interested in collecting data about individuals that are using vCluster. Rather, vCluster Labs collects data about how vCluster is being used. This entails information about the configuration of vCluster, and the environment where it is deployed (for example, Kubernetes version and CPU architecture).

Each vCluster is deployed with a syncer component, which contains all controllers that make a cluster function. This component collects the data and uploads it to vCluster Labs at regular intervals (once every 1-5 minutes). vCluster Labs provides a documented example of the telemetry payload. The source code responsible for this is fully available in the vCluster [repo](https://github.com/loft-sh/vcluster). The telemetry backend is hosted at "https://admin.loft.sh/analytics/v1/vcluster/". The ingestion service is written and maintained by the vCluster maintainers. The data is saved into a relational database with strict access control.
