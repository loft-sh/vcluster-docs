...
- **Authentication methods**: vCluster supports multiple authentication mechanisms to secure access to the virtual cluster. Each authentication method can be customized with specific settings like certificate authorities, token expiration periods, and validation endpoints to match your security requirements:
  - Client certificate authentication for certificate-based access
  - OpenID Connect (OIDC) integration for enterprise authentication  
  - Webhook-based authentication for custom validation
  - Service account token validation for Kubernetes-native authentication
  - Request header authentication when working through proxies
