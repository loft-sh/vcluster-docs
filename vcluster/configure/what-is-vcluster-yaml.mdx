...
```yaml
controlPlane:
  backingStore: # This backing store option is an Enterprise feature and requires a license. Remove it if you don't have a license.
    etcd:
      embedded: 
        enabled: true
  ingress:
...
1. [Embedded etcd](/vcluster/configure/vcluster-yaml/control-plane/components/backing-store/etcd/embedded) as the vCluster control plane's backing store to reduce management overhead (Enterprise feature).
