...
Every virtual cluster requires configuration settings that define how it operates within your host Kubernetes cluster. The `vcluster.yaml` file is the central configuration source for these settings. The `vcluster.yaml` file controls everything from the control plane architecture to resource sync behavior between the virtual and host clusters.
...
- **Authentication methods**: Configure the authentication mechanisms that secure access to the virtual cluster. These settings include:
  - Client certificate authentication requirements
  - Service account token validation rules  
  - OpenID Connect (OIDC) provider settings
  - Webhook token authentication endpoints
  - Request header authentication for proxy integration
  - Authentication token expiration and rotation policies

Each method requires specific configuration such as certificate authorities, issuer URLs, or webhook endpoints depending on your security requirements.
...
When you create a virtual cluster, the vCluster CLI reads your configuration file and applies all settings during deployment. To create a new virtual cluster with a configuration file:
...
Configuration changes often become necessary as requirements evolve. vCluster supports updating existing virtual clusters with new configurations. To update an existing virtual cluster:

<Flow>
<Step>
Edit your `vcluster.yaml` file with the desired changes. For example, enable a new sync feature or adjust resource limits.
</Step>
...
For a complete guide to updating virtual clusters, see the [deployment changes documentation](../manage/deploy-changes/).
