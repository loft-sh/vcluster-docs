...
The file is optional. If you don't provide one, vCluster uses shared nodes (the Control Plane Cluster's node pool) with all resources syncing to a single namespace. Most production deployments benefit from explicit configuration.
...
- **Shared nodes**: Use all or a labeled subset of the Control Plane Cluster's worker nodes.
...
When using shared nodes, the tenant cluster shares certain resources with the Control Plane Cluster to function properly. The sync configuration controls this in both directions:

- **From Control Plane Cluster to tenant**: Configure which Control Plane Cluster resources appear inside the tenant cluster through [host-to-virtual synchronization](/vcluster/configure/vcluster-yaml/sync/from-host/). Common examples include nodes, storage classes, and priority classes.
- **From tenant to Control Plane Cluster**: Define which tenant cluster resources sync to the Control Plane Cluster through [virtual-to-host synchronization](/vcluster/configure/vcluster-yaml/sync/to-host/). This typically includes pods, services, and persistent volume claims.
...
* [Basics](../deploy/control-plane/binary/basics)
* [High Availability](../deploy/control-plane/binary/high-availability)
