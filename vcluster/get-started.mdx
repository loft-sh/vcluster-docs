...
- [**vCluster Control Plane**](/architecture/README.mdx): Contains a Kubernetes API server, a controller manager, a data store mount and the Syncer.
- [**Syncing resources**](/architecture/README.mdx): vCluster runs your workloads by syncing pods from the virtual cluster to the host cluster.
- [**Pod scheduling**](/architecture/README.mdx): By default, vCluster reuses the host cluster scheduler to schedule workloads.
- [**Storage**](/architecture/README.mdx): You can use the host's storage classes without the need to create them in the virtual cluster.
- [**Networking**](/architecture/README.mdx): vCluster syncs resources such as Service and Ingress resources from the virtual to the host cluster.
- [**Nodes**](/architecture/README.mdx): By default, vCluster creates pseudo nodes for every pod `spec.nodeName` in the virtual cluster.
...
1. (Optional) Configure vCluster with `vcluster.yaml`.
   ...
   By default, vCluster uses vanilla Kubernetes as the Kubernetes distribution for the virtual cluster. If you want to change the [Kubernetes distribution](/vcluster-yaml/control-plane/components/distro/README.mdx), you must change it before you deploy vCluster. Create a file called `vcluster.yaml` and configure the distribution. This example configures a different Kubernetes distribution (i.e K3s).
   ...
   Refer to the `vcluster.yaml` [reference docs](/vcluster-yaml/README.mdx) to explore all configuration options.
...
Configure features in a `vcluster.yaml` file. These examples show you how to configure some popular features. See the `vcluser.yaml` [configuration reference](/vcluster-yaml/README.mdx) for how to configure additional features.
