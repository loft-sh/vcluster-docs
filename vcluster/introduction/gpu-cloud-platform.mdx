...
Combine Standalone with [Private Nodes](../deploy/worker-nodes/private-nodes/overview.mdx) to add accelerators from any supported infrastructure provider, whether that's your own bare metal or a cloud GPU instance joined into a tenant cluster.
...
- [Stacks](/docs/platform/next/understand/what-are-stacks) — standardize the applications that make up each tenant's GPU environment
- [Certified Stacks](/docs/platform/next/integrations/certified-stacks) — browse tested integrations bundled with vCluster Platform
