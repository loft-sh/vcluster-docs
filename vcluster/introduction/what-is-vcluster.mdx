...
vCluster helps AI cloud providers deliver highly scalable, resilient compute clusters as self-service tenant products, including managed Kubernetes, Slurm, inference, Ray, and other cluster types. Each tenant gets a dedicated <GlossaryTerm term="tenant-cluster">cluster</GlossaryTerm>, provisioned on your infrastructure or directly on bare metal, with its own control plane and RBAC. Tenants get an isolated cluster experience, with the isolation level set by the worker node model you choose.
...
vCluster works across a range of infrastructure configurations. Each cluster is backed by a dedicated virtualized control plane, built on a certified Kubernetes distribution. That's what lets tools built for the standard Kubernetes API, such as `kubectl`, Helm, Argo, and Crossplane, work against a cluster without modification. It's the same foundation that higher-level cluster types like Slurm, Ray, and inference-serving stacks run on as workloads.
