...
## Feature Availability Across Tenancy Models and Product Tiers

The table below shows all vCluster features, which tenancy models they support, and their availability across product tiers (Open Source, Free, Dev, Prod, Scale, Enterprise).

Features are automatically sorted alphabetically and display:
- **Feature type badges**: [OSS], [Free], or [Paid]
- **Tenancy models**: Which deployment models support the feature (Shared, Private, Standalone)
- **Product tier availability**: Checkmarks (✓) show which tiers include each feature
