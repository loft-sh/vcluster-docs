...
vCluster provides a comprehensive set of features across multiple product tiers, from open source capabilities to enterprise-grade functionality. The table below shows all available features, their product tier availability (OSS, Free, Dev, Prod, Scale, Enterprise), and compatibility with different tenancy models (Shared Nodes, Private Nodes, Standalone).

<!--
  FeatureTable: Complete list of all vCluster features across all tiers.
