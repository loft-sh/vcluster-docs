...
From the cluster's perspective, GPU workloads use the same Kubernetes APIs they would use on a regular cluster:
...
- If the required driver, runtime configuration, device plugin, or DRA driver is missing from the node or cluster, vCluster cannot make the device appear by itself.

For private nodes, each cluster can run its own GPU Operator, device plugin, DRA driver, scheduler, and accelerator CRDs.
...
Review [GPU and device isolation](../security/shared-nodes-hardening.mdx#gpu-and-device-isolation) for the tenant-isolation gaps specific to shared GPU nodes.

## Install the NVIDIA GPU Operator in a cluster

With [Private Nodes](../deploy/worker-nodes/private-nodes/overview.mdx), the cluster can install the NVIDIA GPU Operator directly because its workloads run on dedicated worker nodes. This is the common pattern for AI cloud and inference provider platforms: the cluster owns the GPU Operator, device plugin, DCGM Exporter, MIG Manager, and related CRDs for its private node pool.

:::note
Install the GPU Operator inside the cluster only when that tenant owns the GPU node software stack. On shared host nodes, the platform team usually installs the GPU driver stack and device plugin on the control plane cluster nodes instead.
:::
...
1. Provision or join the GPU nodes to the cluster.
...
4. Decide whether the cluster needs MIG, NVIDIA vGPU, CDI, GPUDirect, or DCGM metrics.
5. Confirm `kubectl` and `helm` point at the cluster, not the control plane cluster.
...
Confirm the cluster sees `nvidia.com/gpu` on the private GPU nodes:
...
If the pod stays pending, check node readiness, taints, GPU resource names, project quotas, allowed node types, and whether the private node has joined the cluster. If Operator pods fail, check the node OS, kernel, container runtime, driver installation mode, and the NVIDIA Operator logs.
...
- [`deviceClasses`](../configure/vcluster-yaml/sync/from-host/device-classes.mdx) syncs allowed `DeviceClass` resources from the control plane cluster to the cluster.
...
Once `deviceClasses` sync is enabled, platform administrators create [`DeviceClass`](https://kubernetes.io/docs/reference/kubernetes-api/resource/device-class-v1/) resources on the control plane cluster and choose which classes are visible in each cluster.

For private nodes, tenants can also run the DRA driver and related controllers inside their cluster when they own the worker-node software stack.
...
| DRA device allocation | Vendor DRA driver and `DeviceClass` resources | Syncs allowed DRA objects between the control plane cluster and cluster |
...
To make GPU or accelerator workloads work in a cluster:
...
When several applications make up the GPU environment, use a [Stack](/docs/platform/understand/what-are-stacks) to deploy them repeatedly with dependency ordering and health gates. See [Create a Stack template](/docs/platform/administer/templates/create-stack-templates) to package GPU operators, schedulers, monitoring, and dependent applications.
