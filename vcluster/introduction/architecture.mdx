...
Virtual clusters are fully functional Kubernetes clusters, but how they are deployed depends on what tenancy model that you want for your cluster. 
...
- A **syncer**, which is a component that synchronizes resources to and from the virtual cluster and host cluster and facilitates workload management on the host's infrastructure. This is only applicable when the worker nodes are comprised from the shared host cluster.
...
_Applicable only to Shared Nodes and Dedicated Nodes_
