...
Every tenant cluster is built on a certified Kubernetes distribution. This is true whether it's delivered to tenants as managed Kubernetes directly or as the foundation for a Slurm, Ray, or inference cluster running on top. This page covers that shared control plane and worker-node architecture.
...
* **Underlying cluster pod to tenant cluster service** — Pods on the underlying cluster can access services within a tenant cluster by [replicating](../configure/vcluster-yaml/networking/replicate-services.mdx#cluster-to-control-plane-cluster) tenant cluster services to any namespace on the underlying cluster.
