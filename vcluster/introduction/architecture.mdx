...
Each virtual cluster includes its own DNS service, which is [CoreDNS](../configure/vcluster-yaml/control-plane/components/coredns) by default. The DNS service lets pods within the virtual cluster resolve the IP addresses of other services running in the same virtual environment. This capability is anchored by the syncer component, which maps service DNS names within the virtual cluster to their corresponding IP addresses in the host cluster, adhering to Kubernetes' DNS naming conventions.

The vCluster Pro edition has [CoreDNS embedded](../configure/vcluster-yaml/control-plane/components/coredns) directly into the virtual control plane pod, minimizing the overall footprint of each virtual cluster.
