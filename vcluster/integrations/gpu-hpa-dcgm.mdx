...
- NVIDIA GPU drivers and the NVIDIA device plugin installed on the private GPU nodes, usually through the [NVIDIA GPU Operator](../introduction/gpu-and-accelerator-support.mdx#install-the-nvidia-gpu-operator-in-a-cluster)
- `helm` and `kubectl` connected to the vCluster

## Step 1: Install DCGM exporter

:::info Already running the NVIDIA GPU Operator?
The GPU Operator deploys DCGM Exporter by default. Installing the standalone chart below alongside it creates two competing DaemonSets on the same GPU nodes. If the Operator manages your GPU stack, skip this step. To add Kubernetes pod labels such as endpoint, customer, or tier to the exported metrics, enable the optional label enrichment shown in [GPU and accelerator support](../introduction/gpu-and-accelerator-support.mdx#install-the-nvidia-gpu-operator-in-a-cluster). Then continue with Step 2, and follow the GPU Operator path in Step 3 for the ServiceMonitor.
...
KEDA is often a good fit when your serving signal is naturally expressed as a Prometheus query. [Install KEDA](https://keda.sh/docs/latest/deploy/) in the cluster, then create a `ScaledObject` that queries Prometheus.
