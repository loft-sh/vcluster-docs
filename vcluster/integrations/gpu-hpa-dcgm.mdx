...
GPU and inference autoscaling help when your workloads have variable GPU or request demand:
...
| Time to first token | You serve streaming responses and need to protect interactive latency. |
| Tokens per second | You need throughput-based scaling for generated-token workloads. |
| p95 or p99 latency | You scale from customer-visible latency, often with smoothing to avoid flapping. |
