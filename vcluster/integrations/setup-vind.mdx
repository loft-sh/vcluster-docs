...
The [`setup-vind`](https://github.com/loft-sh/setup-vind) GitHub Action provisions Kubernetes clusters via [vCluster's Docker driver (vind)](../configure/vcluster-yaml/experimental/docker.mdx) for CI/CD testing. It is a drop-in replacement for `setup-kind` that gives you a full virtual cluster running in Docker, usable for e2e tests, multi-cluster setups, and more.
