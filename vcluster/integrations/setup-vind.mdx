...
The [`setup-vind`](https://github.com/loft-sh/setup-vind) GitHub Action provisions Kubernetes clusters using [vCluster's Docker driver (vind)](../configure/vcluster-yaml/experimental/docker.mdx) for CI/CD testing. It replaces `setup-kind` with a full cluster running in Docker, usable for end-to-end (e2e) tests, multi-cluster setups, and more.
...
<InterpolatedCodeBlock
  code={`- uses: loft-sh/setup-vind@v1\n  with:\n    kubernetes-version: "[[VAR:KUBERNETES_VERSION:${DEFAULT_K8S_VERSION.slice(1)}]]"`}
  language="yaml"
/>
...
| <code>image: kindest/node:{DEFAULT_K8S_VERSION}</code> | <code>kubernetes-version: "{DEFAULT_K8S_VERSION.slice(1)}"</code> | No node image needed |
