...
With vCluster Platform 4.12, use a Platform Stack when separate Apps or Argo CD Applications need explicit ordering. Stack tasks can wait for healthy dependencies and pass outputs to later tasks. See [Create a Stack template](/docs/platform/next/administer/templates/create-stack-templates).

When you use Argo CD without a Platform Stack, put hard dependencies in one Application. Use `argocd.argoproj.io/sync-wave` annotations to order resources inside it. Sync waves don't order separate Applications.
