...
description: Compose and deploy an inference serving stack in a cluster and expose an inference endpoint.
---
...
An inference serving stack deploys inside the cluster. vCluster provides the tenant Kubernetes API, GPU access, resource sync, and templates for installing the stack.
...
Deliver the serving stack declaratively with the [Argo CD integration](/platform/integrations/argocd). Platform creates and manages the `ArgoCDApplication` object. Argo CD delivers the manifests from your Git repository into the cluster. See [Build a direct engine template](#build-a-direct-engine-template) for the direct pattern.

:::note
This page focuses on the private-node endpoint pattern, where the cluster owns its worker-node software stack. Provider-owned shared serving pools can also run behind a product API, but they need a different tenancy and routing design.
:::
...
Choose which component owns the generated workloads, routes, and autoscalers before you build the cluster template.
...
- A cluster with [Private Nodes](../deploy/worker-nodes/private-nodes/overview.mdx) enabled.
- At least one GPU node attached to the cluster.
...
- For production delivery, an [Argo CD connector](/platform/integrations/argocd/connect-argocd) enabled on the cluster and a Git repository holding your runtime manifests.
...
Before installing the serving stack, confirm the cluster can see GPU capacity and any routing resources your selected pattern expects.
...
For a direct deployment that uses an imported Gateway, confirm the cluster can see it before creating routes:
...
If the Gateway is missing, fix the cluster template before deploying the runtime. For imported Gateways, the template must enable `sync.fromHost.gateways` and `sync.toHost.gatewayApi.httpRoutes`. For tenant-owned Gateways, the template must enable the Gateway API sync resources your controller requires.
...
Apply the manifest inside the cluster:
...
Validate the Service from inside the cluster:
...
Package the engine as an Argo CD Application so a template update or Git commit rolls out consistently across each applicable cluster. Store the Deployment, Service, and route in a Helm chart. Then declare an `ArgoCDApplicationTemplate` that points at it.

Keep model identity and customer-specific values as parameters so the same template serves every endpoint tier. The `source.helm.values` block below references each parameter with Go template syntax (`{{ .Values.parameterName }}`). This is the same parameter mechanism [templates use for clusters](/platform/administer/templates/parameters), applied here to the Helm values that render the chart.
...
Reference the template and pass its parameter values from the cluster's `vcluster.yaml`, alongside the [Argo CD connector](/platform/integrations/argocd/connect-argocd):
...
Argo CD syncs the Application into the cluster once the connector registers it. `selfHeal: true` reverts manual drift, and a Git commit to the template path rolls out on the next reconcile.
...
With vCluster Platform 4.12, use a [Stack](/docs/platform/understand/what-are-stacks) when separate Apps or Argo CD Applications need explicit ordering. Stack tasks can wait for healthy dependencies and pass outputs to later tasks. See [Create a Stack template](/docs/platform/administer/templates/create-stack-templates).

When you use Argo CD without a Stack, put hard dependencies in one Application. Use `argocd.argoproj.io/sync-wave` annotations to order resources inside it. Sync waves don't order separate Applications.

Plan for minutes, not seconds, for the sync itself. Argo CD's reconcile interval defaults to a few minutes and is configurable. If you don't trigger syncs explicitly, a template change reaches each cluster on its next scheduled sync rather than instantly. A freshly created cluster can take a while to reach fully `Healthy` across its Applications, depending on connector registration and image pull time.
...
2. The cluster imports approved Gateways with `sync.fromHost.gateways`.
3. The runtime's Argo CD Application creates `HTTPRoute` resources in the cluster alongside the Deployment and Service.
4. vCluster syncs the tenant route to the control plane cluster and enforces the Gateway attachment policy.

The cluster template needs Gateway sync enabled. The following example maps a platform-owned Gateway to a tenant-facing Gateway and allows routes from selected tenant namespaces:
...
Once Argo CD syncs the Application, confirm the route from inside the cluster:
...
- The cluster reaches `Ready` and the private GPU node joins.
