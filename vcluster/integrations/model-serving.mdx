...
Deliver the runtime declaratively with the [Argo CD integration](/platform/integrations/argocd). Platform syncs the manifests below as an `ArgoCDApplication` from your Git repository, so a template update rolls out consistently across every tenant cluster running it.
...
Reference the template from the tenant cluster's `vcluster.yaml`, alongside the [Argo CD connector](/platform/integrations/argocd/connect-argocd):
...
Keep model identity and customer-specific values as `ArgoCDApplicationTemplate` parameters so the same template serves every endpoint tier. Keep driver configuration, GPU presentation mode, and node image details in the GPU stack and node type instead of embedding them into the runtime Application.
