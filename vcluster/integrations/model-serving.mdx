...
Set `CreateNamespace=true` so Argo CD creates the target namespace itself. This option only takes effect when the Application also sets `spec.destination.namespace`, so the template sets it from the `namespace` parameter. The platform resolves the destination cluster from `target: vcluster`, but it doesn't set the namespace. Without both, the Application fails to sync into a namespace that doesn't already exist, unlike the smoke test above where `kubectl create namespace` handled that manually.
