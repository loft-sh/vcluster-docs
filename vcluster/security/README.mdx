...
For granular, platform-managed admission policies that tenants cannot override, use [central admission control](../configure/vcluster-yaml/policies/admission-control.mdx). Central admission control is a Pro feature. It configures webhooks in the tenant cluster. The webhooks point through the vCluster proxy to a service in the control plane cluster or to an external policy service. Tenants can install their own admission controllers, but they cannot mutate or delete the centrally configured hooks.
