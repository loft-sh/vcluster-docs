...
These policies apply through your own policy engine, not vCluster, so none of them depend on a vCluster edition. Where they run differs. The first evaluates Pods inside each tenant cluster, reached through the [`policies.centralAdmission`](./shared-nodes-hardening.mdx#prerequisites) webhook wiring, before vCluster ever syncs the Pod to the control plane cluster. The other two evaluate objects natively on the control plane cluster, after translation.

If the same policy engine serves both paths, exclude your platform's own system namespaces from the first policy's `match` block. Otherwise it also evaluates the control plane cluster's own Pods. Infrastructure workloads that rely on a bare `operator: Exists` toleration to run everywhere would get denied once you switch it to `Enforce`.
