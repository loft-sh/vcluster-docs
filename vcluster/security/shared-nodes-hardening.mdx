...
- **Node and kubelet disclosure.** `networking.advanced.proxyKubelets.byHostname` and `byIP` enable a vCluster-mediated kubelet proxy, which backs kubelet metrics scraping and `kubectl top node` against synced nodes. `kubectl logs` and `exec` don't depend on this setting. They use an independent Pod subresource redirect. On a supported, patched vCluster version, kubelet responses for pods and per-pod stats are already scoped to the tenant's own pods, and container log access is validated against pod and container ownership. Metric samples without a pod or PVC label, typically node-level metrics like overall CPU, memory, and network usage, aren't tenant-scoped and remain visible to any tenant with the proxy enabled. Disable `proxyKubelets` unless you need the metrics workflow, both to reduce node-level disclosure and attack surface, and as a safeguard on any version that predates the pod-level scoping. When [`sync.fromHost.nodes`](../configure/vcluster-yaml/sync/from-host/nodes.mdx) is enabled, set `clearImageStatus: true` so co-tenant image names and tags aren't visible through the synced node object. Require kubelet authentication and authorization. Disable the kubelet read-only port on every shared node, regardless of these settings.
...
- [ ] Tenant workload nodes carry a dedicated label and taint, distinct from infrastructure, ingress, metrics, and control-plane node roles. The tenant-pool toleration is injected through `enforceTolerations`. Tenant-cluster and host-side admission deny explicit or wildcard tolerations capable of matching protected infrastructure taints, while host-side admission permits the exact tenant-pool toleration and required Kubernetes node-condition tolerations.
