...
- vCluster v0.34.0 or later. Earlier versions don't scope kubelet-proxy pod-listing and container-log responses to the tenant's own pods.
...
- **Node and kubelet disclosure.** Disable [`networking.advanced.proxyKubelets`](../configure/vcluster-yaml/networking/advanced.mdx) unless you need the metrics workflow it backs (`kubectl top node` and kubelet metrics scraping). Node-level metric samples without a pod or PVC label aren't scoped to the tenant and remain visible whenever the proxy is enabled, regardless of vCluster version. On vCluster v0.34.0 and later, its pod-listing and container-log paths are scoped to the tenant's own pods. Disabling `proxyKubelets` is a safeguard mainly for versions older than that. `kubectl logs` and `exec` are unaffected either way, since they use an independent, ownership-validated path. When [`sync.fromHost.nodes`](../configure/vcluster-yaml/sync/from-host/nodes.mdx) is enabled, pair it with `clearImageStatus: true`. Require kubelet authentication and authorization, and disable the kubelet read-only port on every shared node regardless of these settings.
...
- **Tenant offboarding.** Deprovisioning a tenant is a separate control from onboarding it. Revoke the tenant's Platform and RBAC credentials first. Normal vCluster deletion removes the resources it synced or created on the control plane cluster, so most cleanup is automatic. Imported host resources, such as a referenced DeviceClass or NetworkAttachmentDefinition, correctly remain, since the tenant never owned them. Verify storage cleanup separately. A PersistentVolume with `reclaimPolicy: Retain` survives deletion by design, and that's the storage class's decision to preserve data, not a vCluster gap. Confirm no synced or controller-generated objects remain in the control plane cluster after deletion. This includes `virt-launcher` Pods and other host-side objects a controller generated from the tenant's custom resources.
