...
- **Tenant-cluster admission** sees the tenant-authored Pod before that injection happens. It can deny tenant-authored tolerations for protected infrastructure taint keys without ever seeing the toleration `enforceTolerations` adds later.
- **Host-side admission** sees the translated Pod after injection, so it can't distinguish a tenant-supplied toleration from an injected one. It has to allowlist the exact tenant-pool toleration and deny protected infrastructure taint keys specifically, rather than blocking tenant-supplied tolerations in general.
...
- [ ] Tenant workload nodes carry a dedicated label and taint, distinct from infrastructure, ingress, metrics, and control-plane node roles. The tenant-pool toleration is injected through `enforceTolerations`. Tenant-cluster admission denies tenant-authored tolerations for protected infrastructure taint keys, and host-side admission allowlists only the exact tenant-pool toleration.
