...
Only sync tenant NetworkPolicies where the platform boundary itself sits at a precedence a tenant's own NetworkPolicy can't override. Kubernetes' AdminNetworkPolicy and BaselineAdminNetworkPolicy are built for this. So are CNI-specific tiered policies, such as a Calico GlobalNetworkPolicy or a Cilium cluster-wide policy. Otherwise, leave `sync.toHost.networkPolicies` disabled, or add control plane cluster admission that denies synced NetworkPolicies with an `ipBlock` peer outside an explicit allowlist.
...
[Node sync](../configure/vcluster-yaml/sync/from-host/nodes.mdx) passes host GPU capacity and allocatable counts through to the tenant cluster unfiltered. Unlike CPU, memory, and ephemeral storage, which vCluster recomputes net of other tenants' usage, GPU resource counts on a synced node reflect the real host state. GPU pools per node are typically small. On a small pool, this can disclose another tenant's real-time GPU allocation more precisely than the CPU and memory disclosure covered in [Availability, disclosure, and lifecycle](#availability-disclosure-and-lifecycle). Apply the same guidance there to any shared node exposing GPU or other extended resources.
...
- **Node and kubelet disclosure.** [`networking.advanced.proxyKubelets`](../configure/vcluster-yaml/networking/advanced.mdx) mediates a kubelet proxy behind metrics scraping and `kubectl top node`. Its pod-listing and container-log paths aren't scoped to tenant ownership, so a tenant can list every pod on the node and read another tenant's container logs. Node-level metric samples aren't scoped either. `kubectl logs` and `exec` are unaffected, since they use an independent, ownership-validated path. Disable `proxyKubelets` unless you need the metrics workflow. It's the primary mitigation here, not defense in depth. When [`sync.fromHost.nodes`](../configure/vcluster-yaml/sync/from-host/nodes.mdx) is enabled, pair it with `clearImageStatus: true`. Require kubelet authentication and authorization, and disable the kubelet read-only port on every shared node regardless of these settings.
