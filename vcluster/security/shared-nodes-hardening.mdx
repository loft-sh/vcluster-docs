...
Only sync tenant NetworkPolicies where the platform boundary itself sits at a precedence a tenant's own NetworkPolicy can't override. Kubernetes' AdminNetworkPolicy is built for this, since a `Deny` action there takes effect regardless of any NetworkPolicy. `BaselineAdminNetworkPolicy` doesn't qualify. It's evaluated after NetworkPolicy and is designed to be overridden by it, so it can't protect this boundary. CNI-specific tiered policies, such as a Calico GlobalNetworkPolicy or a Cilium cluster-wide policy, can also work if they sit at a higher precedence. Otherwise, leave `sync.toHost.networkPolicies` disabled, or add control plane cluster admission that denies synced NetworkPolicies with an `ipBlock` peer outside an explicit allowlist.
...
A device node under `/dev`, for example `/dev/nvidia0`, mounted through a `hostPath` volume bypasses device-plugin accounting entirely. Treat it as the same class of gap as the PersistentVolume `hostPath` breakout in [Storage breakout prevention](#storage-breakout-prevention). Cover it with the same control plane cluster admission that denies `hostPath` PersistentVolumes.
