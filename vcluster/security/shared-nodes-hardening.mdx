...
- **`policies.podSecurityStandard: restricted`**. Default: none
...
- **`policies.resourceQuota`, `policies.limitRange`**. Default: `auto`
  ...
  Enabling one automatically enables the other. Prevents one tenant from exhausting shared node or etcd capacity.
...
- **`sync.toHost.networkPolicies.enabled: true`**. Default: `false`.

  Required so tenant-authored NetworkPolicies actually reach the control plane cluster CNI. Without it, a tenant's own NetworkPolicy objects exist only inside the tenant cluster and enforce nothing on shared nodes.
