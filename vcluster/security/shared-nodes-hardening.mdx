...
Give each tenant vcluster its own IngressClass and controller instance where practical. If tenants must share one controller, add control plane cluster admission that restricts which hostnames a given tenant namespace's Ingress can claim. Confirm your specific ingress controller's own behavior on cross-namespace hostname collisions rather than assuming isolation.
...
- [ ] Tenant Ingress hostname claims can't collide across tenants, either through dedicated per-tenant IngressClasses and controllers, or host admission restricting claimable hostnames on a shared controller. Gateway API sync, where used, has an `allowedRoutes` hostname allowlist configured on the imported Gateway.
