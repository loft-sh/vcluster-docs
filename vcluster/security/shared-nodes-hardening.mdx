...
- Use a label and taint dedicated to tenant workloads, applied only to nodes meant to run tenant compute. Inject the matching toleration for every synced Pod with [`sync.toHost.pods.enforceTolerations`](../configure/vcluster-yaml/sync/to-host/core/pods/README.mdx), so tenants don't need permission to set it themselves.
...
- Taint sensitive and infrastructure nodes separately from the tenant pool, and block tenant-supplied tolerations for those taints through admission. A taint alone doesn't reject a tenant workload if the tenant can add a matching toleration, and vCluster preserves tenant-set tolerations by default. Scope the admission rule to infrastructure taints only, since blocking every tenant-supplied toleration would also block the platform-injected tenant-pool toleration.
...
- [ ] Tenant workload nodes carry a dedicated label and taint, distinct from infrastructure, ingress, metrics, and control-plane node roles. The tenant-pool toleration is injected through `enforceTolerations`, and admission blocks tenant-supplied tolerations for every other taint.
