...
Most controls in this guide are available in open source vCluster. Two of them aren't. [Central admission control](../configure/vcluster-yaml/policies/admission-control.mdx) and [VirtualClusterTemplate](/platform/administer/templates/create-templates) enforcement are Enterprise features, and each is called out where it appears.
...
- [VirtualClusterTemplate](/platform/administer/templates/create-templates) enforcement through vCluster Platform, an Enterprise feature, so tenants can't loosen the baseline.
...
Enforce this configuration through required [VirtualClusterTemplates](/platform/administer/templates/create-templates), an Enterprise feature, so tenants can't loosen it. A tenant with unrestricted `vcluster.yaml` access can undo every setting in this list.
