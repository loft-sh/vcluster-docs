...
:::note
All Corefile server blocks must bind on port **1053**, not the standard DNS port 53. The cluster's CoreDNS process listens on port 1053 to avoid privilege requirements. A custom Corefile that binds on port 53 does not match the listener and silently breaks DNS resolution. UDP packets are dropped with no error in pod logs.
:::
