...
- **kubectl**: Use when vCluster CLI and Helm are not available in the environment or when working in a restricted environment with limited tool access. This method provides direct access to the raw internal configuration file when other methods are failing or when only kubectl access to the control plane cluster is available.
...
The vCluster CLI provides a comprehensive view of your cluster configuration, showing both user-defined settings and applied defaults. Use the following command when you need the complete effective configuration that vCluster uses to manage your tenant cluster:
