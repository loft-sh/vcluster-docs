---
title: Check connection to a vCluster or control plane cluster
...
When working with Kubernetes clusters, you can determine if you're connected to a vCluster or control plane cluster using the following methods:
...
echo "Connected to control plane cluster"
exit 1
```

This script first checks the current context name. If that doesn't indicate a vCluster, it then checks for vCluster node labels. If neither method detects a vCluster, it assumes you're connected to the control plane cluster.
