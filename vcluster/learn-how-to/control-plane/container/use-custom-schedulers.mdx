...
sidebar_position: 6
description: How to deploy and use custom schedulers in both control plane and cluster
---
...
- Run a custom scheduler in the cluster.
- Use vCluster Hybrid scheduling, so Pods from the cluster can use schedulers from both control plane and cluster.

How pods are scheduled with Hybrid scheduling enabled:
- Pods without `spec.schedulerName` use the default scheduler inside the cluster.
...
- Pods whose `spec.schedulerName` matches a scheduler running inside the cluster are scheduled by that virtual
  scheduler.
...
newer. It requires shared host nodes and host nodes have to be synced into the cluster. This guide enables node
...
## Create control plane and cluster
...
Apply manifests in the cluster:
...
- one pod is using the custom cluster scheduler.
