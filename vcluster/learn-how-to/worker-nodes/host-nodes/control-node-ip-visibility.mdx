...
- **Privacy**: Protects sensitive infrastructure information from cluster users.
...
vCluster automatically replaces the actual node IP addresses with obfuscated values. For example, if a host node has IP `192.168.1.100`, it appears in the cluster with a generated IP like `10.0.0.1`.
...
1. Compare node information between control plane and clusters:

```bash title="Check control plane cluster node"
kubectl get nodes -o wide
```

```bash title="Check cluster node"
