---
title: Sync Calico network policies to the control plane cluster
sidebar_label: Calico network policies
sidebar_position: 7
...
- No CustomResourceDefinition for `projectcalico.org` on the control plane cluster. That absence is what makes this an aggregated API resource. vCluster generates a schemaless CRD in the cluster instead of copying one. See [Aggregated API resources](../configure/vcluster-yaml/sync/to-host/advanced/custom-resources.mdx#aggregated-api-resources).
...
- Exactly one cluster per control plane cluster namespace, which is the default and only supported layout.

:::warning This approach doesn't protect a shared namespace
Some deployments running v0.24 or earlier could force more than one cluster into the same control plane cluster namespace. That option was deprecated on introduction and removed in v0.25. If you're still running such a deployment, a namespace-scoped selector doesn't separate those tenants, and no patch on `spec.selector` makes it safe.
:::

### Set up cluster variables

Set the control plane and cluster contexts and the namespace where vCluster runs.
...
<Flow id="calico-selector-scoping-example">
  <Step>
    <Highlight color="secondary">Cluster</Highlight> Create two namespaces. Each namespace has a web server and a client with identical labels:
  ...
  <Step>
    <Highlight color="secondary">Cluster</Highlight> Apply a policy in `demo` only:
  ...
  <Step>
    <Highlight color="secondary">Cluster</Highlight> The client in `demo` can reach the selected web server in the same namespace:
  ...
  <Step>
    <Highlight color="secondary">Cluster</Highlight> The identically labeled client in `demo2` can't reach the protected web server in `demo`:
  ...
  <Step>
    <Highlight color="secondary">Cluster</Highlight> The web server in `demo2` keeps serving traffic because its namespace has no policy:
  ...
  <Step>
    <Highlight color="secondary">Cluster</Highlight> The tenant object still reads the selectors it was created with, not the composed values from the control plane cluster:
