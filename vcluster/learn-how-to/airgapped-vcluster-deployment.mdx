...
- [vCluster Standalone Air-gapped](../deploy/control-plane/binary/security/air-gapped.mdx)
- [Private Nodes Air-gapped](../deploy/worker-nodes/private-nodes/security/air-gapped.mdx)

The standard method for installing vCluster is using Helm with the vCluster Labs charts repository. For air-gapped environments without internet access, you'll need to host the Helm chart locally. This guide demonstrates how to use a private GitLab instance as a web server to host the vCluster Helm chart.
...
1. Add and update the vCluster Helm repository on a machine with internet access:
...
After creating the configuration, verify the deployment by creating the tenant cluster through the platform UI.
