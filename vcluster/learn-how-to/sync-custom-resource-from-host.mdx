...
description: Walk through syncing a namespaced custom resource from the control plane cluster to a cluster.
---
...
Enable from-host syncing for `Example` custom resources in your cluster configuration:
...
- Syncs all `Examples` from the `foobar2` namespace to the `default` namespace in the cluster.

:::tip create cluster
Create or update a cluster following the [vCluster quick start guide](../quick-start/shared-nodes.mdx).
:::

### Sync a namespaced custom resource to the cluster
...
<Flow id="namespaced-custom-resources-from-host-example">
  ...
  #### Ensure the custom resource synced to the cluster

  The custom resource should now be accessible in the cluster. If you edit a field on the tenant object, it stays until the control plane cluster changes that same field, which then overwrites it.

  <Step>
    Check the custom resource in the cluster:
  ...
  <Step>
    Now edit the example in the control plane cluster and see that the change syncs to the cluster. To set replicas to 4, run:
  ...
  #### Verify the cluster copy updated

  <Step>
    <Highlight color="secondary">Cluster</Highlight> Check the number of replicas:
