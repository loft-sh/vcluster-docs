...
sidebar_position: 3
description: Why a single CoreDNS replica runs out of headroom in a cluster, and why embedded CoreDNS can't scale the same way.
---

CoreDNS handles DNS resolution for every pod in the cluster. By
default, it runs a single replica (`controlPlane.coredns.deployment.replicas: 1`)
as a separate Deployment, sized for small and medium clusters. See
...
You can switch a live cluster between embedded and separate CoreDNS
