...
The platform periodically checks for activity to determine when an environment should enter or remain in sleep mode. This check interval—called the **Sleep Mode Last Activity Update Interval**—defines how often the platform updates the "last active" timestamp for each environment.
