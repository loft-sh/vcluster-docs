...
Typically, clusters are accessed through the platform proxy, that is, requests to the
cluster API server, are proxied through the platform itself. This behavior allows for the platform to
act as a single endpoint for all clusters in the platform deployment. Because of this
...
<Tabs
    ...
    <TabItem value="ui">
      <Flow id="create-vcluster-with-accesspoint">
        ...
        <Step>
          Click the <Button>Configure</Button> button to skip selecting a cluster
          template.
        </Step>
        <Step>
        ...
        <Step>
          Finish configuring anything else you'd like on your cluster, then
          click the
