...
In this case, the secret `type` will remain unchanged by vCluster.Pro, and the data will be populated as-is from the project secret. Care must be taken that the project secret data matches the expected format of the chosen secret type.
