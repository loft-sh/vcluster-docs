...
import PartialVault from "@site/docs/platform/api/_partials/resources/projects/spec/vault.mdx";

vCluster.Pro supports syncing secrets from a HashiCorp Vault instance into both project secrets and global secrets.
This feature allows you to use secrets from Vault in your projects, vclusters and global vCluster.Pro tenant without copying them manually or using third-party tools.
...
You must update the [vCluster.Pro configuration](/platform/api/resources/config.mdx) with the Vault object to set up a global Vault integration.

<Flow id="hashicorp-vault-global-setup">
  <Step>
    Log in to the vCluster.Pro UI as admin and navigate to the <NavStep>Admin</NavStep> section using the menu on the left.
  </Step>
  <Step>
    Edit the vCluster.Pro configuration by adding the Vault object with the following
    properties:
...
For further configuration options, see the [vCluster.Pro configuration reference](/platform/api/resources/config.mdx) and the [Vault configuration options](#configuration-options).
:::

  </Step>
  <Step>Save the changes to the configuration using the <Button>Apply</Button> button and wait for vCluster.Pro to restart.</Step>
</Flow>

### Project-Based Vault Integration

You need to update the [project specification](/platform/api/resources/project/README.mdx) with the Vault object to set up a project-based Vault integration.
...
For further configuration options, see the [Project reference](/platform/api/resources/project/README.mdx) and the [Vault configuration options](#configuration-options).
:::
