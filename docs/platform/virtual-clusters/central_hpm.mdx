...
This feature is an extension to the existing [hostpath mapper](/guides/logging/hpm.mdx) component of vCluster and requires an active vCluster Platform license.
Currently, when enabled, hostpath mapper can support the following usecases:
1. [Enabling container based logging used by tools like fluentd, logstash etc.](/guides/logging/elk_stack.mdx) inside vCluster
2. [Enabling pod based logging used by loki](/guides/logging/grafana_loki.mdx) inside vCluster
...
This is a loft-integrated feature, and the related docs can be found [here](/platform/virtual-clusters/central-hostpath-mapper).
