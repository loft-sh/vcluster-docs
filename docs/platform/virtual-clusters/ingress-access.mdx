...
In vCluster.Pro, typically, virtual clusters are accessed via the vCluster.Pro proxy, that is, requests to the
virtual cluster API server, are proxied through vCluster.Pro itself. This behavior allows for vCluster.Pro to
act as a single endpoint for all virtual clusters in the vCluster.Pro deployment. Because of this
behavior, vCluster.Pro is also able to act as a central point of authentication and authorization, and
to log all interactions (if vCluster.Pro Auditing is licensed and enabled).

In some situations you may prefer to access a virtual cluster API server directly, that is, not
via the vCluster.Pro proxy. This behavior can be enabled with the virtual cluster 'AccessPoint' feature.
