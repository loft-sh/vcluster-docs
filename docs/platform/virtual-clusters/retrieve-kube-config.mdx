...
manually construct a kube config or use the vCluster Platform CLI to automatically create and update your
kube config.

## Retrieve via CLI

The easiest way to retrieve a kube config for your virtual cluster is by using the vCluster Platform CLI.
...
:::info
Depending on if you have direct [cluster endpoints](../clusters/multi-region-mode.mdx) or
[ingress endpoints for virtual clusters](../virtual-clusters/ingress-access.mdx)
enabled, the resulting kube config will look different
:::

## Kube Config Without CLI

You can also create the kube config manually by creating an [access key](../users/access-keys.mdx) for
