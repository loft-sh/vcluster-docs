...
chart contains values that allow for configuring the stage of the application. vCluster.Pro
administrators could expose this App to vCluster.Pro users, and rather than relying on the users to
properly set a helm value of "dev" or "test", the vCluster.Pro administrator can instead simply provide
...
These values are behind the `vCluster.Pro` key!
