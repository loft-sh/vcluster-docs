In vCluster.Pro, you can also configure _multiple_ SSO providers together. This can be a very handy feature if you
...
The latter is configured by using the "connectors" option. Below is an example vCluster.Pro config "auth"
section that contains three SSO providers (one OIDC, and two GitHub providers). When users log
into vCluster.Pro with such a configuration present, they will see three options for authentication as
...
auth:
  github:
    ...
    redirectURI: https://my-vcluster-pro-url/auth/github/callback
  oidc:
    clientId: test
    clientSecret: test
    issuerUrl: https://example-issuer.com
    redirectURI: https://my-vcluster-pro-url/auth/oidc/callback
  # If you want to use multiple SSO configs of the same type you can use the connectors option
  connectors:
    # the id of the connector that will show up in the vcluster-pro callback url
    - id: test
      ...
      github:
        ...
        redirectURI: https://my-vcluster-pro-url/auth/test/callback
```
