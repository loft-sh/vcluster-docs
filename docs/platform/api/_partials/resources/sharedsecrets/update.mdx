...
```bash
kubectl edit sharedsecrets.management.loft.sh my-global-secret -n vcluster-pro
```
...
```bash
kubectl patch sharedsecrets.management.loft.sh my-global-secret -n vcluster-pro \
        --type json \
