---
title: vCluster.Pro Management Roles
sidebar_label: vCluster.Pro Management Roles
---
...
`ClusterRole` to each connected cluster in the vCluster.Pro deployment.
...
In many vCluster.Pro deployments you can get away with using the pre-defined Management Roles, but you
are always free to create more of your own if needed.

:::info vCluster.Pro Default Objects
If you don't see any Management Roles present in your vCluster.Pro installation, they have either been
deleted, or, more likely, your vCluster.Pro instance was deployed with the `DISABLE_DEFAULT` environment
variable set which causes vCluster.Pro to _not_ install default objects, including the Management Roles.
:::
...
<Flow id="create-management-role">
  <Step>
    In the vCluster.Pro UI, select the <NavStep>Users</NavStep> field on the
    left-most menu bar.
  </Step>
  <Step>
...
Management Role can be done via the vCluster.Pro UI.
...
>
  ...
  <TabItem value="team">
    <Flow>
      <Flow id="apply-role-user">
        <Step>
          In the vCluster.Pro UI, select the <NavStep>Users</NavStep> field on
