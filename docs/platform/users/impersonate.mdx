...
import PartialClusterAccessCreateUI from '@site/docs/platform/clusters/_partials/cluster-access/create-ui.mdx'

To test configurations and permissions, vCluster.Pro provides a feature to impersonate users within the
vCluster.Pro UI and see everything through the eyes of a specific users. This is very useful if you want
to check if a user has permissions to access an object.

## Enable Impersonation

To enable impersonation, make sure the user has appropriate permissions, only vCluster.Pro management
...
vCluster.Pro lets you connect a variety of SSO providers for authentication but for the sake of
simplicity, let's just manually create a user to learn more about vCluster.Pro's cluster access features:
...
Remember: Everything you do in vCluster.Pro UI, including creating a user, is effectively a kubectl
...
vCluster.Pro allows admins with appropriate RBAC permissions to impersonate users. Let's try this to see
how vCluster.Pro UI would look like for our newly created user:
...
You can also use the vCluster.Pro CLI as the impersonated user, to do this, simply run the following
...
Let's give our test user Anna access to one of the clusters connected to this vCluster.Pro instance:

<PartialClusterAccessCreateUI />

:::note Single Sign-On + Cluster Access
You can connect a variety of SSO providers to vCluster.Pro. To automatically give users access to
...
Directory, Okta, SAML, etc.), check out the [SSO Group Sync](../users/sso-group-sync.mdx) section for
more details.
:::
