...
defined across the whole project or on a per-user basis. Additionally, some additional resource types specific to vCluster.Pro are outlined below.
...
vCluster.Pro objects such as virtualclusterinstances can be instantly enforced as soon as consumption exeeds the limit, but on-cluster resources such as `limits.cpu` cannot be immediately enforced to avoid blocking requests for a long time.
