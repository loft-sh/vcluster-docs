...
vCluster.Pro provides several points of integration with [Argo CD](https://argo-cd.readthedocs.io/), a popular GitOps tool
...
Argo CD integration is enabled on a _per-project_ basis -- the project layer is where admins tell vCluster.Pro where the
...
vCluster.Pro also provides the ability to use vCluster.Pro as an SSO provider for Argo CD to allow users to authenticate via vCluster.Pro.
After enabling this setting, users who browse to the Argo CD instance will see a button to login via vCluster.Pro. All
members of the project will be able to log in via vCluster.Pro and gain access to Argo CD. For more information, see
[Configuring SSO](#configuring-sso)

Finally, vCluster.Pro Projects can be imported into Argo CD's App Projects. This is an advanced feature allowing users to
...
<Flow id="create-vcluster-template-ui">
  ...
  <Step>
    ...
    vCluster.Pro credentials. For more info, see the section Configuring SSO
    section below.
  </Step>
</Flow>
...
This allows users to login to the ArgoCD instance using their vCluster.Pro credentials.

:::caution vCluster.Pro Host Config Required
This requires `loftHost` key to be configured first. See [vCluster.Pro Configuration](/platform/admin/config.mdx).
...
The vCluster.Pro domain must be resolvable from the ArgoCD instance and the browsers of any user logging in via this method.
:::
...
<Flow id="create-vcluster-template-ui">
  ...
  <Step>
    ...
    roles that vCluster.Pro will assign to members when logging in to Argo CD
    via SSO
  </Step>
</Flow>
