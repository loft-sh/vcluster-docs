...
vCluster.Pro provides several points of integration with [Rancher](https://www.rancher.com/), which is a multi-cluster orchestration platform for Kubernetes. 
...
You can enable member Synchronization which synchronizes Rancher Project Member Roles into vCluster.Pro Project Member roles and Rancher Cluster Roles. There a couple of mappings available by default but you can define your own custom mappings.
...
You must update the [vCluster.Pro configuration](/platform/api/resources/config) with the Rancher object to set up a global Rancher integration. This enables the platform to communicate with Rancher via its API and perform the necessary actions required for the syncing.
