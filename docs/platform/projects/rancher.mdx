...
You must update the [vCluster.Pro configuration](../api/resources/config) with the Rancher object to set up a global Rancher integration. This enables the platform to communicate with Rancher via its API and perform the necessary actions required for the syncing.
