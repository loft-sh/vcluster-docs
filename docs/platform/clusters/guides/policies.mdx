...
Policies allow you to customize admission control and extend vCluster.Pro. As vCluster.Pro internally uses Kubernetes resources for anything (including UI requests), you can customize almost everything with policies.
...
Policies are based on [jsPolicy](https://jspolicy.com) and you will need to install the jsPolicy app into the cluster if you want to use policies. JsPolicy is a recommended app in vCluster.Pro and can be installed via the vCluster.Pro UI.

<figure class="frame">
  <img src="/docs/media/v1/ui/policies/install-js-policy.png" alt="JsPolicy" />
  <figcaption>Installing jsPolicy via the vCluster.Pro UI</figcaption>
</figure>
...
Policies can be created either through the vCluster.Pro UI or through directly creating `JsPolicy` objects in the cluster. For a more extensive documentation how to write policies, please check the [jsPolicy docs](https://www.jspolicy.com/docs/writing-policies/workflow).
...
You can delete a policy either via the vCluster.Pro UI or kubectl directly.
...
This section holds several examples for useful policies within vCluster.Pro. For more examples and explanations checkout the [jsPolicy documentation](https://www.jspolicy.com/docs/quickstart).
...
This policy will deny pod creation only for a single vCluster.Pro user called `test`.
...
vCluster.Pro does not require you to use jsPolicy for policies, and you can always use your own preferred solution. Every policy engine should work fine with vCluster.Pro. However, if you want to use the policies view in vCluster.Pro, this currently only works with [jsPolicy](https://github.com/loft-sh/jspolicy).

We decided to integrate jsPolicy instead of another policy engine, because jsPolicy is the most flexible one that can cover the most use cases within vCluster.Pro. Especially with [controller policies](https://www.jspolicy.com/docs/using-policies/policy-types#controller) and [mutating webhooks](https://www.jspolicy.com/docs/using-policies/policy-types#mutating) that are not at all or only partly supported in other policy engines, jsPolicy has a huge advantage in terms of flexibility and expressiveness.
