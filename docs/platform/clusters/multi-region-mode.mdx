...
Running vCluster.Pro in Multi-Region Mode provides a way to avoid the central vCluster.Pro instance and directly connect to a connected vCluster.Pro cluster. By default, all Kubernetes contexts that are created through vCluster.Pro will route all Kubernetes traffic (such as `kubectl get pods` etc.) through the central vCluster.Pro instance.
This allows vCluster.Pro to handle authentication, authorization, auditing, sleep mode activity etc.
...
- **loft.sh/direct-cluster-endpoint-insecure** (Optional): If this is `true`, vCluster.Pro will create Kubernetes contexts with `insecure-skip-tls-verify: true` for this endpoint
