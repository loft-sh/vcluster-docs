...
2. The [Setup vCluster.Pro](https://github.com/loft-sh/setup-vcluster) action is used to install the vCluster CLI.
3. The `vcluster login` command is used to log in to the organization's vCluster.Pro instance. Environment variables `LOFT_URL` and `ACCESS_KEY` are populated using [GitHub secrets](https://docs.github.com/en/actions/security-guides/using-secrets-in-github-actions).
