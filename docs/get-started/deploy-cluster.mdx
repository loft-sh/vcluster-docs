...
To create a vCluster using ArgoCD, you will create a `vcluster.yaml` file and a file `my-vcluster.yaml` containing your ArgoCD `Application`
definition.

First, create the `vcluster.yaml` with the following contents:
```yaml
controlPlane:
  distro:
    k3s:
...
Second, create the ArgoCD `Application` file `my-vcluster.yaml`. We're using the `vcluster` Helm chart and the `vcluster.yaml` to pass the chart values,
...
Wait for the vCluster custom resource to report a ready status:
```bash
