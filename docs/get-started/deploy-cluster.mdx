...
You have installed [kubectl](https://kubernetes.io/docs/tasks/tools/#kubectl) and have access to a Kubernetes cluster to use as the host cluster. You do not need admin permissions on the host cluster.
...
`vcluster create` creates a new context that starts with "vcluster_my-cluster" and updates your kubeconfig file to point to that context. You can use the CLI to quickly connect to your virtual cluster. Execute:
...
- `vcluster disconnect`: Sitch back to your default (host) context.
...
@TODO link to other exposing methods or delete sentence
...
@TODO is this really tabs? or should we just list the kubectl command?
<Tabs
  groupId="get-started"
  defaultValue="cli"
  values={[
