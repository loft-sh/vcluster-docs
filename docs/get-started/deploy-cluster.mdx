...
## vCluster key concepts and components

- [**Control plane**](/architecture/control-plane.mdx): The vCluster control plane contains a Kubernetes API server, a controller manager, a data store mount, and the vCluster Syncer component.
...
```bash
vcluster connect my-vcluster --kube-config my-vcluster-kubeconfig.yaml --update-current=false
```

This writes the virtual cluster's kubeconfig to the specified file.
