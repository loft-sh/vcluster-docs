...
You may use the `clusterctl get kubeconfig` command to retrieve the kubeconfig. Here's an example where it is written to
a file and used with `kubectl` to list the vCluster's namespaces:
```bash
export CLUSTER_NAME=my-vcluster
export CLUSTER_NAMESPACE=my-vcluster
