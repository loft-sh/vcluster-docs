...
1. Look for the NGINX pods.
   ...
   The vCluster `my-cluster-0` pod contains the virtual cluster’s Kubernetes API server and some additional tools. There’s also a CoreDNS pod, which vCluster uses, and the two NGINX pods.
