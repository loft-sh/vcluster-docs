...
You configure your virtual clusters using the vCluster configuration file called `vcluster.yaml`. This guide includes some examples to illustrate what you can do. See the `vcluster.yaml` [config section](/vcluster/vcluster-yaml/README.mdx) for detailed configuration options.
...
By default, vCluster creates a pseudo node for every pod `spec.nodeName` that it encounters within the virtual cluster. The fake node has a different name and made-up spec.
...
See the [vcluster.yaml node section](/vcluster/vcluster-yaml/sync/from-host/nodes.mdx) for a detailed config reference.
...
See the `vcluster.yaml` [config section](/vcluster/vcluster-yaml/networking/replicate-services.mdx) for a detailed config reference.
