...
By default, vCluster creates a [pseudo node](/architecture/nodes.mdx) for every pod `spec.nodeName` that it encounters within the virtual cluster. The pseudo node has a different name and made-up spec. If you need to access specific nodes for scheduling, you can enable syncing "real" nodes, which changes how vCluster shows nodes inside the virtual cluster.
