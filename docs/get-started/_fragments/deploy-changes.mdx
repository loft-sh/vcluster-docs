...
Cluster API changes can be applied by re-generating the cluster custom resource using the `clusterctl` CLI:
```bash
...
After the changes have been applied, you may wait for the VCluster custom resource to report a ready status:
```bash
