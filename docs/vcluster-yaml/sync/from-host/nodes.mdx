...
By default, vCluster syncs psuedo nodes from the host cluster where there are virtual cluster pods running. Pseudo nodes only have real values for the CPU, architecture, and operating system, while everything else is randomly generated. A single pseudo node can either represent a single real node on the host cluster, or it can represent multiple real nodes. If there are no more pods on a node, vCluster deletes the pseudo node. 
...
Enable syncing real nodes from the host cluster to the virtual cluster as well as syncing back from the virtual cluster any changes to node labels and taints. Enabling this will add RBAC permissions to the Syncer component to allow it to modify the host nodes and statuses.
```YAML
sync:
  fromHost:
    nodes:
...
Enable syncing real nodes and while clearing all status.images from the nodes when it is synced to the virtual cluster. For certain multi-tenant use cases such as multi-customer tenancy, images can leak sensitive information about a node.
```YAML
sync:
  fromHost:
    nodes:
