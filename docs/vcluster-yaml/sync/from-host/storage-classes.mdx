...
By default, vCluster will not sync StorageClasses from the host cluster to the virtual cluster. While it's not strictly required when you're syncing PersistentVolumes and PersistentVolumeClaims, syncing StorageClasses may be required for infromational purposes or ensuring that storage is handled consistently between the host and virtual clusters.
