...
Pods run inside the underlying host cluster. vCluster's Syncer component syncs Pods between host and virtual cluster.
...
See [Host cluster to virtual cluster](/vcluster/vcluster-yaml/networking/replicate-services#host-cluster-to-virtual-cluster)

### Pod (vCluster) --> Service (different vCluster instances)
See [Mapping services across vCluster instances](/vcluster/vcluster-yaml/networking/resolve-dns#mapping-services-across-vcluster-instances)

## From The Host Cluster

### Pod (Host) --> Service (Inside vCluster)
See [Virtual cluster to host cluster](/vcluster/vcluster-yaml/networking/replicate-services#virtual-cluster-to-host-cluster)
