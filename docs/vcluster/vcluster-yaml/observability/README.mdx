...
To scrape those metrics, send an `Authorization` header with a valid virtual cluster service account token that has permissions to access the `/metrics` endpoint within the vcluster.
