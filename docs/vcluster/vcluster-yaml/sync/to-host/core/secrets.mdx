---
title: Secrets
sidebar_label: secrets
sidebar_position: 2
description: Configuration for ...
---

import SyncAllResource from '@site/docs/_partials/config/sync/toHost/secrets.mdx'

{/*
`secrets.enabled`:
 - covered by the generic section here: https://www.vcluster.com/docs/syncer/config#enable-or-disable-synced-resources
`secrets.all`:
 - https://www.vcluster.com/docs/syncer/config#sync-all-secrets-and-configmaps
*/}
Sync [Secrets](https://kubernetes.io/docs/concepts/configuration/secret/) used by pods. Apps frequently need secret data to function, so syncing is enabled by default. 

### Sync only utilized secrets from virtual to host cluster

```YAML
sync:
  toHost:
    secrets:
      enabled: true
```

### Sync all secrets from virtual to host cluster

vCluster only knows about a couple of virtual cluster resources that actually use secrets and tries to sync only those into the host cluster. You can enable syncing all virtual cluster secrets to avoid the problem that needed secrets are not synced to the host cluster.

```YAML
sync:
  toHost:
    secrets:
      enabled: true
      all: true
```

## Config reference

<SyncAllResource/>