...
By default, this is disabled. Sync [CSIStorageCapacity](https://kubernetes.io/docs/concepts/storage/storage-capacity/) resources from the host cluster to the virtual cluster if the `.nodeTopology` matches a synced node. Use this in conjunction with the virtual scheduler inside of vCluster for more advanced scheduling needs. Syncing the CSIStorageCapacity resources from the host helps the virtual scheduler make decisions about which node to schedule pods to.
