...
This feature allows you to customize how vCluster exports the kubeconfig file to a secret, so you can use it, for example, in your ArgoCD or Terraform pipelines. vCluster always creates a kube config in a secret called `vc-NAME` in the namespace where vCluster is deployed. When this option is specified, vCluster will create an additional secret with the given configuration.
