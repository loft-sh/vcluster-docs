...
import PluginOverview from '@site/docs/_fragments/plugin-overview.mdx'
...
Most of the settings are for the init container from which vCluster copies the plugin. Adjust `controlPlane.statefulSet.resources` [configuration](/vcluster/vcluster-yaml/control-plane/deployment/statefulset.mdx) to ensure the vCluster deployment has enough CPU  and memory to also run the plugin.

:::note
- `plugin[*].resources` does not change the syncer's resource requests.
- `plugin[*].config` content ends up in an environment variable `PLUGIN_CONFIG` that is read by the plugin. This configuration has no specific format.
:::
