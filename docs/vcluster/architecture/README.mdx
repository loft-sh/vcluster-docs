...
- A **data store**, which is a connection to or mount of the data store where the API stores all resources. SQLite is used by default, but you can [choose other data stores](/docs/vcluster/vcluster-yaml/control-plane/components/backing-store/) like etcd, MySQL, and PostgreSQL.
...
- A **scheduler**, which is an optional component that schedules workloads. By default, vCluster reuses the host cluster scheduler to save on computing resources. If you need to add node labels or taints to control scheduling, drain nodes, or utilize custom schedulers, you can [enable the virtual scheduler](/docs/vcluster/vcluster-yaml/control-plane/deployment/advanced/virtual-scheduler).
...
Higher-level Kubernetes resources, such as Deployment, StatefulSet, and CRDs only exist inside the virtual cluster and never reach the API server or data store (etcd) of the host cluster.
...
Each virtual cluster includes its own DNS service, which is [CoreDNS](/docs/vcluster/vcluster-yaml/control-plane/components/coredns) by default. The DNS service lets pods within the virtual cluster resolve the IP addresses of other services running in the same virtual environment. This capability is anchored by the syncer component, which maps service DNS names within the virtual cluster to their corresponding IP addresses in the host cluster, adhering to Kubernetes' DNS naming conventions.
