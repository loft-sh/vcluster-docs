...
Virtual clusters are fully functional Kubernetes clusters nested inside a physical host cluster in order to provide better isolation and flexibility to support multi-tenancy. 
...
## Components
Compared to a physical cluster, a virtual cluster does not have its own node pools or networking. Instead, a virtual cluster has its own control plane that schedules workloads inside the host cluster.

vCluster spins up a virtual cluster and has components to manage interaction between the virtual cluster and the host cluster. By default, vCluster runs as a single pod scheduled by a StatefulSet consisting of a Control Plane and Syncer.
...
The vCluster Pro edition combines CoreDNS into a single pod together with the Control Plane and Syncer for a reduced footprint.
