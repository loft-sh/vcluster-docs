...
- _`KUBERNETES_DISTRO`_ with your virtual cluster's existing Kubernetes distribution. **You can't change the Kuberntes distribution or backing store during an upgrade**. 
- _`VALUES_FILE`_ with the path to the `values.yaml`file you used to configure your vCluster instance.
...
```bash
helm -n VCLUSTER_NAMESPACE get values VCLUSTER_NAME
```
...
See the [vCluster config reference](/vcluster/vcluster-yaml/README.mdx) for configuration details.