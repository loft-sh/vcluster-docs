...
This guide shows you how to upgrade vCluster v0.19 to v0.20 by converting your Helm values configuration file to the new `vcluster.yaml` configuration file. 

As part of the v0.20.0 release, Loft Labs introduced a new `vcluster.yaml` configuration file with a totally new structure. This file replaces your existing `values.yaml`. Pre-v0.20.0 `values.yaml` files are not compatible with v0.20.0. You need to convert to the new format in order to upgrade existing vCluster instances.
...
Loft Labs has tested the conversion path from v0.19.x to v0.20.0 and therefore recommends moving to v0.19.5 before converting to v0.20.0.
...
The v0.20.0 vCluster CLI has been extended to support converting from the old format to the new using `vcluster convert config`.
...
## Convert your vCluster instance
