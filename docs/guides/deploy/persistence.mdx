---
title: Persist vCluster data
sidebar_label: Persist vCluster data
---
...
In order to use an emptyDir to store the data instead of a persistent volume, please create a `vcluster.yaml` with the following contents:
...
The following example could be used to launch a vCluster that connects to a PostgresSQL database named k3s. Create a `vcluster.yaml` with:
...
The following example could be used to connect the vCluster to a MySQL database using client certificate authentication. Create a `vcluster.yaml` with:
