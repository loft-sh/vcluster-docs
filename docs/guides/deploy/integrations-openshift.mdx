...
import NonRootSegment from '@site/docs/_fragments/non-root-vcluster.mdx'
import OpenshiftSegment from '@site/docs/_fragments/deploy-to-openshift.mdx'


By default, OpenShift doesn't allow running containers with the root user, but it assigns a random UID from the allowed range automatically, which means that you can skip the steps described in the [Running as non-root user](/guides/security/rootless-mode.mdx) section of this document and your vCluster should run as a non-root user by default.
