...
:::caution
Running as non-root is currently supported only for the k3s distribution. While other distributions provided by vCluster may make use of the `securityContext` field from the `vcluster.yaml` file, we do not guarantee that they will work as expected.
:::
