...
Add your `vcluster.yaml` config file to the `valueFiles` array in your ArgoCD `Application` file. You may use any of the options for passing values covered by the [ArgoCD Helm documentation](https://argo-cd.readthedocs.io/en/stable/user-guide/helm).
...
Remove `export HELM_VALUES=$(cat vcluster.yaml)` if you don't have custom configuration.
