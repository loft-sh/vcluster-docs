...
separate namespaces to communicate with each other, despite the standard network policy not
...
The virtual cluster functionality of vCluster.Pro comes from the popular open source project
[vCluster](https://vcluster.com). vCluster.Pro provides a centralized management layer for virtual
clusters, allowing users to provision virtual clusters in any vCluster.Pro managed cluster (or virtual
cluster!). vCluster.Pro can also import existing virtual clusters such that they
can then be managed from the central vCluster.Pro instance!
