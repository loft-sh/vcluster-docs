...
**Create vCluster.Pro Config For Microsoft**

vCluster.Pro is able to use Microsoft OAuth2 flow to identify the end user through their Microsoft account.

To tell vCluster.Pro to use Microsoft for SSO, navigate to `Admin > Config` in vCluster.Pro and adjust your config as shown below:

```yaml
auth:
  # Tell vCluster.Pro to use microsoft authentication
  microsoft:
    #
    #
    ...
    redirectURI: https://vcluster-pro.my.domain/auth/microsoft/callback
    #
    #
...
After saving the new vCluster.Pro configuration, vCluster.Pro will restart itself and you should be able to log in via Microsoft.
