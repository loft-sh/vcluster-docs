...
**Create vCluster.Pro Config For Google**

vCluster.Pro is able to use Google’s OpenID Connect provider as an authentication source.

To tell vCluster.Pro to use Google for SSO, navigate to `Admin > Config` in vCluster.Pro and adjust your config as shown below:

```yaml
auth:
  # Tell vCluster.Pro to use google authentication
  google:
    #
    #
...
After saving the new vCluster.Pro configuration, vCluster.Pro will restart itself and you should be able to log in via Google.
