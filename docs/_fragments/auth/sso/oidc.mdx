...
**Create vCluster.Pro Config For OIDC**

vCluster.Pro is able to use an OIDC provider as an authentication source.

To tell vCluster.Pro to use OIDC for SSO, navigate to `Admin > Config` in vCluster.Pro and adjust your config as shown below:

```yaml
auth:
  # Tell vCluster.Pro to allow OIDC for authentication
  oidc:
    #
    #
    ...
    redirectURI: https://vcluster-pro.my.domain/auth/oidc/callback
    #
    #
    ...
    emailClaim: "email"
    # (Optional) vCluster.ProUsernameClaim is the JWT field to use as the user's ID (Kubernetes name) and username
...
After saving the new vCluster.Pro configuration, vCluster.Pro will restart itself and you should be able to log in via your OIDC provider.
