...
To tell vCluster.Pro to automatically apply certain account templates to new users when they sign in via SSO for the first time, navigate to `Admin > Config` in vCluster.Pro and add the following lines to your `auth.oidc` section of the config:
...
auth:
  oidc:
    ...
    - name: loft-all-clusters-default     # Name of template 1
      sync: true                          # If enabled vCluster.Pro will sync changes in the cluster account template to the created accounts
...
Usually it is recommended to enable sync via the `clusterAccountTemplates[*].sync` or `groupClusterAccountTemplates[*].clusterAccountTemplates[*].sync` option, as this enables you to maintain multiple accounts and resources just with a single cluster account template. vCluster.Pro will watch the cluster account templates and update all users and teams that use this template automatically as soon as the template changes.
:::

:::info Update Cluster Account Templates
vCluster.Pro will make sure that the cluster account templates that you specify in `clusterAccountTemplates` and `groupClusterAccountTemplates` will be synced to the user every time the users SSO tokens are refreshed (e.g. during login or after a certain time automatically). This means that if you for example add a new cluster account template in `auth.oidc.clusterAccountTemplates` this will also be set on an already existing user after their credentials were refreshed.
:::
