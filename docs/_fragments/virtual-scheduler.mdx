...
1. Labeling nodes inside the virtual cluster has no effect on scheduling, unless [enabling real node syncing](/vcluster-yaml/sync/from-host/nodes) with the syncBackChanges enabled.
