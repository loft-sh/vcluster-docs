...
- [**vCluster Control Plane**](/vcluster/architecture/): Contains a Kubernetes API server, a controller manager, a data store mount and the Syncer.
- [**Syncing resources**](/vcluster/architecture/): vCluster runs your workloads by syncing pods from the virtual cluster to the host cluster.
- [**Pod scheduling**](/vcluster/architecture/): By default, vCluster reuses the host cluster scheduler to schedule workloads.
- [**Storage**](/vcluster/architecture/): You can use the host's storage classes without the need to create them in the virtual cluster.
- [**Networking**](/vcluster/architecture/): vCluster syncs resources such as Service and Ingress resources from the virtual to the host cluster.
- [**Nodes**](/vcluster/architecture/): By default, vCluster creates pseudo nodes for every pod `spec.nodeName` in the virtual cluster.
...
1. (Optional) Configure vCluster with `vcluster.yaml`.
   ...
   By default, vCluster uses vanilla Kubernetes as the Kubernetes distribution for the virtual cluster. If you want to change the [Kubernetes distribution](/vcluster/vcluster-yaml/control-plane/components/distro/), you must change it before you deploy vCluster. Create a file called `vcluster.yaml` and configure the distribution. This example configures a different Kubernetes distribution (i.e K3s).
   ...
   Refer to the `vcluster.yaml` [reference docs](/vcluster/vcluster-yaml/) to explore all configuration options.
...
Configure features in a `vcluster.yaml` file. These examples show you how to configure some popular features. See the `vcluser.yaml` [configuration reference](/vcluster/vcluster-yaml/) for how to configure additional features.
