...
- **Fake Nodes**: (default) vCluster creates fake nodes for each `spec.nodeName`. If there are no more pods on a node, vCluster deletes the fake node (no cluster role for vCluster is needed).
