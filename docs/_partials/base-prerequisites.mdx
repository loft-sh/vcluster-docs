...
Run the command `kubectl auth can-i create clusterrole -A` to verify that your current kube-context has administrative privileges.
    ...
    :::
- `helm`: Helm <Mark color="#ff6600">v3.10</Mark> is required for deploying the platform. Refer to the [Helm Installation Guide](https://Helm.sh/docs/intro/install/) if you need to install it.
