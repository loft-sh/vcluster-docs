A cluster is a fully functional Kubernetes cluster that runs inside a namespace of
another Kubernetes cluster. Typically, the cluster that the cluster is installed in is
referred to as the control plane cluster.

Clusters, being fully functional Kubernetes clusters in their own right, can be a very
...
permitting this. In both of these (and many more!) scenarios, a cluster may be a perfect
solution.

The diagram below briefly outlines the attributes of clusters as compared to using
...
The cluster capability of vCluster Platform comes from the popular open source project
[vcluster](https://vcluster.com). vCluster Platform provides a centralized management layer for
clusters, allowing users to provision clusters in any control plane cluster vCluster
Platform manages, or even inside another cluster. vCluster Platform also offers the capability to import existing clusters such that they
can then be managed from the central vCluster Platform instance.

## Why use tenant Kubernetes clusters?

Clusters can be used to partition a single physical cluster into multiple logical,
...
Clusters handily address these challenges by providing a dedicated Kubernetes API server
per cluster, thus bypassing the challenges of cluster-scoped resources and sharing a
single control plane.

Clusters also provide more stability than namespaces in many situations. The
...
Clusters are not only highly functional, they can often be a huge cost saver for
organizations. Many teams scale to address the problems outlined here by simply adding
additional physical Kubernetes clusters. With clusters, administrators are able to have
many clusters within a single physical cluster, this results in big savings by not
having to pay for multiple control planes, and more often than not results in vastly simplified
cluster maintenance. This makes clusters ideal for running experiments, continuous
integration, and setting up sandbox environments.

Finally, clusters can be configured independently of the physical cluster. This is great
...
<!-- vale on -->
    ...
    - Reuse and share services across multiple clusters with ease.
- **Cost Savings:**
