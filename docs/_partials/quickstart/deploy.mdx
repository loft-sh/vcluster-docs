...
import TerraformDeployFile from '!!raw-loader!@site/docs/_partials/quickstart/terraform-deploy-main.tf'
...
This uses Helm to generate the Kubernetes deployment files from the `vcluster.yaml` file before applying with `kubectl`
...
To deploy vCluster using ArgoCD, you need two files:

- `vcluster.yaml` for your vCluster configuration.
