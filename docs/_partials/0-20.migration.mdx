...
a cluster had configuration options using CLI flags, annotations and different `values.yaml` options. With the new `vcluster.yaml`, all configuration options are now easily configured in a single source.
...
* If you are using [vcluster-hostpath-mapper](https://github.com/loft-sh/vcluster-hostpath-mapper), upgrade it to v0.1.1-beta.1 or later for compatibility with the new vcluster.yaml configuration file in v0.20.
* Review the [list of changes](/vcluster/reference/migrations/0-20-cli-changes) in the vCluster CLI as commands, subcommands, and flags were removed, renamed or added.

## Upgrade the CLI

Before upgrading cluster instances, upgrade to the latest version of the vCluster CLI.
...
If you don't have the `values.yaml`, use Helm to retrieve the user defined configuration for your currently running cluster.
...
- `<KUBERNETES_DISTRO>` with your cluster's existing Kubernetes distribution. **You can't change the Kubernetes distribution or backing store during an upgrade**.
...
the `loft-sh/vcluster-pro` image repository. This allows users to seamlessly test and adopt any Enterprise feature without disrupting their existing
open source features. Enterprise features are integrated into the `loft-sh/vcluster-pro` image but remain inactive by default to ensure that your experience remains consistent with the open source version unless you specifically activate them with a vCluster Platform Enterprise plan.
...
#### EKS distribution support

The v0.20 removed the support of EKS as a cluster distribution.

:::note reminder
The Kubernetes distribution of the cluster doesn't need to match the distribution of your control plane cluster. You can use any distribution of vCluster to deploy vCluster on EKS clusters.
:::

In order to continue using your clusters that have EKS as the distribution, you can convert to using the k8s distribution and replace the images with the EKS images. While this was tested when v0.20.0 was released, it is not continuously tested. It's recommended spinning up new clusters with the k8s distribution and migrate your workloads to them.
...
While downgrading to an older version of the vCluster CLI is supported, you cannot downgrade cluster instances to a prior version of vCluster (for example, from 0.20+ back to 0.19). Make sure you have tested the upgrade sufficiently before rolling out this change to critical systems.
:::
...
You can spin up two clusters, one at older version, such as 0.19 and one at newer version, for example 0.20+.
