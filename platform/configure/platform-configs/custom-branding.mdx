...
<FeatureTable names="custom-branding,advanced-ui-customizations" />


The platform UI can be customized to match your corporate design, enhancing brand consistency and user experience.
<!-- vale off -->
Custom branding can be configured using the platform config. Add the following YAML configuration to your platform config (the `config` section of the [`values.yaml`](../introduction.mdx)) or through vCluster Platform UI **Admin > Config**:
<!-- vale on -->
