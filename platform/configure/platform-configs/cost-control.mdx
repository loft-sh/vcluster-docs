...
The platform comes with the cost control dashboard enabled by default, offering insights into potential [cost savings](https://www.vcluster.com/cost-savings) through clusters.
...
clusters the platform deploys and manages [Prometheus](https://prometheus.io/) and
...
config](./cost-control.mdx).


To turn off the cost control dashboard, which is enabled by default, set `costControl.enabled` to `false` in platform configs:

```yaml title="Turning off the Cost Control Dashboard in values.yaml"
config:
  costControl:
    enabled: false
```
...
```yaml title="Default Cost Settings in values.yaml"
config:
  costControl:
    settings:
      # Average CPU cost
...
```yaml title="Default Global Prometheus Settings in values.yaml"
config:
  costControl:
    global:
      metrics:
...
```yaml title="Default Cluster Prometheus Settings in values.yaml"
config:
  costControl:
    cluster:
      metrics:
...
```yaml title="Default OpenCost Settings in values.yaml"
config:
  costControl:
    cluster:
      opencost:
...
For individual cluster configuration, the [Cluster](/docs/platform/api/resources/clusters) resource allows for further
