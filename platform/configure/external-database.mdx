...
| **External database** | Yes — all replicas share a single Kine-backed database | No — uses embedded SQLite | Yes — all regions share a single Kine-backed database |
...
- **Cost control unavailable**: The cost control feature requires the embedded
...
a single AWS region using EKS, an external RDS database (Kine), ALB ingress,
...
-   PodDisruptionBudget ensuring at least one replica is always available
