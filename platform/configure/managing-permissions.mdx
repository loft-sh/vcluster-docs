...
description: Learn how to apply RBAC policies.
sidebar_position: 5
---

<!-- TODO:(piotr1215) move managing permissions to administer section -->
One of the core goals of vCluster Platform is to provide vCluster Platform admins the ability to manage permissions for
...
custom vCluster Platform resources to provide a more ergonomic way to manage access.

This guide is intended to provide a practical showcase of applying vCluster Platform RBAC policies in
...
Before continuing, it would be a good idea to make sure you are familiar with vCluster Platform
[Users and Teams](/platform/users/what-are-users-and-teams.mdx), as well as
[SSO Configuration Options](/platform/users/sso/openid-connect.mdx) as building out a complete RBAC design will
almost certainly require these resources/configurations. Lastly, it is also a great idea to make
sure you are up to speed with [Kubernetes RBAC](https://kubernetes.io/docs/reference/access-authn-authz/rbac/).
...
- We also have a "power user" (we'll call this user "dvader") who should have full access to the
  connected clusters, but _not_ be able to create admin users or do other vCluster Platform administration type
...
[^1]:
    By default, users granted the Project User role can only provision virtual clusters
...
[^2]:
    This means we cannot use the "built in" Management Role (ClusterRoleTempalte kind)
...
[^3]:
    Once again this means we cannot use the "built in" Management Role "Project Admin"
...
For guide, a fresh vCluster Platform instance has been installed and a second connected
...
Projects are one of the core organizational components to vCluster Platform. Introduced in vCluster Platform 3.0, Projects
give us a simple cluster agnostic way to group, view, and apply policies to resources in vCluster Platform.
...
Management Role shows up as an available option in the vCluster Platform UI for roles to apply to Users or
Teams on a Project.
...
templates for Virtual Clusters ("by using the "_" for the name), and we have explicitly allowed
both Clusters for this Project. We could have used a single entry of "_" to allow all Clusters
for this Project, but by explicitly defining the Clusters we can ensure that members of this
Project are not accidentally given access to any Clusters that may be added to vCluster Platform in the future.
...
The easiest way to validate your RBAC policies is to simply impersonate Users who you would
...
Check out the [Impersonate](../users/managing-users/impersonate.mdx) docs section for the basics of impersonation.
...
edit) all connected Clusters, but _not_ see any Projects (other than the Default Project). This
...
but has no visibility to the "acme" Project, and cannot see any Clusters at all (but _can_
create virtual clusters in the permitted clusters). This User should _not_ be able to create
Spaces, but _can_ create Virtual Clusters in either of the Connected Clusters.

Finally, impersonating the "acme" test User, you should see that you have access only to the
"acme" Project, and that you do _not_ have a "Create" option for either spaces or virtual
clusters. Creating a space or virtual cluster with the admin user (and ensuring that the _owner_
...
management, none of these Users should be able to act on any of those types of resources in vCluster Platform.
...
### vCluster Platform SSO Configuration

Configuring SSO in vCluster Platform is documented [here](../users/sso/openid-connect.mdx), for purposes of this
...
critical to ensure that you are properly telling vCluster Platform where it can find the attributes you care
about. In this case we are telling vCluster Platform that the username will be defined under an attribute
...
be configured in such a way to appropriate pass groups to vCluster Platform such that users logging into vCluster Platform
via the provider can be properly categorized.

### Mapping Groups to vCluster Platform Teams

Once you have established which group from SSO you would like to map to which Team in vCluster Platform, you
...
vCluster Platform and the SSO provider. There can be miscommunication about which fields map to which
attributes, which can cause SSO integration to not work as desired. It is always a good idea to
watch the vCluster Platform logs when testing SSO integration. vCluster Platform will log the parsed SAML response, you
...
You can also confirm the User and Teams by viewing the User Management pane in the vCluster Platform UI --
...
Typically, we advise against limiting access to individual objects and instead recommend vCluster Platform
...
The following manifests represent the "end state" of vCluster Platform, or the configuration state at the
...
Big disclaimer, this Keycloak setup is _strictly for lab/testing purposes_ -- it is insecure in
...
<details>
    <summary>Keycloak vCluster Platform Realm JSON (very long.)</summary>
