...
Both vCluster Platform and vCluster Platform Agent are using the `vcluster-platform` helm chart. By setting values in `values.yaml`, you can choose to install or upgrade vCluster Platform or vCluster Platform Agent. You can also control the post-deployment platform behaviour and the
behaviour of agents in other managed host clusters.
...
3. **Agent settings** - Control how the platform agent operates in connected clusters
   - Set through the top-level `agentValues:` section of the `vcluster-platform` chart. You can [override](#understanding-agent-values) these per-cluster with annotations
   - Located in the `vcluster-platform` Helm chart ([loft-sh/loft](https://github.com/loft-sh/loft/tree/master/chart))
   - Examples: `replicaCount`, `resources`, `ingress`, `persistence`, etc.
