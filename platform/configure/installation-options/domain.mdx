...
After deploying the platform, you may want to access the deployment through a routable IP address or a resolvable domain name. You may also wish to secure the deployment with a TLS certificate. This is particularly useful for sharing platform access with other team members who may or may not have direct access to Kubernetes.

:::tip Requirements
Setting up the platform for access through a routable IP or domain name, and securing it with TLS requires the following:
...
The platform, like any other service in Kubernetes, can be exposed in multiple ways. **The LoadBalancer method is recommended.** The NGINX ingress controller examples below are preserved for reference but the ingress-nginx project is deprecated upstream. Use a LoadBalancer or a Gateway API-compatible controller for new deployments. For cluster endpoint planning, see [Gateway API](/docs/vcluster/key-features/gateway-api) and [Migrate from ingress-nginx to Gateway API](/docs/vcluster/manage/ingress-nginx-to-gateway-api).

<details>
  <summary>

### [Deprecated]: External access through NGINX ingress controller
...
<Tabs
  ...
  values={[
    { label: 'Domain through ingress', value: 'ingress', },
    { label: 'Domain through load balancer', value: 'lb', },
  ]
}>
...
Make sure to follow the [Load Balancer > AWS ELB + ACM](#external-access-through-loadbalancer) guide above.
...
tls:
  enabled: true # Clusters running on the same host as the platform directly use the platform service.
...
## Import clusters with certificate validation {#importing-existing-tenant-clusters}

When importing externally managed clusters, you can enable secure communication by using the same certificate authority configured for the platform.
...
2. Import the cluster with certificate validation:

```bash title="Import a cluster with certificate validation"
...
The imported cluster restarts and reports `Ready` once it establishes a secure connection to the platform.
