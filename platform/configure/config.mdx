...
For high availability and scalability, the platform agent can be [deployed with multiple replicas](../configure/high-availability). However, certain agent tasks must be executed by only one pod 
...
Thus, by default, the leader election is enabled when [installing vCluster through Helm](../../install/helm) or [using vCluster CLI](../../install/quick-start-guide). 
This ensures that only one pod performs “singleton” tasks while 
