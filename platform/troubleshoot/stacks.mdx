...
| `FeatureNotAllowed` | At least one task requires a feature outside the active plan. App tasks require the Apps feature, available on every plan. Argo CD Application tasks require the Argo CD Integration feature, which isn't available on the free plan. | Remove the unlicensed task, or use a plan that includes the missing feature. |
...
| `OutputsConflict` | Another object already owns the Secret name the controller needs for captured outputs. | Read the Ready condition message and inspect Secrets in the project namespace. Don't delete a Secret until you confirm the StackInstance doesn't own it. |
