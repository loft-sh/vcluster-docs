...
A `Healthy` Stack with a missing or unexpected child application can mean two tasks share a name. vCluster Platform doesn't currently reject a duplicate task name, so the later task's child silently replaces the earlier one's. Check `status.tasks[]` for repeated `name` values, then rename the conflicting task in the StackTemplate.
