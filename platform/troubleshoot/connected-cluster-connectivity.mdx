...
2. **During reconciliation**, after at least one successful connection: `status.phase` is `Failed`, stays empty longer than expected, or is `Initialized` with a non-empty `status.reason` describing a pod health problem. Go to [Failures during reconciliation](#failures-during-reconciliation) and match the reason.
