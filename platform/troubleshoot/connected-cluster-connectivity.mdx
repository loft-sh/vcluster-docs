...
The platform UI's **Infra Management > Control Plane Clusters** view also reports an `Online` state for the cluster. That state only reflects reality for clusters connected through the agent's network tunnel (`spec.networkPeer: true`). It has no meaning for a cluster registered with a direct kubeconfig secret (`spec.config`) or for the cluster the platform itself runs on (`spec.local: true`), so don't rely on it in those two cases.
...
For the agent side, go to **Infra Management > Control Plane Clusters**, click **Edit** for the cluster, open the **Agent** tab, and add under **Extra Agent Values**. See [Set agent values using the UI](../administer/clusters/advanced/agent-config.mdx#set-agent-values-using-the-ui).
