...
description: Identify which vCluster Platform project quota or admission quota denied a cluster, namespace, Machine, or Kubernetes resource request.
---
...
Management denials come from the vCluster Platform management API before the requested management resource is accepted. They apply to platform-owned objects, such as cluster instances, namespace instances, and Machines. These denials answer the question: "Can this project, user, or team create or wake another platform-managed resource?"
...
| `virtualclusterinstance`, `spaceinstance`, or `nodeclaim` | The management resource that consumes quota. Clusters consume `virtualclusterinstances`, namespaces consume `spaceinstances`, and Machines consume `nodeclaims`. |
...
<Flow id="identify-management-quota-denial">
  <Step>
    **Retrieve the denied management resource**
    ...
    Use the resource type from the denial message to retrieve the denied management object. For clusters and namespaces, the namespace where the management object lives is the project namespace.
...
<Flow id="identify-kubernetes-admission-quota-denial">
  <Step>
    **Identify the connected-cluster namespace**
    ...
    Start with the namespace where the denied Kubernetes resource was created. If the denial happened from inside a cluster or namespace instance, get the connected-cluster namespace from the instance's `spec.clusterRef.namespace` field.
...
| An `.active` quota blocked a wake-up request | Increase the active-resource quota, or put other inactive clusters or namespaces to sleep before retrying the wake-up. |
...
For storage limits inside clusters, combine project quotas with per-tenant-cluster ResourceQuota objects or policy enforcement. See [Enforce storage quotas per cluster](../how-to/enforce-storage-quotas.mdx).
