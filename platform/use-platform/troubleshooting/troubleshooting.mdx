...
If you can access vCluster Platform through port-forwarding only, this is usually an indicator that the problem
might be related to a misconfigured LoadBalancer or Ingress Controller.

### Quota and admission denials

If creating or waking a cluster, namespace, Machine, pod, service, or PVC fails with a quota-related `Forbidden` error, use the [quota and admission troubleshooting runbook](../../troubleshoot/quota-admission.mdx) to identify the project, owner, quota expression, and generated cluster quota that denied the request.

### Connected cluster connection failures

If a connected cluster never finishes connecting, goes offline or flaps after connecting, or gets stuck mid-upgrade, use the [connected cluster connectivity runbook](../../troubleshoot/connected-cluster-connectivity.mdx) to match the cluster's status reason to a cause and resolution.

## vCluster

### Debug shell

The Debug Shell opens a terminal in an ephemeral container attached to a vCluster control plane pod.
...
This role should be assigned to users who already have access to the cluster.
