...
The secret will be synchronized with the project secret momentarily. Secrets may be synchronized in any namespace or cluster that belong to the project.

## Syncing Secrets in Intervals

Secrets inside clusters will be synchronized whenever the project secret is created or modified. However, since it would be inefficient to watch all secrets in all clusters,
vCluster Platform synchronizes the cluster secrets on an interval. This means there's a small delay before a cluster secret is synchronized once it's created.
