...
<Flow id="sleep-automatic-ui">
  ...
  <Step>
    In the drawer that appears on the right, expand the <Expander>Sleep</Expander> section. This only works for {props.name}s without a template, if you want to change the sleep mode configuration for a {props.name} created by a template, do the changes in the Templates view
  </Step>
  <Step>
