...
property that references the template renders the matching field, so a template
referenced for user data must set `cloudInitTemplate` and one referenced for
network data must set `networkDataTemplate`. If the matching field is unset,
provisioning fails.
