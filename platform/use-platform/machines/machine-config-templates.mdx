...
type or Machine instead of inlining it on each one.

Each template holds exactly **one** of `cloudInitTemplate` or
`networkDataTemplate`. Setting both, or neither, is rejected.
...
output before it is applied to the Machine.
...
| `spec.cloudInitTemplate` | Go template that renders a `#cloud-config` document. Mutually exclusive with `networkDataTemplate`. |
| `spec.networkDataTemplate` | Go template that renders a cloud-init `network-config` document. Mutually exclusive with `cloudInitTemplate`. |
