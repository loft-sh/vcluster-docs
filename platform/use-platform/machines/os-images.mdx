...
:::note OS images inside a Tenant
A [Tenant](../../understand/what-are-tenants.mdx) reaches the platform's OS images only
where a capability grants them by label, and can upload its own only where the capability
allows it. See
[Grant object access](../../administer/tenants/create-and-configure-tenants.mdx#grant-object-access).
:::
