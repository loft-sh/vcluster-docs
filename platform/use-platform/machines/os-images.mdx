...
For the [NICo driver](../../vmetal/drivers/bare-metal/nico.mdx#prepare-an-operating-system-image), set `machines.vcluster.com/os-image-type` to `nico`.
An image-based definition requires `nico.vcluster.com/os-image-url`. Set `nico.vcluster.com/os-image-sha` to the image checksum. NICo requires exactly one root filesystem UUID or label.
Use `nico.vcluster.com/os-image-disk` to select the target disk.

At first use, the driver creates or finds the corresponding operating system in the consuming NICo tenant.
Alternatively, set `nico.vcluster.com/os-image-id` to use an existing NICo operating system.
See the [NICo image configuration](../../vmetal/drivers/bare-metal/nico.mdx#prepare-an-operating-system-image) for the complete example, required NICo settings, and disk overwrite precautions.
