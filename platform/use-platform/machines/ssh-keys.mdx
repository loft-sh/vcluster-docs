...
:::note SSH keys inside a Tenant
A [Tenant](../../understand/what-are-tenants.mdx) never sees the platform's own SSH keys.
The capability only decides whether the Tenant may register keys of its own, and those
names carry the `<tenant>--` prefix.
:::
