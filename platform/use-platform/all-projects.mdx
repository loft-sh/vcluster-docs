...
description: Use the All Projects view to browse and filter clusters, namespaces, machines, and network environments across active projects you can access from a single screen.
---

import AllProjectsSelected from '@site/static/media/platform/screenshots/all-projects-selected.png';

By default, every view in vCluster Platform is project-scoped. When you are working inside a project, the clusters, namespaces, machines, and secrets you see belong only to that project. This is by design. Projects are isolation boundaries, and members of one project cannot see another project's resources.
...
<figure className="platform-screenshot">
  <img src={AllProjectsSelected} alt="All Clusters view with the All Projects mode active, showing the globe icon and All Projects label in the project switcher, along with the cluster table and project and status filter controls" />
  <figcaption>All Clusters in All Projects mode. The project filter and status filter let you narrow the table without leaving the cross-project context.</figcaption>
</figure>
...
Clicking the globe icon from any project view switches to All Projects and opens the equivalent section. For example, clicking from a project's Clusters page opens All Clusters. Clicking from a section that has no All Projects equivalent (such as Secrets) opens All Clusters by default.
...
All Projects provides three views. Each table includes a project column and filters so you can scope the list without leaving the cross-project context. Visible table columns are configurable by each user, so the columns you see might differ from another user's view.

| View | Use it to |
| --- | --- |
| [All Clusters](./virtual-clusters/create/create-no-template.mdx) | Review clusters, their status, project, template, owner, version, and control plane placement. |
...
| [All Machines](./machines/overview.mdx) | Review machines, their project, assigned cluster, infrastructure provider, network environment, and operating system image. |

Network environments aren't owned by a project, so they have no cross-project view. Manage them under **Bare Metal > Networking**. The **Network Environment** column in All Machines links there.
...
Clicking a project link in a row opens that project's equivalent page. For example, clicking a project link in the All Clusters table opens that project's cluster list.
