...
## Include Stacks in a tenant cluster template

vCluster 0.37 supports Stacks through `deploy.stacks` in the tenant cluster's `vcluster.yaml`. Use a Stack when applications need dependency ordering, health gates, or output handoff.

Add the Stack in the template's **Stacks** configuration section or YAML. See [Use a Stack](./use-stacks.mdx#declare-a-stack-in-vclusteryaml) for the configuration shape. Template authors can also [create a StackTemplate](../../administer/templates/create-stack-templates.mdx) for users to select.
