...
Use one App for an independently managed application. Use a Stack when several applications must deploy in order, wait for healthy dependencies, or pass outputs.

## Understand licensing

Stacks don't have a separate license entitlement. App tasks use the Apps entitlement, which is available on every plan. Argo CD Application tasks also require the Argo CD Integration entitlement, which isn't available on the free plan.

If a task isn't licensed, the StackInstance is blocked with the `FeatureNotAllowed` reason.

## Install a Stack from the catalog

The UI can show **Stacks & Apps** or **Apps**, depending on your permissions and the installed Platform patch level.
...
You need access to the StackTemplate and permission to create StackInstances in the project. Published output values require separate permission to get the `stackinstances/outputs` subresource.

## Add a Stack during tenant cluster creation
...
The StackInstance is created for the new tenant cluster. It is reconciled with that tenant cluster's lifecycle.

## Declare a Stack in vcluster.yaml
