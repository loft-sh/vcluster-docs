...
vCluster Platform uses the `VirtualClusterInstance` CRD to manage virtual clusters. The key field controlling management behavior is `spec.external`, which determines whether the Platform only monitors or fully manages the virtual cluster lifecycle.
...
- **`external: true`** - Platform provides visibility but doesn't manage lifecycle (read-only mode)
- **`external: false`** - Platform fully manages the vCluster lifecycle (full control)

:::info Learn more about spec.external
For detailed information about the `spec.external` field and its implications, see the [VirtualClusterInstance API reference](../../api/resources/virtualclusterinstance/virtualclusterinstance.mdx).
:::

## Add for visibility only

Use this approach when you want to keep managing the vCluster with your existing tools (Helm, Argo CD, etc.) while gaining visibility in the Platform UI.
  ...
  <Step>
    **Add Host Cluster to the Platform (Optional)**: If you want to add the host cluster to the platform, then
    ...
    <InterpolatedCodeBlock
      code={`CLUSTER_ID=[[VAR:CLUSTER_ID:my-host-cluster]]
UI_CLUSTER_NAME="[[VAR:UI_CLUSTER_NAME:My Host Cluster]]"
vcluster platform add cluster $CLUSTER_ID --display-name "$UI_CLUSTER_NAME"`}
  ...
  <Step>
    **Add Virtual Cluster to the Platform**: Before running this command, be sure that your kubecontext is set to the
    ...
    <InterpolatedCodeBlock
      code={`VIRTUAL_CLUSTER_ID=[[VAR:VIRTUAL_CLUSTER_ID:my-vcluster]]
PROJECT=[[VAR:PROJECT:default]]
VIRTUAL_CLUSTER_IMPORT_NAME="[[VAR:VIRTUAL_CLUSTER_IMPORT_NAME:My vCluster]]"
