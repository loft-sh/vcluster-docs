---
title: Add existing clusters
sidebar_label: Add Existing Clusters
sidebar_position: 2
description: Add externally deployed clusters to Platform with visibility-only or full management capabilities
---
...
Any running cluster can be added to the Platform. When adding a cluster to the Platform, you can choose between two integration levels based on your needs.

:::tip Related documentation
For conceptual understanding of externally vs. Platform-deployed clusters, see [Externally deployed clusters](/docs/platform/understand/externally-vs-platform-deployed).

See the [Platform CLI hub](../../reference/platform-cli.mdx) for UI-to-CLI equivalents across common Platform tasks, including cluster commands.
:::
...
The Platform uses a VirtualClusterInstance resource to track clusters. The management level is controlled automatically based on how you add the vCluster. For more details, see the [VirtualClusterInstance API reference](../../api/resources/virtualclusterinstance/virtualclusterinstance.mdx).
:::

## Add for visibility and access

Use this approach when you want to keep managing the vCluster with your existing tools (Helm, Argo CD, or similar) while gaining visibility and access through the Platform, including SSO integration.
...
<Flow id="add-virtual-cluster-visibility">
  ...
  <Step>
    **Add Cluster to the Platform**: before running this command, be sure that your kubecontext is set to the
...
Then, add the cluster to the platform by running:
<InterpolatedCodeBlock
...
For [GitOps deployments](/docs/platform/install/gitops), create the [VirtualClusterInstance](../../api/resources/virtualclusterinstance/virtualclusterinstance.mdx) directly in your Git repository. Tools like [Argo CD](/docs/platform/integrations/argocd) or Flux apply the manifest, and the Platform manages the cluster:
...
Verify that the cluster is correctly managed by the Platform. You can check this using the CLI:
...
# Check Platform-managed clusters
...
- **[Project assignment](/docs/platform/administer/projects/create)**: Organize clusters into projects with quotas
...
- **[Resource quotas](/docs/platform/administer/projects/quotas)**: Enforce resource limits using <GlossaryTerm term="project">project</GlossaryTerm> settings
...
**Single source of truth principle:** when `external: false`, the Platform becomes the sole manager of the cluster. This means:
...
- **Reconciliation conflicts**: If external tools continue to manage the cluster after setting `external: false`, both systems may fight to enforce their desired state
...
2. **GitOps compatibility**: Use VirtualClusterInstance manifests in your GitOps repository, letting Argo CD deploy them while Platform manages the resulting clusters
...
- Learn about [managing cluster access](/docs/platform/use-platform/virtual-clusters/manage-access)
