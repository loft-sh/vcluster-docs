...
can configure the scheduling and retention of snapshots in the vCluster config options under **Automatic Snapshots**. Though snapshot configuration is configured on the virtual cluster itself, the functionality of scheduling snapshots is in vCluster Platform.
...
| `schedule` | No | Configure the schedule of taking snapshots by specifying a [cron](https://en.wikipedia.org/wiki/Cron) schedule. | `0 0 * * *` |
| `retention.period` | No | Number of days that a snapshot will be stored in the storage service. | `30` |
| `retention.maxSnapshots` | No | Number of snapshots that can be stored in the storage service. | `110` |
...
| Option  | Description  |
|---|---|
| `storage.type`  |  Defines the type of storage used to store the snapshot, the platform supports the following types: S3 and OCI. |

### S3 buckets

Snapshots can be stored in an S3 bucket using the s3 protocol. 
...
| `storage.type.s3.podIdentityServiceAccount` | Name of the AWS pod identity service account. |
| `storage.type.s3.credential` | Reference the S3 credentials as a Kubernetes secret
| `storage.type.s3.credential.name` | Name of the Kubernetes secret
| `storage.type.s3.credential.namepsace` | Namespace of the Kubernetes secret 
...
```yaml title="Example configuring snapshots into s3 with a pod identity
external:
  platform:
    autoSnapshot:
    enabled: true
    ...
    storage:
      type: s3
        s3:
          ...
          # Service account to use for pod identity 
...
<Step>
  ...
  Create a Kubernetes secret that stores your AWS credentials. Place this secret 
  in the same namespace where the vCluster is deployed.
...
| `storage.type.oci.credential.namepsace` | Namespace of the Kubernetes secret |
...
<Step>
  ...
  Create a Kubernetes secret that stores your credential to your OCI registry. Place this secret 
  in the same namespace where the vCluster is deployed.
  ...
  * `password`
  ...
  ```bash title="Create credentials secret"
  kubectl create -f - <<EOF
  apiVersion: v1
  kind: Secret
  type: Opaque
...
Snapshots transition through different statuses as part of their execution lifecycle. A typical flow starts with the
 snapshot being in the `Scheduled` state, then moves to the `In Progress` state while it is being stored. If the snapshot and storage is successful,
  the snapshot moves to the `Stored` state. If an error occurs, it temporarily enters the `Error` state before retrying, and could end in the `Failed` state if it cannot be stored. 
