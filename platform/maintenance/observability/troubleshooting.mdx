...
## A cluster gets no metrics-writer credential

A cluster that delivers metrics through a connector requests its `metrics-writer`
credential from Platform. When Platform can't resolve that connector into a write endpoint the
cluster's collector could reach, it refuses the request. No credential Secret is
delivered, and the cluster logs the reason on every resync.

Platform reports the same finding as an `ObservabilityConnectorUnresolved` condition on the
cluster's instance, which is where the admin who owns the connector can see it:
...
|---|---|---|
| `WriteGatewayInternalOnly` | The connector's gateway Service is `ClusterIP`, and this cluster's collector doesn't run on the Platform cluster. | Set `serviceType: LoadBalancer` on the connector Secret. See [When clusters require a LoadBalancer gateway](./install-observability-gateway.mdx#when-clusters-require-a-loadbalancer-gateway). |
| `ObservabilityConnectorNotFound` | The connector named on the cluster doesn't exist in the Platform namespace. | Create the connector, or correct the name on the cluster. |
...
| `ObservabilityNotConfiguredOnInstance` | The cluster enables observability without naming a connector. | Name a connector on the cluster, or disable the integration. |

The condition carries only causes an admin has to fix. A connector is shared across
clusters, so causes that clear on their own are left off the condition. One example is a
gateway still waiting for its LoadBalancer address. Leaving them off keeps the condition from
flapping on every cluster using that connector. Those causes appear only in the requesting
cluster's own log. An absent condition therefore means either that the connector resolves or
...
| Scope denied | Key scope doesn't include the requested cluster or cluster. |
