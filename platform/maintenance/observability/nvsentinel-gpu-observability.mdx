---
title: Observe GPU nodes with NVSentinel
...
[GPU observability templates](./gpu-observability-templates.mdx). You reference the template from an `ArgoCDApplication` to deploy
NVSentinel into a cluster that runs GPU nodes, typically a cluster with private
...
- The [Argo CD integration](../../integrations/argocd/overview.mdx) is enabled.
- An Argo CD connector is registered on the target cluster. See [Connect to Argo CD](../../integrations/argocd/connect-argocd.mdx).
- The [NVIDIA GPU Operator](https://docs.nvidia.com/datacenter/cloud-native/gpu-operator/latest/index.html) is deployed in the same cluster, per the [deployment order](./gpu-observability-templates.mdx#deployment-order).
- cert-manager is deployed, if any component you enable needs internal TLS. See [When you need cert-manager](#when-you-need-cert-manager).
...
Deploy NVSentinel into the cluster whose GPU nodes it should watch. For a cluster
with private GPU nodes, target the cluster itself.
...
spec:
  ...
  destination:
    virtualCluster:
      name: my-vcluster            # replace with your cluster name
      target: vCluster             # deploy into the cluster
  templateRef:
    name: nvsentinel
  parameters:
...
<Flow>
  ...
  <Step>
    Navigate to <Label>Clusters</Label> and open the cluster with the GPU nodes to monitor.
  </Step>
  <Step>
...
[Tear down the GPU stack](./gpu-observability-templates.mdx#tear-down-the-gpu-stack).
...
| `dryRun` | `true` | Log and record intended actions without cordoning, draining, or remediating. See [Dry run](#dry-run). |
...
- [GPU observability templates](./gpu-observability-templates.mdx) for the full GPU template
