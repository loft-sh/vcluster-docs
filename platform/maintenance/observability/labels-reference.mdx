...
| `vcluster_platform_project` | Cluster project. | `X-Vcluster-Platform-Project` |
...
| Label | Meaning |
|---|---|
| `vcluster_platform_hosted_on` | Registered control plane cluster that hosts the cluster. |
...
|---|---|---|---|
| `cluster` | Fleet monitoring with OpenTelemetry | Control plane cluster hosting the cluster (this guide's pipeline always pairs it with tenant identity) | `vcluster_platform_hosted_on`. `vcluster_platform_cluster` is reserved for cluster-scoped metrics with no tenant identity, which this guide's pipeline doesn't produce. |
| `vcluster_name` | Fleet monitoring with OpenTelemetry | Cluster name | `vcluster_platform_instance` |
| `vcluster_project` | Fleet monitoring with OpenTelemetry | Project | `vcluster_platform_project` |
| `vcluster_project_namespace` | Fleet monitoring with OpenTelemetry | Namespace backing the cluster | No direct equivalent — the gateway doesn't expose the backing namespace as a scope label |
| `vcluster_user` | Fleet monitoring with OpenTelemetry | Cluster owner | No direct equivalent — not part of the authenticated scope model |
| `vcluster_virtual_namespace` | Fleet monitoring with OpenTelemetry | Namespace as seen inside the cluster | No direct equivalent |
| `vcluster_virtual_pod` | Fleet monitoring with OpenTelemetry | Pod as seen inside the cluster | No direct equivalent |
| `loft_project_name` | Aggregating Metrics | Project | `vcluster_platform_project` |
| `loft_virtualcluster_name` | Aggregating Metrics | Cluster name | `vcluster_platform_instance` |
