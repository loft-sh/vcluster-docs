...
- Grafana datasource ConfigMaps for fleet, control plane cluster, and cluster
...
| `serviceType` | No | Gateway Service type. Valid values are `ClusterIP`, the default, and `LoadBalancer`. Must be `LoadBalancer` when a cluster delivers metrics through this connector and its collector runs outside the Platform cluster. See [When clusters require a LoadBalancer gateway](#when-clusters-require-a-loadbalancer-gateway). |
...
- **Allow embedding.** Set `security.allow_embedding = true` so the Fleet Observability page
  and each cluster's Observability tab can embed the dashboards in an iframe. See
...
### When clusters require a LoadBalancer gateway

A `ClusterIP` gateway is only reachable from inside the Platform cluster, so it serves
exactly one kind of cluster: one with shared nodes whose control plane runs on the
Platform cluster itself. That cluster's collector is a pod on a Platform cluster node,
so it resolves the gateway's in-cluster Service hostname like any other pod there. An
install where the Platform cluster only hosts shared-nodes clusters needs no load balancer.

Every other cluster runs its collector where that hostname doesn't resolve:

- A cluster hosted on a connected control plane cluster runs its collector on that control
  plane cluster's nodes.
- A cluster with private nodes runs its collector on its own nodes, whichever control
  plane cluster carries its control plane.
- A vCluster Standalone cluster runs on its own machine.

Rather than hand out an address those clusters can't reach, Platform refuses them the
...
surfaces, see
[A cluster gets no metrics-writer credential](./troubleshooting.mdx#a-cluster-gets-no-metrics-writer-credential).
