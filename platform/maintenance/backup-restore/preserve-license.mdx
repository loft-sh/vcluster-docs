...
2. Navigate to **Admin > Platform Config > License and Billing**
...
The `loft-license-cache` secret is worth calling out because its name suggests otherwise. It holds a cached copy of the last license the platform retrieved, so the platform can keep running while the license server is unreachable. It carries no identity. The platform rebuilds it on the next successful retrieval. Only `loft-cert` determines the instance ID. See [How licensing works](../../understand/licensing.mdx) for the full retrieval and caching model.
