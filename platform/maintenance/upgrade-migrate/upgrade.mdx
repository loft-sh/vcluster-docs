...
If you have apps deployed on a cluster, cluster, or space, upgrading to Platform 4.12 doesn't disrupt them. Each app's underlying Helm release keeps running exactly as before. What changes is how the platform manages that release. Platform 4.12 removes the old resources that used to represent a deployed app:
...
This removal doesn't affect the `spec.template.helmRelease` field on a cluster or cluster template. That field describes the vCluster chart's own Helm release, and is unrelated to the removed app-deployment resources.
:::

### Migrate an existing app deployment

These resources are replaced by [**AppInstance**](../../api/resources/appinstance.mdx), which deploys an [App](../../understand/what-are-apps.mdx) into a cluster, space, or connected cluster. Migrating an existing app to AppInstance is a UI action, not a CLI or API migration.

To find the migration action, open the App's cluster or cluster and click the **Apps** button. This is the same view used to [update an installed App](../../use-platform/apps/updating.mdx#update-an-installed-app-version).

The Apps table's Type column tells these apart. A **Legacy App** row shows the **Migrate** action when its original App still exists, and the App must include the version referenced by the Helm release. A **Template App** row is declared in the cluster or space configuration itself, continues to use that configuration, and doesn't need this migration.
...
Platform 4.12 ignores `snapshots.auto.storage.<provider>.credential.secretNamespace` for standalone clusters, and instead always reads the credential Secret from the Platform project namespace that contains the VirtualClusterInstance.

Before upgrading each affected standalone cluster:
...
3. Remove `secretNamespace` from the cluster configuration and any template that supplies it.
...
snapshots:
  auto:
    storage:
      ...
      s3:
        credential:
          # Platform 4.12 ignores this namespace for standalone clusters.
...
Pod-based clusters continue to resolve `secretNamespace` in their connected control plane cluster. For standalone requirements and storage limitations, see [Auto snapshots on standalone clusters](/docs/vcluster/configure/vcluster-yaml/snapshots#auto-snapshots-on-standalone-tenant-clusters).
