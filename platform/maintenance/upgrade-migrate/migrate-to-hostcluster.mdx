...
- **Cluster dependencies**  
  If the platform has cluster instances running on the same Kubernetes control plane cluster, these instances do not migrate automatically. They require a separate migration process after the platform has been moved. Migrating only the platform without considering clusters can cause service disruptions and dependency issues.

- **Cluster agent on the destination cluster**  
  If the destination Kubernetes cluster has a cluster agent, it must be decommissioned before migration. The agent can interfere with the new platform installation, potentially leading to scheduling conflicts and unexpected behavior.
...
<Step>
Optional: Stop `ArgoCD` sync of any clusters.
</Step>
...
Validate the core capabilities of the platform:
  ...
  - Log access: Verify log retrieval from multiple clusters.
  - Cluster creation: Create a test cluster.
