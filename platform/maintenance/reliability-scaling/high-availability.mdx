...
Platform HA keeps the vCluster Platform management plane available during replica or node failures. Cluster availability also depends on the cluster control plane and the Kubernetes cluster that hosts it. For more information, see [What happens during control plane outages?](../../understand/control-plane-outages.mdx).
