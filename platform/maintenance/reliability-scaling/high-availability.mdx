...
<FeatureTable names="platform-external-db" />
...
Platform HA keeps the vCluster Platform management plane available during replica or node failures. Tenant cluster availability also depends on the tenant cluster control plane and the Kubernetes cluster that hosts it. For more information, see [What happens during control plane outages?](../../understand/control-plane-outages.mdx).
...
platform starts with leader election mode. You can do this using Helm on an existing platform installation:
