...
:::info Distinct from cluster file logging
This is the platform's own process logging. vCluster charts v0.37 and later have a separate, unrelated `logging.file.enabled` option for cluster logs, configured in `vcluster.yaml` rather than the platform's Helm values.
:::
