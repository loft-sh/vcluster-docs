...
| [Workload logs](#workload-logs) | Output from applications running inside clusters | [Central HostPath Mapper](./central-hostpath-mapper.mdx) |
...
At all levels, a fixed set of built-in rules run before the level's own rules. Some requests are never logged, including internal access-review and token requests, and kubeconfig retrieval for a cluster. Sensitive `create` requests, such as access keys and cluster registration, stay at metadata level even when the configured level would otherwise capture the full payload. Long-running streaming requests, such as `pods/log` and `pods/exec`, also stay at metadata level regardless of the configured level. See [Default policy exclusions](../../configure/platform-configs/audit.mdx#default-policy-exclusions) for the full list of affected resources and why.
...
Clusters introduce a naming problem. When vCluster syncs pods to the control plane cluster, it rewrites their names. A pod named `web-6d4f9b` inside a cluster appears as `web-6d4f9b-x-default-x-my-vcluster` on the control plane cluster. Log collectors see only the physical name and cannot correlate entries back to the originating cluster workload.
...
- **Central HostPath Mapper**: A single DaemonSet on the control plane cluster handles path remapping for all clusters. Recommended for fleet deployments.
- **Per-cluster HostPath Mapper**: A DaemonSet installed in each cluster's namespace. Better suited to small environments or where per-cluster isolation is needed.

See [Central HostPath Mapper](./central-hostpath-mapper.mdx) for setup instructions for both options.
