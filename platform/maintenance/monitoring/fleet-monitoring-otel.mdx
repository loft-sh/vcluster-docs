...
When managing many tenant clusters across Control Plane Clusters, you need per-tenant
...
import time and pushed to a central Prometheus via `remote_write`.
...
Because identity labels are enriched at import time, every query can filter and
...
OTel Collectors. Both dashboards use the identity labels enriched at import
