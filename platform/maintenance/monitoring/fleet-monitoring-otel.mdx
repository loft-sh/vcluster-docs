...
sidebar_position: 4
description: "Monitor clusters across shared and private nodes using OpenTelemetry Collectors with automatic label enrichment and Prometheus remote write."
---
...
import UnmanagedMetricsLabels from './_fragments/unmanaged-metrics-labels.mdx'

<UnmanagedMetricsLabels />

When managing many clusters across <GlossaryTerm term="control-plane-cluster">control plane clusters</GlossaryTerm>, you need per-tenant
visibility into resource consumption and API health without deploying monitoring
inside each <GlossaryTerm term="tenant-cluster">cluster</GlossaryTerm>. This guide explains how to configure OpenTelemetry
Collectors to collect workload and control plane metrics from across multiple
clusters. All metrics are enriched with <GlossaryTerm term="vcluster">vCluster</GlossaryTerm> identity labels at
...
[Aggregating Metrics](./aggregating-metrics.mdx) instead. This guide covers advanced
fleet monitoring with `remote_write`, Target Allocator, and per-tenancy-model collectors.
...
- Cluster architecture:
  ...
  - Two clusters running on the local cluster:
    - One cluster sharing the nodes of the local cluster (Shared Nodes tenancy model).
    - One cluster with private nodes (Private Nodes tenancy model).
  - An external cluster connected to vCluster Platform.
  - Two clusters running on the connected cluster with the same configuration.

- Collector architecture:
  - A central Prometheus with the remote write receiver enabled.
  - One OTel Collector Deployment with Target Allocator per control plane cluster (scrapes shared-nodes clusters and their control planes using ServiceMonitors).
  - One OTel Collector DaemonSet per private-nodes cluster (scrapes local kubelet, cAdvisor, and <GlossaryTerm term="api-server">API server</GlossaryTerm> metrics from inside the cluster).
...
metrics in the batch, causing cross-contamination between clusters on the same
...
pod labels/annotations set by the vCluster <GlossaryTerm term="syncer">syncer</GlossaryTerm>
...
The private-nodes collector runs inside each cluster as a DaemonSet with one
...
Since the collector runs inside the cluster, it can't access Control
...
and `pod` to `vcluster_virtual_pod`. Inside a private-nodes cluster, the
...
- Prometheus Operator CRDs installed on the control plane cluster (`ServiceMonitor`,
  `PodMonitor`).
- Clusters deployed with a ServiceMonitor enabled. This allows scraping
...
- Clusters with dedicated/private nodes.
...
Deploy one shared-nodes collector per control plane cluster. First, register the App
  ...
  config:
    ...
    values: | # yaml
      ...
      - apiVersion: rbac.authorization.k8s.io/v1
        ...
        - kind: ServiceAccount
          name: patch-webhook-sa
          # Keep this namespace aligned with spec.defaultNamespace below.
...
- **`operator.targetallocator.mtls: true`**: Each cluster exposes its API
...
Deploy one private-nodes collector into each private-nodes cluster. All
...
spec:
            ...
            config:
              scrape_configs:
                - job_name: 'kubelet'
                  ...
                  scheme: https
                  tls_config:
                    insecure_skip_verify: {{ .Values.kubelet.insecure }}
                  authorization:
                    credentials_file: /var/run/secrets/kubernetes.io/serviceaccount/token
                  relabel_configs:
  ...
  - description: Skip TLS verification for the connection to Prometheus
    ...
    variable: prometheus.insecure
  - defaultValue: true
    description: Skip kubelet certificate verification for clusters whose serving certificates use a separate CA or lack IP SANs
    label: Skip kubelet TLS Verification
    type: boolean
    variable: kubelet.insecure
  recommendedApp:
  - virtualcluster
```
...
:::info Key configuration details
- **DaemonSet mode**: Private-nodes clusters have dedicated nodes. A DaemonSet
...
- **`external_labels` instead of `k8sattributes` for identity**: The collector
  runs inside the cluster, so it can't access control plane cluster namespace labels.
...
<Flow id="deploy-otel-private">
  ...
  <Step>
    Click the project containing your private-nodes cluster.
  </Step>
  <Step>
    Click on the cluster, then navigate to the <NavStep>Config</NavStep>
    tab.
  </Step>
  <Step>
...
| Prometheus Password | No | Basic auth password |
| Prometheus Skip TLS Verification | No | Skip TLS verification for the Prometheus connection |
| Skip kubelet TLS Verification | No | Skip kubelet certificate verification for kubelet scrapes. The default is `true`. |

The default skips kubelet certificate verification because kubelet serving certificates
commonly use a separate certificate authority or omit node IP subject alternative names.
Set **Skip kubelet TLS Verification** to `false` only when the collector can validate the
kubelet serving certificate for every node address it scrapes.

:::info
Repeat these steps for each private-nodes cluster.
:::
...
Because identity labels are enriched at collection time, every query can filter and
...
**Why:** <GlossaryTerm term="etcd">etcd</GlossaryTerm> is the persistence backend. High latencies here (especially for
...
Nodes clusters. Use this dashboard to monitor resource consumption and
...
| Section | Panels |
|---------|--------|
| Project summary | Total Projects, Total clusters, Total Pods, Total CPU (cores), Total Memory, Max Error Rate (stat panels) |
| Resource usage by project | CPU by Project (cores), Memory by Project, Pods by Project, Clusters by Project (bar charts) |
...
A detailed drill-down view for individual clusters. Use this dashboard to
investigate resource consumption and control plane health for a specific tenant
cluster.

**Dashboard JSON file:** [Download dashboard JSON](/media/dashboards/vcluster-detail.json)
