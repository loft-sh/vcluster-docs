---
title: Fleet monitoring labels reference
sidebar_label: Labels reference
sidebar_position: 6
description: Reference for vcluster_platform labels stamped by the fleet monitoring Write Gateway.
---
...
| Literal alternation regex | Allowed | `vcluster_platform_instance=~"api|worker"` |
| Negative equality | Rejected | `vcluster_platform_instance!="api"` |
| Negative regex | Rejected | `vcluster_platform_instance!~"api"` |
| Wildcard regex | Rejected | `vcluster_platform_instance=~".*"` |
...
`vcluster_project`, or `loft_virtualcluster_name`. For the fleet monitoring gateway,
