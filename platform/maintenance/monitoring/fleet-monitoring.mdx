...
from across multiple tenant clusters. Metrics can be aggregated by cluster,
project, and tenant cluster. Because the architecture uses Prometheus
...
flowchart TB
    ...
    subgraph local["Local Cluster"]
        subgraph localPrivVC["Tenant Cluster (Private)"]
            LocalPromAgent["Prometheus agent"]
        end

        LocalKPS["Kube-Prom-Stack"]

        subgraph localSharedVC["Tenant Cluster (Shared)"]
            LocalSharedWorkload["Workload"]
        end
    end

    subgraph connected["Connected Cluster"]
        subgraph connPrivVC["Tenant Cluster (Private)"]
            ConnPromAgent["Prometheus agent"]
        end

        ConnKPS["Kube-Prom-Stack"]

        subgraph connSharedVC["Tenant Cluster (Shared)"]
...
- Cluster Architecture:
  ...
  - Two tenant clusters running on the local cluster:
    ...
    - One tenant cluster with two private nodes and node-to-node VPN (Private Nodes tenancy model).
  - An external cluster that is connected to vCluster Platform (hosting vCluster Platform agent).
  - Two tenant clusters running on the connected cluster:
    - One tenant cluster sharing the nodes of the connected cluster (Shared Nodes tenancy model).
    - One tenant cluster with two private nodes and node-to-node VPN (Private Nodes tenancy model).

- Prometheus Architecture
  - A central Prometheus (remote_write receiver)
  - A Prometheus Operator (to scrape each tenant cluster's own metrics using `ServiceMonitors`) and a Prometheus Agent (remote_writer) per Cluster
  - A Prometheus Agent (remote_writer) per tenant cluster with private nodes (Private Nodes tenancy model).

## Deploy Prometheus agent and operator on each cluster
...
Tenant clusters with shared nodes must be deployed with a ServiceMonitor. This
allows scraping their API server and controller metrics from the Prometheus
agent running on the control plane cluster. Enable this in your `vcluster.yaml`:
...
<InterpolatedCodeBlock
  ...
  # We need kube-state-metrics to make use of the "vcluster.loft.sh"
  # Pod labels, which are configured using the sub chart values below.
  kubeStateMetrics:
    enabled: true
  ...
  # ---------------------------------------------------------------------------
  # Prometheus Operator to be able to scrape from ServiceMonitors
  # ---------------------------------------------------------------------------
  prometheusOperator:
    enabled: true
    resources:
...
## Deploy Prometheus agent on each tenant cluster with Private Nodes

Each tenant cluster with private nodes needs its own Prometheus instance to
scrape kubelet metrics from its dedicated nodes and forward them to the central
Prometheus using remote write.

:::info Prerequisites
Tenant clusters with private nodes must be deployed with node-to-node vCluster VPN enabled. Add the following to your `vcluster.yaml`:
...
Below steps must be repeated for each tenant cluster.
:::

<PageVariables VCLUSTER_NAME="private-vcluster-1"/>

**1. Connect to the tenant cluster:**
...
Save the following as `prometheus-virtualcluster-values.yaml` and set the name of the tenant
cluster. This is necessary to be able to aggregate any workload
running on the private nodes to their corresponding tenant cluster.
...
<InterpolatedCodeBlock
  ...
  # All other scrape_configs are provided using extraScrapeConfigs.
  # ---------------------------------------------------------------------------
  scrapeConfigs:
    prometheus:
      enabled: false
...
1. To provide a set of labels that makes it possible to aggregate by cluster, project, and tenant cluster.
...
about tenant clusters as labels. These metrics always return `1` and can
therefore be joined using PromQL to make those labels available for
aggregation later.
...
Following labels are attached:
- `kind`: Kind of the tenant cluster (StatefulSet or Deployment)
- `name`: Name of the tenant cluster
- `namespace`: Namespace of the tenant cluster
- `instance_name`: Name of the accompanying VirtualClusterInstance (only present if the tenant cluster is registered with the platform)
- `instance_namespace`: Namespace of the accompanying VirtualClusterInstance (only present if the tenant cluster is registered with the platform)
- `creation_timestamp`: Creation timestamp of the tenant cluster
...
maps the tenant cluster name to its corresponding `instance_info` entry,
copying labels like `cluster` and `project` into the result. Subsequent queries
use the same technique.

#### kube-apiserver request latency (p99, by verb) (tenant cluster only)
...
#### kube-apiserver request latency (p95, non-WATCH) (tenant cluster only)
...
#### etcd backend latency (p99, by operation) (tenant cluster only)
...
#### kube-apiserver request rate (by verb) (tenant cluster only)
...
#### kube-apiserver request rate (by resource) (tenant cluster only)
...
#### REST client outbound request rate (by code) (tenant cluster only)
...
#### Network I/O rate (by namespace) (tenant cluster only)
...
#### kube-apiserver error rate (4xx/5xx, by code) (tenant cluster only)
...
#### kube-apiserver error ratio (errors / total) (tenant cluster only)
...
#### etcd request errors (tenant cluster only)
...
#### Container OOM kills (tenant cluster only)
...
#### REST client error rate (outbound 5xx) (tenant cluster only)
...
#### Container CPU usage (top pods) (tenant cluster only)
...
#### Container memory working set (top pods) (tenant cluster only)
...
#### CPU throttling ratio (by pod) (tenant cluster only)
...
#### kube-apiserver inflight requests (tenant cluster only)
...
#### kube-apiserver flow-control queue depth (tenant cluster only)
...
#### Workqueue depth (by queue name) (tenant cluster only)
...
#### Long-running WATCH connection count (tenant cluster only)
