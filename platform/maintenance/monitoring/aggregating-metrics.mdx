...
collect workload metrics from across multiple clusters and aggregate by
Project, <GlossaryTerm term="vcluster">vCluster</GlossaryTerm>, and Space. This approach uses shared OpenTelemetry DaemonSets on the <GlossaryTerm term="control-plane-cluster">control plane cluster</GlossaryTerm> without requiring individual collector installations per <GlossaryTerm term="tenant-cluster">cluster</GlossaryTerm>.
...
<Flow id="deploy-prometheus">
  <Step>
Go to <NavStep>Infrastructure > <GlossaryTerm term="control-plane">Control Plane</GlossaryTerm> Clusters</NavStep>.
  </Step>
  <Step>
...
Deploy the built-in OpenTelemetry App on each connected cluster.
...
| `loft_virtualcluster_name` | The cluster name |
...
<img src={GrafanaExample} width="100%" alt="Grafana dashboard showing CPU and Memory usage by Project, Space, and Cluster"/>
