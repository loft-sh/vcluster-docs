...
import PlatformMachineLayer from '@site/static/media/platform/diagrams/platform-machine-layer.svg';
...
- **Management API.** Serves Platform resources: projects, clusters, VirtualClusterInstances, templates, users, access keys, and the node providers, node types, and Machines that describe physical and virtual capacity.
...
A connected cluster is a Kubernetes cluster registered with Platform. Tenant cluster control planes and project workloads run there, as do the provisioning components a node provider deploys. Connecting a cluster installs the Platform agent, which maintains an outbound connection to Platform and keeps cluster-side state in sync.
...
Platform manages the physical and virtual machines underneath the fleet through vMetal. vMetal isn't a separate service. It's the common provisioning model Platform exposes across a portfolio of drivers, and it ships and versions with Platform.

- **Node provider.** Targets one connected cluster and deploys the driver components that run there, such as Metal3, Ironic, and a DHCP server for bare metal. It groups capacity into node types.
- **Node type.** Selects machines by label and sets their provisioning defaults, such as OS image and SSH keys.
- **Machine.** One claim on one provisioned server, either physical or virtual. The `NodeClaim` resource represents the request and tracks its lifecycle.

A provisioned server becomes one of three things, decided by the request rather than by separate machinery. It joins a tenant cluster as a worker, carries that cluster's own control plane, or stays a server you reach over SSH with no Kubernetes on it.

That third case is why the boundaries above matter. A tenant cluster control plane normally runs as a workload on a connected cluster, but a standalone control plane runs directly on a provisioned machine instead. Platform still needs the connected cluster that runs the provisioning components, just not one to host that control plane.

<figure>
  <PlatformMachineLayer style={{width: '100%', height: 'auto'}} role="img" aria-label="Platform defines node providers, node types, and Machines. A connected cluster runs the provisioning components a node provider deploys and hosts tenant cluster control planes by default. Each provisioned machine becomes a worker that joins a tenant cluster, a control plane that runs on the machine itself rather than in the connected cluster, or an independent server that joins no cluster and is reached over SSH." />
  <figcaption>A provisioned machine becomes a worker, a control plane that runs on the machine itself, or an independent server that joins no cluster</figcaption>
</figure>

[Dive deeper into vMetal →](../vmetal/overview.mdx)
