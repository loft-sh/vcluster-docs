...
Tenant cluster templates configure the Kubernetes environment itself. [Apps](../understand/what-are-apps.mdx) install individual applications into that environment. When an environment needs several Apps or Argo CD Applications with dependency ordering, health gates, or output handoff, use a [Stack](../understand/what-are-stacks.mdx) to manage them as one unit.
...
Node providers integrate with cloud APIs (AWS, GCP, Azure, and others) to provision VMs that join as private nodes. For bare metal infrastructure, [vMetal](/docs/platform/next/vmetal/drivers/bare-metal/metal3/server-lifecycle) handles physical server provisioning using the same model.
...
[Configure node providers →](../vmetal/concepts/machine-types-and-nodetypes.mdx)
