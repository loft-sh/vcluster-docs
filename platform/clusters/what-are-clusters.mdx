...
A cluster is a Kubernetes cluster that can be connected to and managed from the vCluster Platform UI. This can be a local Kubernetes cluster where vCluster Platform is installed in or an external cluster which is deployed on a cloud provider for instance.
