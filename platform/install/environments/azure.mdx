...
accept the warning in your browser. For production use, replace the default self-signed certificate with a valid [TLS certificate](/platform/configure/installation-options/domain#configure-tls).
:::
...
:::info
For more information on how to configure a custom domain, see the [Configure external access and TLS documentation](/platform/configure/installation-options/domain).
:::
...
You can also use Google as an identity provider and [configure SSO](../../configure/platform-configs/single-sign-on#providers) to enable user authentication to the platform.
