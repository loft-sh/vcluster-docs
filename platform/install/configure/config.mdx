...
If you decide to configure vCluster Platform helm release with  `additionalCA`, it will be automatically used by all vClusters managed by the vCluster Platform to establish a connection between vClusters and the Platform. You don't need to tweak individual vCluster configuration to enable this.
This will enable secure communication between your vClusters and the vCluster Platform.
```yaml
additionalCA: "(base64 encoded CA)"
```

Also, if you need to import externally managed vCluster to the vCluster Platform, you can also enable a secure connection by grabbing the value from the  existing vCluster Platform helm release, you can do this by running:

```shell
helm get values --namespace [namespace where vCluster Platform is deployed] [loft helm release name] --all | grep additionalCA
```

Then, using you can import vCluster using vCluster CLI command:
```shell
  vcluster platform add vcluster [your vCluster name] \
--namespace=[namespace where vCluster runs] \
--project=[project name] \
--ca-data [base64 string grabbed from the command above]
```

Once the command succeeds, imported vCluster should restart and report Ready phase.
