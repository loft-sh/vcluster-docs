...
A common use case is configuring network access between the vCluster platform and managed vClusters to allow API communication.
If both the platform and vCluster run on the same host cluster, you should also configure the `NO_PROXY` (or `no_proxy`) environment variable to bypass proxies for internal traffic if you are using proxies.
