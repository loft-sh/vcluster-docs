...
The most basic GitOps platform deployment consists of a ArgoCD `Application` with the platform chart and your desired values.
...
The preceding ArgoCD `Application` would deploy the platform into the
current-context of configured for your Kubernetes cluster, in the `vcluster-platform` namespace. In this example, values are being passed to set some basic configurations including:
...
One of the great benefits of the platform is that you can easily manage resources located in many physical clusters by adding them to the platform and using it as your central point of management. When you add a "connected" cluster to the platform, a `Cluster` resource is created and a platform Agent is installed in the cluster to handle local reconciliation tasks. If you are managing the platform via GitOps, you may also wish to manage these connected clusters in a similar fashion, rather than letting the platform install and manage the Agent.
...
Each cluster requires two resources: a `Cluster` object that simply defines the cluster name and the owner of the cluster inside the platform, and an associated `Secret` that contains relevant configuration information for the platform to connect to the cluster.
...
You may have noticed a strange annotation on the `Cluster` resource preceding --
`argocd.argoproj.io/sync-options: SkipDryRunOnMissingResource=true`. This
annotation tells ArgoCD to skip the dry run/check of the resource. You need this
...
Here is a basic example, but you'll need to update `$PLATFORM_VERSION` with a valid platform version.
<!-- vale off -->
