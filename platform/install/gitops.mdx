...
If you are already using GitOps practices with your applications deployed into your Kubernetes cluster(s), then
...
dependency name, in this case, "vcluster-control-plane". For example, if you wanted to set the `replicaCount`
...
manager, cluster(s). After adding the relevant cluster(s) as available
...
[API documentation](/platform/api/resources/project/project.mdx) where you can find example manifests and argument references for nearly all vCluster Platform
