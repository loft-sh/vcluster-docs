...
1. **Api Server**: vCluster Platform starts an internal Kubernetes api server that handles most of the functional requests vCluster Platform receives.
   Since vCluster Platform saves everything in Kubernetes resources rather than storing any data in volumes, it can easily be scaled horizontally.
...
For reasons of simplicity and performance, both components are compiled into the same binary in vCluster Platform. Scaling of vCluster Platform works a little bit differently than with Kubernetes components
