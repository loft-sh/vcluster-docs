...
After populating the registry with the artifacts, you'll install vCluster Platform, connect host clusters and then deploy virtual clusters. Each step requires
additional configuration to use your private registry.

## Populate images to private registry

<!-- vale off -->
Each [vCluster Platform GitHub release](https://github.com/loft-sh/loft/releases) and [vCluster GitHub release](https://github.com/loft-sh/vcluster/releases) includes multiple release assets to help you upload images to a private registry. These assets include text files listing all required images for the release, along with two scripts to automate pulling and pushing those images.

### Image text files

<!-- vale off -->
In each [vCluster Platform release](https://github.com/loft-sh/loft/releases), a file lists the container images required to install vCluster Platform:
...
The difference between the scripts in the different releases is the default name of the tarball that is created. These instructions override the defaults to ensure
...
Installing vCluster Platform and connecting hosts to the platform requires you to configure a `vcluster-platform.yaml`.
...
After vCluster Platform is installed, you'll want to connect host clusters to the platform. These host clusters need to be able to
