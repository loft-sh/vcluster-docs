...
- On the installation computer:
    ...
    - A `kube-context` with admin access to the Kubernetes cluster (verify with `kubectl auth can-i create clusterrole -A`)
      ...
      :::
    - <Mark color="lightgray">[Optional]</Mark> An offline license key for the platform (provided by Loft)
- <Mark color="lightgray">[Optional]</Mark> A private Docker registry accessible to both the installer computer and the air-gapped Kubernetes cluster
