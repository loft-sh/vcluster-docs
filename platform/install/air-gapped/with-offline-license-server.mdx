...
The license server stores state in PostgreSQL (embedded or external) and exposes a `/license/v2` API endpoint that vCluster Platform calls for license checks. The platform contacts it on the same schedule it would use for the hosted license server. For that schedule and the surrounding caching behavior, see [How licensing works](../../understand/licensing.mdx).
...
:::warning insecureSkipVerify disables verification everywhere
This value turns off TLS verification for the platform and all its managed components, not just the license server connection. Prefer trusting the signing CA instead, by supplying it as a base64-encoded value in the `additionalCA` Helm value. See [Configure a custom domain](../../configure/installation-options/domain.mdx) for how to produce that value. Setting `insecureSkipVerify: true` causes `additionalCA` to be ignored, so use one or the other.
:::
