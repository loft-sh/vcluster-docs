...
The platform database does not store core platform state. Cluster definitions, project settings, users, and all other platform objects are stored as Kubernetes custom resources on the control plane cluster. This database exists solely for usage reporting.
...
| Metric | Description |
|---|---|
| Cluster count | Total number of clusters across the platform |
...
Use the [cluster resource-usage API](../../api/resources/virtualclusterinstance/resourceusage.mdx) to inspect the live per-vendor breakdown. Read the `gpus` field for cross-vendor data. The existing `capacity` field only includes `nvidia.com/gpu` for GPU capacity.
