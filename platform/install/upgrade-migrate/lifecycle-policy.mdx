...
Due to the rapid development of the platform, each minor version receives active support for 3 months after its release. This period is referred to as End of Support (EOS). After EOS, the version enters the maintenance phase, during which support is limited. During active support, updates from the active development branch can be backported upon request. During End of Life (EOL), only critical security updates are backported.
...
\* Due to the number of breaking changes from vCluster v0.19 to v0.20 and vCluster Platform v3.4 to v4.0, v3.4 has an extended End of Support end date. Review the [migration guide](../../reference/migrations/4-0-migration.mdx) to upgrade your platfrom from v3.4 to v4.0.
