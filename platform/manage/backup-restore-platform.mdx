...
Make sure that you are not installing vcluster Platform (and vCluster Platform Agents) into cluster(s) with vCluster Platform already
running! Multiple vCluster Platform instances running on the same cluster(s) can lead to conflicts in
