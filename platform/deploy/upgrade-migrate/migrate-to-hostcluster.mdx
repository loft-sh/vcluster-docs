...
:::warning
The platform must run on a dedicated Kubernetes cluster. It must have its own external access (either through an ingress controller or a load balancer) and DNS record before migration. Ensure these conditions are met before proceeding.
:::
