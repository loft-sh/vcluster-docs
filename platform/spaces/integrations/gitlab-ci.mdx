...
e2e:
  ...
  stage: test
  before_script:
    - vcluster login $LOFT_URL --access-key $LOFT_ACCESS_KEY
    - vcluster platform create space "${CI_PROJECT_NAME}-${CI_COMMIT_SHORT_SHA}-${CI_PIPELINE_ID}" --use
    - apk add go make
  script:
    ...
    - make e2e
  after_script:
    - vcluster platform delete space "${CI_PROJECT_NAME}-${CI_COMMIT_SHORT_SHA}-${CI_PIPELINE_ID}"
