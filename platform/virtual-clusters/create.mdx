...
Each virtual cluster that is created in vCluster Platform belongs to a Project. There are two primary ways which
virtual clusters can be created inside vCluster Platform: from a template, and manually.

Virtual clusters created from a [template](../virtual-clusters/templates.mdx) inherit all template
...
Project owners and vCluster Platform admins can create virtual clusters manually, that is without a template.

## Create From Template

Project members can create a new virtual cluster through the vCluster Platform UI or CLI:
...
Regardless of how you create a virtual cluster from a template, either by the UI or the CLI, vCluster Platform
