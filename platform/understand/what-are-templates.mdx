...
Templates define a baseline configuration applied to every cluster or space created from them. They are how Platform administrators enforce consistency across a fleet without requiring each user to configure resources manually.
...
- A base `vcluster.yaml` configuration defining the cluster's control plane, worker node type, backing store, and sync settings
- Resource quotas, limit ranges, and network policies for namespace-level isolation
- Kubernetes objects deployed into the cluster at creation time, such as tooling, pre-populated test data, or CRDs
- [Apps](what-are-apps.mdx) — packaged applications deployed into the cluster when it is created
- [Stacks](what-are-stacks.mdx) — dependency-aware groups of Apps and Argo CD Applications
