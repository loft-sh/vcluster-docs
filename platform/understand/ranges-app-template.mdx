---
title: What are numeric input ranges in app templates
sidebar_label: Numeric input ranges
description: Learn how to set minimum and maximum values for numeric parameters in app templates
---

import Flow, { Step } from '@site/src/components/Flow';

# What are numeric input ranges in app templates?

## What are template parameters? {#what-are-template-parameters}

App templates use parameters to make them customizable. When users deploy resources based on these templates, they provide values for these parameters:
...
## Available parameter constraints {#available-parameter-constraints}

Templates support various parameter types and constraints:
...
## Set numeric ranges in the UI {#set-numeric-ranges-in-the-ui}
...
Access the template creation interface in the platform.
</Step>
...
The platform actively validates user input against your defined range. If a value is outside this range, the user cannot proceed with creating the resource.

## Define ranges in yaml {#define-ranges-in-yaml}
...
## Use cases for numeric ranges {#use-cases-for-numeric-ranges}

Setting numeric ranges is valuable for:

- **Resource allocation**: Ensuring clusters receive sufficient but not excessive resources.
...
For more information on other parameter types, including selection from predefined options, see the [template parameters documentation](../administer/templates/parameters.mdx).