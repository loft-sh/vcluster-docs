...
Platform tracks each connected cluster's agent tunnel as a `NetworkPeer` object, and reflects overall connection health on the `Cluster` resource's `status.phase`: empty while the agent hasn't connected yet, `Initialized` once it's connected and healthy, or `Failed` if reconciliation hit an error. `status.reason` and `status.message` carry a short code and the underlying error for whichever step failed.

The platform UI's Host Clusters view also shows an `Online` state. That state reflects the `NetworkPeer` connection, so it's only meaningful for clusters connected through the agent path (`spec.networkPeer: true`). It doesn't apply to a cluster registered with a direct kubeconfig secret (`spec.config`) or the cluster Platform itself runs on (`spec.local: true`).
