...
This is why teams are useful beyond the UI. Platform permissions, project membership, admission policies, and cluster access can all reason about users and teams as Kubernetes-style subjects and groups.
...
On a platform with Tenants, a user belongs to exactly one Tenant and stays there. That binding is what scopes everything the user can see, and it can't be changed after the user is created.

Team membership can't cross that line. A user bound to a Tenant can't join a platform-level team, and a platform user can't join a Tenant's team. Either write is refused, because a team carries permissions and the combination would give a tenant user operator-level rights inside their own scope.

Users on a platform with no Tenants are unaffected, and belong to the platform.

[Understand Tenants →](./what-are-tenants.mdx)
...
Many platform resources can be owned by either a user or a team. Platform records that ownership through owner fields on the resource and through ownership labels applied to resources such as namespaces, space instances, cluster instances, and access keys.
