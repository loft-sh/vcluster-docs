...
Stacks don't have a separate license entitlement. App tasks use the [Apps](./what-are-apps.mdx) feature, which is available on every plan. Argo CD Application tasks also require the [Argo CD Integration](../integrations/argocd/overview.mdx) feature, which isn't available on the free plan.

Certified Stacks use the same resources and controller. vCluster Platform bundles certified StackTemplates and their supporting resources for tested integrations such as NVIDIA Run:ai.
