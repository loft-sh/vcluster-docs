...
Stacks let vCluster Platform deploy several applications as one dependency-aware unit. Use them when an environment needs more than a collection of independently installed Apps.
...
Stacks don't have a separate license feature. App tasks use the [Apps](./what-are-apps.mdx) feature, which is available on every plan. Argo CD Application tasks also require the [Argo CD Integration](../integrations/argocd/overview.mdx) feature, which isn't available on the free plan.
