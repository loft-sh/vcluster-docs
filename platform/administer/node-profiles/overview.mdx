...
NodeProfiles apply to private nodes only. They can't be used by clusters whose control plane runs on a shared or standalone <GlossaryTerm term="control-plane-cluster">control plane cluster</GlossaryTerm>, or by cloud-hosted clusters.
:::
...
Each <GlossaryTerm term="tenant-cluster">cluster</GlossaryTerm> runs a reconciler that watches its assigned private worker nodes and keeps them aligned with the NodeProfile they reference. The reconciler only manages the keys it owns:
...
Convergence is prompt, not tied to a fixed interval. The reconciler watches for profile changes and re-converges affected nodes as soon as they occur. If the cluster's connection to the platform is temporarily unavailable, convergence resumes once the connection is restored.
...
2. The cluster's `privateNodes.defaultProfile`.
...
A profile resolved through the default-profile fallback chain, rather than an explicit selection, isn't promoted this way. Defaults stay dynamic, so changing or clearing the cluster's or project's default re-converges the affected nodes without requiring a new join.

## How NodeProfile relates to other node objects

[Node providers](../node-providers/overview.mdx) and [node types](../node-providers/overview.mdx#node-types) describe how a node is provisioned, which infrastructure it comes from and what hardware it has. NodeProfile describes what the node looks like once it's part of the cluster, independent of where it came from.
