...
<Step>
If the original cluster is going to be connected to the platform as a new connected cluster:

Update the `VirtualClusterInstances` and `SpaceInstances` to reference the new cluster name using kubectl:

```bash title="Update cluster references in VirtualClusterInstances"
kubectl get virtualclusterinstances.management.loft.sh -A

kubectl patch virtualclusterinstances.management.loft.sh [instance-name] -n [namespace] --type=merge -p '{"spec":{"clusterRef":{"name":"[new-cluster-name]", "namespace":"loft-clusters"}}}'
```

```bash title="Update cluster references in SpaceInstances"
kubectl get spaceinstances.management.loft.sh -A

kubectl patch spaceinstances.management.loft.sh [instance-name] -n [namespace] --type=merge -p '{"spec":{"clusterRef":{"name":"[new-cluster-name]", "namespace":"loft-clusters"}}}'
```

:::note
Update these references before restoring the backup to the new cluster. Otherwise, when you connect the original cluster to the new platform, the virtual clusters and spaces might reference a non-existent cluster.
:::
</Step>
