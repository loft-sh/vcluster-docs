...
<FeatureTable names="connected-clusters" />



vCluster Platform manages clusters that run across one or more connected clusters.
A connected cluster is a Kubernetes cluster that serves as a control plane cluster, where cluster control planes run. To enable management through vCluster Platform,
...
Direct access to a control plane cluster grants broad permissions to platform resources and clusters running on it. Grant this access only to privileged platform administrators. Unprivileged users should access clusters directly rather than through the control plane cluster or its pods.
:::

See the [Platform CLI hub](../../reference/platform-cli.mdx) for UI-to-CLI equivalents across common Platform tasks, including connecting clusters.
...
:::info
If the cluster doesn't finish connecting, goes offline after connecting, or gets stuck mid-upgrade, see [Troubleshoot connected cluster connection failures](../../troubleshoot/connected-cluster-connectivity.mdx).
:::
