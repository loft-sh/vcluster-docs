...
When provisioning infrastructure with Terraform, you can automate the complete cluster registration workflow. This approach registers the cluster in vCluster Platform and installs the agent, making the cluster immediately available for cluster deployment.
...
The automation requires an admin-level access key to create cluster-specific access keys through the API. You can create an access key through the [vCluster Platform UI](../../authentication/access-keys.mdx#create-an-access-key) or CLI.
...
2. **Retrieve cluster-specific access key**: Get the unique key for this cluster using the API
...
- Retrieve the cluster-specific access key using the API
- Install the Platform agent using Helm
...
Navigate to <NavStep>Infra Management > Control Plane Clusters</NavStep> in the Platform interface. Your cluster should appear with "Connected" status within moments.
...
**Use scoped keys when possible** - While this workflow requires an admin key for the initial setup, consider using [scoped access keys](../../authentication/access-keys.mdx#scoped-access-keys) for other automation tasks.
...
- [Create clusters](../../../use-platform/virtual-clusters/add-virtual-clusters.mdx) on the registered control plane cluster
