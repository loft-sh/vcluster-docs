...
| --- | --- | --- |
| APIService location | Aggregated on the control plane EKS cluster | Local, inside each region's embedded API server |
| Authentication | Control plane cluster bearer token (for example, an EKS IAM token) | [Platform access key](../../../authentication/access-keys.mdx) |
| APIService visibility on control plane EKS | Shows the registration | Shows nothing |
...
scope it to the project, user, or cluster the caller needs.
...
the control plane EKS endpoint stops working after the upgrade. There's no in-place
compatibility shim.

Update callers to use the platform HTTPS endpoint. Switch authentication
from your control plane cluster bearer token (for example, an EKS IAM token) to a
platform access key.

Standard (non-multi-region) platform deployments are unaffected. The control plane
EKS APIService keeps working as before.

### Troubleshoot common errors

| Symptom | Cause | Resolution |
| --- | --- | --- |
| `404` from the control plane EKS endpoint | Caller is using the control plane EKS endpoint on a multi-region platform | Switch to the platform HTTPS endpoint shown in [Call the management API](#call-the-management-api) |
| The control plane EKS cluster has no `v1.management.loft.sh` APIService | Expected on multi-region, as the APIService lives inside the embedded API server | None; verify from inside the embedded API server if needed |
