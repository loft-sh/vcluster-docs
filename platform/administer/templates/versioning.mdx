...
When creating a virtual cluster from a template—whether through the UI or CLI—you can specify a **template version string**. If omitted, the latest available version is selected by default.
...
To always upgrade to the latest available version, use `X.X.X` as the version string.

This allows you to keep virtual clusters aligned with the latest template versions without manual intervention.
