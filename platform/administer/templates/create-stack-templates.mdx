...
<Flow id="deploy-starter-stack">
  ...
  <Step>
    Save the following resources as `starter-stack.yaml`. Set the project namespace and tenant cluster name for your destination, and set `spec.owner` to a user or team that can reach it. App tasks deploy as their owner, so a Stack with an App task needs one.
