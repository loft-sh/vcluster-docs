...
Published outputs are available through the StackInstance outputs subresource. They aren't written to `status`. See the [StackInstanceOutputs reference](../../api/resources/stackinstanceoutputs.mdx) for the response schema. Access to sensitive output values requires separate `get` permission on `stackinstances/outputs`. See [Stack permissions](../users-permissions/permissions/stacks.mdx).
