...
Give each task a unique DNS-label-safe `name`. A Stack supports up to 20 tasks. Each task can set its own `timeout`. A task without one uses the Stack's `defaults.taskTimeout`, or 10 minutes if that isn't set either. Set exactly one task type:
...
:::caution
vCluster Platform doesn't currently reject a duplicate task name. A repeated name silently replaces the earlier task's child application, and the StackInstance can still report `Healthy`.

For an `app` task, also keep the combined StackInstance and task name short. The generated child name only enforces the 63-character Kubernetes object-name limit, but Helm's own release-name limit is 53 characters. A longer combination fails to deploy with a `release name is invalid` error that doesn't mention length.
:::
...
Published outputs are available through the StackInstance outputs subresource. They aren't written to `status`. Access to sensitive output values requires separate `get` permission on `stackinstances/outputs`. See [Stack permissions](../users-permissions/permissions/stacks.mdx).
