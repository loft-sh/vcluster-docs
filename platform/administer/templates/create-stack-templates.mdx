...
import StarterStack from "!!raw-loader!./_code/starter-stack.yaml";
...
A `StackTemplate` is a reusable, cluster-scoped blueprint. A `StackInstance` applies that blueprint to one cluster or control plane cluster. Each task creates either an AppInstance or an ArgoCDApplication.
...
Both features are available on every plan, including the free plan. If a task's feature is ever removed from the active plan, the StackInstance remains blocked with the `FeatureNotAllowed` reason. Learn more about [Apps](../../understand/what-are-apps.mdx) and the [Argo CD integration](../../integrations/argocd/overview.mdx).
...
<Flow id="deploy-starter-stack">
  ...
  <Step>
    Save the following resources as `starter-stack.yaml`. Set the project namespace and cluster name for your destination, and set `spec.owner` to a user or team that can reach it. App tasks deploy as their owner, so a Stack with an App task needs one.

    <InterpolatedCodeBlock
      code={StarterStack}
      language="yaml"
      title="starter-stack.yaml"
    />
  </Step>
  <Step>
    Create the StackTemplate and StackInstance:
  ...
  <Step>
    Wait for both tasks to report `Healthy`:
...
A task can capture one scalar value from a Secret or namespaced Kubernetes resource. A later task reads the value with `{{ .Outputs.<task>.<output> }}` and must list the source task in `dependsOn`. See [Template values and outputs in Stacks](../../how-to/use-go-template.mdx#template-values-and-outputs-in-stacks) for more Go template examples.
...
Published outputs are available through the StackInstance outputs subresource. They aren't written to `status`. See the [StackInstanceOutputs reference](../../api/resources/stackinstanceoutputs.mdx) for the response schema. Access to sensitive values requires separate permission to retrieve StackInstance outputs. See [Stack permissions](../users-permissions/permissions/stacks.mdx).

## Test a stack template

Create a StackInstance that references the template. A destination and, once set, an owner are both immutable after creation. A cluster destination must be in the same project as the StackInstance. There's no cross-project form of this field, so deploying the same Stack into clusters across several projects takes one StackInstance per project, all referencing the same template.
...
- [Manage Stacks declaratively](../../how-to/gitops-end-to-end.mdx#manage-stacks-declaratively) to store StackTemplates and StackInstances in Git.
- [Stack permissions](../users-permissions/permissions/stacks.mdx) to configure RBAC for Stack resources.
- [StackTemplate reference](../../api/resources/stacktemplate.mdx) for the full field schema.
