...
Template parameters enable customization of tenant cluster templates. The same mechanism applies to SpaceTemplate, App, and ArgoCDApplicationTemplate resources. An ArgoCDApplication supplies values to an ArgoCDApplicationTemplate's parameters. It doesn't define a parameter schema of its own. Platform administrators define a set of parameters that expose specific fields to users during tenant cluster creation. These parameters allow customization of values like replica counts, image tags, or ingress domains without granting access to the full configuration.
...
`ArgoCDApplicationTemplate.spec.parameters` defines the parameter schema (types, validation, defaults) the same way tenant cluster templates do. `ArgoCDApplication.spec.parameters` supplies values against that schema. It doesn't define parameters itself.

Both `ArgoCDApplicationTemplate` and inline `ArgoCDApplication` specs can interpolate the built-in `loft` platform-specific values described above, whether or not a parameter schema is involved. When an `ArgoCDApplication` targets a control plane cluster (`destination.cluster`) rather than a tenant cluster, `loft.project` and `loft.virtualClusterName` are empty strings. That destination isn't scoped to a project or tenant cluster.
