...
Template parameters enable customization of cluster templates. The same mechanism applies to SpaceTemplate, App, and ArgoCDApplicationTemplate resources. An ArgoCDApplication supplies values to an ArgoCDApplicationTemplate's parameters. It doesn't define a parameter schema of its own. Platform administrators define a set of parameters that expose specific fields to users during cluster creation. These parameters allow customization of values like replica counts, image tags, or ingress domains without granting access to the full configuration.

When a user creates a cluster from a parameterized template, the platform presents a form that collects values for each parameter. These values are injected into the rendered configuration using Go templating syntax (`{{ .Values.parameterName }}`).
...
You can apply validation rules to ensure that users provide valid, safe, and expected input when filling out template parameters. These rules improve consistency and reduce configuration errors during cluster creation.
...
parameters:
  - variable: environment_type
    ...
    description: Select the target environment for this cluster
    type: string
    required: true
    options:
      - development
...
- variable: cluster_name
  label: Cluster Name
  description: Enter a name for your cluster (3-20 characters, lowercase letters and hyphens only)
  type: string
  required: true
...
- variable: custom_config
  ...
  description: Additional YAML configuration to apply to the cluster
  type: multiline
...
- variable: instance_size
  label: Instance Size
  description: Choose the resource allocation tier for your cluster
  type: string
  required: true
  options:
    - value: small
...
| VirtualCluster          | `virtualClusterName`      | Name of the cluster (if applicable)  |
| Cluster                 | `cluster`                 | Name of the physical Kubernetes cluster      |
| VirtualClusterNamespace | `virtualClusterNamespace` | Namespace of the cluster             |
...
### Argo CD application parameter values

`ArgoCDApplicationTemplate.spec.parameters` defines the parameter schema (types, validation, defaults) the same way cluster templates do. `ArgoCDApplication.spec.parameters` supplies values against that schema. It doesn't define parameters itself.

Both `ArgoCDApplicationTemplate` and inline `ArgoCDApplication` specs can interpolate the built-in `loft` platform-specific values described above. This works whether or not a parameter schema is involved. When an `ArgoCDApplication` targets a control plane cluster (`destination.cluster`) rather than a cluster, `loft.project` and `loft.virtualClusterName` are empty strings. That destination isn't scoped to a project or cluster.

### Connected control plane cluster parameter values

In addition to Platform-specific parameter values, annotations on the Platform connected control plane cluster `Cluster` resource are available under the `loft.clusterAnnotations` key for the `clusterRef.cluster` selected to deploy the cluster instance.
