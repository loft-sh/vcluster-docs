...
Templates standardize cluster, namespace, App, and Stack configurations. Create templates through the vCluster Platform UI or by applying manifests directly to your cluster with the management API.
...
<Tabs>
  <TabItem value="virtual-cluster-template" label="Cluster template" default>

<Flow id="create-vcluster-template-ui">
  <Step>
   Go to <NavStep>Tenant Management > Cluster Templates</NavStep> and, if shown, click the <Label>Clusters</Label> tab.
  </Step>
  <Step>Click <Button>Create Cluster Template</Button>.</Step>
  <Step>
    Provide your cluster template with a name by replacing the `my-template` placeholder name, or by
...
Apps are Helm charts, Kubernetes manifests, or bash scripts that define an application to be deployed into a cluster, namespace, or cluster.

App templates can be created through the UI.

<Flow id="create-app">
  <Step>
    Go to <NavStep>Tenant Management > Stacks & Apps</NavStep>, or <NavStep>Tenant Management > Apps</NavStep> if that's what your sidebar shows. If a tab bar appears, click the <Label>Helm Apps</Label> tab.
  </Step>
  <Step>
  ...
  <Step>
    On the <Label>Definition</Label> tab, select the type of app (`kubectl`, Helm, or bash), and provide the necessary details. These vary depending on your application type and environment.
  </Step>
  <Step>
...
<TabItem value="stack-template" label="Stack template">

A Stack template defines a dependency-ordered group of Apps and Argo CD Applications.

<Flow id="create-stack-template">
  <Step>
    Go to <NavStep>Tenant Management > Stacks & Apps</NavStep>, and select the <Label>Stacks</Label> tab.
  </Step>
  <Step>
    Click <Button>Create Stack Template</Button>.
  </Step>
  <Step>
    Add tasks, dependencies, parameters, and published outputs.
  </Step>
  <Step>
    Click <Button>Create Stack Template</Button>.
  </Step>
...
See [Create a Stack template](./create-stack-templates.mdx) for the task schema, outputs, and a complete example.
...
<Flow id="create-namespace-template-ui">
  <Step>
    Go to <NavStep>Tenant Management > Cluster Templates</NavStep> and, if shown, click the <Label>Namespaces</Label> tab.
  </Step>
  <Step>
...
Templates include configuration sections that define behavior, content, and access. The following options apply to both cluster templates and namespace templates.
...
The **Apps** section lets you select pre-built applications that install automatically when clusters are created from the template. Choose from available apps such as ArgoCD, Prometheus Stack, Ingress controllers, or custom applications specific to your organization.

Selected apps deploy immediately after cluster creation. Development teams have required tools and services available without manual installation. This approach standardizes the application stack across clusters. It also reduces setup time for development teams.

The **Objects** section defines Kubernetes manifests that are applied during cluster creation. These include ConfigMaps, Secrets, NetworkPolicies, or custom ClusterRoles that establish baseline configurations. For an example of injecting a custom ClusterRole to control in-cluster permissions, see [Custom ClusterRoles](../users-permissions/permissions/vcluster.mdx#custom-clusterroles).


### Management access

Management access defines who can view, edit, or delete the template. It applies to the template itself, not to the clusters created from it.
...
This does not affect any clusters or resources that were already created from the template.

:::warning Delete a template

If you delete a template, any cluster, app, or namespace that references it can no longer be edited or upgraded through the Platform UI’s form-based editor.
...
<Flow>
  <Step>
    Go to <NavStep>Tenant Management > Cluster Templates</NavStep>, or <NavStep>Tenant Management > Stacks & Apps</NavStep>, or <NavStep>Tenant Management > Apps</NavStep> if that's what your sidebar shows. If a tab bar is shown, click the tab for the template type (<Label>Clusters</Label>, <Label>Namespaces</Label>, <Label>ArgoCD Apps</Label>, or <Label>Helm Apps</Label>).
  </Step>
  <Step>
...
The vCluster sync configuration determines which resources are shared between the cluster and control plane cluster. Unrestricted sync settings can allow users to:
  ...
  values: |
    sync:
      toHost:
        ...
        pods:
          enabled: true
        # Disable syncing resources that could affect the control plane cluster
        persistentVolumes:
          enabled: false
        priorityClasses:
          enabled: false
      fromHost:
        ...
        # This prevents access to control plane cluster secrets
```

### Policy enforcement

Use the `policies` section to enforce security standards within the cluster:
...
<details>
<summary>Cluster template</summary>

The following template creates a cluster with ResourceQuota and LimitRange objects to control resource usage. It configures persistent storage, includes RBAC rules for operational tasks, and enables ingress support for apps.
...
The `loft.sh/import-argocd: 'true'` label uses the legacy project-level Argo CD integration. For new setups, configure Argo CD registration through the connector-based integration instead. See [Connect to Argo CD](../../integrations/argocd/connect-argocd.mdx).
...
<details>
<summary>Argo CD cluster template</summary>
...
```yaml title="Argo CD cluster template"
apiVersion: management.loft.sh/v1
kind: VirtualClusterTemplate
metadata:
...
<details>
<summary>Parameterized cluster template</summary>
...
```yaml title="Parameterized cluster template"
apiVersion: management.loft.sh/v1
kind: VirtualClusterTemplate
metadata:
...
spec:
  ...
  description: This template deploys a sleepy cluster
  template:
    metadata: {}
    instanceTemplate:
      metadata: {}
...
<details>
<summary>Versioned cluster template</summary>
...
```yaml title="Versioned cluster template"
kind: VirtualClusterTemplate
apiVersion: management.loft.sh/v1
metadata:
  name: default-template
spec:
  displayName: Default Cluster Template
  description: This cluster template deploys a vCluster with embedded etcd and integrated CoreDNS.
  owner:
    team: loft-admins
  template:
    metadata: {}
