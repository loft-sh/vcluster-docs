...
Before applying a template, complete the following:
...
After connection, you can apply a template using `kubectl`, Helm, or ArgoCD.
...
Virtual cluster templates include configuration sections that control template behavior, content, and access.


### Template parameters

Template parameters make templates dynamic by exposing configurable fields that users can customize during virtual cluster deployment. When creating parameters, you define the variable name, data type, user-friendly label, and description to guide users.

Parameters can include validation rules such as allowed options, required fields, and default values. For example, you might create parameters for admin passwords, environment types, resource limits, or feature toggles. Users see these parameters as form fields when deploying virtual clusters. They can customize deployments without modifying the underlying template.
...
Management access controls who can view, edit, or delete the virtual cluster template itself. This section manages template permissions, not permissions for virtual clusters created from the template.

You can set an owner with full permissions and add additional users or teams with specific permission levels such as view-only or full access. Management access ensures that only authorized platform administrators can modify templates. Teams can still deploy virtual clusters from approved templates.

The permission model supports role-based access controls. This prevents unauthorized template modifications that could affect standardization or security policies.
...
Deleting a virtual cluster template removes it from the platform and prevents it from being used for future cluster creation. 
This does not affect any virtual clusters that were already created from the template. You can delete a template using either the user interface or the command line.
...
<Flow>
  ...
  <Step>
    Select **Virtual Clusters** from the templates menu.
  </Step>
  <Step>
    Find the template you want to delete.
  </Step>
  <Step>
    Click the delete button or options menu for the template.
  </Step>
  <Step>
...
Delete the template using kubectl:

```bash
kubectl delete virtualclustertemplate <template-name>
```
...
:::warning 
Virtual clusters that reference a deleted template cannot be edited or upgraded through the Platform UI.

To update a virtual cluster that references a deleted template, you must edit the virtual cluster's custom resource directly. This requires editing the associated CustomResourceDefinition (CRD).
:::
