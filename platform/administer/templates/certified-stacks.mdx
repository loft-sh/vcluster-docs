...
The bundle also includes the Apps referenced by these StackTemplates. All four Run:ai StackTemplates use App tasks. They require the Apps entitlement, which is included on every plan. They don't require the Argo CD Integration entitlement.
...
- an ingress LoadBalancer and working DNS behavior
...
Use the `runai-tenant` VirtualClusterTemplate for the supported path. It creates a tenant cluster and deploys a `run-ai-dedicated-control-plane` StackInstance named `runai`.
...
After creation, monitor the `runai` StackInstance. Verify the ingress, GPU Operator, NVIDIA Run:ai operator, and scheduler. A healthy Helm task alone doesn't prove that the NVIDIA Run:ai scheduler is ready.
