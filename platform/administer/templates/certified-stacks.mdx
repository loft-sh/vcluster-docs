...
Certified templates are read-only, and the UI has no copy or duplicate action. To customize one, create your own StackTemplate from its YAML under a different name. See [Customize a certified stack](#customize-a-certified-stack). Platform updates can replace bundled resources during an upgrade.
...
Certified objects are read-only. The only row action on a certified template is <Button>View</Button>, and **Save Changes** stays disabled on the template page. The UI has no copy or duplicate action, so build your own template from the certified YAML.
...
<Flow id="customize-certified-stack-ui">
  ...
  <Step>
    Find the certified template, and click <Button>View</Button>. The **CR** tab of the editor pane shows the full resource.
  </Step>
  <Step>
  ...
  <Step>
    Make your changes, then click <Button>Create Stack Template</Button>.
  </Step>
</Flow>

To work outside the UI, export the template through the management API:

```bash
vcluster platform connect management
kubectl get stacktemplates.management.loft.sh run-ai-dedicated-control-plane -o yaml > my-stack.yaml
```

Edit `my-stack.yaml` the same way, then run `kubectl apply -f my-stack.yaml`.

Your template is a separate resource. It doesn't receive changes from later certified bundles, and a Platform upgrade that ships a new bundle doesn't replace it.
...
For exhaustive parameters and implementation details, read the upstream [dedicated control-plane](https://github.com/loft-sh/certified-stacks/blob/main/run-ai/dedicated-control-plane/README.md) and [central control-plane](https://github.com/loft-sh/certified-stacks/blob/main/run-ai/central-control-plane/README.md) guides.
