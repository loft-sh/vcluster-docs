...
:::warning
Creating a tenant cluster from the `runai-tenant` VirtualClusterTemplate can fail during Stack installation if the tenant cluster name is too long. `deploy.stacks` includes the tenant cluster name in the generated StackInstance name. The NVIDIA Run:ai chart combines that name with its component names, which can exceed the 63-character limit for the `thanos-query-grpc` Service name. A reported 18-character tenant cluster name produced a 64-character Service name. This is a known issue. Until it's fixed, keep the tenant cluster name to 17 characters or fewer, and check your Platform release notes to confirm whether it's resolved.
:::
