...
:::caution
Keep the tenant cluster name to 17 characters or fewer. The NVIDIA Run:ai chart combines the generated StackInstance name with its own component names to build Kubernetes object names, including the `thanos-query-grpc` Service. A longer tenant cluster name can push that name past the 63-character Kubernetes object name limit.
:::
