...
The platform uses [Kubernetes role-based access control (RBAC)](https://kubernetes.io/docs/reference/access-authn-authz/rbac/) to manage access for platform resources and objects, including platform custom resources and clusters.

RBAC manages user and team permissions by assigning them roles that specify what actions they can perform. These permissions are applied using *ClusterRoleBindings*, which connect a specific role to a user or team, which defines their access across platform resources and clusters.

:::note Tenants narrow every role
On a platform with [Tenants](../../understand/what-are-tenants.mdx), a user belongs to
exactly one Tenant and that binding can't change. A Tenant isn't a role layer. It narrows
whatever role the user holds, so effective access is the role intersected with what the
Tenant was granted.

Team membership can't cross the line either. A user bound to a Tenant can't join a
platform-level team, and a platform user can't join a Tenant's team. Either write is
refused.
:::
...
| **Connected cluster** | A control plane cluster that hosts cluster control planes. |
| **Cluster** | A certified Kubernetes distribution managed by Platform.|
...
| Cluster | Isolated Kubernetes environment | Provides full API compatibility with dedicated control plane |
