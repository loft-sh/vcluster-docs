...
Unlike clusters or namespaces, Stacks don't have a per-object **Permissions** tab in the UI. Grant access with a ClusterRoleTemplate instead, as shown below.
...
A user who creates a StackInstance must also be authorized for its destination cluster or control plane cluster. Task type licensing and destination access checks are independent of the resource rules in this page.
