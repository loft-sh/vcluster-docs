...
# Access keys in vCluster platform

## Overview

Access keys provide authentication credentials for the vCluster platform. Users can use these keys with the vCluster CLI or HTTP utilities like curl to interact with the platform programmatically.
...
The vCluster platform supports limiting access key permissions by scoping them to specific resources. You can scope an access key to restrict its use to:
...
Scoped access keys work particularly well for automated processes such as CI/CD pipelines. When implementing access keys in automated workflows, consider using scoped keys to improve security by limiting access to only the required resources.
...
CI/CD integration often requires programmatic access to vCluster resources. Create project-scoped access keys for build pipelines to enable automated building and deployment while limiting potential security exposure.
...
<Flow id="create-access-key">
  ...
  <Step>
    ...
    prompt as you will not be able to fetch this key again!
  </Step>
  <Step>
