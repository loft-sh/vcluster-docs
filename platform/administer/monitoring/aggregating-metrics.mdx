...
The OpenTelemetry Collector Agent on each node will push metrics about the workloads running on that node to the Prometheus instance. The metrics will have the vCluster, vCluster Platform and Kubernetes metadata associated with the workloads as labels.
