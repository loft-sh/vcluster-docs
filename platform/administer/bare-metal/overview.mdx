...
The platform manages server power: it powers a server on when a Machine claims it and powers it off when the Machine is removed. Register servers in a powered-off state so that unclaimed servers stay shut down until the platform provisions them.
