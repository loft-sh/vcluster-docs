...
spec:
  ...
  properties:
    ...
    # Optional: reference a NetworkEnvironment for network defaults.
...
- **Cluster private nodes**: Provision VMs that join clusters as private worker nodes through [auto nodes](/docs/vcluster/deploy/worker-nodes/private-nodes/auto-nodes/).
...
<Flow>
  ...
  <Step>
    ...
    A cluster can also create a Machine through auto nodes.
  </Step>
...
| `vcluster.com/network-environment` | References a `NetworkEnvironment` whose properties merge into the Machine. |
| `vcluster.com/user-data` | Provides inline cloud-init user data. |
| `vcluster.com/user-data-template-config` | References a [MachineConfigTemplate](../../use-platform/machines/machine-config-templates.mdx) that renders user data. |
