...
| vCluster KubeVirt CSI driver | Creates DataVolumes on the control plane cluster to back tenant PVCs and removes them according to the tenant PV reclaim lifecycle. |
...
Do not delete these DataVolumes directly from the platform UI or control plane cluster.
...
If the image boots but does not join a cluster, make sure the guest image supports `cloudInitNoCloud`.
