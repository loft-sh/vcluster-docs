...
- The VM template, cloud-init network data, and guest OS interface names agree on which interface should receive the configured IP address.

## Create a network attachment definition

Use the platform UI when you want an administrator-managed NAD that VM node types can reference.
...
## Attach a virtual machine to a network attachment definition

Set `kubevirt.vcluster.com/network-attachment-definition` to the NAD name.
...
Use these related properties to tune the injected interface:
...
| `kubevirt.vcluster.com/network-data` | Base64-encoded cloud-init network data. Use this when automatic network configuration is not enough. |
