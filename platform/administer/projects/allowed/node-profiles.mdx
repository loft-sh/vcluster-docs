...
With this spec, nodes in this project can use either `gpu-training` or `general-compute`. A node that joins without an explicit profile, and whose <GlossaryTerm term="tenant-cluster">cluster</GlossaryTerm> has no `privateNodes.defaultProfile`, falls back to the project's `general-compute` default.
...
`defaultNodeProfile` sets the project fallback profile. A node uses this value only when it doesn't select a profile explicitly and the cluster's `privateNodes.defaultProfile` is also unset.
...
A profile is resolved in the order described in [Assignment and precedence](../../node-profiles/overview.mdx#assignment-and-precedence). An explicit selection wins first, then the cluster's default, then the project's default. Whichever source wins, the result is validated against `allowedNodeProfiles` before the profile is applied.
