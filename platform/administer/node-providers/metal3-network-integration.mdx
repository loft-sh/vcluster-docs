...
The `metal3.neutronEnabled` NodeProvider field enables Neutron integration. Within Neutron mode, the `metal3.vcluster.com/network-provider` property selects an optional fabric provider on a network environment (a cluster-scoped [`NetworkEnvironment`](../../vmetal/concepts/network-environments.mdx) resource). Set the property to `netris` for Netris, or leave it empty for a flat network.

:::info Requires a network environment
Neutron and Netris require a network environment. They support Machines provisioned independently or used as [private nodes](/docs/vcluster/deploy/worker-nodes/private-nodes) for a <GlossaryTerm term="tenant-cluster">cluster</GlossaryTerm>. Use private nodes and a separate Netris server cluster for each network environment when tenants are untrusted.
:::
...
The two flat modes keep cluster control planes and APIs separate and manage their addresses independently. Every server still shares the same layer 2 network, so these modes don't provide network isolation. Only Netris moves that isolation boundary into the hardware fabric.
...
Flat Neutron also requires `metal3.vcluster.com/network-cidr` in the effective properties of every NodeClaim. The platform can't allocate the tenant port without this CIDR. The following example defines the CIDR on a node type and binds its NodeClaims to a network environment assigned to their Tenant:
    ...
    nodeTypes:
      - name: compute
        properties:
          ...
          # Select this NetworkEnvironment by name. It must be assigned to the claim's Tenant or the platform.
...
metadata:
  ...
  labels:
    # Assign the environment to a Tenant. Use an empty value to reserve it for the platform.
    tenant.vcluster.com/exclusive-to: tenant-a
spec:
...
spec:
  metal3:
    ...
    nodeTypes:
      - name: compute
        properties:
          # Select the NetworkEnvironment shown later on this page by name.
          vcluster.com/network-environment: tenant-a-network
```
...
metadata:
  name: tenant-a-network
  labels:
    # The assignment and spec can't be changed after creation.
    tenant.vcluster.com/exclusive-to: tenant-a
spec:
  providerRef: metal3-provider
  properties:
