...
References a CDI `DataSource`. The provider sets the DataVolume's `sourceRef` to this DataSource. If you only supply `<name>`, the provider looks up the DataSource in the node provider namespace from `spec.kubeVirt.clusterRef.namespace`. Use `<namespace>/<name>` to select a different namespace explicitly. Mutually exclusive with `kubevirt.vcluster.com/image-url`.
