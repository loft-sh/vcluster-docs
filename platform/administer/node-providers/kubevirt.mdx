...
3. Create Multus NetworkAttachment definition `netris-<node provider name>-<server cluster name>` using the bridge plugin with the `nebrvx-<vnet vxlan id>` bridge
...
- Bridge with the `nebrvx-<vnet vxlan id>` must exist on the host cluster nodes
