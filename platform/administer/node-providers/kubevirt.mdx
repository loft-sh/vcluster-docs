...
`deploy.kubevirt.enabled` is immutable after the provider is created, as are `clusterRef.cluster` and `clusterRef.namespace`. To change them, create a new provider.
:::
