...
Specifies a reference to a Multus NetworkAttachmentDefinition.
This allows connecting the VM to another network (e.g., through a bridge).
It must exist in the respective namespace in the host cluster.

Automatically injects a network `multus` and interface using that network in the VM template.
If it already exists in the template, it will be overwritten.
...
5. Inject a network and interface into the VM template (see [below](#kubevirt-vcluster-com-network-attachment-definition))
...
**Prerequisites:**
- [Netris integration](/vcluster/configure/vcluster-yaml/integrations/netris) must be enabled and configured in the vCluster configuration
...
For more information on Netris integration, see the [Netris integration documentation](/vcluster/configure/vcluster-yaml/integrations/netris).
