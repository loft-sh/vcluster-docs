...
Properties are key-value pairs on the NodeProvider, NodeType, or NodeClaim that control provisioning behavior. Properties are merged with NodeClaim overriding NodeType overriding NodeProvider.
...
Useful when a tenant wants to create a one-off VM whose shape does not match any predefined NodeType.

:::warning Standalone Machines only
...
References a CDI `DataSource`. The provider sets the DataVolume's `sourceRef` to this DataSource. If only `<name>` is supplied, the DataSource is looked up in the same namespace as the VM. Mutually exclusive with `kubevirt.vcluster.com/image-url`.

#### `kubevirt.vcluster.com/root-disk-size`

**Type:** `string` (Kubernetes quantity, e.g. `40Gi`)
...
A partial `DataVolumeTemplate` that is deep-merged onto the defaults before the platform applies the image source and disk size. Use this to override fields like `storage.accessModes` or `storage.storageClassName`. The DataVolume's `source` and `sourceRef` are always set by the platform based on the image properties and cannot be overridden here.
...
References a Multus `NetworkAttachmentDefinition` that already exists in the target namespace on the host cluster. The provider injects a `multus` network and a corresponding interface into the VM template using this NAD. If a network or interface with the same name already exists in the template, it is overwritten.

This property is the supported way to attach a VM to an additional network (e.g., a bridge). Set it on the NodeType for VMs that should share a network, or on the NodeClaim for a one-off attachment.
...
Names the [NodeEnvironment](./overview.mdx#node-environments) whose properties are merged into the NodeClaim's effective property set. Resolved from the merged properties of NodeProvider, NodeType, and NodeClaim. When set, takes precedence over the typed `NodeClaim.spec.environmentRef` field.

Use this to pull environment-level defaults in declaratively at the provider or type level rather than per-claim, or simply to keep network configuration in one place separate from provider and type definitions.

### SSH and user data

The `vcluster.com/ssh-keys`, `vcluster.com/user-data`, `vcluster.com/user-data-template`, and `vcluster.com/user-data-template-secret` properties behave identically across providers. See the [metal3 reference](./metal3.mdx#ssh-and-user-data) for details. The templating values available are:

| Field | Description |
|-------|-------------|
| `.Values.NodeClaim` | The NodeClaim being provisioned |
| `.Values.Project` | The project the node belongs to |
| `.Values.Properties` | Merged properties |
...
1. Query the Netris server cluster to collect subnet, gateway and vxlan id information
2. Allocate an IP address from the subnet
3. Create Multus NetworkAttachment definition `netris-<node provider name>-<server cluster name>` using the bridge plugin with the `nbr-<vnet vxlan id>` bridge
4. Inject cloud-init network configuration with the allocated IP
5. Inject a network and interface into the VM template via the [`kubevirt.vcluster.com/network-attachment-definition`](#kubevirt-vcluster-com-network-attachment-definition) property
...
- `kubevirt.vcluster.com/network-attachment-definition`
    ...
    If specified, the integration will not create another Multus NetworkAttachmentDefinition, but instead use this one.
