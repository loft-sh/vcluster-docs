...
<figure>
  <KubeVirtNodeProvisioningFlow style={{width: '100%', height: 'auto'}} role="img" aria-label="A NodeClaim references a NodeType for its VM template and requests a node from the vCluster Platform. The platform creates a KubeVirt VirtualMachine on the control plane cluster from that template, with a join-token Secret injected through cloud-init. The VM registers as a worker node in the cluster using that token. Deleting the NodeClaim deletes the VirtualMachine and removes the node." />
  <figcaption>The platform turns a NodeClaim into a KubeVirt VirtualMachine on the control plane cluster, which joins the cluster as a worker node through cloud-init. Deleting the NodeClaim reverses the flow.</figcaption>
</figure>
...
4. When the VM boots, `cloud-init` executes the script from the disk, configuring the machine and registering it as a node with the cluster.
...
### Choose where VirtualMachines are created

`spec.kubeVirt.namespaceStrategy` controls which namespace on the control plane cluster a node's `VirtualMachine` object is created in:

- `Provider` (default): every `VirtualMachine` is created in the node provider's own namespace, `clusterRef.namespace`.
- `namespaceStrategy: VirtualCluster`: each `VirtualMachine` is created in the namespace of the cluster whose `NodeClaim` requested the node, instead of the provider's namespace.

```yaml
spec:
  kubeVirt:
    namespaceStrategy: VirtualCluster
```

The VirtualCluster strategy falls back to the `Provider` behavior when it can't resolve a usable cluster namespace within `clusterRef.cluster`. This happens in the following cases:

- The `NodeClaim` has no cluster reference.
- The referenced VirtualClusterInstance doesn't exist.
- The cluster runs in a different connected cluster from the KubeVirt provider.
- The cluster is standalone and doesn't run in a namespace on the provider cluster.

As a result, `namespaceStrategy: VirtualCluster` has no effect for standalone clusters.
...
This property is currently only compatible with standalone Machines. It cannot be used with NodeClaims created by a cluster's node pool. This is not enforced. Setting it on a node-pool claim will produce undefined behavior.
:::
...
When the NetworkEnvironment carries the `kube-ovn.vcluster.com/subnet` property, the platform fills these in automatically, each only when you have not set it yourself. Set any of them explicitly on the NodeType or NodeClaim to override the default.
...
Instead of pointing VMs at an existing kube-ovn network, you can have the platform create one for you. Set `kube-ovn.vcluster.com/vpc` on the [NetworkEnvironment](../../vmetal/concepts/network-environments.mdx) to provision the kube-ovn resources on the control plane cluster. This property gates provisioning. Without it, the platform creates nothing. Provisioning runs as part of the NetworkEnvironment's infrastructure lifecycle, so the network exists before any VM uses the environment and is removed when the environment is deleted.

A VM attaches to a network provisioned this way through the same [attachment properties](#attach-vms-to-a-kube-ovn-network). When the NetworkEnvironment carries `kube-ovn.vcluster.com/subnet`, those attachment defaults are filled in automatically.

#### `kube-ovn.vcluster.com/vpc`

**Type:** `string`

Name of the kube-ovn `Vpc`. Setting it enables provisioning and creates the VPC, subnet, and `NetworkAttachmentDefinition`. When the VPC does not exist, the platform creates it. A pre-existing VPC is only modified when this NetworkEnvironment manages it.
