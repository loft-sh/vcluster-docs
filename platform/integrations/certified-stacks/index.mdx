...
Certified Stack orchestration doesn't have a separate license entitlement. Each task uses the entitlement for the resource it creates. The bundled NVIDIA Run:ai templates contain App tasks, and [Apps](../../understand/what-are-apps.mdx) are available on every plan.
...
Earlier Certified Stacks are repository-based reference implementations. They can include Terraform, Helm, and other orchestration outside the Platform Stack API. Those implementations remain available in the repository for users who rely on them.
