...
| --- | --- | --- | --- |
| Dedicated control plane | Each cluster runs its own NVIDIA Run:ai control plane and GPU Operator. | Deleting the tenant removes its dedicated StackInstance. | Independent NVIDIA Run:ai and GPU Operator lifecycles per tenant. |
...
Use the `runai-tenant` VirtualClusterTemplate. It creates a cluster and declares a [`deploy.stacks`](/docs/vcluster/configure/vcluster-yaml/deploy#platform-stacks) entry named `runai`. This entry references the `run-ai-dedicated-control-plane` StackTemplate. Platform generates the StackInstance name from the cluster name and Stack configuration name.
...
3. `run-ai-central-control-plane` installs the agent and tenant components inside that cluster.

The `runai-tenant-central-control-plane` VirtualClusterTemplate creates the registration and tenant StackInstances. Secrets carry registration facts, registry credentials, and certificate trust into the cluster.
