...
Deleting a tenant cluster follows a separate cleanup path. Depending on the destination and connector, Platform either requests cascading deletion or relies on tenant cluster deletion to remove workloads.

The generated Argo CD `Application` doesn't carry the `resources-finalizer.argocd.argoproj.io` finalizer, and there's no way to add it. `metadata` on `ArgoCDApplication` and `ArgoCDApplicationTemplate` only supports labels and annotations.
