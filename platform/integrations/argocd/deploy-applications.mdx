...
`ArgoCDApplicationTemplate` supports the same parameter definitions (types, validation, defaults) as tenant cluster templates. Both templated and inline `ArgoCDApplication` specs can use the built-in platform-specific values (project, tenant cluster name, and so on) described in [Parameters](../../administer/templates/parameters.mdx#argo-cd-application-parameter-values).
...
Deleting a tenant cluster follows a separate cleanup path. Platform always deletes the `ArgoCDApplication` object, but whether the underlying Argo CD `Application` cascades depends on the connector and destination:

- With the Akuity connector, Platform always requests cascading deletion, regardless of destination.
- With a self-hosted Argo CD connector and `target: vcluster` (the default), Platform skips cascading delete. Argo CD doesn't prune the workloads, so they're orphaned in the tenant cluster unless its own teardown removes them first.
- With a self-hosted Argo CD connector and `target: host`, Platform requests cascading deletion, since the control plane cluster, and the workloads on it, outlives the tenant cluster.
