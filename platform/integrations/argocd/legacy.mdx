...
with a platform access key. The control plane EKS endpoint isn't a valid target on
...
of the speed and ease of creating clusters, as well as the power of Argo CD for managing and
deploying applications within those clusters.

Argo CD Integration has three major components or levels, with each level adding additional
integration capabilities.

1. Sync clusters to Argo CD
...
The simplest level of integration is syncing clusters into Argo CD as available clusters
for deployments. In this deployment model, clusters that are part of an Argo enabled
project, and have the `loft.sh/import-argocd` label set to `true` will be automatically "synced"
to Argo CD. These clusters will show up as clusters that are available for deployments
in the Argo CD UI. When the clusters are deleted, vCluster Platform will automatically remove the
...
under <NavStep>Project Config</NavStep>.
:::
...
>
  <TabItem value="creation">
    <Flow id="enable-argocd-project-creation">
      ...
      <Step>
        ...
        <Label>Clusters</Label> from the dropdown box in the <Label>
          Where is Argo CD running
        </Label> dropdown. This tells vCluster Platform if your Argo CD instance lives
        in a connected physical cluster, or if it is running inside of a cluster.
      </Step>
      <Step>
  ...
  <TabItem value="existing">
    <Flow id="enable-argocd-project-existing">
      <Step>
        Select the appropriate project in the project selector dropdown (top left), then click <NavStep>Project Config</NavStep>.
      </Step>
      <Step>
      ...
      <Step>
        ...
        in a connected control plane cluster, or inside a cluster.
      </Step>
      <Step>
        In the next dropdown to the right, select the name of the cluster or
        cluster that your Argo CD instance is deployed in.
...
configured using Helm values, or in the `Platform` > `Platform Config` section of the UI on the `Config` pane.
:::
...
:::warning Moving clusters between projects
Migrating a cluster between projects while Argo CD integration is enabled is not supported. Because Argo CD applications are project-scoped, this operation deletes all Argo CD applications managed by Platform for that cluster and removes their workloads. Applications are not recreated in the destination project. The cluster may also get stuck in `Terminating`.
...
Once a project has Argo CD enabled, clusters within that project are eligible to be
synced to the Argo instance as available clusters. You can enable cluster import into
Argo CD during the cluster creation step, or by enabling this feature on existing
clusters.

:::info Make Sure Your Project Has Argo CD Enabled.
If the project your cluster resides in does _not_ have Argo CD integration enabled, you
...
<Tabs
  defaultValue="creation"
  values={[
    { label: "During Cluster Creation", value: "creation" },
    { label: "On Existing Cluster", value: "existing" },
  ]}
>
  <TabItem value="creation">
    <Flow id="enable-argocd-import-creation">
      ...
      <Step>
        Click on Clusters and the{" "}
        <Button>New Cluster</Button> button.
      </Step>
      <Step>
        In the popup, optionally select the cluster template, then click
        the
        <Button>Create</Button> button.
      </Step>
      <Step>
        Find your desired cluster in the cluster list. Slide the
...
:::warning Integration Errors
If there are any errors adding your cluster to Argo CD, the cluster status will
...
The Argo CD integration can be disabled on a per cluster or per project level by toggling the same sliders
used to enable it. Disabling the integration at the cluster level simply removes it as a registered cluster
in Argo CD. Disabling the integration at the project level removes all clusters from Argo CD, so be careful
when disabled Argo at this level.

:::warning Disabling at the project level deletes all managed applications
When Argo CD integration is disabled at the project level, all Argo CD applications managed by Platform for clusters in that project are deleted. The workloads deployed by those applications are also removed.
:::
