...
Disabling the integration removes the cluster from Akuity, uninstalls the agent namespace from the cluster, and deletes every `ArgoCDApplication` object Platform generated for it from `deploy.argoCD.applications`. This applies whether the integration is configured using `vcluster.yaml`, a VirtualClusterInstance manifest, or a Cluster object. Standalone `ArgoCDApplication` objects you created directly aren't deleted, but their remote Argo CD Application is removed along with the cluster's registration, and the CR's `Synced` condition turns `False` until the cluster registers again.
:::
