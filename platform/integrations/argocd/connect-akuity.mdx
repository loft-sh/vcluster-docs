...
Disabling the integration removes the cluster from Akuity and uninstalls the agent namespace. This applies whether the integration is configured using `vcluster.yaml`, a VirtualClusterInstance manifest, or a Cluster object.

For a tenant cluster, Platform deletes the `ArgoCDApplication` objects it generated from `deploy.argoCD.applications`, including ones with `target: host`. Independently created `ArgoCDApplication` objects aren't deleted, including ones using `spec.destination.cluster` when that control plane cluster's integration is disabled.

Platform removes their previously synced backend Applications, but the `ArgoCDApplication` object itself remains and reports a connector error. Re-enabling the integration reconciles it again. You don't have to recreate it.
:::
