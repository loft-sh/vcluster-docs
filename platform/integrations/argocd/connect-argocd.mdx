...
This guide connects vCluster Platform to a self-hosted Argo CD server. At the end you will have a connector that registers clusters and control plane clusters with Argo CD automatically.
...
|----------|---------|-----|
| `clusters` | `get`, `create`, `update`, `delete` | Register and deregister clusters and control plane clusters |
| `applications` | `get`, `create`, `update`, `delete`, `sync` | Create and sync Argo CD Applications across all projects (`*/*`). `sync` is required to stop in-progress sync operations when deleting a cluster with installed apps |
...
Add the `integrations.argoCD` block to the cluster's `vcluster.yaml`. The `connector` field references the Secret name from [Step 1](#step-1-create-a-connector).
...
When the VirtualClusterInstance reconciles, Platform registers the cluster with Argo CD using the Platform proxy as the API server endpoint and a scoped access key as the bearer token.
