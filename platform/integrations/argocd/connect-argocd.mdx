...
Argo CD supports two authentication methods, controlled by the `authType` field. Apply the Secret with `kubectl apply`.
...
|-------|----------|-------------|
| `authType` | Yes | Authentication method: `"token"` or `"basic"` |
