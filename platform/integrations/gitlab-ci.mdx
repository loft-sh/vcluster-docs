...
## Clusters for merge requests

This example shows how to create and delete a Cluster for running end-to-end tests for
...
3. The `before_script` then creates a cluster using
   [predefined GitLab variables](https://docs.gitlab.com/ee/ci/variables/predefined_variables.html)
   to create a unique name. Optionally, the `--upgrade` flag can be used to reuse an existing cluster instead of creating a new one.
...
6. Finally `after_script` deletes the cluster, and passes `--delete-namespace` to ensure the
   corresponding namespace for the cluster is deleted. By using `after_script` we can ensure the cluster
   is deleted even if the tests fail. You may wish to skip this step if reusing the cluster with the `--upgrade` flag when creating the cluster.
