...
5. Generate `karpenter-values.yaml` and set it to an environment variable. You will then need create a credential linked to the service account and identity.

```bash
export ISSUE_URL=$(az aks show --name <cluster-name> --resource-group <resource-group-name> --query "oidcIssuerProfile.issuerUrl" -o tsv)
```

6. You will need a service account for karpenter and an identity.
