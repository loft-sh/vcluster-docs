...
4. The following permissions in vCluster Platform:
    1. Create projects
    2. create vClusters
5. The following Kubernets permissions for kubectl operations:
    1. Create `NodePools.karpenter.sh/v1beta1`.
    2. Create `Deployments.apps/v1`.
5. EKS specific:
    ...
    2. The following Kubernetes permissions for kubectl operations:
        1. Create `EC2NodeClass.karpenter.sh/v1beta1`.
    3. Set the environment variables: `CLUSTER_NAME`, `ARM_AMI_ID`, `AMD_AMI_ID`, `K8S_VERSION`.
    4. Delete the gp2 StorageClass with `kubectl delete storageclass gp2` and replace with gp3 by applying following yaml:
6. AKS specific:
    1. Create `AKSNodeClass.karpenter.sh/v1beta1`
