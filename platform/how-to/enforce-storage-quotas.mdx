---
title: Enforce storage quotas per cluster
sidebar_label: Storage quota enforcement
sidebar_position: 7
description: Enforce per-cluster storage limits inside vCluster Platform clusters using ResourceQuota objects or Kyverno policies applied at provisioning time through templates.
---

import StorageQuotaEnforcementDiagram from '@site/static/media/diagrams/storage-quota-enforcement.svg';

vCluster Platform [project quotas](../administer/projects/quotas.mdx) cap aggregate storage consumption across all clusters in a project, but they don't distribute limits across namespaces or enforce per-PVC caps inside individual clusters. Per-cluster enforcement fills that gap. A ResourceQuota or Kyverno policy applied inside the cluster controls how much storage any single namespace can consume and how individual PVCs may be sized.

This guide covers two patterns for per-cluster enforcement. Both can be applied automatically at provisioning time through templates so that every new cluster starts with the right guardrails in place.
...
## Pattern 1: ResourceQuota inside the cluster

A Kubernetes ResourceQuota object enforces aggregate limits on PVC count and total storage requests. It's namespace-scoped, so enforcing limits across all namespaces in the cluster requires one ResourceQuota per namespace. Adding one to a cluster template deploys it inside every cluster created from that template.

### Add a ResourceQuota to a cluster template

In the `objects` field of a VirtualClusterTemplate, include a ResourceQuota for each namespace you want to constrain. vCluster Platform applies the objects inside the cluster at provisioning time.
...
For per-namespace enforcement that applies when a namespace is created inside a cluster, use a Space template with a ResourceQuota in the `objects` field instead. See [Create a template](../administer/templates/create-templates.mdx).
:::
...
Install Kyverno inside the cluster as an [App](../administer/templates/create-templates.mdx) in the cluster template. The examples below use `kyverno.io/v1`. Adjust the API version and validation syntax for the Kyverno version installed in your clusters.
...
### Embed Kyverno policies in a cluster template

Add ClusterPolicy manifests to the `objects` field of a VirtualClusterTemplate to deploy them inside every cluster created from that template. Multiple manifests are separated by `---`.
...
The platform applies `objects` before `apps`. ClusterPolicy manifests in the `objects` field are applied before the Kyverno App finishes installing its CRDs and admission webhook, so the apply will fail if Kyverno is not already present. To avoid this, either pre-install Kyverno in the cluster before using this template, or package the ClusterPolicy manifests as a dedicated App listed after the Kyverno App rather than using the `objects` field.
...
- [Create a template](../administer/templates/create-templates.mdx): add objects and Apps to a cluster template
