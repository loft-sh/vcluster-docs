...
This example defines a reusable Stack with two App tasks, then deploys it to a tenant cluster. Argo CD sync waves create the referenced template before the instance. The Stack controller, not Argo CD, enforces the task dependency and health gate.
...
- [Enable sleep mode](/docs/vcluster/next/operate/sleep-wakeup) to reduce costs on idle tenant clusters
