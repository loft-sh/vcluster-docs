...
A [Stack](../understand/what-are-stacks.mdx) is defined by two resources, `StackTemplate` and `StackInstance`. Store both with the other Platform resources in your Git repository. A StackTemplate is cluster-scoped. Create each StackInstance in its project's `p-<project-name>` namespace.
