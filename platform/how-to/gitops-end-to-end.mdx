...
sidebar_position: 1
description: Complete guide to managing vCluster Platform and clusters with ArgoCD, from platform installation to cluster deployment.
---

import InterpolatedCodeBlock from "@site/src/components/InterpolatedCodeBlock";

This guide walks through the full lifecycle of managing platform resources with ArgoCD, covering platform installation, cluster connections, project creation, and cluster deployment. Each section references the detailed documentation for that step and provides the ArgoCD-specific configuration needed.
...
After the platform is running, connect control plane clusters where clusters will be deployed. In a GitOps workflow, you can manage connected clusters as Kubernetes resources alongside your platform installation.
...
To also register the control plane cluster with Argo CD so it can receive shared infrastructure applications directly, add `spec.argoCD` to the Cluster object after creating a connector (see [Step 3](#3-create-a-project)):

```yaml
spec:
  argoCD:
    enabled: true
    connector: argocd-main   # name of your connector Secret
```
...
Projects organize clusters and control access, templates, and quotas. In a GitOps workflow, define projects as Project custom resources.
...
With the platform, cluster, and project in place, deploy clusters through ArgoCD. There are two approaches depending on how you want to manage the cluster lifecycle.
...
:::tip
The namespace follows the pattern `p-<project-name>`. The platform uses this convention to associate clusters with projects.
:::

When the VirtualClusterInstance reconciles, Platform registers the cluster with Argo CD using the connector credentials. You can then declare Argo CD Applications in the cluster configuration. See [Deploy applications](../integrations/argocd/deploy-applications.mdx).

### Option B: Deploy with Helm chart directly

Deploy the vCluster Helm chart through an ArgoCD Application. This approach gives you direct control over the Helm release but does not provide platform lifecycle features unless you [register the cluster afterward](../use-platform/virtual-clusters/add-virtual-clusters.mdx).
...
## Manage stacks declaratively

A [Stack](../understand/what-are-stacks.mdx) is defined by two resources, `StackTemplate` and `StackInstance`. You can store both with the other Platform resources in your Git repository. A StackTemplate is cluster-scoped. Create each StackInstance in its project's `p-<project-name>` namespace.

This example defines a reusable Stack with two App tasks, then deploys it to a cluster. Argo CD sync waves create the referenced template before the instance. The Stack controller, not Argo CD, enforces the task dependency and health gate.
...
Set `spec.owner` on the StackInstance. App tasks deploy as their owner, and a GitOps controller applies the manifest with its own service account rather than a Platform identity. The owner is immutable once set, so a later commit can't change it.

Declare the referenced App or Argo CD application templates in an earlier sync wave or manage them separately. A referenced StackTemplate is resolved on every StackInstance reconciliation, so a committed template change can update existing instances. Review `prunePolicy` before removing tasks. Its default value, `Retain`, reports removed child applications as orphans instead of deleting them.

For the complete task and output schema, see [Create a Stack template](../administer/templates/create-stack-templates.mdx). For Stack resource permissions, see [Stack permissions](../administer/users-permissions/permissions/stacks.mdx).
...
platform-gitops/
  ...
  resources/
    ...
    stacks/
      example-platform.yaml     # StackTemplate + StackInstance
```

## Next steps

- [Configure platform settings](../configure/introduction.mdx) for authentication, audit logging, and more
- [Set up templates](../administer/templates/create-templates.mdx) to standardize cluster configurations
- [Configure project quotas](../administer/projects/quotas.mdx) to control resource usage
- [Enable sleep mode](/docs/vcluster/manage/sleep-wakeup) to reduce costs on idle clusters
