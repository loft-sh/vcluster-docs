...
A virtual cluster is a fully functional Kubernetes cluster that runs inside a namespace of a host Kubernetes cluster. Virtual clusters solve namespace limitations in multi-tenant environments by allowing users to deploy their own Custom Resource Definitions (CRDs) or enable communication between pods across namespaces that would otherwise be restricted by standard [NetworkPolicies](https://kubernetes.io/docs/concepts/services-networking/network-policies/#networkpolicy-resource).
