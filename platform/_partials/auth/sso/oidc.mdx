...
To configure OIDC for SSO, use the following YAML configuration to update your platform config (the `config` section of the [`values.yaml`](../../../configure/introduction.mdx)) or through vCluster Platform UI <!-- vale off -->**Admin > Config**<!-- vale on -->:
...
auth:
  ...
  oidc:
    ...
    # (Optional) Resource is the expected audience for access tokens during token exchange.
    # Required when using OIDC token exchange with access tokens (RFC 8693).
    # For Entra ID, set this to the Application (client) ID as a bare GUID.
    # See: /docs/platform/administer/authentication/token-exchange
    #resource: ""
...
After saving the new configuration, the platform should restart. You should then be able to log in using your OIDC provider.
...
Once OIDC is configured, CI/CD pipelines and automated systems can exchange OIDC tokens for platform access keys without storing static credentials. See [OIDC token exchange](../../../administer/authentication/token-exchange.mdx).
