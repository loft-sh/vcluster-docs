...
To configure the platform to use Microsoft for SSO, use the following YAML configuration to update your platform config (the `config` section of the [`values.yaml`](../../../configure/introduction.mdx)) or through vCluster Platform UI <!-- vale off -->**Admin > Config**<!-- vale on -->:
