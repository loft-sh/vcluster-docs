...
Your current kube-context must have administrative privileges, which you can verify with `kubectl auth can-i create clusterrole -A`
    ...
    :::
- `helm` installed: Helm v3.10.0+ is required for deploying the platform. Refer to the [Helm Installation Guide](https://helm.sh/docs/intro/install/) if you need to install it.
