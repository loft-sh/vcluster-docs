...
Your current kube-context must have administrative privileges, which you can verify with `kubectl auth can-i create clusterrole -A`
    ...
    :::
- `helm` installed: Helm <Mark color="#ff6600">v3.10</Mark> is required for deploying the platform. Refer to the [Helm Installation Guide](https://Helm.sh/docs/intro/install/) if you need to install it.
