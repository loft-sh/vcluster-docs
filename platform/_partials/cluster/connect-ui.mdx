import Flow, { Step } from '@site/src/components/Flow'
import NavStep from '@site/src/components/NavStep'
import Button from '@site/src/components/Button'
import Label from '@site/src/components/Label'
import Input from '@site/src/components/Input'
import Expander from '@site/src/components/Expander'

<Flow id="cluster-connect-ui">
  <Step>
    Go to <NavStep>Infra Management > Control Plane Clusters</NavStep>.
  </Step>
  <Step>
    Click <Button>Connect Cluster</Button>.
  </Step>
  <Step>
    Give your cluster a name in <Label>Display Name</Label>.
    Optionally, give a name for the underlying kubernetes resource in the <Label>Control Plane Cluster ID</Label> field, or leave it empty to autogenerate one.
    Then click <Button>Next</Button>.
  </Step>
  <Step>
    Copy & execute the displayed vCluster CLI or Helm command.
  </Step>
  <Step>
    Wait until vCluster Platform installs the agent in your connected cluster.
  </Step>
  <Step>
    Once successful, use the displayed CLI command to create a space or cluster in the newly connected control plane cluster.
  </Step>
  <Step>
    Click <Button>Finish</Button> to go to the <NavStep>Control Plane Clusters</NavStep> view.
  </Step>
</Flow>
