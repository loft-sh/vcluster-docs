...
Set on a control plane cluster HTTPRoute to tell vCluster's HTTPRoute syncer to preserve a single named host-side HTTPRouteRule verbatim during tenant-to-host reconciliation. The platform sleep agent uses this to keep a managed wake-override rule alive while a tenant namespace sleeps.
