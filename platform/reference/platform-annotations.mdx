...
These annotations control node provider and network environment behavior.
...
### machines.vcluster.com/is-default-environment {#machines-vcluster-com-is-default-environment}

**Type:** Annotation

**Example:** `machines.vcluster.com/is-default-environment: "true"`

**Used on:** NetworkEnvironment

**Set by:** User-configurable

Marks the network environment as the default for its `spec.providerRef`. A NodeClaim that names no environment binds to the default for its provider, if the project allows it. The value must be exactly `"true"`. Each provider can have only one default, and setting the annotation on a second environment doesn't clear the first. While two are marked, claims that rely on the default fail. The **Set as default** switch in the network environment form sets this annotation. See [Network environments](../vmetal/concepts/network-environments.mdx#how-a-machine-selects-a-network).
