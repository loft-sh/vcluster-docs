...
The name of the <GlossaryTerm term="tenant-cluster">tenant cluster</GlossaryTerm> an object is associated with.
...
Records which NodeProvider provisioned this joined node in the tenant cluster.
...
Tells vCluster to preserve request mirror filters on this HTTPRoute during synchronization from tenant cluster to control plane cluster.
...
Links the platform NodeClaim to the cluster-scoped karpenter NodeClaim in the tenant cluster. Set by vcluster-pro's karpenter cloud provider integration.
...
Links a worker NodeClaim to the tenant cluster Node it joins as. The platform sets this label on both the claim and hands it to the kubelet as a node label, and the tenant-side node-init controller matches them to write the Kubernetes node name back onto the claim.
...
These annotations in the `sleepmode.loft.sh` namespace control auto sleep behavior for namespaces and tenant clusters. See each annotation's **Used on** field for exactly where to set it.
...
Specifies the number of seconds of inactivity after which the namespace or tenant cluster should automatically sleep.
...
Specifies the number of seconds of inactivity after which the namespace or tenant cluster should be automatically deleted.
...
Specifies a cron schedule for when the namespace or tenant cluster should automatically sleep. Uses standard cron format.
...
Specifies a cron schedule for when the namespace or tenant cluster should automatically wake up.
...
Forces the namespace or tenant cluster to sleep immediately, regardless of activity.
...
For vCluster 0.33.0 or later, keeps the tenant cluster control plane running and
lets the tenant cluster manage workload sleep. The platform agent doesn't take
...
Ignores all activity when determining whether the namespace or tenant cluster should sleep.
...
effect on tenant clusters. Use it for namespace sleep mode only.
...
Ignores tenant cluster requests when determining namespace activity.
...
annotation has no effect on tenant clusters. Use it for namespace sleep mode
only.
...
Provide a comma-separated list. This annotation has no effect on tenant
...
`resource[.group]=name1 name2`. This annotation has no effect on tenant
clusters. Use it for namespace sleep mode only.
...
Marks an access key whose API requests should not count as activity for sleep mode purposes. The ArgoCD integration sets this on its access keys so ArgoCD reconcile traffic does not keep a tenant cluster awake.
...
When set, the namespace or tenant cluster remains asleep when it receives that
traffic.
...
Unix timestamp of when the namespace or tenant cluster entered auto sleep. Present only when sleeping.
...
Identifies a metrics-writer access key issued for a tenant cluster to write observability data to the platform's metrics backend.
...
Part of the legacy project-level Argo CD integration. When set to `"true"`, the platform automatically registers this tenant cluster or cluster with Argo CD using the project-level configuration.
...
Part of the legacy project-level Argo CD integration. Tracks which Argo CD AppProject destinations are managed by the project controller. This prevents the controller from removing destinations managed by tenant clusters when syncing project specifications.
