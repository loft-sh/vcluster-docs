...
This page documents the well-known annotations and labels in the `loft.sh` namespace used by <GlossaryTerm term="vcluster">vCluster</GlossaryTerm> Platform for managing clusters, projects, spaces, users, teams, and integrations.
...
**Example:** `loft.sh/streaming-connection-idle-timeout: "14400"`

**Used on:** Cluster

**Set by:** User-configurable

Sets the idle timeout, in seconds, for streaming connections such as exec,
port-forward, and logs to this cluster. The setting applies to new connections.
...
### platform.vcluster.com/vcluster-instance-name {#platform-vcluster-com-vcluster-instance-name}
...
The name of the <GlossaryTerm term="tenant-cluster">cluster</GlossaryTerm> an object is associated with.
...
The namespace of the cluster an object is associated with.
...
**Example:** `vcluster.loft.sh/node-provider: "aws-node-provider"`

**Used on:** Node

**Set by:** Platform

Records which NodeProvider provisioned this joined node in the cluster.
...
The <GlossaryTerm term="control-plane">control plane</GlossaryTerm> endpoint for accessing this vCluster.
...
Indicates the original namespace of an object synced from the vCluster to the <GlossaryTerm term="control-plane-cluster">control plane cluster</GlossaryTerm>.

### vcluster.loft.sh/preserve-host-rule {#vcluster-loft-sh-preserve-host-rule}

**Type:** Annotation

**Example:** `vcluster.loft.sh/preserve-host-rule: "loft-sleep-wakeup-rule"`

**Used on:** HTTPRoute

**Set by:** Platform

Set on a control plane cluster HTTPRoute to tell vCluster's HTTPRoute <GlossaryTerm term="syncer">syncer</GlossaryTerm> to preserve a single named host-side HTTPRouteRule verbatim during tenant-to-host reconciliation. The platform sleep agent uses this to keep a managed wake-override rule alive while a tenant namespace sleeps.

### vcluster.loft.sh/preserve-request-mirror-filters {#vcluster-loft-sh-preserve-request-mirror-filters}

**Type:** Annotation

**Example:** `vcluster.loft.sh/preserve-request-mirror-filters: "true"`

**Used on:** HTTPRoute

**Set by:** Platform

Tells vCluster to preserve request mirror filters on this HTTPRoute during synchronization from cluster to control plane cluster.

### vcluster.loft.sh/default-node-profile {#vcluster-loft-sh-default-node-profile}

**Type:** Annotation

**Example:** `vcluster.loft.sh/default-node-profile: "gpu-enabled"`

**Used on:** VirtualClusterInstance

**Set by:** Platform

Mirrors the owning project's default node profile onto the VirtualClusterInstance. The value is the node profile name. This annotation is absent when the project has no default profile configured.

### vcluster.loft.sh/node-claim-name {#vcluster-loft-sh-node-claim-name}

**Type:** Label

**Example:** `vcluster.loft.sh/node-claim-name: "my-node-claim"`

**Used on:** Secret (bootstrap-token)

**Set by:** Platform

Identifies the NodeClaim name associated with a bootstrap-token Secret used for node registration.

### vcluster.loft.sh/node-claim-namespace {#vcluster-loft-sh-node-claim-namespace}

**Type:** Label

**Example:** `vcluster.loft.sh/node-claim-namespace: "default"`

**Used on:** Secret (bootstrap-token)

**Set by:** Platform

Identifies the NodeClaim namespace associated with a bootstrap-token Secret used for node registration.

### vcluster.loft.sh/karpenter-node-claim {#vcluster-loft-sh-karpenter-node-claim}

**Type:** Label

**Example:** `vcluster.loft.sh/karpenter-node-claim: "node-claim-abc123"`

**Used on:** NodeClaim (platform)

**Set by:** Platform

Links the platform NodeClaim to the cluster-scoped karpenter NodeClaim in the cluster. Set by vcluster-pro's karpenter cloud provider integration.

### vcluster.loft.sh/uid {#vcluster-loft-sh-uid}

**Type:** Label

**Example:** `vcluster.loft.sh/uid: "a1b2c3d4-e5f6-7890-abcd-ef1234567890"`

**Used on:** NodeClaim, Node

**Set by:** Platform

Links a worker NodeClaim to the cluster Node it joins as. The platform sets this label on both the claim and hands it to the kubelet as a node label, and the tenant-side node-init controller matches them to write the Kubernetes node name back onto the claim.
...
**Used on:** Deployment, StatefulSet, ReplicaSet

**Set by:** Platform

Marks a workload that the platform scaled to zero for sleep. The platform uses
this label to find the workload when it wakes the namespace.
...
These annotations in the `sleepmode.loft.sh` namespace control auto sleep behavior for namespaces and clusters. See each annotation's **Used on** field for exactly where to set it.
...
**Used on:** Namespace, VirtualClusterTemplate or inline VirtualClusterInstance under `spec.template.metadata.annotations`

**Set by:** User-configurable

Specifies the number of seconds of inactivity after which the namespace or cluster should automatically sleep.
...
Specifies the number of seconds of inactivity after which the namespace or cluster should be automatically deleted.
...
Specifies a cron schedule for when the namespace or cluster should automatically sleep. Uses standard cron format.
...
Specifies a cron schedule for when the namespace or cluster should automatically wake up.
...
Forces the namespace or cluster to sleep immediately, regardless of activity.
...
### sleepmode.loft.sh/scope {#sleepmode-loft-sh-scope}

**Type:** Annotation

**Example:** `sleepmode.loft.sh/scope: "workloads-only"`

**Used on:** VirtualClusterInstance

**Set by:** User-configurable

For vCluster 0.33.0 or later, keeps the cluster control plane running and
lets the cluster manage workload sleep. The platform agent doesn't take
over auto sleep for this instance.
...
Excludes this workload from auto sleep. When the namespace sleeps, its existing
Pods continue running. The sleep quota prevents Kubernetes from creating a
replacement if an excluded Pod stops while the namespace is asleep.
...
Ignores all activity when determining whether the namespace or cluster should sleep.
...
Ignores supported Ingress, HTTPRoute, and Istio traffic when determining
activity. Useful when automated web requests should not prevent sleep.
...
Ignores requests from users in specific groups when determining activity.
Provide a comma-separated list of user group names. This annotation has no
effect on clusters. Use it for namespace sleep mode only.
...
Ignores cluster requests when determining namespace activity.
...
Ignores requests to specific Kubernetes resources when determining activity.
Provide a comma-separated list of `resource` or `resource.group` values. This
annotation has no effect on clusters. Use it for namespace sleep mode
only.
...
Ignores requests with specific Kubernetes API verbs when determining activity.
Provide a comma-separated list. This annotation has no effect on
clusters. Use it for namespace sleep mode only.

### sleepmode.loft.sh/ignore-resource-verbs {#sleepmode-loft-sh-ignore-resource-verbs}

**Type:** Annotation

**Example:** `sleepmode.loft.sh/ignore-resource-verbs: "pods=get list,deployments.apps=get"`

**Used on:** Namespace

**Set by:** User-configurable

Ignores specific Kubernetes API verb combinations for specific resources.
Format: `resource[.group]=verb1 verb2,resource2[.group]=verb3`. This
annotation has no effect on clusters. Use it for namespace sleep mode
only.
...
**Example:** `sleepmode.loft.sh/ignore-resource-names: "pods=monitoring-pod,configmaps=config1"`

**Used on:** Namespace

**Set by:** User-configurable

Ignores requests to specific named resources. Format:
`resource[.group]=name1 name2`. This annotation has no effect on
clusters. Use it for namespace sleep mode only.
...
### sleepmode.loft.sh/ignore-activity {#sleepmode-loft-sh-ignore-activity}

**Type:** Label

**Example:** `sleepmode.loft.sh/ignore-activity: "true"`

**Used on:** AccessKey

**Set by:** Platform

Marks an access key whose API requests should not count as activity for sleep mode purposes. The ArgoCD integration sets this on its access keys so ArgoCD reconcile traffic does not keep a cluster awake.
...
Disables automatic wakeup from supported Ingress, HTTPRoute, and Istio traffic.
When set, the namespace or cluster remains asleep when it receives that
traffic.
...
Disables Istio metrics-based activity tracking for the connected cluster. Other
configured activity sources, such as proxied Kubernetes API requests and HTTP
request mirrors, can still update activity.

### sleepmode.loft.sh/request-mirror-controller-allowlist {#sleepmode-loft-sh-request-mirror-controller-allowlist}

**Type:** Annotation

**Example:** `sleepmode.loft.sh/request-mirror-controller-allowlist: "example.com/controller"`

**Used on:** Cluster

**Set by:** User-configurable

Comma-separated list of `GatewayClass.spec.controllerName` values to treat as
request-mirror-supporting for HTTPRoutes on this connected cluster. Use it when
a compatible controller doesn't advertise the `HTTPRouteRequestMirror` feature
in `GatewayClass.status.supportedFeatures`.
...
Unix timestamp of when the namespace or cluster entered auto sleep. Present only when sleeping.
...
### sleepmode.loft.sh/http-route-request-mirror {#sleepmode-loft-sh-http-route-request-mirror}

**Type:** Annotation (read-only)

**Example:** `sleepmode.loft.sh/http-route-request-mirror: "true"`

**Used on:** HTTPRoute

**Set by:** Platform

Indicates that the platform owns and manages request mirror filters on this HTTPRoute for sleep mode functionality.

### sleepmode.loft.sh/http-route-sleeping {#sleepmode-loft-sh-http-route-sleeping}

**Type:** Annotation (read-only)

**Example:** `sleepmode.loft.sh/http-route-sleeping: "true"`

**Used on:** HTTPRoute

**Set by:** Platform

Indicates that the HTTPRoute should continue reconciling to sleep mode configuration if present or be restored to normal operation when removed.
...
### platform.vcluster.com/metrics-writer {#platform-vcluster-com-metrics-writer}

**Type:** Label

**Example:** `platform.vcluster.com/metrics-writer: "true"`

**Used on:** AccessKey

**Set by:** Platform

Identifies a metrics-writer access key issued for a cluster to write observability data to the platform's metrics backend.
...
Part of the legacy project-level Argo CD integration. When set to `"true"`, the platform automatically registers this cluster or cluster with Argo CD using the project-level configuration.

For new setups, use the connector-based integration instead. See [Connect to Argo CD](../integrations/argocd/connect-argocd.mdx).

### loft.sh/user-managed-destinations {#loft-sh-user-managed-destinations}
...
Part of the legacy project-level Argo CD integration. Tracks which Argo CD AppProject destinations are managed by the project controller. This prevents the controller from removing destinations managed by clusters when syncing project specifications.
...
### loft.sh/argocd-application-id {#loft-sh-argocd-application-id}

**Type:** Label

**Example:** `loft.sh/argocd-application-id: "my-app"`

**Used on:** Resources associated with ArgoCD applications

**Set by:** Platform

Identifies the ArgoCD application resource name associated with this resource.

### loft.sh/argocd-registered-app-name {#loft-sh-argocd-registered-app-name}

**Type:** Annotation

**Example:** `loft.sh/argocd-registered-app-name: "my-app"`

**Used on:** VirtualClusterInstance

**Set by:** Platform

Stores the ArgoCD application name that was last successfully registered. Used internally to track registration state.

### loft.sh/argocd-registered-cluster-name {#loft-sh-argocd-registered-cluster-name}

**Type:** Annotation

**Example:** `loft.sh/argocd-registered-cluster-name: "vcluster-prod-abc123"`

**Used on:** VirtualClusterInstance

**Set by:** Platform

Stores the ArgoCD cluster name that was last successfully registered. Used to detect and migrate name changes when a platform license is added and the cluster name gains an instance-ID hash suffix.
...
### platform.vcluster.com/made-by-connector {#platform-vcluster-com-made-by-connector}
...
These labels are used for platform observability features including fleet metrics and monitoring.

### platform.vcluster.com/fleet-observability-connector {#platform-vcluster-com-fleet-observability-connector}

**Type:** Label

**Example:** `platform.vcluster.com/fleet-observability-connector: "true"`

**Used on:** Connector

**Set by:** Platform

Identifies observability connectors that provide data to the platform-wide fleet metrics Grafana instance.

### platform.vcluster.com/self-signed {#platform-vcluster-com-self-signed}

**Type:** Label

**Example:** `platform.vcluster.com/self-signed: "true"`

**Used on:** Secret

**Set by:** Platform

Marks a gateway serving certificate Secret as issued by the platform controller. Distinguishes platform-managed certificates from user-provided certificates.
...
### loft.sh/machine-key {#loft-sh-machine-key}

**Type:** Label

**Example:** `loft.sh/machine-key: "mkey1234567890abcdefghijklmnopqrstuvwxyz0123456789ab"`

**Used on:** NetworkPeer

**Set by:** Platform

Indexes a NetworkPeer by its Tailscale machine public key, encoded as 52 base32 characters to satisfy Kubernetes' label value length limits.
...
### loft.sh/network-peer-persistence {#loft-sh-network-peer-persistence}
...
### loft.sh/ssh-keys {#loft-sh-ssh-keys}

**Type:** Annotation

**Example:** `loft.sh/ssh-keys: "ssh-rsa AAAAB3NzaC1yc2EAAAADAQAB...,ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAA..."`

**Used on:** AccessKey

**Set by:** User-configurable

Specifies a comma-separated list of SSH public keys that are allowed for network peers created by this access key. Used to restrict network peer authentication.

## Shared and project secrets {#shared-project-secrets}

These annotations and labels are used for secret management.

### loft.sh/encrypted {#loft-sh-encrypted}

**Type:** Annotation

**Example:** `loft.sh/encrypted: "true"`

**Used on:** Secret

**Set by:** Platform

Marks a Secret whose data is encrypted at rest. Paired with encryption on write and decryption on read operations.

### loft.sh/tenant-config-secret {#loft-sh-tenant-config-secret}

**Type:** Label

**Example:** `loft.sh/tenant-config-secret: "true"`

**Used on:** Secret

**Set by:** Platform

Marks a Secret as a tenant's backing config Secret. Platform backup selects Secrets by this label to collect per-tenant configuration (SSO client secrets, UI settings) alongside the Tenant resource.
...
These annotations and labels are used for StackInstance management and task tracking.

### platform.vcluster.com/stack {#platform-vcluster-com-stack}

**Type:** Label

**Example:** `platform.vcluster.com/stack: "my-stack-instance"`

**Used on:** Resources created by StackInstance tasks

**Set by:** Platform

Identifies the StackInstance that created this resource. The stack controller uses this label to list all child resources belonging to a specific stack instance.

### platform.vcluster.com/stack-config-name {#platform-vcluster-com-stack-config-name}

**Type:** Label

**Example:** `platform.vcluster.com/stack-config-name: "postgres-db"`

**Used on:** Stack custom resources

**Set by:** Platform

Stores the name a stack was configured under in vcluster.yaml. The resource name is derived from this value but cannot be turned back into it.

### platform.vcluster.com/stack-task {#platform-vcluster-com-stack-task}

**Type:** Annotation

**Example:** `platform.vcluster.com/stack-task: "deploy-database"`

**Used on:** Resources created by StackInstance tasks

**Set by:** Platform

Records the task name that created this resource. Used to link a child resource to its task definition and to detect orphaned resources when tasks are removed from the stack.

### platform.vcluster.com/stack-retry {#platform-vcluster-com-stack-retry}

**Type:** Annotation

**Example:** `platform.vcluster.com/stack-retry: "deploy-database"`

**Used on:** StackInstance

**Set by:** User-configurable

Requests a retry of a specific task by name, or all applicable tasks when set to `"all"`. A failed AppInstance can't be re-triggered this way; edit its Stack task instead. The Stack controller processes the request and then removes the annotation. See [Retry a stack](../troubleshoot/stacks.mdx#retry-a-stack) for the procedure and limitations.
...
### loft.sh/app-instance {#loft-sh-app-instance}

**Type:** Annotation

**Example:** `loft.sh/app-instance: "default/my-app-instance"`

**Used on:** HelmRelease

**Set by:** Platform

Stamped onto every Helm release deployed by an AppInstance to attribute the release to the AppInstance that manages it. The value format is `<namespace>/<name>`.
...
### platform.vcluster.com/cleanup-stacks {#platform-vcluster-com-cleanup-stacks}

**Type:** Finalizer

**Example:** `finalizers: ["platform.vcluster.com/cleanup-stacks"]`

**Used on:** VirtualClusterInstance

**Set by:** Platform

Blocks VirtualClusterInstance deletion until every managed StackInstance is deleted, independently of the Argo CD integration finalizer.
...
### platform.vcluster.com/cleanup-nico-tenant {#platform-vcluster-com-cleanup-nico-tenant}

**Type:** Finalizer

**Example:** `finalizers: ["platform.vcluster.com/cleanup-nico-tenant"]`

**Used on:** Tenant

**Set by:** Platform

Blocks Tenant deletion until platform-managed network infrastructure allocations and provider relationships are released.
...
### platform.vcluster.com/cleanup-observability-connector {#platform-vcluster-com-cleanup-observability-connector}

**Type:** Finalizer

**Example:** `finalizers: ["platform.vcluster.com/cleanup-observability-connector"]`

**Used on:** Connector

**Set by:** Platform

Blocks connector deletion until the observability datasource is cleaned up.

### platform.vcluster.com/cleanup-observability-gateway {#platform-vcluster-com-cleanup-observability-gateway}

**Type:** Finalizer

**Example:** `finalizers: ["platform.vcluster.com/cleanup-observability-gateway"]`

**Used on:** Secret

**Set by:** Platform

Blocks connector secret deletion until the observability gateway is cleaned up.

### platform.vcluster.com/cleanup-nico-os-image {#platform-vcluster-com-cleanup-nico-os-image}

**Type:** Finalizer

**Example:** `finalizers: ["platform.vcluster.com/cleanup-nico-os-image"]`

**Used on:** OSImage

**Set by:** Platform

Blocks OSImage deletion until every NICo OperatingSystem the platform materialized from it is deleted.

### platform.vcluster.com/cleanup-os-image-blob {#platform-vcluster-com-cleanup-os-image-blob}

**Type:** Finalizer

**Example:** `finalizers: ["platform.vcluster.com/cleanup-os-image-blob"]`

**Used on:** OSImage

**Set by:** Platform

Blocks OSImage deletion until the image's bytes and any open upload session are gone from its image store.
...
These annotations control node provider behavior.

### vcluster.com/pause-deploy {#vcluster-com-pause-deploy}

**Type:** Annotation

**Example:** `vcluster.com/pause-deploy: "true"`

**Used on:** NodeProvider

**Set by:** User-configurable

Pauses Helm install, upgrade, and cleanup of the node provider's deployed components, such as Metal3/Ironic, Multus, and the DHCP server. Node type sync and provider startup keep running, and existing Helm releases are left untouched. Remove the annotation to resume normal deployment reconciliation.

## Tenants {#tenants}

### tenant.vcluster.com/owner {#tenant-vcluster-com-owner}

**Type:** Label

**Example:** `tenant.vcluster.com/owner: "acme"`

**Used on:** Any resource a Tenant creates

**Set by:** Platform

Names the Tenant that created the resource. Platform stamps it at creation, keeps it on
update, and refuses any change that would alter it. A Tenant can neither see nor set it,
and its absence means the resource belongs to the platform. Mutually exclusive with
`tenant.vcluster.com/exclusive-to`.

### tenant.vcluster.com/exclusive-to {#tenant-vcluster-com-exclusive-to}

**Type:** Label

**Example:** `tenant.vcluster.com/exclusive-to: "acme"`

**Used on:** Machines and network environments

**Set by:** A platform admin, by hand

Assigns a platform-owned resource to one Tenant exclusively. Nothing derives it, because
no field in the Tenant spec expresses exclusivity. The value is the Tenant name, and an
empty value reserves the resource for the platform. Removing the label returns the
resource to the pool, except on a network environment, where the assignment can't be
changed or removed once set. Mutually exclusive with `tenant.vcluster.com/owner`.

### nico.vcluster.com/org {#nico-vcluster-com-org}

**Type:** Annotation

**Example:** `nico.vcluster.com/org: "acme-org"`

**Used on:** Tenant

**Set by:** A platform admin

Opts a Tenant into NICo and names the NICo tenant org it maps to. The Tenant controller
reports the materialized org in `status.nico`. A Tenant is bound to a single NICo provider,
and so to a single NICo site.

### nico.vcluster.com/node-provider {#nico-vcluster-com-node-provider}

**Type:** Annotation

**Example:** `nico.vcluster.com/node-provider: "nico-eu-west"`

**Used on:** Tenant

**Set by:** A platform admin

Names the NICo node provider a Tenant onboards through. Platform resolves the provider in
this order:

1. **From granted node types.** Each node type granted to the Tenant, by name or by label,
   names its node provider. If those resolve to exactly one NICo provider, Platform uses it
   and ignores this annotation.
2. **From this annotation.** If no granted node type resolves to a NICo provider, Platform
   uses the provider named here. This onboards a Tenant before any node type is granted.

If neither applies, the Tenant reports `MissingNodeProvider`. If granted node types resolve
to more than one NICo provider, it reports `MultipleNodeProviders`, because a Tenant is
limited to one. An empty value reports `EmptyNodeProvider`, and a value naming a provider
that isn't NICo reports `NotANICoProvider`.

This annotation is interim. The target design derives the provider only from granted node
types, so expect it to change.

See [Tenant boundary reference](../administer/tenants/tenant-boundary-reference.mdx) for
how these labels resolve into what a Tenant can see.
...
### loft.sh/managed-by {#loft-sh-managed-by}

**Type:** Label

**Example:** `loft.sh/managed-by: "project-controller"`

**Used on:** Various generated resources

**Set by:** Platform

Names the controller that manages a generated resource. Used for resource ownership tracking and lifecycle management.
