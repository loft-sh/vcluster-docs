...
These annotations and labels are used for stack management, including stack instances, task tracking, and output management.
...
Triggers a retry of a specific task by name, or retries all tasks when set to `"all"`. The stack controller processes this annotation and removes it after initiating the retry.

### loft.sh/stack-output-sources {#loft-sh-stack-output-sources}

**Type:** Annotation

**Example:** `loft.sh/stack-output-sources: '{"database_url":"Secret/default/credentials","region":"ConfigMap/default/config"}'`

**Used on:** Secret

**Set by:** Platform

Records the source location for each value stored in a stack outputs Secret, enabling the platform to determine which outputs contain sensitive data based on whether they originated from Secrets.

### platform.vcluster.com/stack-output-captured-sources {#platform-vcluster-com-stack-output-captured-sources}

**Type:** Annotation

**Example:** `platform.vcluster.com/stack-output-captured-sources: '{"database_url":"Secret/default/credentials","region":"ConfigMap/default/config"}'`

**Used on:** Secret

**Set by:** Platform

Records the source each stored value was read from in a stack outputs Secret. A value whose source no longer matches the declared sources annotation is considered stale and is neither served nor passed on.

### platform.vcluster.com/stack-output-declared-sources {#platform-vcluster-com-stack-output-declared-sources}

**Type:** Annotation

**Example:** `platform.vcluster.com/stack-output-declared-sources: '{"database_url":"Secret/default/credentials","api_key":"Secret/default/api-config"}'`

**Used on:** Secret

**Set by:** Platform

Records the source every declared output reads now, with or without a value. Used by readers that do not render the stack to determine if a stored value is stale.
...
Blocks VirtualClusterInstance deletion until every VCI-owned Stack CR is torn down, independently of the ArgoCD integration finalizer.
