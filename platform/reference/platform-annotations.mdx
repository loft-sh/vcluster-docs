...
Requests a retry of a specific task by name, or all applicable tasks when set to `"all"`. A failed AppInstance can't be re-triggered this way; edit its Stack task instead. The Stack controller processes the request and then removes the annotation.
