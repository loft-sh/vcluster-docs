...
When the Machine is deleted, the platform powers the server off and clears its image, user data, and network data references. If platform IPAM allocated an address for the Machine, the platform releases it immediately so a later Machine can reuse it. It restores pre-existing managed DHCP annotations and makes the BareMetalHost available for reuse.
