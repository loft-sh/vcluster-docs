...
<figure>
  <Metal3ProvisioningFlow style={{width: '100%', height: 'auto'}} role="img" aria-label="A Machine request causes the Metal3 provider to select and configure an available BareMetalHost on the control plane cluster. Bare Metal Operator reconciles it, and Ironic powers the external physical server and installs its operating system. The provisioned server then becomes one of three things. It joins a tenant cluster as a worker node, it carries that tenant cluster control plane as a systemd service, or it joins no cluster and is reached over SSH. Deleting the Machine clears provisioning data and returns the host to the available pool." />
  <figcaption>The platform writes the requested state to an available BareMetalHost, and Metal3 and Ironic provision the external server. The provisioned server then becomes a worker node, a tenant cluster control plane, or a server in no cluster at all. Deleting the Machine returns the host to the available pool.</figcaption>
</figure>
...
What happens once the operating system boots depends on what you requested the Machine for. The server can join a tenant cluster as a worker, carry that cluster's own control plane, or be a server you reach over SSH with no Kubernetes on it. Everything before that point is identical. See [Two use cases, three outcomes](../../../overview.mdx#two-use-cases-three-outcomes).

When the Machine is deleted, the platform powers the server off and clears its image, user data, and network data references. It waits for the BareMetalHost to return to its original reusable state, restores pre-existing managed DHCP annotations, and then releases any address allocated by platform IPAM. A later Machine can reuse the host and address after that cleanup completes.

## How it works: Provisioning

After the platform selects a BareMetalHost, it prepares the server for provisioning:

1. The platform merges provisioning properties from the NodeProvider, node type, network environment, and Machine. The most specific level wins: Machine-level properties override node type properties, which override NodeProvider properties.
2. If a network property (`metal3.vcluster.com/network-cidr` or `metal3.vcluster.com/network-ip-range`) is configured, the platform allocates an IP from that pool. See [Network configuration](./networking.mdx#ipam).
3. The platform generates a user data configuration and, when an address was allocated, a network data configuration, and stores each in a Kubernetes Secret on the control plane cluster.
4. The provider sets the BareMetalHost's `userData` and `networkData` references to these Secrets, and the image information from the configured [OSImage](../../../images/os-images.mdx) or direct image properties.

Ironic then provisions the server through power management, PXE boot, and an in-memory installer that writes the OS to disk. The server boots into the provisioned OS with the user data and network data configuration applied.

The platform always merges the resolved SSH keys into that user data. What it appends alongside them depends on the Machine:

| Machine | The platform also appends | Result |
|---|---|---|
| Worker for a <GlossaryTerm term="tenant-cluster">tenant cluster</GlossaryTerm> | A registration script that joins the server to that cluster | A Kubernetes node in that cluster |
| Control plane for a tenant cluster | The vCluster install script | A server running the cluster's control plane as a systemd service |
| Requested on its own | Nothing further | A provisioned server you reach over SSH |

See [Independently provisioned machines](../../../deploy/independently-provisioned-machines.mdx), [Control plane machines](../../../deploy/control-plane-machines.mdx), and [Tenant cluster nodes](../../../deploy/tenant-cluster-nodes.mdx) for each path.
...
This walkthrough covers the essential steps to go from a connected control plane cluster to a provisioned bare metal server. Steps 1 through 4 apply to every use of vMetal. Step 5 is a branch point.

The walkthrough spans two clusters, so every command names its context. `NodeProvider`, `OSImage`, `SSHKey`, and `NodeClaim` are management cluster resources. `BareMetalHost` and BMC Secrets live on the control plane cluster named by `clusterRef.cluster`. Replace each placeholder with a name from `kubectl config get-contexts`.

Before you begin:

- Confirm your hardware, BMCs, and network meet the driver's [requirements](./requirements.mdx). The BMC has to be reachable from the control plane cluster, and each server needs a NIC that can PXE boot on the provisioning network.
...
- Create an [OSImage](../../../images/os-images.mdx#create-an-osimage) named `ubuntu-noble`, or change the node type to reference an existing Metal3-compatible OSImage.
- Check your license. vMetal requires a license that includes it, and requesting a Machine without a tenant cluster additionally requires the `machine-management` feature. In the platform, navigate to **Admin > License and Billing**.

:::warning Decide on SSH access before you provision anything
SSH keys are injected into the server's cloud-init at provision time. A server that's already provisioned doesn't pick up a key you add afterwards, so you'd have to release and reprovision it.

Add `vcluster.com/ssh-keys` to the node type in step 1 if you'll want shell access to these servers, even when they're destined to become Kubernetes nodes. It's how you inspect a node whose kubelet never came up. See [Register an SSH key](../../../deploy/independently-provisioned-machines.mdx#1-register-an-ssh-key).
:::
...
spec:
  ...
  metal3:
    ...
    - name: "compute-node"
      ...
      properties:
        vcluster.com/os-image: ubuntu-noble
        # Optional, but add it now if you'll ever want shell access.
...
Everything to this point is the same whatever you do with the capacity. From here, what you claim the server for decides which guide to follow. All three create a `NodeClaim`, use the same node types and the same server pool, and differ by a field or two on the request.

| You want | Follow |
|---|---|
| A server you reach over SSH, with no Kubernetes on it | [Independently provisioned machines](../../../deploy/independently-provisioned-machines.mdx) |
| A tenant cluster whose control plane runs on your own hardware | [Control plane machines](../../../deploy/control-plane-machines.mdx) |
| Dedicated worker capacity for an existing tenant cluster | [Tenant cluster nodes](../../../deploy/tenant-cluster-nodes.mdx) |
...
| Cloud-init appends | Nothing beyond SSH keys | A join command | The vCluster install script |
| Result | A server you reach over SSH | A worker node in the cluster | The cluster's control plane |

In every case the platform selects an eligible server, allocates an IP, renders cloud-init, and hands the server to Ironic for provisioning. Everything in steps 1 through 4 is identical across all three. See [Machine and server concepts](../../../concepts/machine-lifecycle.mdx) for the model behind them.
</Step>
</Flow>
...
For how the platform determines each node type's capacity and cost, see [Capacity](../../../concepts/machine-types-and-nodetypes.mdx#capacity) and [Cost](../../../concepts/machine-types-and-nodetypes.mdx#cost).
...
References an [OSImage](../../../images/os-images.mdx) resource by name. The OSImage's properties are used to configure the image URL, checksum, and checksum type. This is the recommended way to configure OS images.
...
References a [MachineConfigTemplate](../../../images/machine-config-templates.mdx) by name. The provider renders the template's `networkDataTemplate` into a cloud-init `network-config` document when `metal3.vcluster.com/network-data` isn't set. See [Machine Config Templates](../../../images/machine-config-templates.mdx) for the template syntax and available variables.
...
References [SSHKey](../../../images/ssh-keys.mdx) resources by name. The public keys are included in the user data during provisioning.

#### `vcluster.com/user-data`

**Type:** `string`

Custom cloud-init configuration merged with the generated user data. Accepts any valid cloud-config directives, including `packages`, `write_files`, and `runcmd`. The platform appends to this document rather than replacing it. It always adds the resolved SSH keys, and adds a cluster registration command only when the Machine has a tenant cluster to register with. Anything the platform appends runs after your own `runcmd` entries.
...
References a [MachineConfigTemplate](../../../images/machine-config-templates.mdx) by name. The provider renders the template's `cloudInitTemplate` into a cloud-config document when `vcluster.com/user-data` isn't set. See [Machine Config Templates](../../../images/machine-config-templates.mdx) for the template syntax and available variables.
