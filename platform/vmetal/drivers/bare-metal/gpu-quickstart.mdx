...
This guide walks through provisioning GPU bare metal servers as private nodes in a tenant cluster, building on the NodeProvider and server registration from [Get started](./metal3.mdx#get-started). It covers GPU-specific prerequisites, node type configuration, and workload verification.
...
Complete [Get started](./metal3.mdx#get-started) first. You need a working NodeProvider with Metal3 deployed and at least one `BareMetalHost` in `available` state.
...
- [SSH keys](./metal3.mdx#ssh-and-user-data) configured as `SSHKey` resources if you need post-provision server access.
...
If you don't have a GPU-ready image, use a standard cloud image and install the vendor driver through `vcluster.com/user-data`. See [Configuration properties](./metal3.mdx#configuration-properties) for user data options.
