...
- The target control plane cluster is [connected to vCluster Platform](../../../administer/clusters/connect-cluster.mdx).
- KubeVirt and CDI are installed on the target cluster, or the [KubeVirt node provider is configured to deploy them](./kubevirt.mdx#infrastructure-deployment).
...
:::tip
Root disk DataVolumes for KubeVirt-backed Machines are usually generated automatically from [image properties](./kubevirt.mdx#image-configuration) or an [OS image](../../images/os-images.mdx). Use the steps below instead for secondary or manually managed disks.
:::
...
The [KubeVirt node provider](./kubevirt.mdx) can also synthesize a root-disk `DataVolumeTemplate` automatically when a Machine, NodeType, NodeProvider, or [OS image](../../images/os-images.mdx) sets `kubevirt.vcluster.com/image-url` or `kubevirt.vcluster.com/image-datasource`.
...
| vCluster KubeVirt CSI driver | Creates control plane cluster DataVolumes to back tenant PVCs and removes them according to the tenant PV reclaim lifecycle. |
...
Don't delete these DataVolumes directly from the platform UI or the control plane cluster.
