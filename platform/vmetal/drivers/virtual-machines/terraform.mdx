...
The terraform provider allows you to use Terraform scripts to automatically create [nodes](../../concepts/machine-types-and-nodetypes.mdx) and [node environments](../../concepts/machine-types-and-nodetypes.mdx). 
...
[Node types](../../concepts/machine-types-and-nodetypes.mdx) are used by vCluster and the platform to decide what exact machine type should get provisioned. 
...
[Node environments](../../concepts/machine-types-and-nodetypes.mdx) can be configured similar to node templates either inline or using git. Each node environment will be created once per vCluster before the first node claim is provisioned. Only if a vCluster is using a node the provider a new node environment is provisioned.
