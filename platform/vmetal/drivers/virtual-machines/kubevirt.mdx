...
To inspect and operate KubeVirt VMs directly from vCluster Platform, see [Virtual Machines](./overview.mdx).
For administrator guidance on VM disk imports and Multus networks, see [Data Volumes](./data-volumes.mdx) and [Network Attachment Definitions](./network-attachment-definitions.mdx).
...
This describes a Machine that targets a tenant cluster. A Machine requested without one gets no registration script, and the provider creates its VM in the node provider's own namespace. See [Independently provisioned machines](../../deploy/independently-provisioned-machines.mdx), whose walkthrough uses Metal3 but whose model applies to any driver.
...
- `namespaceStrategy: VirtualCluster`: each `VirtualMachine` is created in the namespace of the tenant cluster whose `NodeClaim` requested the node, instead of the provider's namespace.
...
The VirtualCluster strategy falls back to the `Provider` behavior when it can't resolve a usable tenant cluster namespace within `clusterRef.cluster`. This happens in the following cases:

- The `NodeClaim` has no tenant cluster reference.
- The referenced VirtualClusterInstance doesn't exist.
- The tenant cluster runs in a different connected cluster from the KubeVirt provider.
- The tenant cluster is standalone and doesn't run in a namespace on the provider cluster.

As a result, `namespaceStrategy: VirtualCluster` has no effect for standalone tenant clusters.
        ...
        - name: static-standard-nodes
          ...
          nodeTypeSelector:
            - property: vcluster.com/node-type
              value: my-provider.standard-node
```
...
For administrator workflows, ownership, and import troubleshooting, see [Data Volumes](./data-volumes.mdx).
...
For UI workflows, examples, ownership, and NAD troubleshooting, see [Network Attachment Definitions](./network-attachment-definitions.mdx).
...
The `vcluster.com/ssh-keys` and `vcluster.com/user-data` properties behave identically across providers. See the [Metal3 reference](../bare-metal/metal3/overview.mdx#ssh-and-user-data) for details.

To render cloud-init from a reusable template, reference a [MachineConfigTemplate](../../images/machine-config-templates.mdx) with the `vcluster.com/user-data-template-config` property. See [Machine Config Templates](../../images/machine-config-templates.mdx) for the template syntax and available variables.
    ...
    - provider: kubevirt-netris-provider
      static:
        - name: netris-vms
          ...
          nodeTypeSelector:
            - property: vcluster.com/node-type
              value: kubevirt-netris-provider.netris-vms
