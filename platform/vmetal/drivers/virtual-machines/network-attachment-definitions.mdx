...
The property can be set on a NodeProvider, NodeType, NodeClaim, Machine, or [NodeEnvironment](../../concepts/machine-types-and-nodetypes.mdx#node-environments).
...
- [KubeVirt](./kubevirt.mdx#infrastructure-deployment), Multus, and the CNI plugins referenced by the NAD are installed on the target cluster.
- Any network binding plugin selected through `kubevirt.vcluster.com/interface-template` is installed and registered in the KubeVirt custom resource.
...
KubeVirt VM networking spans three layers, and each is configured separately:

- The NAD's CNI configuration defines the host-side network, for example a Linux bridge or SR-IOV device.
- The KubeVirt interface binding (`kubevirt.vcluster.com/interface-binding` or `interface-template`) connects the VM to that network.
- Cloud-init network data (`kubevirt.vcluster.com/network-data`, `network-interface`, `network-nameservers`) configures the interface inside the guest OS.

Keep these layers separate when troubleshooting. A working NAD and binding only get traffic to the VM's virtual NIC. The guest still needs correct IP configuration to use it.
...
Set [`kubevirt.vcluster.com/network-attachment-definition`](./kubevirt.mdx#kubevirt-vcluster-com-network-attachment-definition) to the NAD name.
...
|---|---|
| `kubevirt.vcluster.com/interface-binding` | Sets an in-tree KubeVirt binding method. Supported values are `bridge`, `masquerade`, and `sriov`. Defaults to `bridge`. |
| `kubevirt.vcluster.com/interface-template` | YAML fragment merged into the injected KubeVirt interface. Use `binding: {name: <plugin>}` for a network binding plugin that is already [installed and registered with KubeVirt](https://kubevirt.io/user-guide/network/network_binding_plugins/). The interface name is always forced to `multus`. |
...
| `kubevirt.vcluster.com/network-data` | Base64-encoded cloud-init network data. Use this when automatic network configuration is not enough. See [network-data templates](../../images/machine-config-templates.mdx#network-data-templates). |
...
See the [KubeVirt node provider Netris example](./kubevirt.mdx#netris-integration) for the full provider configuration.

## Operational ownership

NetworkAttachmentDefinitions are namespaced resources on connected Kubernetes clusters, not vCluster Platform resources.
