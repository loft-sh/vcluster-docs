...
Create a [KubeVirt node provider](./kubevirt.mdx) to connect vCluster Platform to a cluster where VMs should run.
...
- Optional [OS images](../../images/os-images.mdx) and [SSH keys](../../images/ssh-keys.mdx) for reusable provisioning configuration.
...
KubeVirt VM configuration is controlled by the provider template, node types, `NodeClaim` properties, and reusable resources.

Common properties include:

| Property | Description |
|---|---|
| `vcluster.com/os-image` | References a reusable [KubeVirt OSImage](../../images/os-images.mdx#kubevirt). |
| `vcluster.com/ssh-keys` | References one or more `SSHKey` resources. |
| `vcluster.com/network-environment` | References a `NodeEnvironment` whose properties merge into the `NodeClaim`. |
| `vcluster.com/user-data` | Provides inline cloud-init user data. |
| `vcluster.com/user-data-template-config` | References a [MachineConfigTemplate](../../images/machine-config-templates.mdx) that renders user data. |
...
For the full provider configuration, template behavior, image properties, and Netris networking example, see the [KubeVirt node provider documentation](./kubevirt.mdx).
