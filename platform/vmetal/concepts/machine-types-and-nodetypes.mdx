...
* NICo, a bare metal provisioning driver. In active development, not yet available.

These node providers implement the common vMetal Machine API and lifecycle. Tenant clusters request capacity through the same model without depending on the underlying infrastructure API. See [vMetal overview](../overview.mdx) for how they fit together.
