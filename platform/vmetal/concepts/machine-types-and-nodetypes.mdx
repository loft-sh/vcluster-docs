---
title: Node providers and node types
sidebar_label: Node providers and node types
sidebar_position: 1
---
...
* [Terraform](../drivers/virtual-machines/terraform.mdx) (powered by [OpenTofu](https://opentofu.org) under the hood)
* [KubeVirt](../drivers/virtual-machines/kubevirt.mdx) <!-- vale Vale.Terms = NO --> for VM-backed private nodes. To inspect and operate KubeVirt VMs directly, see [Virtual Machines](../drivers/virtual-machines/overview.mdx).
* [Nvidia Base Command Manager](../drivers/bare-metal/bcm.mdx) <!-- vale Vale.Terms = NO --> (BCM) for existing BCM-managed clusters.
* [Metal3](../drivers/bare-metal/metal3/overview.mdx) for bare metal provisioning using Metal3 and Ironic.
* Cluster API, for infrastructure provisioned through a Cluster API provider's own contract. Available as a driver mechanism rather than a specific provider.

These node providers implement the common vMetal `NodeClaim` API and lifecycle. Tenant clusters request capacity through the same model without depending on the underlying infrastructure API. See [vMetal overview](../overview.mdx) for how they fit together.
...
Don't create a node provider per tenant cluster. Create one provider, then make its node types available only within the applicable Tenant and Project resource boundaries.
:::
...
* `vcluster.com/node-type`: The node type's fully qualified name, which the platform generates as `<provider>.<node type>`. Maps vCluster node pools to a specific node type. When the qualified and bare names differ, the platform sets both as a comma-separated pair so a selector written against either one still matches. Prefer the qualified form. It's what the API and the platform UI show, and the platform is migrating to it.
* `node.kubernetes.io/instance-type`: The official Kubernetes label, carrying the fully qualified node type name. Unlike `vcluster.com/node-type`, it only ever holds that single value, because Karpenter rejects a list here.
...
Provisioning properties, such as OS image, SSH keys, and network configuration, can be set at the node provider level, the node type level, or the individual node claim level. When the same property is set at more than one level, the most specific level wins: node claim properties override node type properties, which override node provider properties.

## Node claims

When a tenant cluster requests a new node, either dynamically using [Karpenter](https://karpenter.sh/) or through static configuration, vCluster creates a node claim in the platform with requirements. An administrator can also create a node claim directly, with no tenant cluster attached, to provision a server that runs no Kubernetes. See [Independently provisioned machines](../deploy/independently-provisioned-machines.mdx). The platform then selects the node type that best fulfills those requirements. If multiple node types match, the platform chooses the cheapest available one.

Tenant resource boundaries define the maximum set of node types available to an organization. A Project can only narrow that set. Use [`spec.allowedNodeTypes`](../../administer/projects/allowed/node-types.mdx) to restrict which admitted node types the Project's NodeClaims may use.
