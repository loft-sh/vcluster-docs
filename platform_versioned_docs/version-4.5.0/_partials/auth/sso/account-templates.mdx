...
To tell vCluster Platform to automatically apply certain account templates to new users when they sign in via SSO for the first time, navigate to `Admin > Config` in vCluster Platform and add the following lines to your `auth.oidc` section of the config:
...
auth:
  oidc:
    ...
    - name: loft-all-clusters-default     # Name of template 1
      sync: true                          # If enabled vCluster Platform will sync changes in the cluster account template to the created accounts
