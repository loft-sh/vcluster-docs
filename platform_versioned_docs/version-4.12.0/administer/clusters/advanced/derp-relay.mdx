...
<VersionBadge platformVersion="v4.3.0"/>
...
Additional DERP relays supplement the central DERP server rather than replacing it. Agents fall back to the central relay if all custom relays become unavailable.
:::
...
Enable verbose TSNet logging to verify relay activity, as described in [Enable verbose agent and platform logs](../../../troubleshoot/connected-cluster-connectivity.mdx#enable-verbose-agent-and-platform-logs). `LOFT_LOG_TSNET_CONTROL_PLANE` covers the platform's DERP relay and mesh traffic, and `LOFT_LOG_TSNET_AGENT` covers the equivalent logging for an agent acting as a relay.
...
- [Regional Cluster Endpoints](./regional-cluster-endpoints.mdx): Reduce API latency by connecting directly to clusters
- [Networking](./networking.mdx): Required ports for platform communication
- [Troubleshoot connected cluster connection failures](../../../troubleshoot/connected-cluster-connectivity.mdx): Debug connectivity issues
