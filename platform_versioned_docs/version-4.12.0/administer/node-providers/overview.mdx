...
:::warning Deleting a NodeClaim doesn't drain the node
When a NodeClaim is deleted, the platform destroys the underlying machine right away. It doesn't cordon the tenant cluster Node or evict its workloads first, so any pods still running on that Node stop without eviction. Drain the Node from the tenant cluster before you delete the NodeClaim:

<InterpolatedCodeBlock
  code={'kubectl drain [[VAR:NODE_NAME:my-node]] --ignore-daemonsets --delete-emptydir-data'}
  language="bash"
/>

`kubectl drain` cordons the Node before it evicts pods, so no new pods get scheduled there during the drain.
:::
