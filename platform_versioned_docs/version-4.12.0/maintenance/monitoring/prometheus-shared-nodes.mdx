...
This doesn't affect Private Nodes tenant clusters, which have dedicated real nodes with directly reachable IPs.
...
- **Prometheus outside the tenant cluster** (on the control plane cluster or a central monitoring instance): use [Option 1](#option-1-otel-fleet-monitoring-reference-architecture).
...
scrape node and cAdvisor metrics from Shared Nodes tenant clusters. Its self-scrape pattern
only authenticates against the host kubelet on Private Nodes tenant clusters. Use Option 1
...
- **vCluster doesn't rewrite metrics.** Pod and namespace labels reflect the control plane cluster's internal naming, not the tenant cluster's virtual names. On a shared node, metrics from different tenant clusters have overlapping pod/namespace label values.
