...
```bash
kubectl get virtualclusterinstances.management.loft.sh -n p-my-project -o yaml
```
...
```bash
kubectl get virtualclusterinstances.management.loft.sh my-virtual-cluster -n p-my-project -o yaml
```
...
```bash
curl -s "https://$LOFT_DOMAIN/kubernetes/management/apis/management.loft.sh/v1/namespaces/p-my-project/virtualclusterinstances" \
     -X GET --insecure \
...
curl -s "https://$LOFT_DOMAIN/kubernetes/management/apis/management.loft.sh/v1/namespaces/p-my-project/virtualclusterinstances/my-virtual-cluster" \
     -X GET --insecure \
     -H "Authorization: Bearer $ACCESS_KEY"
```
