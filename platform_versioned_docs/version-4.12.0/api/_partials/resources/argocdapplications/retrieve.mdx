...
```bash
kubectl get argocdapplications.management.loft.sh -n p-my-project -o yaml
```
...
```bash
kubectl get argocdapplications.management.loft.sh shared-infra -n p-my-project -o yaml
```
...
```bash
curl -s "https://$LOFT_DOMAIN/kubernetes/management/apis/management.loft.sh/v1/namespaces/p-my-project/argocdapplications" \
     -X GET --insecure \
...
curl -s "https://$LOFT_DOMAIN/kubernetes/management/apis/management.loft.sh/v1/namespaces/p-my-project/argocdapplications/shared-infra" \
     -X GET --insecure \
     -H "Authorization: Bearer $ACCESS_KEY"
```
