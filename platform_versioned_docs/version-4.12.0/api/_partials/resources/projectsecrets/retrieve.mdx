...
```bash
kubectl get projectsecrets.management.loft.sh -n p-my-project -o yaml
```
...
```bash
kubectl get projectsecrets.management.loft.sh my-secret -n p-my-project -o yaml
```
...
```bash
curl -s "https://$LOFT_DOMAIN/kubernetes/management/apis/management.loft.sh/v1/namespaces/p-my-project/projectsecrets" \
     -X GET --insecure \
...
curl -s "https://$LOFT_DOMAIN/kubernetes/management/apis/management.loft.sh/v1/namespaces/p-my-project/projectsecrets/my-secret" \
     -X GET --insecure \
     -H "Authorization: Bearer $ACCESS_KEY"
```
