...
```bash
kubectl get nodeclaims.management.loft.sh -n p-my-project -o yaml
```
...
```bash
kubectl get nodeclaims.management.loft.sh vcluster-62ftb -n p-my-project -o yaml
```
...
```bash
curl -s "https://$LOFT_DOMAIN/kubernetes/management/apis/management.loft.sh/v1/namespaces/p-my-project/nodeclaims" \
     -X GET --insecure \
...
curl -s "https://$LOFT_DOMAIN/kubernetes/management/apis/management.loft.sh/v1/namespaces/p-my-project/nodeclaims/vcluster-62ftb" \
     -X GET --insecure \
     -H "Authorization: Bearer $ACCESS_KEY"
```
