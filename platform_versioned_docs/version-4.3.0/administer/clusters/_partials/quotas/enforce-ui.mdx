...
<Flow id="enforce-quotas-ui">
  ...
  <Step>
    Hover over the cluster access that you want to apply the quota to and click
    on the{" "}
    <Button>
      <svg
    ...
    button to <Label>Edit</Label> the cluster access
  </Step>
  <Step>
    In the drawer that appears on the right, expand the{" "}
    <Expander>Restrictions</Expander> section
  </Step>
  <Step>
    Use the <Label>Enforce Quota</Label> field to specify quotas, e.g. you can
    limit the number of spaces by adding the line <Input>spaces: 3</Input> to
    this quota specification
  </Step>
  <Step>
