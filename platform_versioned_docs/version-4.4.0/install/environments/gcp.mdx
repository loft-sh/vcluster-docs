...
<Flow>
<Step>
[Deploy the platform](/docs/platform/4.4.0/install/quick-start-guide) using the vCluster CLI.
...
accept the warning in your browser. For production use, replace the default self-signed certificate with a valid [TLS certificate](/docs/platform/4.4.0/configure/domain#configure-tls).
:::
...
:::info
For more information on how to configure a custom domain, see the [Configure external acess and TLS documentation](/docs/platform/4.4.0/configure/domain).
...
You can also use Google as an identity provider and [configure SSO](/docs/platform/4.4.0/configure/single-sign-on/providers/google) to enable user
