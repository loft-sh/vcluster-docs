...
<Flow>
<Step>
[Deploy the platform](/docs/platform/install/quick-start-guide) using the vCluster CLI.
...
accept the warning in your browser. For production use, replace the default self-signed certificate with a valid [TLS certificate](/docs/platform/configure/domain#configure-tls).
:::
...
:::info
For more information on how to configure a custom domain, see the [Configure external acess and TLS documentation](/docs/platform/configure/domain).
...
You can also use Google as an identity provider and [configure SSO](/docs/platform/configure/single-sign-on/providers/google) to enable user
