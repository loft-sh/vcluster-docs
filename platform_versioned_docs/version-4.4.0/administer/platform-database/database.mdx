...
1. Set $PLATFORM_NS and $PLATFORM_NAME, based on the name and namespace of the vCluster Platform Deployment or StatefulSet.
...
3. Scale down vCluster Platform Deployment or StatefulSet to 0.

```bash
kubectl scale <deployment or statefulSet> -n $PLATFORM_NS $PLATFORM_NAME --replicas 0
```
...
```bash
kubectl scale <deployment or statefulSet> -n $PLATFORM_NS $PLATFORM_NAME --replicas $DESIRED_REPLICAS
```

6. Confirm that vCluster Platform database becomes available.

```bash
kubectl rollout status <deployment or statefulSet>/$PLATFORM_NAME -n $PLATFORM_NS
```
