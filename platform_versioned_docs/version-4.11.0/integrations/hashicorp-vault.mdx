...
This feature allows you to use secrets from Vault in your projects, virtual
clusters and global vCluster Platform tenant without copying them manually or using third-party tools.
...
### Optional: Store the Vault token in a Kubernetes secret
...
You must update the [vCluster Platform configuration](../api/resources/config.mdx) with the Vault object to set up a global Vault integration.

<Flow id="hashicorp-vault-global-setup">
  <Step>
    Log in to the vCluster Platform UI as admin and navigate to <NavStep>Platform > Platform Config</NavStep>.
  </Step>
  <Step>
...
For further configuration options, see the [vCluster Platform configuration reference](../api/resources/config.mdx) and the [Vault configuration options](#configuration-options).
:::
...
Adjust the annotations to your Vault secret accordingly.
:::
