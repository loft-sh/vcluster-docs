...
<Flow id="enforce-quotas-ui">
  ...
  <Step>
    In the configuration sheet that opens, expand the{" "}
    <Expander>Restrictions</Expander> section.
  </Step>
  <Step>
    Use the <Label>Quota</Label> field to specify quotas. For example, limit the number of namespaces by adding the line `spaces: 3`.
  </Step>
  <Step>
