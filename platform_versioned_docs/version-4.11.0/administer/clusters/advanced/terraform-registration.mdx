...
The automation requires an admin-level access key to create cluster-specific access keys through the API. You can create an access key through the [vCluster Platform UI](../../authentication/access-keys#create-an-access-key) or CLI.
...
**Use scoped keys when possible** - While this workflow requires an admin key for the initial setup, consider using [scoped access keys](../../authentication/access-keys#scoped-access-keys) for other automation tasks.
