...
You can specify credentials either directly inside the app or using a project secret.
...
You can also reference a project secret using:
```yaml
apiVersion: management.loft.sh/v1
kind: App
metadata:
...
### Use dynamic credentials (ECR and so on)

For dynamic credentials, such as ECR credentials, use [external-secrets](https://external-secrets.io/) and export the credentials to a project secret.

For example, for ECR credentials, this can be done using:
```yaml
...
If an app instance fails to deploy because of a `usernameRef` or `passwordRef`, see [Secret and credential resolution failures](../../troubleshoot/template-sync-failures.mdx#secret-and-credential-resolution-failures).
