...
:::note Loft Router
vCluster provides a hosted domain for the vCluster platform, referred as the Loft Router.
...
helm upgrade vcluster-platform vcluster-control-plane --install \
 --repo https://charts.loft.sh/ \
 --namespace vcluster-platform \
 --create-namespace \
 --values values.yaml \
...
- `admin.password` - Sets the password of an administrator user. By default, the username is `my-admin-password`.
