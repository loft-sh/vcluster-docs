...
Add the following YAML configuration to your platform config (the `config` section of the [`values.yaml`](../../configure/introduction.mdx)) or through vCluster Platform UI <!-- vale off -->**Admin > Config**<!-- vale on -->:

```yaml title="Multiple SSO providers"
auth:
  github:
    clientId: test123
