...
<FeatureTable names="auto-nodes-bcm" />
...
This property may be omitted when the `netris.vcluster.com/server-cluster` is set.
...
The BCM node provider can use the [Netris integration](/vcluster/configure/vcluster-yaml/integrations/netris) for automated network configuration and IP management.
The following properties require the integration to be enabled and [configured](/vcluster/configure/vcluster-yaml/integrations/netris#configure-netris-integration).
