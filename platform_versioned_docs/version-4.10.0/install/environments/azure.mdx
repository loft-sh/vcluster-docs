---
title: Install vCluster Platform on Azure using AKS
sidebar_label: AKS
sidebar_position: 4
description: Learn how to install and configure vCluster Platform on Azure with AKS.
---
...
This guide provides instructions for installing the platform on Azure using [Azure Kubernetes Service (AKS)](https://learn.microsoft.com/en-us/azure/aks/).
...
<InterpolatedCodeBlock
  code={
`export RESOURCE_GROUP_NAME=[[VAR:RESOURCE_GROUP_NAME:vcluster-demo-ResourceGroup]]
...
After the AKS cluster is running, install the platform.

### Install the platform

<Flow>
<Step>
[Install the platform](/docs/platform/install/quick-start-guide) using the vCluster CLI.
...
```bash title="Install the platform using vCluster CLI
vcluster platform start
```

The command prompts you to enter the email address for the admin user:

```bash title="Expected installation output"
...
After the platform is installed, your default browser opens with the platform UI, and you should see output similar to the following:
...
This completes the basic platform installation. For additional configuration and available features, see the [Next steps section](#post-install-steps).
...
:::note
This assumes the platform is installed in the `vcluster-platform` namespace which is a
...
accept the warning in your browser. For production use, replace the default self-signed certificate with a valid [TLS certificate](../../configure/installation-options/domain.mdx#configure-tls).
:::
...
:::info
For more information on how to configure a custom domain, see the [Configure external access and TLS documentation](../../configure/installation-options/domain.mdx).
:::
...
<InterpolatedCodeBlock
  code={'# Deploy platform pods only on user node pools\n' +
'nodeSelector:\n' +
...
<InterpolatedCodeBlock
  code={'# Allow scheduling on user node pools with custom taints\n' +
'tolerations:\n' +
...
:::info
For more information about platform configuration options, see [platform configuration](../../configure/installation-options/overview.mdx).
:::
...
You can also use Google as an identity provider and [configure SSO](../../configure/platform-configs/single-sign-on.mdx#providers) to enable user authentication to the platform.

## Clean up resources

If you installed the AKS cluster using this guide, and want to clean up the resources, run the
following command:
