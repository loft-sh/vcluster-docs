...
You can also start sleep manually from the tenant cluster's three-dot menu in the Platform UI.
...
The platform tracks activity based on requests that are proxied through the platform. Direct access to the tenant cluster API server (for example, through ingress access) is not tracked by the platform's inactivity detection.

Gateway API HTTPRoute endpoints can participate in auto sleep when the selected Gateway controller supports request mirroring. Gateway endpoints may still be usable when request mirroring is not supported, but automatic wakeup from HTTP traffic is not available through that controller. If Gateway API CRDs are installed after vCluster Platform is already running, restart Platform with `kubectl rollout restart deployment/loft -n vcluster-platform` so Gateway API sleep-mode controllers are discovered.

If you need more advanced activity detection, see the [vCluster sleep mode documentation](/docs/vcluster/configure/vcluster-yaml/sleep) for configuration options including custom activity annotations and Gateway API caveats.
