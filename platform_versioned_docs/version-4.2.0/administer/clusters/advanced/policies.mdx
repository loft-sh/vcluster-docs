...
description: Learn how to create and use policies with vCluster.
---
...
Policies are based on [jsPolicy](https://jspolicy.com) and you will need to install the jsPolicy app into the cluster if you want to use policies. JsPolicy is a recommended app in vCluster Platform and can be installed via the vCluster Platform UI.

<figure class="frame">
  <img src="/docs/media/v1/ui/policies/install-js-policy.png" alt="JsPolicy" />
  <figcaption>Installing jsPolicy via the vCluster Platform UI</figcaption>
</figure>
...
Policies can be created either through the vCluster Platform UI or through directly creating `JsPolicy` objects in the cluster. For a more extensive documentation how to write policies,  check the [jsPolicy docs](https://www.jspolicy.com/docs/writing-policies/workflow).
...
Go to the Clusters > Policies view and click 'Add Policy'.
...
This is the actual JavaScript payload that is executed if a policy matches a resource and operation. Checkout the [jsPolicy documentation](https://www.jspolicy.com/docs/why-jspolicy) and [jsPolicy examples](https://github.com/loft-sh/jspolicy/tree/main/examples) for more information about how the payload could look like.
...
There are multiple ways to create policies for the cluster. For all options,  check the [jsPolicy documentation](https://www.jspolicy.com/docs/writing-policies/workflow). This guide will cover the easiest option by creating a simple `JsPolicy` object.
...
Go to the Clusters > Policies view and click 'delete' in the actions column.
...
<figure class="frame">
  <img
    src="/docs/media/v1/ui/policies/policy-violations.png"
...
This section holds several examples for useful policies within vCluster Platform. For more examples and explanations checkout the [jsPolicy documentation](https://www.jspolicy.com/docs/quickstart).
...
vCluster Platform does not require you to use jsPolicy for policies, and you can always use your own preferred solution. Every policy engine should work fine with vCluster Platform. However, if you want to use the policies view in vCluster Platform, this currently only works with [jsPolicy](https://github.com/loft-sh/jspolicy).

We decided to integrate jsPolicy instead of another policy engine, because jsPolicy is the most flexible one that can cover the most use cases within vCluster Platform. Especially with [controller policies](https://www.jspolicy.com/docs/using-policies/policy-types#controller) and [mutating webhooks](https://www.jspolicy.com/docs/using-policies/policy-types#mutating) that are not at all or only partly supported in other policy engines, jsPolicy has a huge advantage in terms of flexibility and expressiveness.
