...
| `patch`           | PATCH               | Partially modifies a virtual cluster instance through the API, without replacing the full spec                           | Standard     |
| `delete`          | DELETE              | Permanently removes a virtual cluster instance via UI or API                              | Standard     |
| `deletecollection`| DELETE with filters | Deletes multiple virtual cluster instances matching specific criteria (e.g., label selector)           | Standard     |
| `use`             | N/A                 |  Grants access to a virtual cluster instance: fetches kubeconfig, connects to the vCluster, and enables interaction with its resources     | Non-standard |
