...
You can also start sleep manually from the virtual cluster's three-dot menu in the Platform UI.
...
The platform tracks activity based on requests that are proxied through the platform. Direct access to the virtual cluster API server (for example, using ingress access) is not tracked by the platform's inactivity detection.
