---
title: Install vCluster Platform on Google Cloud using GKE
sidebar_label: GKE
sidebar_position: 4
description: Learn how to install and configure vCluster Platform on Google Cloud with GKE.
---
...
This guide provides instructions for installing the platform on Google Cloud using [Google Kubernetes Engine (GKE)](https://cloud.google.com/kubernetes-engine).
...
gke-vcluster-demo-default-pool-a1b2c3d4-e5f6   Ready    <none>   5m    v1.30.5-gke.1443001
gke-vcluster-demo-default-pool-a1b2c3d4-g7h8   Ready    <none>   5m    v1.30.5-gke.1443001
```
...
After the GKE cluster is running, install the platform.
...
:::info
For more information on how to configure a custom domain, see the [Configure external acess and TLS documentation](../../configure/installation-options/domain.mdx).
...
You can also use Google as an identity provider and [configure SSO](../../configure/platform-configs/single-sign-on.mdx) to enable user
authentication to the platform.


## Cleanup

If you installed the GKE cluster with this tutorial, and want to clean up the resources, run the
following command:
