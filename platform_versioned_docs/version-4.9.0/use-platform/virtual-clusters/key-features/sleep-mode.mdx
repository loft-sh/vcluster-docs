...
The platform tracks activity based on requests that are proxied through the platform. Direct access to the tenant cluster API server (for example, using ingress access) is not tracked by the platform's inactivity detection.
